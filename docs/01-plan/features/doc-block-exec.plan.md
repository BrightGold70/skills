...
**v1.102 re-ran all four with `dfae038` as the right-hand side and the interval holds there too**:
...
re-run at `dfae038` and not left at `b3be433` for a reason that is the rule and not a courtesy: its
corpus is `h-mad/` and `handoff/`, which is wider than the four feature documents, and this
revision's own stamp rule sends every wider-corpus reading to `dfae038`.** It would otherwise have
been carried silently across two commits on the strength of the four documents being byte-identical
across them — which is a fact about `docs/`, and says nothing whatever about `h-mad/`.
...
rather than the phrase beside it. v1.102 is measured at `dfae038`.** **`00b961f` also appears in
this revision's readings and the two are not interchangeable by assumption, so the relation is
measured**: `00b961f` is the commit v1.101's bytes landed at, `dfae038` is the tree v1.102 was
written against, and
`git diff --stat 00b961f dfae038 -- <plan> <spec> <impl-plan> <design>` is **empty** — the two
commits between them touched `docs/handoffs/` alone
(`git diff --name-only 00b961f dfae038 | sed 's|/[^/]*$||' | sort -u` prints `docs` and
`docs/handoffs`). **So a reading whose corpus is one of the four feature documents is stamped
`00b961f`, the commit those exact bytes landed at, and is identical at `dfae038`; a reading whose
corpus is wider than the four is stamped `dfae038`.** The wider-corpus readings this revision takes
are the codex-leg ledger, whose corpus is `docs/01-plan/features/`, and the two repository-wide
commands that establish the relation itself, the `rev-list` and the `diff --name-only` above. **The
ledger was run at both shas and reads `72`/`85` at each**, so the choice of stamp changes no value
here — which is stated rather than relied on, because the next round's pair will not be
byte-identical and the choice will then matter.
**v1.102 re-derives no `docs/`-scoped figure that v1.101 already stamped at `b3be433`, and does not
re-stamp them either**: `b3be433` is immutable and those readings are correct there, so a mass
re-stamp would replace a set of true sentences with a set of newly-taken ones and add a defect
surface for no argument. **The readings v1.102 *does* take are listed, never counted** — a cardinal
over a prose list is the population-short-by-N claim this section writes a rule against, and this
list is prose. They fall in two kinds, and the kinds are what matters: readings that **replace** a
`b3be433` value that was wrong, expired or unrunnable — the codex-leg ledger, the retired
spec-immobility premise, the cardinal-list check's post-repair delta, and residual (2)'s divergence
pair — and readings that are **new**, taken to settle a cross-document state no earlier revision
measured: the three `pgid` discharge commands, the two still-owed spec commands, and the spec
enumeration diff, and the `h-mad/`/`handoff/` interval closure. Each is published at its own site
with its own command. *The freeze* keeps its meaning —
...
`b3be433` — the commit this revision is measured at, not the one the previous revision was — and
...
Inherited-unverified at `dfae038`, the commit v1.102 is measured at — re-stamped from v1.101's
`b3be433` rather than carried, because a register that names a commit older than the revision's own
...
paragraph that defines it**. **The v1.101→v1.102 move is discharged by the interval argument run
rather than recalled**: `git rev-list --oneline b3be433..dfae038` returns exactly **three** commits,
of which only `00b961f` (v1.101's own landing) touches any of the four feature documents — the other
two, `df04e8e` and `dfae038`, touch `docs/handoffs/` alone. `00b961f`'s Version History entry
...
**The v1.99→v1.101 move it replaces was discharged the same way**: `git rev-list --oneline
...
v1.102. **v1.101 is deliberately not in that predicate**: v1.101 *did* execute one member, the six
screen-two legs, which is why they re-enter below stamped to it rather than covered by the interval,
and a lead saying no member's last execution is v1.101 would be false for exactly that bullet.
...
the *revision* that ran it, so for that member the register can say the reading is not v1.102's
...
- The **five stdlib probes of the carve-out table's `700c599` block** — last executed **v1.99**, at
  `700c599`, and **not re-run in v1.100 or v1.101**. **They are members now and were not before, and
...
- The **six screen-two legs** — last executed **v1.101**, at `b3be433`, and **not re-run in
  v1.102**. **They left this register for v1.101 alone and re-enter here now**, by the same rule the
...
above was walked and its bullets counted for this sentence, not carried from v1.101's `six`** — the
count moved because this revision re-entered a member, and a cardinal over a list this revision edits
is the exact shape §Measurements writes a rule against. That is what this document records for them,
and reconstructing a revision number from
a stamp is the recall the register exists to refuse. A stamp is enough for the register's purpose —
it says the reading is not v1.102's — named by number, and v1.101 excluded from the predicate for
the reason the lead gives, since that revision executed the screen-two legs; a deixis here would
...
and it must be re-measured on every revision, without exception. **Both halves, re-derived by v1.102
at `00b961f` — the commit v1.101 landed at and the one this revision is measured against — with the
sha inside the command so the reading is reproducible without a working tree:**

```
git ls-tree -r --name-only 00b961f -- docs/01-plan/features/ \
...
git ls-tree -r --name-only 00b961f -- docs/01-plan/features/ \
  ...
  | sed 's/.*audit\.v//;s/\.teammate\.md//' | sort -n | tail -1    # -> 85
```

**codex `72` against teammate `85` at `00b961f`. v1.101's published pair was `72`/`83` and the
teammate half had been wrong since `8c6539a` — the commit v1.100 was authored against — which is what
"without exception" was written to prevent and what two consecutive revisions then did.** The
series, one run of the command above per sha, is the whole argument: `1cbddb7` **72/83** ·
`700c599` **72/83** · `8c6539a` **72/84** · `b3be433` **72/84** · `00b961f` **72/85**. v1.99 landed at `8c6539a` and
...
`00b961f` landed them and took the half to `85`, which is why the pair above is stamped there and
not carried. **The same prediction applies to this reading**: `72`/`85` is correct at `00b961f` and
stops being correct the moment round fifteen's own reports are committed, so v1.103 re-runs it at its
own measurement commit rather than reading this line.
...
**12** for teammate at `b3be433` (it was `11` at `1cbddb7`), so a reader who takes `84` for a file
count is off by seventy-two. (2) **The
