...
**v1.97 re-ran both commands again with `7d8e797` as the right-hand side and the interval holds
there too**: `git diff --name-only 74e126f 7d8e797 -- h-mad handoff` prints nothing and
`git diff --name-only 74e126f 7d8e797 | sed 's|/.*||' | sort -u` prints `docs` alone, as do the same
two with `4e4a00c` on the left; the body still carries **60** occurrences of `4e4a00c` at `7d8e797`
by the `awk`-prefixed form above, this document not having moved in `f91a74b..7d8e797`
(`git diff --stat f91a74b 7d8e797 -- <doc>` is empty). **The phrase
...
refuse. **Residual, stated because it is not closed:** the body-scoped self-counts stamped
`4e4a00c` in this paragraph are readings of the v1.94 body and are *not* claimed at `68a70d6`;
the interval closure covers `h-mad/` and `handoff/`, and this document is neither.
...
rather than the phrase beside it. v1.97 is measured at `7d8e797`.** *The freeze* keeps its meaning —
`4e4a00c`, v1.95's freeze and the sha the phrase stayed pinned to through v1.96 even though v1.96's
own round was frozen at `68a70d6` — and it stays a **past-tense label on readings already taken**. Body-scoped at `7d8e797` the phrase sits on **32** lines, **20** of them carrying
`4e4a00c` on the same line
(`git show 7d8e797:<doc> | awk '/^## Version History/{exit}{print}' | grep 'the freeze' | grep -c '4e4a00c'`);
those twenty are readings with their sha beside them and are deliberately not re-typed, since a mass
re-stamp is a larger defect surface than the two standing sentences this rule repoints. Residual,
...
`grep -vc` — returns the other **12** at `7d8e797` and is a *shape* filter and never a verdict,
because this document hard-wraps and most of those twelve carry their sha on the adjacent line.
Both figures are readings of a **landed** body and move when v1.97 lands.

**The `docs/`-scoped figures this revision re-derived, stated as one closure rather than as ten
re-stamps.** The members are **listed, never counted**, and that distinction is the point: this
...
`7d8e797`, and **returned exactly its published value**, so its `4e4a00c` stamp is left as written —
...
(`git diff --stat cf3a862 7d8e797 -- <spec>` empty, as `cf3a862 4e4a00c` was). **The screen used to
...
a walk that holds at `7d8e797` says nothing about the next freeze — it is re-taken there, not
...
|---|---|---|---|---|---|---|
| The carve-out table published as five rows | **v1.93** | `8909ec4` | `Five members` → **1** | the v1.94 stamp driver run over that same body → **13** matching lines resolving to **seven** distinct probes | `Five members` → **0** at `6f0ee85` (v1.92) | v1.94, `7982c18` — `Seven members` → **1** |
| The register said three OS probes | **v1.94** | `7982c18` | `three OS probes` → **1** | the carve-out table's OS/runtime rows at `7982c18` → **five** (argparse `exit_on_error`, group kill and escape AC-5.2, `rmtree` on `0o000`, the reader-less FIFO, the naturally emptied group) | `three OS probes` → **0** at `8909ec4` (v1.93) | v1.95, `06ef40f` — `**five** OS- or runtime-determined probes` → **1** |
| The screen-two clause said four legs | **v1.94**, then carried unrepaired through v1.95 (`06ef40f`, same clause, same four-leg enumeration, same six executed legs) | `7982c18` | `four screen-two legs` → **1** | §Measurements' executed screen-two legs at `7982c18` → **six**: the four the sentence names plus the multi-word-gap probe and the cardinal-alternation probe, both already in that body (`three importing test files` → **2**, `printf 'zero files` → **1**) | `four screen-two legs` → **0** at `8909ec4` (v1.93) | v1.96, `f91a74b` — `**six** screen-two legs` |

Every grep cell above is one command,
`git show <commit>:<doc> | awk '/^## Version History/{exit}{print}' | grep -cF '<needle>'`,
and two details in it are load-bearing rather than tidy. **`-F`**, because two of these needles
...
`grep: repetition-operator operand invalid`, measured on the `06ef40f` body. **The `awk` prefix**, because
...
derivation is `git log --oneline --reverse -S'- v1.NN: Plan audit' -- <doc> | head -1`. (2) The
...
Inherited-unverified at `7d8e797`, the commit v1.97 is measured at — re-stamped from v1.96's
`68a70d6` rather than carried, because a register that names a commit older than the revision's own
measurement commit is the same detachment the closure paragraph above repairs, and this document's
body is byte-identical across `f91a74b..7d8e797` so nothing in the list changed by moving the stamp
(`git diff --stat f91a74b 7d8e797 -- <doc>` is empty): the
...
named below; and — new in v1.97 — the **body-scoped `74e126f` self-counts** of the closure
paragraph above (whole-file **30**, body **26**, narrowed **11**, all three stamped `4e4a00c` and
all three readings of the v1.94 body). **That last member is here because the register's population
statement admits no exception**: through v1.96 those three were handled by an inline "Residual,
stated because it is not closed" sentence, and an inline residual is not a register entry, so they
sat outside both the interval closure and this list — which the paragraph above defines as a defect
in the register rather than a licensed gap. **They are entered as unverified rather than
...
either on this document or on the spec, and the closure above reaches neither. **v1.94, v1.95, v1.96
and v1.97 have none of them re-run these legs, and all four say so rather than letting the older sha
read as a fresh pass**: the two spec
locators in this section are re-derived on the surfaces that state them — at `4e4a00c` by v1.95 and
v1.96, and again at `7d8e797` by v1.97, both returning **1** — and
the **six**
...
`7d8e797`. **The cardinal is six because the section below was walked and its stamped legs counted,
...
and it must be re-measured on every revision, without exception. **Both halves, re-derived by v1.97
at `7d8e797`, with the sha inside the command so the reading is reproducible without a working
tree:**

```
git ls-tree -r --name-only 7d8e797 -- docs/01-plan/features/ \
...
**codex `72` against teammate `83` at `7d8e797`.** Through v1.96 this pair was carried at `4e4a00c`
