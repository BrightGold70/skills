...
last commit, which lands design v1.112 and v1.113. **A figure that has moved at four consecutive design
...
```
$ D=docs/02-design/features/doc-block-exec.design.md
$ for s in 09e9307 cb4fe99 cac6edc ccd8ebd 0021c77 7fc5f94 da03ca5; do
    printf '%s total=' "$s"
...
moved. **All seven `git show` readings are of committed blobs and a reader with nothing but a clone can
re-derive every one of them**, which is the property the discharge restores.
...
once at `7fc5f94`, so the mechanism is measured rather than merely specified. **The self-match is stated because a needle quoted in
...
**86**/**86** at `0021c77`, **87**/**87** at `7fc5f94` and **88**/**88** at `da03ca5`. **The pair is what
...
seven, which is what establishes that no row is duplicated, and a single names figure quoted across
...
`88` above is a reading of a **sibling design table**; the realisation is a reading of
...
with nothing on any surface able to say whether that is a partial build or a wrong one. Both
halves are derived here, at `6a1693c`, in one walk:

```
$ for f in h-mad/tests/mutation-specs/doc_block_exec.json \
           h-mad/tests/mutation-specs/doc_block_exec_task3.json; do
    python3 -c "import json,sys;print(len(json.load(open(sys.argv[1]))['mutations']), sys.argv[1])" "$f"
  done
32 h-mad/tests/mutation-specs/doc_block_exec.json
28 h-mad/tests/mutation-specs/doc_block_exec_task3.json
$ python3 - <<'PY'
import json, subprocess
P = 'h-mad/tests/mutation-specs/'
spec = set()
for f in ('doc_block_exec.json', 'doc_block_exec_task3.json'):
    spec |= {m['name'] for m in json.load(open(P + f))['mutations']}
body = subprocess.run(['git', 'show', '6a1693c:docs/02-design/features/doc-block-exec.design.md'],
...
PY
matrix 88 on-disk 60 both 58
on-disk-not-in-matrix ['kill-skipped-after-collect-failure', 'stderr-not-closed']
matrix-not-on-disk 30
```

**THE TWO FILES ARE DISJOINT SLICES OF ONE POPULATION, AND DISJOINTNESS IS MEASURED RATHER THAN
ASSUMED**, because a row count alone cannot tell a staged slice from a duplicated one: the two name
sets intersect in **0** names and their union is **60** distinct names, which is why `32 + 28` and
the union agree. **Unit: distinct mutation NAMES, not rows and not matching lines.** So at
`6a1693c` the files hold **60** rows of which **58** are matrix members, and the matrix's
realisation is **58 of 88** — Tasks 1-2 and Task 3 written, Tasks 4 and 5 unwritten. **`60` and
`58` are BOTH published because the gap between them is the finding**, and quoting either alone
mis-states the other: "60 of 88" over-reports coverage by two, and "58 rows on disk" contradicts
the row count a reader gets from the files.
...
sweep uses `rglob("tests/mutation-specs/*.json")`, both verified present at `6a1693c` with
`grep -c 'specs_dir.glob("\*.json")'` → **1** and `grep -c 'rglob("tests/mutation-specs/\*.json")'`
→ **1**. **That is what makes the per-task split harness-legal and what makes it a STAGING artifact
rather than a convention this document should ratify**: the file names do not partition the
population by any stated rule — Tasks 1-2 sit in the *unqualified* `doc_block_exec.json` while Task
3 sits in a suffixed sibling — and no surface in any of the four documents names a per-task
convention (`grep -c 'doc_block_exec_task3'` over all four returns **0 0 0 0** at `6a1693c`). A
document that ratified the naming would be ratifying an asymmetry; a document that demanded a merge
would be demanding one on the strength of a total that was never a claim about a file. **Neither is
written here. The glob is the contract, and it is stated as such.**

**TWO REALISED ROWS ARE NOT MATRIX MEMBERS, THEY ARE NAMED RATHER THAN COUNTED, AND THE DESIGN OWES
THEM**: differencing the 60-name union against the matrix's 88 names returns exactly
`kill-skipped-after-collect-failure` and `stderr-not-closed` — the two rows added at 5e when Task
3's stderr closure and post-collect kill gained discriminating tests. **The total in §Deliverables
is deliberately NOT bumped to 90 on their account.** The design owns the matrix; a plan publishing
a total that no design surface derives is precisely the failure the seven-sha derivation above
exists to prevent, and a `+2` applied here would make this document the sole attributor of a figure
it cannot defend. Routed to the design as owed, and the difference is re-run at every freeze because
it closes the moment the matrix carries them. **THE DISCHARGE IS IN FLIGHT AND ITS TRIGGER IS NAMED
WITHOUT ITS NUMBER, WHICH IS THE ONLY FORM THAT ARMS NO STAMP**: a design revision carrying these
two rows is being authored in the same round as this one, so the total above and the two
differences are stamped at `6a1693c` and at nothing later, and **every one of them is re-derived
by the walk in this block at the sha that lands that batch — never edited to a working-tree reading
of a sibling's post-edit body**, which is the defect the batch-stamp paragraph above exists to
prevent and the reason no figure here is written forward. §Deliverables states the total and this
block produces it, so the two surfaces are re-read together at that sha, and a `not-in-matrix` list
that comes back empty is what closes this routing rather than any edit made in advance of it. **The difference in the OTHER direction is MEASURED,
not subtracted, and the two answers differ**: the matrix carries **30** names no spec file yet
realises — `len(matrix - spec)` from the same walk, printed above — and **not** the `88 - 60 = 28`
that the published totals invite. The two rows outside the matrix are counted in the 60 and not in
the 88, so the naive subtraction under-reports the unrealised set by exactly them; **whenever two
sets are not nested, the difference is run and never derived from the totals**, which is the whole
reason this walk prints both directions. That **30** is *incidental* rather than load-bearing — it
stands at 30 only because Tasks 4 and 5 have not run, and no rule holds it there.
