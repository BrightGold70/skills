...
`git diff --name-only 74e126f fbc2ea0 -- '*.py' | wc -l` → **6**, against **0** at `dfae038`,
**2** at `af19d53` and **2** at `09e9307`. **The two readings are read together and the arithmetic
...
settled relation. Only one of those 6 files moves a count here — `grammar_corpus.2026-09-03.cd979362.py`,
...
rather than the phrase beside it. v1.105 is measured at `cac6edc`, and at nothing else** — v1.104
was measured at `fbc2ea0`, and that sha survives here only as the stamp on readings v1.104 took,
never as a second answer to *measured at*.
...
never as a second answer to *measured at*. So every reading v1.105 takes is run at `cac6edc` with
...
v1.104's — and does not move. **A third clause the freeze-sha rule needs and v1.103 did not state:
a reading taken over *this revision's own post-edit body* is stamped to the tree v1.105 ships and
never to `cac6edc`, which does not contain these edits** — which is why the two self-counting
...
**WHAT "MEASURED AT" DOES AND DOES NOT PROMISE, STATED HERE BECAUSE v1.105's MOVE IS THE FIRST ONE
THAT EXPIRES `h-mad`-SCOPED READINGS RATHER THAN INHERITING THEM.** The rule says every reading
this revision *takes* is taken at its measurement commit; it does not say every figure in the
document was re-taken this round, and the register two sections below is where the difference is
recorded rather than left to a reader's optimism. Between `fbc2ea0` and `cac6edc` exactly one
...
`grep -l 'sys.path.insert(0, str(SCRIPT_DIR))' h-mad/tests/test_*.py | wc -l` → **13**. **The one
thing this enumeration cannot do is prove itself complete**, and it is not offered as complete: a
figure outside both the moved set and this re-run set is a defect in the register below rather than
a licensed gap, which is the rule that register already states about itself.
...
is one.
**The four documents are byte-identical across `09e9307..fbc2ea0`**
(`git diff --stat 09e9307 fbc2ea0 -- <plan> <spec> <impl-plan> <design>` is empty), **and this
document is byte-identical from `09e9307`, its own v1.103 landing commit, through `fbc2ea0`**
(`git diff --stat 09e9307 <sha> -- <doc>` is empty at `6fc71ab`, `f91dd2c` and `fbc2ea0` alike).
**The predecessor closure is left as v1.103 wrote it and is not re-run**: it held across
`3f70eb3..af19d53`, which is a fact about blobs at commits that have not moved. **That is a fact
about blobs and is deliberately not used as a licence to skip a re-run**: what it licenses is
...
`git diff --name-only 09e9307 fbc2ea0 -- h-mad handoff` prints nothing, so `fbc2ea0` — which adds
four probe files under `docs/03-analysis/probes/doc-block-exec/` and touches nothing else — expires
no `h-mad`-scoped reading, which is the opposite of what `af19d53` did to v1.103 and is why the
retirement in the closure paragraph above is not repeated here.
...
Inherited-unverified at `fbc2ea0`, the commit v1.104 is measured at — re-stamped from v1.103's
`af19d53` rather than carried, because a register that names a commit older than the revision's own
...
paragraph that defines it**. **The v1.103→v1.104 move is discharged by the interval argument run
rather than recalled**: `git rev-list --oneline af19d53..fbc2ea0` returns exactly **four** commits,
of which exactly **one** — `09e9307` — touches any of the four feature documents, by
`git diff --name-only <sha>^ <sha> -- <plan> <spec> <impl-plan> <design>` run per commit, which
prints **3** paths there (plan v1.103, design v1.108, impl-plan v1.52; the spec was not revised in
that batch) and **0** at each of `6fc71ab`, `f91dd2c` and `fbc2ea0`. `09e9307` is this document's
own v1.103 landing.
...
- The **`awk` boundary probe's five legs** — last executed **v1.96**, stamped `awk version
  ...
  **only for the revision that executes it** and re-enters at the next revision that does not. v1.97
  ...
  document returns `v1.94`, `v1.96` and `v1.99` plus this revision's own entry, and **each non-v1.96
...
- The **fence-body de-indentation case**, the fifteenth of the scanner grammar corpus — last
  ...
  The same walk with `/de-indent/` returns `v1.35`, `v1.95`, `v1.96` and `v1.99` plus this entry, and
...
over the body of this document returns **9** on the tree this revision ships. It is **not**
decremented from v1.104's `ten`: v1.105 removes one member and adds none, and a cardinal
over a list this revision edits is the exact shape §Measurements writes a rule against, so it is
counted rather than adjusted. **The walk's stop address moved with the list and is republished with
...
members:` at end of line and its stop address is the residual paragraph's own opening, so a future
bullet added *after* the residual paragraph is invisible to it and a future list whose lead sentence
is re-worded breaks it silently — which is why the walk is published with both addresses rather than
...
artifacts that do **not exist yet** (`29` names, `86` mutations, `8` rows), which are contract
...
inside this very batch, which is the narrowed rule earning itself within one round rather than at
some later freeze**: the design's r18 revision adds one matrix row, so the figure this document
states is **86** over the tree this batch ships and **85** at `cac6edc`. Under the old carve-out
that second move would have been invisible for the same reason the first was; under the narrowed
one it is a reading with a command and a stamp, and the stamp is *the batch* rather than a sha
because the sibling's edits are not committed yet — the third clause of the freeze-sha rule,
applied to a sibling's post-edit body instead of to this document's own. **The axis is not
...
```
$ D=docs/02-design/features/doc-block-exec.design.md
$ for s in 09e9307 cb4fe99 cac6edc; do
    printf '%s total=' "$s"
...
**The last two invocations read `$D` in the WORKING TREE and not through `git show`, and that is
the point of them rather than a shortcut**: the design's r18 revision is not committed while this
one is written, so `86` is a reading over a sibling's post-edit body and is stamped to the tree
this batch ships. The three `git show` readings above stay exactly as they are — a reading of a
committed blob does not move when a sibling's working file does. **Residual, and it is the sharper
one**: a working-tree reading is reproducible only until the batch lands, after which the same
command must be re-run as `git show <batch-commit>:$D`; the two forms are published together so a
later reader can see which one produced which number instead of inferring it.
**THIS PARAGRAPH IS THE SINGLE HOME OF THAT OBLIGATION, AND THE SET IT GOVERNS IS ENUMERABLE
BY COMMAND** — the batch stamp appears in two spellings, one naming the tree and one naming the
batch, and every occurrence of either means the tree this revision ships and expires into a
`git show <batch-commit>:` reading when it lands. A stamp with no route back to a sha is the
defect; the route is stated here once, and the other sites point at this paragraph by its bolded
lead-in rather than repeating it, because one obligation copied to every stamp is one thing to
re-word per stamp when the batch commits. **The enumeration is taken by command and reported to
the round rather than written here, and the reason is this document's standing rule about
self-counts**: the census needles would sit inside the corpus they count, so a published integer
beside them is falsified by the sentence publishing it — the first draft of this paragraph did
exactly that and moved its own figure from six to ten in the act of stating it. The command is
`awk '/^## Version History/{exit}{print}' <doc> | tr '\n' ' ' | grep -o '<spelling>' | wc -l`,
run once per spelling, and its two readings are taken over the shipped body after the last edit.
...
`sed 's/^| *//; s/ *|.*//' | sort -u | wc -l` gives **85** distinct names at `cb4fe99` and at
`cac6edc`, **81** at `09e9307` and **86** over the working design this batch ships, so the total is
a count of *rows* and of *names* alike at every reading and no row is duplicated. **Residual, exact**: the anchor is that header string, so a design revision that
...
- **Heading selector differential** — the old `docsections.titled_section` regex
  ...
  **At `fbc2ea0` the softened set is no longer empty on either corpus and the shape is a
  DIFFERENT one**: `titleless=1` on tracked and on glob, `closing_hash` still `0` tracked / `5`
  glob, `tab_form=0` on both. Its one member is the empty ATX heading enumerated above.
  **At `cac6edc` the softened set is empty again on both corpora — `titleless=0`, `tab_form=0`,
  `closing_hash` still `0` tracked / `5` glob — and the two zeroes are NOT the same zero.** The
  `1861157` zero meant *the shape had not been looked for correctly*; the `cac6edc` zero means *the
  one member found was removed from the corpus as a defect in its own right*. **A screen reading
  `0` says nothing about which of those it is**, which is why this bullet publishes the sha series
  rather than the current value, and why the `fbc2ea0` reading is kept beside the `cac6edc` reading
  instead of being overwritten by it. **An empty
...
- **Setext census** — the ATX-only assumption measured directly rather than through the selector
  ...
  **RE-RUN AT THE MEASUREMENT COMMIT `cac6edc`, BOTH PROBES, AND THE READING IS UNCHANGED.** The 09-04
  ...
  read out of the same invocation rather than asserted beside it. **This member therefore LEAVES
  the inherited-unverified register for v1.105**, by the rule that register states about itself —
  a probe leaves only for the revision that executes it, and re-enters at the next revision that
  does not.
...
and it must be re-measured on every revision, without exception. **Both halves, re-derived by v1.104
at `fbc2ea0` — this revision's one measurement commit — with the
sha inside the command so the reading is reproducible without a working tree:**

```
git ls-tree -r --name-only fbc2ea0 -- docs/01-plan/features/ \
  | grep -E 'doc-block-exec\.plan\.audit\.v[0-9]+\.codex\.md$' \
  | sed 's/.*audit\.v//;s/\.codex\.md//' | sort -n | tail -1      # -> 87
git ls-tree -r --name-only fbc2ea0 -- docs/01-plan/features/ \
  | grep -E 'doc-block-exec\.plan\.audit\.v[0-9]+\.teammate\.md$' \
  | sed 's/.*audit\.v//;s/\.teammate\.md//' | sort -n | tail -1    # -> 87
```

**codex `87` against teammate `87` at `fbc2ea0`.** `00b961f` appears below only as the stamp on the
...
reading published above at `fbc2ea0`. **That is four consecutive revisions outrun by their own
round's reports, which is the residual being right four times and not a new defect.** The same
prediction applies to the reading above: `87`/`87` is correct at `fbc2ea0` and stops being correct
the moment round seventeen's own reports are committed, so v1.105 re-runs it at its own measurement
commit rather than reading this line.
...
`00b961f`, **73**/**14** at `af19d53` and **74**/**15** at `fbc2ea0` — the codex reports ran
`1..72` contiguously and cycles `86` and `87` are the only two since, so at `fbc2ea0` a reader who
takes the published `87`/`87` for a file count is off by `87 - 74` on the codex half and
`87 - 15` on the teammate half, the arithmetic
