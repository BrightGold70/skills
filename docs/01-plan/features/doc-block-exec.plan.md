...
**86** at `0021c77` and **87** over the design this batch ships, the second of those carrying a
fresh batch stamp that expires the same way. **A figure that has moved at three consecutive design
...
```
$ D=docs/02-design/features/doc-block-exec.design.md
$ for s in 09e9307 cb4fe99 cac6edc ccd8ebd 0021c77; do
    printf '%s total=' "$s"
...
$ awk '/^\| mutation \| guard it removes \(mechanism\) \| killed by/{f=1;next}
       f&&/^\|---/{next} f&&/^\|/{n++;next} f{exit} END{print "batch total=" n+0}' "$D"
batch total=87
$ awk '/^\| mutation \| guard it removes \(mechanism\) \| killed by/{f=1;next}
       f&&/^\|---/{next} f&&/^\|/{print;next} f{exit}' "$D" \
    | sed 's/^| *//; s/ *|.*//' | sort -u | wc -l | sed 's/^ *//;s/^/batch names=/'
batch names=87
$ awk '/^\| mutation \| guard it removes \(mechanism\) \| killed by/{f=1;next}
       f&&/^\|---/{next} f&&/^\|/{print;next} f{exit}' "$D" \
    | grep -c 'the mutation targets `SKILL.md`' | sed 's/^/batch skill-md-target=/'
batch skill-md-target=1
```
...
moved. **The five `git show` readings are of committed blobs and a reader with nothing but a clone can
re-derive every one of them**, which is the property the discharge restores.
**THE LAST THREE INVOCATIONS READ `$D` IN THE WORKING TREE, AND THE ROUND-NINETEEN BATCH STAMP IS
RE-ARMED BY THEM**: the design's round-nineteen revision is written and not yet committed while
this one is written, so `87` is a reading over a sibling's post-edit body and is stamped to the tree
this batch ships, never to `0021c77`. **The row that arrived is NAMED rather than counted**, for
...
`ccd8ebd` and at the shipped design shows ONE arrival and ZERO departures**, so no row was renamed
and the delta is entirely new coverage, and the `SKILL.md`-target count stays **1** across the move,
which is what makes the split **86** helper-source **+ 1** rather than a second registry row.
...
same way**: when the round-nineteen batch lands, the three working-tree invocations are re-run as
`git show <r19-batch-commit>:$D` and the `87` becomes a blob reading. **Residual,
and it is the sharper one**: this document cannot detect a sibling's matrix moving — only running
the walk can — so the trigger is the batch landing and not a change this document can observe, and
the working-tree reading above is reproducible by nobody once the tree moves again.
...
re-word per stamp when the batch commits. **THE GOVERNED SET WENT EMPTY AND WAS RE-ARMED INSIDE THIS REVISION, AND BOTH READINGS ARE
...
**1 - 1 = 0**. The design's round-nineteen revision then shipped with a new matrix row, this
revision took a working-tree reading of it, and the needle now returns **7**: the quoted spelling
above, plus **6** live stamps, so the governed set is **7 - 1 = 6**. **The six are enumerated by
their surface rather than counted, because a bare six is the shape this paragraph exists to
refuse** — **three** in the §Deliverables mutation-spec cell (the total's stamp, the batch reading
beside it, and the totals-and-names pair list) and **three** under §Measurements (the
matrix-derivation paragraph's own stamp, the totals-and-names sentence following the walk, and the
carve-out-narrowing paragraph that dates the figure's third move). **The enumeration is by hand and
the needle is the check on it**, which is why both are stated: the count catches a site the
enumeration forgets, and the enumeration catches a count that drifted. **The self-match is stated because a needle quoted in
...
two live stamps expire together at the round-nineteen batch commit**, into
`git show <r19-batch-commit>:` readings, and the needle is re-run over every revision's own shipped
body. **The spelling `this revision ships` is a DIFFERENT thing and is
...
**86**/**86** at `0021c77` and **87**/**87** over the design this batch ships. **The pair is what
...
six, which is what establishes that no row is duplicated, and a single names figure quoted across
