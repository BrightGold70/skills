...
> Source: docs/02-design/features/pin-agents-tail-banner.design.md (post-audit, v1.33)
...
- [ ] AC-3.17 (spec AC-1.4, FR-1): **MIXED fixture, caller-observable.** Two candidates: one
      ...
      **The node's NAME is historical and is deliberately not changed.** Since v1.30 it asserts a
      ...
      declined just 7 of these 24, which is why the anchor was replaced — see the block in Task 2,
      which is normative):
...
- [ ] AC-3.18: `_agent_pv_re`'s OWN source comment is corrected in the same edit. It currently
      ...
      **Exact code** (replaces the final line of `_agent_pv_re`'s comment; every line above it in
      ...
      ```sh
        ...
        # documentation changed` MATCH, 24 of 24 such probes. That is safe for Passes 1-2,
...
| AC-4.6 | `test_tail_pass_rival_prose_does_not_suppress` | RED: PASS | mut `rival-re-prose-unsafe` |
