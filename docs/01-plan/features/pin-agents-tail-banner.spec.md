...
shared `_agent_pv_re`, which matches prose 35/35 — see AC-1.4). The launch command line is not a
...
- **Description**: A pass between Pass 2 and Pass 3 reads each unresolved candidate's tail
  ...
  INDEPENDENT literals, not a wrapper around `_agent_pv_re` — that helper matches prose 35/35 and
...
- **Acceptance Criteria**:
  - AC-1.1: Given a candidate pool of one pane whose tail matches the agent's `_agent_tail_re`
    ...
    AC-1.4 measures matching ordinary prose 35/35 — and no other pane's, `_orca_find <agent>` prints
  ...
  - AC-1.4: **Prose naming the agent is not a signature either, and the tail pass uses a
    ...
    matches 35 of 35 such probes, and it is shared with Passes 1-2, whose inputs are short
    ...
    **case-insensitively**, which declines 35/35 while all 12 real banner and status-line
    ...
    case-sensitive match declines 9 of those 12 positives while still declining all 35
