...
(`doc_block_exec.json`, which at completion realizes the design's matrix table one row per row — **91** of them,
90 anchored in the helper's source and 1 in `h-mad/SKILL.md`; `doc_block_exec_wire.json` 8; `docsections.json` 8)
must report `ALL_CAUGHT`. **The 91 is the DESIGN TABLE's count, which that file realizes — it is not a reading
of the file's row count, and phrasing it as one is what let a pre-D9 mirror read as a measurement** (v1.58).
**The total moved 90 → 91 at v1.60 and the mover is named**: design v1.116 answered
`docs/03-analysis/doc-block-exec.5d-decisions.md` §D13(a) by adding `alias-leftover-unreported`, a row
of the helper's source, so the `SKILL.md` half is unmoved at 1. **`ALL_CAUGHT` is NOT satisfiable
against `doc_block_exec.json` as it stands**: the file holds **90** rows and is missing exactly that
one, so a harness run over it reports `ALL_CAUGHT` over 90 and certifies nothing about the guard
§D12 requires. The gap is stated at the Task 4 closure predicate below, and closing it is the 5e
implementer's artifact rather than this document's (v1.60).
...
- **The stated LIMIT of the new-module RED shape, and it is a limit of the SHAPE and not of any one
  fixture** (`docs/03-analysis/doc-block-exec.5d-decisions.md` §D5, settled 2026-09-06 from the
  Task 1 GREEN dispatch): every task below prescribes a RED that begins with a module-level
  `import h_mad_doc_block_exec`, so before the module exists **every test in the file fails at
  COLLECTION with `ModuleNotFoundError` and not one test BODY runs**. A RED whose failure mode is a
  collection error therefore **certifies nothing about the bodies of the tests it collects** — it
  proves only that the module is absent, which was already known before the RED was written. Every
  defect in a test body is invisible until GREEN makes the import succeed. **Measured, not
  reasoned**: 22 of Task 1's 46 tests carried a defect of exactly that kind (a fixture whose name
  contained U+002F SOLIDUS, so `tmp_path / name` wrote into a directory that did not exist), the
  RED was scored clean, and all 22 surfaced as `FileNotFoundError` in the first second of the first
  GREEN run. **What follows for a 5d operator**: a collection-error RED is evidence of absence only,
  the per-test failure-mode statement each task carries is a PREDICTION until GREEN, and the first
  GREEN run is where the test bodies are first measured — so a GREEN dispatch that reports a batch
  of body-level failures is the RED gate working late, not a new defect class. It does not weaken
  the RED requirement; it says what the RED does and does not buy.
...
- **Mutation spec** `h-mad/tests/mutation-specs/doc_block_exec.json`: `root` is `../..`,
  ...
  seventeen took that population from two to FIVE, round nineteen re-derived it and it was SIX, and
  the 5e round settled the standing membership question below and took it to SEVEN, and reading the
  two rows D9 added — which no round had read, because they postdate the v1.56 derivation — took it
  to EIGHT, and
  ...
  of the eight that reds **three** further tests; and `field-escape-removed` (Task 4), whose
  ...
  emits no `SUBST_OVERLAP` head at all. **It reds TWO further named tests.** **The SEVENTH member is
  `timeout-validation-removed` (Task 3), added at v1.57 by the round that settled the standing
  membership question recorded below — it was a member before that decision, on the CLI half alone,
  and the previous revision reported it rather than deciding it on its own authority.** Its canonical
  key is `tests/test_h_mad_doc_block_exec.py::test_nonpositive_timeout_refuses_before_spawn`, and
  dropping the whole predicate from the ONE validator both `run_block` and `main` call also reds
  `tests/test_h_mad_doc_block_exec.py::test_unrepresentable_timeout_refuses_before_spawn` — D7's
  test, whose refusal arm then reaches `communicate` and raises `OverflowError` — and Task 4's
  `tests/test_h_mad_doc_block_exec.py::test_cli_bad_timeout_values`, at the CLI surface.
  **The first of those two is MEASURED and the second is a claim about a task that has not landed,
  and they are written differently here on purpose**: on the tree this revision ships, the mutant
  applied to a scratch copy of `h-mad/{scripts,tests}` gives `3 failed, 95 passed` —
  `test_nonpositive_timeout_refuses_before_spawn`, `test_unrepresentable_timeout_refuses_before_spawn`
  and `test_block_leaves_the_working_tree_untouched`, of which the third is an **artifact of the
  scratch tree** and not of the mutant, established by running the UNMUTATED copy first
  (`1 failed, 97 passed`, that test alone, because it shells `git status` and the scratch is not a
  repository). `test_cli_bad_timeout_values` does not exist yet — `grep -n 'def
  test_cli_bad_timeout_values' h-mad/tests/test_h_mad_doc_block_exec.py` has no hit — so its red
  cannot be measured until Task 4 lands and is carried as a claim, exactly as `field-quoting-removed`
  and `field-escape-removed` carry theirs. With this re-derivation the members that red MORE than one
  further test are exactly four — `field-quoting-removed` (three), `spawn-valueerror-unmapped`
  (two), `intersect-check-removed` (two) and `timeout-validation-removed` (two) — and the other
  four red exactly one each:
  `final-write-close-not-in-finally`, `duplicate-heading-takes-first`, `field-escape-removed` and
  `kill-skipped-after-collect-failure`.
  **The EIGHTH member is `kill-skipped-after-collect-failure` (Task 3), and it is here because the
  derivation rule was RE-RUN over the two rows D9 added rather than assumed to be unaffected by
  them** — v1.56 derived this enumeration over 88 rows, D9 took the population to 90, and the two
  new rows had never been read by any round. Its canonical key is
  `tests/test_h_mad_doc_block_exec.py::test_collect_failure_kills_running_group`, and skipping the
  group kill when collection fails also reds
  `tests/test_h_mad_doc_block_exec.py::test_poll_oserror_is_launch_failed_collect` — the canonical
  key of a DIFFERENT row, `poll-oserror-unmapped`, whose own guard this mutant leaves untouched.
  **Its sibling `stderr-not-closed` was read the same way and is NOT a member**, which is the control
  that stops the pair being admitted as a pair: on the same scratch copy it is `2 failed, 96 passed`,
  its own key `test_stderr_is_closed_after_drain_timeout` and nothing else, against
  `3 failed, 95 passed` for the member — and in both runs one failure is the
  `test_block_leaves_the_working_tree_untouched` scratch artifact established by the unmutated
  control at `1 failed, 97 passed`, not collateral.
  Four plus four is the eight, and the two bins are disjoint. Task 2's own row text already
  ...
  API-level assertion untouched. Those eight are
  ...
  read each of the 8 `doc_block_exec_wire.json` rows, the 8 `docsections.json` rows and the 91
  `doc_block_exec.json` rows for a per-row clause of the form *goes red with it* / *reds too* /
  *stays a regression test, not this row's key*, and take the members from that reading.
  **The population moved 90 → 91 at v1.60 and the new member's reading is OWED, not assumed**:
  §D13(a)'s `alias-leftover-unreported` is the ninety-first, and the derivation rule above reads the
  SPEC's rows — the row is not in `h-mad/tests/mutation-specs/doc_block_exec.json` yet
  (`MATRIX-NOT-ON-DISK ['alias-leftover-unreported']` at this tree), so there is nothing to read and
  the reading falls due when the row lands at 5e. What CAN be read now is the design's matrix row
  itself, and it carries **no** per-row clause of any of the three forms, so on the design's text it
  adds no member; that is a reading of one sentence and is not the spec-side reading the rule
  prescribes, and saying which one was taken is the point (v1.60).
  **The population WAS 90 when this reading was taken and the reading over it is NOT one stamp: 88 of those rows were read at
  v1.56, the two D9 rows at v1.58 (one is a member, one is not — both results are at the eighth
  member above), and the wire-side result below is v1.54's, and the two stamps are separated here
  ...
  and **v1.56 re-derived it over the three rows this revision moves and NONE of them adds a
  member**: D6 strikes `preamble-composed-with-unsubstituted-text` (never a member — its `test` key
  was `test_preamble_and_substitution_compose` and it reded no second named test); D6's replacement
  `substitution-result-not-passed-to-run_block` reds only its own key
  (`test_cli_subst_value_reaches_the_child`, a success-path CLI test no other row's mutant touches);
  and D7's `timeout-upper-bound-removed` reds only
  `test_unrepresentable_timeout_refuses_before_spawn`, since under it
  `test_nonpositive_timeout_refuses_before_spawn` stays green on `0` / `-1` / `nan` / `inf`.
  **A PRE-EXISTING membership question v1.56 made visible and deliberately did NOT decide on its own
  authority, SETTLED at v1.57 by the round**: `timeout-validation-removed` drops the WHOLE predicate
  from the ONE validator both `run_block` and `main` call, so its mutant reds Task 4's
  `tests/test_h_mad_doc_block_exec.py::test_cli_bad_timeout_values` as well as its canonical key
  `test_nonpositive_timeout_refuses_before_spawn` — and, D7's test now existing, that one too. By the
  scope sentence above ("the only rows whose mutant reds a second named test") it is a member, it was
  already a member before v1.56 on the CLI half alone, and it is now listed as the seventh. **That
  was a finding against the enumeration, not a defect D7 introduced**, which is why the resize is a
  round call carried out here rather than a correction of D7; the size moved SIX → SEVEN and the two
  bins moved three-plus-three → four-plus-three, both re-derived above from the member list rather
  than incremented. **The RESIZE did not move the total, and D9 did**: this member's rows were already inside the
  matrix total, so adding it to an enumeration OVER those rows changes no row count anywhere —
  but the total itself moved 88 → 90 at v1.58 for an unrelated reason, D9's two Task 3 rows, and
  v1.57 published it stale; the two facts were carried in one sentence and are separated here,
  ...
  so a multi-site revert must be expressed as one replacement).
  **The spec FILE is written at 5e, not created unanchored at 5d, and that is a correction**
  (`docs/03-analysis/doc-block-exec.5d-decisions.md` §D2, settled 2026-09-06 against the Task 1 RED
  delivery). Earlier wording here said each task appends its rows and the file is created in Task 1;
  that state is unreachable, because `h-mad/tests/mutation-specs/*.json` is swept by **filesystem
  glob**, not by a tracked-file list — reached by locator rather than by a line pin, so this bullet
  adds none:
  `grep -n 'rglob\|specs_dir.glob' h-mad/tests/test_h_mad_mutation_harness.py`, two hits, verified
  on the tree this revision ships — and `h-mad/git-hooks/pre-push` sweeps every tracked `*.json`
  (`grep -n "ls-files -- '\*.json'" h-mad/git-hooks/pre-push`, one hit), and both score an
  unanchored row `ANCHORS_UNREADABLE`. Measured with the Task 1 RED in the working tree, before any
  commit: `test_committed_mutation_specs_are_not_drifted` and
  `test_committed_mutation_harness_anchor_sweep_is_ok` both FAILED, `2 failed, 96 passed`. The
  **positive control** was run before deciding, not reasoned about: with both spec files moved aside
  and `docsections.json` reverted, the same sweep is `11 passed` and the WIRE-PIN still fails on its
  call-record assertion — so the specs were the sole cause and parking them costs the RED nothing.
  **So through 5d the rows live at
  `docs/03-analysis/doc-block-exec.pending-mutation-specs/doc_block_exec.json.pending` and
  `docs/03-analysis/doc-block-exec.pending-mutation-specs/docsections.json.pending`**, and
  `h-mad/tests/mutation-specs/docsections.json` keeps its committed anchored rows so that guard
  stays ARMED through the 5d-to-5e window. **The `.json.pending` suffix is load-bearing**: the
  pre-push hook sweeps every tracked `*.json` anywhere in the repository, so a parked spec still
  ending in `.json` would block the very push it was parked to unblock. At 5e the files move back
  under `h-mad/tests/mutation-specs/`, drop the suffix, and every row is anchored against the source
  that now exists — the one moment an anchor can be verified rather than guessed, since the harness
  refuses any anchor not matching exactly once. Verified after parking, NUL-safe over every tracked
  `*.json` (a whitespace-split first attempt inflated the count with phantom paths and had to be
  redone): `ANCHORS: ANCHORS_OK specs=46 mutations=490 ok=490 drifted=0 unreadable=0
  unclassifiable=0`. **Residual, stated exactly**: nothing detects a parked spec that is never moved
  back. If 5e lands without restoring these two files the feature ships with no mutation coverage
  for the new module and the sweep stays green, because a spec that is not under the glob is not a
  spec. The 5e task is the only thing that closes it.
  **A SIBLING residual of the same shape, and this one has already fired** (`5d-decisions.md` §D10,
  answered at v1.57): nothing detects rows that land at 5e in a file **this document does not name**.
  Task 3's 5e rows were written to `h-mad/tests/mutation-specs/doc_block_exec_task3.json`, which
  was named on **0** lines of this document, of the design, of the plan and of the spec **at the
  freeze `6a1693c`, and that zero is a measurement this very revision moves** — the four documents
  are being revised in parallel to answer §D10, so the same command run afterwards returns a nonzero
  count in every one of them, this bullet included. The command, its unit being MATCHING LINES and
  not occurrences: `for f in docs/01-plan/features/doc-block-exec.{impl-plan,spec,plan}.md
  docs/02-design/features/doc-block-exec.design.md; do git show 6a1693c:$f | grep -c
  'doc_block_exec_task3'; done` prints `0 0 0 0`, and it is stamped at the freeze rather than
  re-derived here for exactly that reason. The zero was **incidental, not load-bearing**: nothing
  forbade naming the file — no document had been revised since it was created. Every published gate
  still read `ALL_CAUGHT`, because the harness scores the file it is handed and the anchor sweep
  scores whatever is under the glob — neither asks whether the rows are where the plan put them.
  **That per-task file is STAGING, not a convention, and the merge is owed by the implementation**:
  `doc_block_exec.json` already holds **32** rows that are exactly Task 1's 25 followed by Task 2's
  7, under a `_why` reading `Task 1 and Task 2 mutation contracts` — **one file holding TWO tasks'
  rows is what refutes a per-task convention**, since under such a convention those two would be two
  files. Three further readings agreed, **all three taken at `6a1693c` and kept stamped there
  because the tree has since moved**: the two files' row-name sets were disjoint and their `command`
  named the SAME test file, so this was one spec split in two; `doc_block_exec_task3.json` was the
  only `*_task[0-9]*.json` among the **42** specs in `h-mad/tests/mutation-specs/`; and `ebd8e27`,
  the commit that created it, justifies its timing under D2 and gives no per-task rationale.
  **THE OPERATOR SETTLED IT AND THE MERGE HAS SINCE EXECUTED** (`8ef45fc` decides, `7afc0ef`
  executes, `8bc2fa7` records the execution and its evidence in
  `docs/03-analysis/doc-block-exec.5d-decisions.md` §D10): the per-task file is merged into
  `doc_block_exec.json` and DELETED. **The current reading of the same two questions, re-run rather
  than inferred from the decision**: the directory now holds **41** specs, and the only sentence
  above that asserted a present-tense property of the tree is replaced by this one. **So the three-spec
  enumeration in the Executive Summary is correct in WHICH FILES it names, and no row total moves for
  this reason** — the total published there did move, 88 → 90, for an unrelated reason (D9), and the
  Summary was separately wrong in PHRASING a design-table total as a file row count; both are
  repaired at v1.58 and neither is a §D10 finding.
  **The 32 above and the end state below are the SAME population read at two times, and saying so is
  the repair**: at `6a1693c` 32 of the table's 90 rows were in that file, 28 more were staged in the
  per-task file, and Task 4's and Task 5's 30 had not been written at all — `32 + 28 + 30 = 90`.
  **The middle addend has since MOVED, and the total has not**: after the merge
  `doc_block_exec.json` holds **60** rows — `python3 -c "import json;print(len(json.load(open('h-mad/tests/mutation-specs/doc_block_exec.json'))['mutations']))"`
  — so at that reading the same partition read `60 + 30 = 90`, and the design table's figure this
  document mirrors is not a count of any file.
  **BOTH sides of that partition have since moved, and they moved for DIFFERENT reasons, which is
  why they are stated separately rather than netted** (v1.60). The TABLE moved 90 → **91**: design
  v1.116 answered §D13(a) with `alias-leftover-unreported`. The FILE moved 60 → **90**: Task 4's 30
  rows landed with its GREEN and are in the working tree, uncommitted at the `c1d8255` freeze —
  the same one-line `python3 -c` above returns `90` on this tree and `60` at `git show
  c1d8255:h-mad/tests/mutation-specs/doc_block_exec.json`. **The two moves do not cancel, and the
  residue is the 5e gap**: a set comparison of the design's matrix names against the file's returns
  `NOT-IN-MATRIX []` and `MATRIX-NOT-ON-DISK ['alias-leftover-unreported']`, so the file is short by
  exactly that one row. **The reading is non-vacuous by control**: drop one on-disk row and
  MATRIX-NOT-ON-DISK goes 1 → 2. Because the residue is exactly one, **Task 5 contributes ZERO rows
  to this spec** — that is derived from the residue, not assumed from Task 5 being unwritten.
  **The 5e obligation was stated so it is checkable rather than assumed, and the checkable half is
  now DISCHARGED**: before the feature is complete all **91** rows live in
  `h-mad/tests/mutation-specs/doc_block_exec.json` and no
  `doc_block_exec_task*.json` remains — `ls h-mad/tests/mutation-specs/ | grep -cE
  '_task[0-9]+\.json$'` **prints `0` at this freeze**, where it printed `1` when v1.57 first ran it.
  Its sibling clause is discharged too: the merge had to reconcile the two files' `target_command`
  (the staged file carried the absolute `/opt/anaconda3/bin/python3.11`, which is not portable), and
  the merged spec ships `["python3.11", "-m", "pytest", "-q"]`. **What is NOT discharged is the row
  count**: **90 of the 91 are written and the one missing is `alias-leftover-unreported`** (v1.60,
  re-derived above; the earlier reading was `60 of the 90`, the remaining 30 being Task 4's and
  Task 5's, and Task 4's 30 have since landed). **This is the one open 5e gap and it does not
  announce itself**: the harness scores what the SPEC holds, so a run over the 90-row file reports
  `ALL_CAUGHT` over 90 and certifies nothing about §D12's alias read-back — a green token against a
  population one short of the matrix. **Adding the row is the 5e implementer's artifact, not this
  document's**; this document is where the closure predicate is stated, and it is stated at Task 4
  below. **Consequently the `ALL_CAUGHT` gate each task
  below states is a 5e gate, not a 5d one** — at 5e, once the task's rows are anchored, run
  `python3.11 scripts/h_mad_mutation_harness.py tests/mutation-specs/doc_block_exec.json`
  and read the `MUTATION:` token; `ALL_CAUGHT` is required before the feature is complete, and a
  task is GREEN at 5d on its pytest summary alone.
- **When each row's payload is fixed — deliberate, not an omission** (impl-plan audit v20, whose
  ...
  ground; the file held 76 rows at that cycle and its table holds 91 now, and the ground is unchanged by the
  ...
  the anchor-*text*-rewritten case, not the anchor-*file*-moved one. **`doc_block_exec.json`'s 90
  rows are NOT wholly on the second side, and saying so was an over-statement of this rule against
  its own row list** (impl-plan audit v47 codex should 2): **87** of them anchor in
  `h-mad/scripts/h_mad_doc_block_exec.py`, which does not exist today, and are wholly on the second
  side; the eighty-eighth is `registry-row-removed`, whose anchor file is `h-mad/SKILL.md` — a file
...
- **Single source of the fence grammar**: **within the two files this feature touches —
  ...
  **23**, `09e9307` **23**, **23** at `cac6edc`, **23** at `0021c77`, and **23** on the tree v1.55 ships — the sweep reads the tree and not this document, so its two v1.55 readings coincide, and the `:264` pin was re-verified with the sweep's own output in the same run. The new member is
  `h-mad/scripts/h_mad_assemble_audit.py:316` `_trim_version_history`, verified at `cac6edc` by
...
- **No sentence in this document states what a sibling document currently says.** A debt or a
  ...
  **PIN EPOCH — set at Phase 5g, and written here because the re-stamp it records would otherwise
  hide exactly what it settles.** This document's provenance moves from `0021c77` to `ed2f1c5`
  (§D3 of the 5d decision sheet; a decision for the round, taken by the orchestrator, not an
  author's edit). The reason it was owed is the feature working as planned: Tasks 1 and 5 landed
  **in the three test files this document pins**, so `h_mad_precheck_doc.py` scored every pin into
  them `PINDRIFT` against a base that predates the implementation — 20 of them, which with the 11
  FR-4 grammar slots put the noise floor at `FAIL issues=31`. **The re-stamp was not taken on
  trust.** All 20 flagged pins were re-read at BOTH shas in one pass, printing the line at
  `0021c77` beside the line on the shipped tree, and the outcome is the thing worth recording:
  **20 of 20 hold at `0021c77`**, which is what makes that sha the epoch rather than an arbitrary
  base, and **2 of 20 also hold at `ed2f1c5`** — `test_the_live_phase5_section_extends_past_its_fenced_blocks`
  in `h-mad/tests/test_docsections.py`, and the `docs/01-plan` glob inside the module-level
  `REAL_AUDIT_REPORTS` tuple in `h-mad/tests/test_h_mad_audit_cycle.py`, both files untouched by
  the feature's source changes. **The other 18 do not hold at `ed2f1c5`, and every one of them
  fails for a reason this document PLANNED**: Task 1 deleted `_fence_aware_end` — definition,
  body and docstring — and `titled_section`'s local `re.search` from `h-mad/tests/docsections.py`,
  and Task 5 hoisted `_gate_block`, `_gate_bash_block` and `_run_recipe` to module level, dropped
  `_gate_bash_block`'s one-line docstring and its hand-rolled `re.findall`, and shifted the module
  head of `h-mad/tests/test_h_mad_collect_report_docs.py` by one line, which is what moves
  `_section` and `_second_surface`. **Those 18 are readings of the PRE-implementation tree and
  stay stamped at `0021c77`** — the rule this document already applies everywhere else, that a
  reading stamped at a blob stays stamped there and only a provenance field moves. Renumbering
  them to the shipped tree is not available and would not be honest if it were: the targets of
  most of them **no longer exist**, which is the deliverable and not a defect.
  **What the re-stamp COSTS, stated rather than left for the next round to find**: once the
  provenance names HEAD, `PINDRIFT` cannot see this class at all, because no file has changed
  since. The epoch is therefore carried in this paragraph rather than by the detector, and it is
  re-derivable without it — `git show 0021c77:h-mad/tests/docsections.py` beside the working file,
  and the same pair for the other two test modules, decides any single pin in two commands.
  ...
  **Re-run at v1.53 against base `fbc2ea0`, which holds v1.52** (decision K a sixth time — v1.53
  ...
  `h-mad/scripts/h_mad_assemble_audit.py:316` (the `_trim_version_history` member residual (a)'s
...
**91** at this batch and this axis contributes none of it; a row on THIS axis, if the design wants
...
- the **tab arm is 0, and it is pinned by nothing at all** — no AC in this document prescribed a
  ...
  case. **No mutation row follows from THIS axis**: the matrix total is **91** at this batch and
...
class _FenceEvent:
    ...
    # NEUTRAL ON EVERY NON-`open` EVENT — the settled contract, not an implementation choice:
...
**The `_FenceEvent` neutral-fields rule, settled and stated here because the shipped Task 1 code
already implements it and this document did not say so**
(`docs/03-analysis/doc-block-exec.5d-decisions.md` §D1, settled 2026-09-06 after the Task 1 RED
dispatch declined to invent the values rather than guessing). `marker`, `run`, `indent` and `info`
carry the opener's values on an `open` event and are **neutral — `marker=None`, `run=0`,
`indent=0`, `info=''` — on every `close`, `body`, `prose` and `heading` event**; a `heading` event
additionally carries its own `level` and `text`. **Why this and not carrying the opener's values
through**: (1) it is the pattern the block above already states for every other kind-scoped field
— `level` is "0 otherwise", `text` is "heading events only", `candidate` is "True only for a
backtick opener" — and three fields neutral outside their kind against four inheriting would be two
rules where the block reads as one; (2) `marker: str | None` is typed optional and nothing but a
non-opener needs the `None` arm; (3) no consumer reads any of the four on a non-`open` event —
`extract` selects on `.candidate` and holds the `open` event it de-indents against, `fence_aware_end`
reads `.kind`, `.level`, `.start` and `.end`, and `find_heading` reads `.text`, `.level` and `.end`;
(4) carrying opener values onto `body`/`close` would make fence geometry readable off a body line,
which is the property the single-scanner rule exists to prevent, and is the exact shape
...
indentation, so nothing downstream can tell a 3-backtick closer from a 5-backtick one, or a 0-space
closer from a 3-space one. That information is used INSIDE `_fence_events` to decide closure and is
deliberately not exported; if a later task needs it, it is a DESIGN change that gains its own field
and its own mutation row, and it is not to be recovered by re-scanning in a consumer.
**No mutation row follows and that is deliberate**: what stands in for one is the fixture —
AC-1.8's `test_fence_events_trace_on_every_hostile_fixture` pins every field of every event on LF
and CRLF copies of each hostile fixture, so an implementation that carries opener metadata through
fails outright. A row for the rule, if the design wants one, is the design's to add.
...
- [ ] AC-2.7 (scan-form clause) `test_substitute_refuses_overlapping_occurrences_of_one_key`: on a block whose text is `aaab`, `substitute(block, {"aa": "X", "ab": "Y"})` raises `OverlappingSubstitution` with `pairs == [("intersect", "aa", "ab", 2)]`. **This fixture exists because it DISCRIMINATES the two scan forms and the `abc` fixture does not** (round-eighteen sheet FACT 4 b): measured on 3.11.8, the prescribed lookahead enumeration `re.finditer(r"(?=" + re.escape(k) + r")", text)` yields `aa` at `[0, 2)` and `[1, 3)` and `ab` at `[2, 4)`, sharing index 2, while the bare `re.finditer(re.escape(k), text)` yields `aa` at `[0, 2)` alone and finds NO intersection — so an implementation written with the bare form passes the `abc` fixture and fails this one. **It NOW CARRIES a mutation row, the addition that took the matrix to 86 at r18; the total moved to 87 with the design's r19 addition on a different axis, reached 88 after the 5d decisions' net +1 on the timeout-bound axis, reached 90 after D9's +2 on the execution-guard axis, and stands at **91** after §D13(a)'s +1 on the alias-refusal axis (`alias-leftover-unreported`, design v1.116; v1.60)**: the design's r18 revision added `intersect-scan-non-overlapping` for exactly this axis, so the row this AC recorded as a design-side debt through the first half of round eighteen has been added and is mirrored in `doc_block_exec.json` below. `intersect-check-removed` does NOT reach the axis — it deletes the predicate outright, which the `abc` fixture already reds — whereas this row keeps the predicate and narrows its SCAN, which only the `aaab` fixture can see. That is the discrimination this test was written for, and it is now a killer rather than a fixture standing beside an unkilled arm.
- [ ] AC-2.8 `test_empty_key_is_refused_by_the_api`: `substitute(block, {"": "v"})` → `BadSubstArg` with `raw == ""`.

**SETTLED (was OPEN-DECISION (r19, 5d)) — ONE shared key-validity predicate, `_is_valid_substitution_key`, with caller-specific diagnostics; the cross-surface equivalence test is REFUTED, not merely unchosen**, source `docs/02-design/features/doc-block-exec.design.audit.v99.codex.md` must 3, resolved by the Phase 6a-prime review (2026-09-07). **SHIPPED at `92ae93a`** ("fix(doc-block-exec): 6b — one key-validity predicate, both surfaces route through it"): `h-mad/scripts/h_mad_doc_block_exec.py:304` defines `def _is_valid_substitution_key(key: str) -> bool` returning `bool(key)`; the API calls it at `:313` (`if not all(_is_valid_substitution_key(key) for key in subs): raise BadSubstArg("")`) and the CLI at `:766` (`if not _is_valid_substitution_key(key): raise BadSubstArg(raw)`). `grep -n '_is_valid_substitution_key' h-mad/scripts/h_mad_doc_block_exec.py` returns exactly those three lines — one definition, two call sites, no third spelling. **Each surface keeping its OWN diagnostic payload is the point of the shape, not a compromise in it**: the CLI holds the raw `--subst` argument and reports it, the API holds only the key and reports `""`, so sharing the PREDICATE never forces sharing the message, and that is why the duplication had looked unavoidable. **THE ALTERNATIVE IS REFUTED, NOT UNCHOSEN**: a cross-surface equivalence test OBSERVES drift after the fact instead of preventing it, and this feature's own Phase-1 precedent is one authoritative implementation that every surface calls — `h-mad/tests/docsections.py:46` and `:56` delegate to `_dbe.fence_aware_end` rather than re-implementing the bounder, and `tests/test_h_mad_doc_block_exec.py::test_docsections_has_no_second_bounder` asserts `"_fence_aware_end" not in names` so the deleted second bounder cannot come back. **Two independent reviewing surfaces reached this same prescription without seeing each other's reasoning.** **PINNED by `tests/test_h_mad_doc_block_exec.py::test_empty_substitution_key_validation_routes_api_and_cli_through_one_predicate`, which pins the ROUTING and not the behaviour**: it monkeypatches `dbe._is_valid_substitution_key` with a recording predicate that raises, drives `dbe.substitute(...)` and then `dbe.main([...])`, and asserts `calls == ["", ""]` — so a surface that stops delegating reds it. The orchestrator mutation-verified BOTH directions at 6b: bypassing the API call site reds the pin, and bypassing the CLI call site reds the pin. **A test asserting only "both surfaces refuse an empty key" passes on the PRE-FIX tree and IS the rejected equivalence test wearing another name** — recorded here explicitly so nobody re-proposes it as a cheaper pin. **RESIDUAL, stated exactly**: the predicate covers the EMPTY-KEY rule only (its whole body is `return bool(key)`). Any FUTURE key-validity rule must be added INSIDE `_is_valid_substitution_key` rather than at either call site, or the duplication this decision removed returns; nothing in the tree enforces that, and the routing pin cannot see it, because it replaces the predicate rather than reading its body.
...
`_validate_timeout(timeout)` first thing (`float()` conversion, then the predicate
`math.isfinite(t) and 0 < t <= _MAX_TIMEOUT_SECONDS`; else
...
_MAX_TIMEOUT_SECONDS = (2**31 - 1) / 1000   # == 2147483.647 — CPython's INT_MAX-milliseconds
                                            # selector limit. Written in THAT derived form and
                                            # never as the bare literal, so the reason is visible.

def _validate_timeout(value: object) -> float:
    """Shared by run_block (first thing) and main (before _reserve): float(value),
    then math.isfinite(t) and 0 < t <= _MAX_TIMEOUT_SECONDS;
    else BadTimeout(value) carrying the raw value."""
...
- [ ] AC-5.6 `test_nonpositive_timeout_refuses_before_spawn`: `timeout=0`, `-1`, `math.nan`, `math.inf` → `BadTimeout` and the recording `Popen` pass-through was never called and no directory was created; `test_unrepresentable_timeout_refuses_before_spawn` (the upper bound, **two arms and the control arm is not optional**): the **refusal arm** runs `timeout=2147483.648` → `BadTimeout`, with the same three assertions the nonpositive test makes — the recording `Popen` pass-through was never called and no directory was created; the **control arm** runs `timeout=2147483.647` — the constant itself, `(2**31 - 1) / 1000` — on a fast block (`echo hi`) and it **RUNS** normally. The control arm is what stops the guard degenerating into "refuse everything large", and it is the arm that fails loudly if a future interpreter moves the limit. **Neither arm is a fault injection**: like the nonpositive test it uses only the recording `Popen` pass-through as a RECORDER, so this test is not a member of the injected population the header above enumerates, and the 13 / 9 / 4 / 6 figures there are unmoved by it.

**SETTLED (was OPEN-DECISION (r19, 5d)) — `preamble-composed-with-unsubstituted-text` MOVES to Task 4 and is RENAMED**, source `docs/03-analysis/doc-block-exec.5d-decisions.md` §D6 (2026-09-06). The premise was re-derived rather than taken: `run_block(block: Block, *, preamble: str | None = None, timeout: float = 30.0) -> RunResult` takes **one** `Block` and every Task 3 AC calls it as `dbe.substitute(block, {...})[0]`, so inside `run_block` there is no `text` against which `text′` could be preferred — `block.text` **is** `text′`, the mutation as specified is a no-op, and `ALL_CAUGHT` over it would be vacuous. **The property is real and was unguarded**, so this is a MOVE and not a withdrawal: Task 4 gains `substitution-result-not-passed-to-run_block` (`main` hands `run_block` the ORIGINAL block instead of the one `substitute` returned, so a `--subst` key reaches the child unexpanded), killed by Task 4's `test_cli_subst_value_reaches_the_child`. **The rename is load-bearing**: the old name says *preamble* and the mechanism has nothing to do with the preamble, so moving it under the old name would carry the wrong mechanism into a new task. **`test_preamble_and_substitution_compose` STAYS in Task 3's AC-3.11 bullet, unchanged** — it stops being a mutation row's `test` key and does not stop being a test, and **Task 3's test count is unmoved by this decision**. **Residual, stated exactly**: (1) a **library** caller that invokes `run_block` directly with an unsubstituted block is guarded by nothing here or elsewhere — that is the caller's own bug, outside this module's surface, and no mutation of this module's source can express it; (2) the moved row guards the CLI's **pass-through** only and says nothing about whether `substitute` computes the right text, which is Task 2's rows' job and is already covered there.

**SETTLED (was OPEN-DECISION (r19, 5d)) — `BadTimeout` gains a representable upper bound, and `OverflowError` is NOT mapped into the launch-failure path**, source `docs/03-analysis/doc-block-exec.5d-decisions.md` §D7 (2026-09-06). **The premise was confirmed and the filed VALUE was not the BOUNDARY**: measured on the pinned interpreter `/opt/anaconda3/bin/python3.11` (3.11.8, conda-forge, darwin), run rather than reasoned, `math.isfinite(1e300) and 1e300 > 0` is `True` so the old guard ACCEPTS it and `communicate(timeout=1e300)` raises `OverflowError: timestamp too large to convert to C _PyTime_t` — an `ArithmeticError`, neither `OSError` nor `ValueError`, so it escapes the mapping — but the edge is nowhere near `1e300`: `1e8` already refuses with a *different* message, `OverflowError: timeout is too large`, and the **exact pair** is `2147483.647` accepted / `2147483.648` refused (a tight binary search over `[2147483.0, 2147485.0]` returns largest accepted `2147483.6470458433`, smallest refused `2147483.647045844`). `(2**31 - 1) / 1000 == 2147483.647` is `True` on the same interpreter: the limit is `INT_MAX` **milliseconds**, about 24.855 days, which is what makes the constant derivable rather than chosen. **It is not deadline-dependent, and that control is what licenses a fixed constant**: re-measured 3 s later the boundary differs by `-1.91e-05` s — search noise, not wall-clock drift — and had the limit been an absolute deadline the pair test would have been flaky by construction. A child that genuinely waits behaves identically: `sleep 0.2; echo y` under `communicate(timeout=2147483.0)` returns `('y\n', '')`. **Why not the mapping alternative**: the `OverflowError` fires inside `communicate`, which is AFTER the spawn and after `mkdtemp`, so mapping it downstream means launching a child and creating a directory for a bound already known to be unhonourable — the exact leak AC-5.6's pre-spawn rule exists to prevent — and it would have to surface as `LaunchFailed(stage="collect")`, filing a caller-input error as a fault of the launch. **Why the bound**: a bound the platform cannot represent is a bound that cannot be honoured, the same category as `inf`, which AC-5.6 already refuses for that reason and no other; the upper bound joins the existing list rather than opening a new one. **Residual, stated exactly**: the constant pins CPython's `INT_MAX`-milliseconds limit **as measured on 3.11.8 / darwin**. On an interpreter or platform whose limit is LOWER, a value between that limit and this constant would still reach `communicate` and raise an unmapped `OverflowError`. That residual is deliberately **not** closed by adding an `except OverflowError` backstop: on the pinned interpreter that branch is unreachable, **an unreachable branch cannot be tested or mutation-verified**, and shipping one would be the appearance of coverage rather than coverage. It is closed by re-running the pair probe on any interpreter this feature is later supported on.

**Mutation rows added here**: `strict-flags-dropped`, `preamble-separator-dropped`,
`cwd-not-passed`, `chmod-0700-removed`,
...
`timeout-upper-bound-removed` (the `<= _MAX_TIMEOUT_SECONDS` clause is dropped from
`_validate_timeout`, leaving `math.isfinite(t) and t > 0`; killed by
`tests/test_h_mad_doc_block_exec.py::test_unrepresentable_timeout_refuses_before_spawn`, whose
refusal arm then reaches `communicate` and raises an unmapped `OverflowError` instead of
`BadTimeout`. **It is MUTUALLY DISCRIMINATING with `timeout-validation-removed`, and in both
directions**: that row drops the WHOLE predicate, this one drops only the upper clause. Under THIS
mutant `test_nonpositive_timeout_refuses_before_spawn` stays green, because `0`, `-1`, `nan` and
`inf` are all still refused by the surviving `math.isfinite(t) and t > 0`. Under THAT mutant the
new test's refusal arm also reds, but that row's canonical key is the nonpositive test and this
row's is the new one, so neither killer can stand in for the other's — one test discriminates each
row's own clause),
...
other composition path — **a member of the eight-row `doc_block_exec.json` population whose mutant
reds a second named test, enumerated in the Conventions bullet** (this site read `five-row` from
round seventeen through v1.56 and was missed when v1.55 took the population to six and again when
v1.57 took it to seven and v1.58 to eight; the size is stated in ONE place, the Conventions enumeration, and every
other mention of it is a pointer that a resize must sweep — `awk '/^## Version History/{exit}{print
NR": "$0}' <this file> | grep -cE 'reds a second named test'` returns the sites; it read **4**
matching lines at the freeze `6a1693c` and reads **5** on the tree this revision ships, the fifth
being the sentence you are reading, which writes the needle into the body it counts — the residual
is stated rather than screened away, because the only cheap alternative is not to publish the
number, and the number is what makes the sweep re-runnable. Of the four pre-existing sites this is
the only one carrying a cardinal on the `doc_block_exec.json` side). **The bare ordinal it carried
...
collateral-red population enumerated in Conventions),
`stderr-not-closed` (the `stderr` handle left open when the drain expires, so the pipe survives the
call; killed by `tests/test_h_mad_doc_block_exec.py::test_stderr_is_closed_after_drain_timeout`) and
`kill-skipped-after-collect-failure` (the group kill skipped when collection itself fails, so a
running leader outlives the helper; killed by
`tests/test_h_mad_doc_block_exec.py::test_collect_failure_kills_running_group`) — **the D9 pair,
added at 5e and absent from this enumeration until v1.58**, which is the whole reason the matrix
total was published stale at 88 for a revision — 28 rows.
**28 is a RE-DERIVATION over the list above AND against the landed spec**, and the two moves that
produced it cancel** (`docs/03-analysis/doc-block-exec.5d-decisions.md` §D6 and §D7): D6 strikes
`preamble-composed-with-unsubstituted-text` from this task, taking it 26 → 25, and D7 adds
`timeout-upper-bound-removed`, taking it 25 → 26. **The unmoved total is arithmetic, not evidence
that nothing changed here**, which is why both addends are named: a reviewer who reads only the
integer would conclude this task was untouched.
...
`float()` conversion, then `math.isfinite(t) and 0 < t <= _MAX_TIMEOUT_SECONDS`, else
`BadTimeout(raw)` — **before** `_reserve`, so no
artifact exists for any input-only refusal; `run_block` re-validates the float it receives, and the
existing `timeout-validation-removed` row covers both call sites — no new row, and D7's
`timeout-upper-bound-removed` is likewise ONE row over the one shared validator) → `--preamble-file`
read as strict UTF-8 (`OSError`/`UnicodeDecodeError` → `PreambleUnreadable`) → **reserve** stdout
then stderr → alias check → `run_block` → final writes. **The block `run_block` receives is the one
`substitute` RETURNED, never the original** — `main` binds the substituted block and hands that on;
that pass-through is what `substitution-result-not-passed-to-run_block` mutates and what
`test_cli_subst_value_reaches_the_child` observes.
...
- [ ] AC-2.1 (subprocess) `test_cli_subst_value_reaches_the_child` — **a NEW test of this task, added because Task 4's AC list held no test that observes the EXECUTED text on the success path while a `--subst` is in play, and binding D6's moved row to one that could not kill it is the non-discriminating-killer defect this feature has already filed twice** (`docs/03-analysis/doc-block-exec.5d-decisions.md` §D6). The document under test holds a tagged block whose body is `echo K`; the invocation is `--subst K=sentinel-value` with `--stdout <tmp_path file>`, so the CHILD's own stdout lands in the stream artifact rather than in the verdict line. Asserts: the run succeeds (`DOCBLOCK: RAN rc=0 …`, exit 0) **and** the stream artifact's bytes contain `sentinel-value` and do **not** contain the bare key `K`. **The value carries no `=`**, deliberately: that is what keeps this test from standing in for `test_subst_value_may_contain_equals`, since `subst-split-on-every-equals` leaves a no-`=` value intact and this test green. **It is the only test in this task that reads the child's stdout with a substitution applied**, which is why the refusal-rendering tests beside it — `test_cli_subst_overlap_detail_lines` and the `BAD_SUBST` / `duplicate_key` tests — cannot see D6's mutant: that mutant moves the SUCCESS path only and leaves every refusal rendering byte-identical.
...
- [ ] AC-3.9 `test_alias_refusal_unlink_failure_reports_leftover` (in-process main, **injected: `os.unlink`**), **SETTLED at `docs/03-analysis/doc-block-exec.5d-decisions.md` §D12** (was an OPEN-DECISION here; impl-plan audit v50 codex must 2). **The alias branch re-reads with `os.path.lexists` after its unlink and, on a surviving file, refuses with a `leftover:` detail line rather than a bare `UNREADABLE reason=stream_paths_alias`.** Built exactly as AC-3.10's `test_rollback_unlink_failure_reports_leftover` is built — the same `os.unlink` injection raising `PermissionError`, bound after `real_unlink = os.unlink` so the test's own `finally` can remove the leftover the injection deliberately created — with the **alias trigger** in place of that test's second-arm `ENOTDIR`: a `--stdout` that is a **fresh** path under `tmp_path` so the first arm's `O_EXCL` succeeds and `created` is True, and a `--stderr` that ALIASES it, so the refusal is `stream_paths_alias` and the branch must remove the reservation it just made. Asserts `UNREADABLE reason=stream_paths_alias`, exit 2, a `leftover:` detail line naming the reservation's path exactly, that path present and **empty**, the block not run, and no traceback. **WITHOUT THE INJECTION THIS TEST IS VACUOUS and it is stated at the AC rather than left to the implementer**: the unlink succeeds, nothing is left over, and a branch carrying no read-back at all passes it — which is exactly the shape the sibling rollback test already exists to refuse. **The alternative D12 weighed — record why the unlink cannot fail silently — is REFUTED by this document's own AC-3.10**, which injects the identical `PermissionError` into the identical `os.unlink` call on the identical kind of artifact and observes the file survive; "it cannot fail silently" is not available as an answer when the same call demonstrably can. **This AC's row question is DISCHARGED and the matrix total is 91** (v1.60): §D12 reported to the DESIGN table whether the alias read-back earns a row of its own — the sibling of `rollback-leftover-unreported`, which covers the ROLLBACK path and not this one — and **§D13(a) answered ADD, design v1.116 shipping `alias-leftover-unreported`** on the ground that `rollback-leftover-unreported`'s killer never enters the alias branch, so the alias mutant survives it. The split is **90 helper + 1 `h-mad/SKILL.md`**, the `SKILL.md` half unmoved because this row mutates the helper. **This test is that row's killer, it is LANDED** (`grep -c '^def test_alias_refusal_unlink_failure_reports_leftover' h-mad/tests/test_h_mad_doc_block_exec.py` → `1`), and it is inside this task's `--expect-fail 62`. **The row has NOT reached the spec**: `h-mad/tests/mutation-specs/doc_block_exec.json` holds 90 rows with `MATRIX-NOT-ON-DISK ['alias-leftover-unreported']`, so the harness cannot yet score this guard — the closure predicate below carries that as the one open 5e obligation.
...
- [ ] AC-4.1 `test_help_anywhere_exits_zero_without_a_verdict` (in-process main, no injection), **SETTLED at `docs/03-analysis/doc-block-exec.5d-decisions.md` §D11** (was an OPEN-DECISION here; impl-plan audit v50 must 3, filed by BOTH legs). **The contract is BROADENED, not pre-scanned: `--help` in ANY argv position yields argparse's own help at exit 0 and never reaches the `BAD_ARGS` verdict.** **The premise was re-derived on the pinned 3.11.8 before the choice was made and it is WIDER than the finding filed** — codex named two shapes and **five** hold, against the exact parser this task prescribes (`allow_abbrev=False`, `error()` overridden to raise `BadArgs`, `exit_on_error` at its default), each case its own parser instance: `--help`; `--bogus --help`; `d.md --heading H --help`; `--help --bogus`; and `d.md --help` **with the required `--heading` absent**. **The two controls are what make that a measurement rather than a probe that never reached `error()` at all**: without `--help`, `--bogus` alone raises `BadArgs('the following arguments are required: doc, -…')` and `d.md --heading` with no value raises `BadArgs('argument --heading: expected one argument')`, both routed exactly as AC-4.1 requires — those two rows stay in `test_malformed_invocation_is_a_verdict` above and are not duplicated here. **One test function over the five shapes**, each its own `main(argv)` call inside `pytest.raises(SystemExit)` and its own `capsys.readouterr()` — the shape `test_malformed_invocation_is_a_verdict` already uses, and the reason it is not written as `@pytest.mark.parametrize` is the RED-count unit stated in the split below. Asserts, per shape: `SystemExit` with `code == 0`; argparse's **usage text on stdout**; and — the discriminating clause — **no line of stdout starts `DOCBLOCK:`**. **That last clause is what makes broadening legitimate rather than a shrug**: a pre-scan implementation that emitted a verdict line *and* the help text would satisfy the first two assertions and fail only this one. **Why not the pre-scan alternative**: it re-implements argparse's own help detection including `-h`, the `--` terminator and `--help` appearing as an option VALUE (`--heading --help`, where it is data and not a request) — new surface with its own defect budget, bought to make a widely-expected CLI behave unusually — and the exemption already existed in this task's prose, which says the override "is never reached, because `--help` is not an error"; that reasoning is position-independent, so this decision widens a stated carve-out rather than creating one. The Audit-gate signal discipline is not breached, because it governs the helper's **verdict** channel and `--help` is a usage request answered on stdout at exit 0. **It adds no mutation row of its own, and the matrix total is 91** — moved 90 → 91 at v1.60 by §D13(a)'s `alias-leftover-unreported` on the unrelated alias-refusal axis, so "unmoved" is no longer the right word for this AC's contribution of zero.
...
**SETTLED (was OPEN-DECISION (r19, 5d)) — the alias-refusal branch DOES verify its unlink with a read-back, and its discriminating failure test is `test_alias_refusal_unlink_failure_reports_leftover`**, source `docs/03-analysis/doc-block-exec.5d-decisions.md` §D12 (2026-09-06), contract written at the AC-3.9 bullet above. **The premise needed CORRECTING before it could be answered, and the correction is carried rather than smoothed away**: the finding reads as though a SHIPPED branch were missing a read-back, and at `7afc0ef`/`861efbe` `h-mad/scripts/h_mad_doc_block_exec.py` is **465 lines** carrying exactly one `lexists` — the retained-cwd cleanup check, and no other — with no `O_EXCL`, no stream-path reservation, no alias detection and no rollback unlink anywhere. The reading is taken by locator rather than by offset, so it survives the very commits this task is about to make: `grep -c 'O_EXCL\|lexists' h-mad/scripts/h_mad_doc_block_exec.py` prints `1`, and `grep -n 'lexists' …` shows that one hit inside the cleanup branch. The whole stream-path apparatus is THIS TASK's deliverable, so the finding stands as a forward constraint on what Task 4 writes and never as a repair to landed code — recorded because acting on it as filed would send an implementer looking for a branch that does not exist. **The alternative answer is REFUTED, not merely rejected**: AC-3.10's `test_rollback_unlink_failure_reports_leftover` injects `PermissionError` into the same `os.unlink` on the same kind of artifact and observes the file survive, so "the unlink cannot fail silently" is unavailable as an answer. **The residual this settlement carried is now DISCHARGED, and the discharge is recorded rather than the routing deleted** (v1.60): §D12 added no row of its own and REPORTED to the design table whether the alias read-back earns a row beside `rollback-leftover-unreported` — which covers the ROLLBACK path only. **`docs/03-analysis/doc-block-exec.5d-decisions.md` §D13(a) answered it and design v1.116 shipped the answer: ADD the row.** `alias-leftover-unreported` is in the design's matrix, so **the matrix total is 91**, split 90 of the helper's source and 1 of `h-mad/SKILL.md` — the `SKILL.md` half unmoved because the new row mutates the helper. **The deciding ground is the killer's REACHABILITY, not the guard's necessity**: `rollback-leftover-unreported`'s killer drives the ROLLBACK path and never enters the alias branch, so deleting this branch's read-back leaves it GREEN and the mutant survives; a row whose killer cannot reach the mutated code is not coverage of it. **The killer named at the AC above is LANDED**, `grep -c '^def test_alias_refusal_unlink_failure_reports_leftover' h-mad/tests/test_h_mad_doc_block_exec.py` returning `1`, and it is inside Task 4's `--expect-fail 62`. **What is NOT discharged is the SPEC**: `h-mad/tests/mutation-specs/doc_block_exec.json` holds 90 rows and `MATRIX-NOT-ON-DISK` is exactly `['alias-leftover-unreported']`, so a harness run over it reports `ALL_CAUGHT` over 90 and says nothing about this guard. That gap is a 5e obligation, stated at Task 4's closure predicate below, and closing it is the 5e implementer's artifact rather than this document's.

**SETTLED (was OPEN-DECISION (r19, 5d)) — the contract is BROADENED and its boundary is TESTED by `test_help_anywhere_exits_zero_without_a_verdict`; argv is NOT pre-scanned**, source `docs/03-analysis/doc-block-exec.5d-decisions.md` §D11 (2026-09-06), contract written at the AC-4.1 bullet above. **The premise was re-derived on the pinned 3.11.8 and the POPULATION is wider than the finding filed**: codex named two help-bearing shapes and **five** short-circuit to argparse's own `SystemExit(0)` with usage on stdout — including `--help --bogus`, which it did not name, and `d.md --help` with the required `--heading` absent — while the two no-`--help` controls, `--bogus` alone and `d.md --heading` with no value, reach the `error()` override and raise `BadArgs` exactly as AC-4.1 requires. **The controls are what make this a measurement**: without them the five could have been the probe never reaching `error()` at all. **The rule this task already states was position-independent and only its stated SCOPE was narrow** — "`--help` is not an error" is true anywhere in argv — so this widens a carve-out the document already had rather than opening a new one, and the Audit-gate signal discipline is untouched because it governs the **verdict** channel while `--help` is answered on stdout at exit 0. **Residual, stated exactly**: the pre-scan alternative is declined on cost, not on principle — it would re-implement argparse's help detection (`-h`, the `--` terminator, and `--help` as an option VALUE) — so a future requirement that `--bogus --help` be a `BAD_ARGS` verdict reopens this decision rather than contradicting it. **§D11 adds no mutation row of its own**, and the matrix total is **91** — it moved 90 → 91 for a reason unrelated to this settlement, §D13(a)'s `alias-leftover-unreported` on the alias-refusal axis (design v1.116), which is why the "unmoved" phrasing v1.59 used here is retired rather than repeated (v1.60).

**Mutation rows added here**: `substitution-result-not-passed-to-run_block` (**moved here from
Task 3 and RENAMED**, `docs/03-analysis/doc-block-exec.5d-decisions.md` §D6: `main` passes the
ORIGINAL block to `run_block` instead of the block `substitute` returned, so a `--subst` key reaches
the child unexpanded. Killed by
`tests/test_h_mad_doc_block_exec.py::test_cli_subst_value_reaches_the_child`, whose stream artifact
then holds the literal key instead of the value. The old name,
`preamble-composed-with-unsubstituted-text`, was withdrawn with the row rather than carried,
because the mechanism has nothing to do with the preamble — any `--subst` success-path test kills
this mutant and none of them touches the preamble. **It is discriminated from every other `subst`
row here**: `subst-split-on-every-equals` and `subst-duplicate-key-last-wins` mis-BUILD the map and
are killed by the argument-parsing tests, while this row builds the map correctly and drops the
RESULT — the map-building tests stay green under it, and the refusal-rendering tests stay green
under it too, because it moves only the success path), `subst-split-on-every-equals`, `subst-duplicate-key-last-wins`,
...
observable output on the same fixture — 30 rows.
**30 is a RE-DERIVATION over the list above, and the addend is named**: D6 moves
`substitution-result-not-passed-to-run_block` into this task, taking it 29 → 30, and it is the only
row this revision adds here. Task 3 loses `preamble-composed-with-unsubstituted-text` in the same
move, so the FEATURE total is moved only by D7's addition and not by D6's.
...
`cli-empty-key-delegated` is discriminated from Task 2's `empty-key-accepted-by-api`, which is NOT
one of this task's rows above — **stated by membership rather than by an integer, because a
restated size beside an enumeration is a second authority and this one had already drifted: it read
"the 28 above" while the same paragraph totalled 29** — by which side is mutated: that row removes
`substitute`'s own guard and is killed by the API
...
25 + 7 + 28 + 31 = **91 rows**, split **90 of the helper's source and 1 of `h-mad/SKILL.md`**.
**Task 4's addend moved 30 → 31 at v1.60 and the assignment is by the KILLER, not by the AC number**:
§D13(a)'s `alias-leftover-unreported` (design v1.116) cites AC-3.9, but its killer
`test_alias_refusal_unlink_failure_reports_leftover` is an **in-process `main`** test and `main` is
the new symbol THIS task introduces, so the killer cannot exist before Task 4; its AC bullet is in
this task's list above, and its sibling `rollback-leftover-unreported` (AC-3.10) already sits at
index **79** of the merged spec, inside Task 4's block of indices 60–89. Tasks 1, 2 and 3 are
unmoved at 25, 7 and 28.
**Tasks 1, 2 and 3 are re-derived from the LANDED SPEC, not from this document** — `python3 -c
"import json;n=[m['name'] for m in json.load(open('h-mad/tests/mutation-specs/doc_block_exec.json'))['mutations']];i=n.index('missing-key-silently-skipped');j=n.index('strict-flags-dropped');print(i, j-i, len(n)-j)"`
prints `25 7 28` **at `c1d8255`, and `25 7 58` on the working tree this revision is written against**
— the third addend is `len(n)-j`, an OPEN tail, so it swallows Task 4's 30 rows the moment they land,
and they have landed uncommitted with Task 4's GREEN. **The command is kept and its two readings are
both stamped rather than the command being quietly rewritten**: `git show
c1d8255:h-mad/tests/mutation-specs/doc_block_exec.json` reproduces the 60-row reading, and the
open-tail form is what a reader must know to interpret either. At `c1d8255` all 60 of those rows
carry `file: scripts/h_mad_doc_block_exec.py`, 0 of them `h-mad/SKILL.md`. **The derivation is over ONE file because §D10's merge deleted the other, and the
form it replaces did not merely read stale — it could not RUN**, since `doc_block_exec_task3.json`
no longer exists; the two task boundaries are named by the rows that open Task 2 and Task 3
(`missing-key-silently-skipped` and `strict-flags-dropped`) rather than by a file split, which is
what makes the split derivable from the merged spec at all. The 60 names are distinct
(`0` duplicates). **Task 4's addend WAS the one that could not be derived that way, and half of it
now can — the halves are separated rather than presented as one measurement** (v1.60): 30 of its 31
rows are on the working tree (spec indices 60–89, landed uncommitted with Task 4's GREEN), and the
thirty-first, `alias-leftover-unreported`, is in the design's matrix and **not** in any spec —
`MATRIX-NOT-ON-DISK ['alias-leftover-unreported']`, `NOT-IN-MATRIX []`, and the comparison is
non-vacuous by control, since dropping one on-disk row takes MATRIX-NOT-ON-DISK 1 → 2. So that `31`
is 30 read from a file plus 1 claimed from the design's table. The single `h-mad/SKILL.md` row in the
90/1 split is `registry-row-removed`, at spec index **84**, inside Task 4's block — a reading of the
file now, where at v1.58 it was a claim about the design's table alone.
**SIX moves land here, each one addend, and each is named with the task it lands in** — the opener
read "Four" while the chain below already named a fifth, D9, so the header count is re-derived from
the list rather than carried, and v1.60 adds the sixth (D13(a)). The move
85 → 86 is the design's r18 addition and the +1 landed in TASK 2 — `intersect-scan-non-overlapping`,
which took that task 6 → 7. The move 86 → 87 is the design's r19 addition and the +1 landed in
TASK 4 — `intersect-kind-ignored`, which took that task 28 → 29 while Tasks 1, 2 and 3 were unmoved
...
convergence, and the disagreement the two documents passed through to reach it, is stated at the row.
**The third and fourth moves are 5d decisions, not sibling readings, and together they are ONE
addend** (`docs/03-analysis/doc-block-exec.5d-decisions.md` §D6 and §D7): D6 MOVES
`preamble-composed-with-unsubstituted-text` out of Task 3 and back in as Task 4's
`substitution-result-not-passed-to-run_block`, which is 26 → 25 and 29 → 30 and **0 on the total**;
D7 ADDS `timeout-upper-bound-removed` to Task 3, which is 25 → 26 and takes the total 87 → 88.
**The FIFTH move is D9, and its absence from this chain is what made 88 stale for one revision**
(`5d-decisions.md` §D9): it ADDS `stderr-not-closed` and `kill-skipped-after-collect-failure` to
Task 3, which is 26 → 28 and takes the total 88 → **90**. Both landed at 5e and both were in the
tree, carrying named killing tests, while this document still read 26 — the chain is written out
move by move precisely so that the next omission is visible as a missing successor rather than as a
wrong integer. **The SIXTH move is §D13(a), and it is the first in this chain that lands in a task
whose code it does NOT mutate** (`5d-decisions.md` §D13(a), shipped as design v1.116): it ADDS
`alias-leftover-unreported`, which takes Task 4 30 → **31** and the total 90 → **91**. The mutated
guard is the alias-refusal branch's `lexists` read-back, but the addend follows the KILLER —
`test_alias_refusal_unlink_failure_reports_leftover` is an in-process `main` test, and `main` is
Task 4's new symbol — and its sibling `rollback-leftover-unreported` is already at spec index 79,
inside Task 4's block. So Tasks 1 and 2 are unmoved at 25 and 7, Task 3 stands at 28 — **unmoved at 26 by
cancellation through D6/D7, then moved to 28 by D9** — and Task 4 stands at 31. **The split is re-derived, not carried**: all five moved rows —
D6/D7's pair, D9's pair and D13(a)'s one — anchor in `h-mad/scripts/h_mad_doc_block_exec.py`, read from the
landed specs' `file` field where the row is on disk and from the design's mechanism column for
`alias-leftover-unreported`, which is not, so the `SKILL.md` half stays at exactly 1
— `registry-row-removed` — and only the helper-source half grew, 86 → 87 → 89 → 90.
**The DESIGN's own matrix total was deliberately NOT restated here through v1.59**, because D6 and D7
moved it and `design-author` had not run when that revision was written, so any integer written then
would have been a reading of bytes that did not yet exist. **At v1.60 the sibling HAS run** — the
design's Version History carries D9 at **v1.114** ("§D10 answered and §D9 landed in the matrix"), the
§D10 merge at **v1.115**, and D13(a) at **v1.116** — so the reading is taken rather than deferred:
the design's own `awk` over its mechanism column returns **91**, a second census under a different
grammar returns 91 rows and 91 DISTINCT names, and the `SKILL.md`-naming-row count is **1**. Both
agree with the 91 and the 1 above. The command stays the authority and is re-taken at the commit
this batch lands rather than carried: run the plan's published `awk` over the design's mechanism
column against `docs/02-design/features/doc-block-exec.design.md` and compare its count and its
`SKILL.md`-naming-row count with the 91 and the 1 above. **A caution the D13(a) round earned**: the
design's summary paragraph spells its total in WORDS ("Ninety-one rows, ninety-one mutations"), which
no numeric `\b91\b` sweep reaches, so a total sweep over that file must carry the spelled-out form. A disagreement is a finding for that round,
not a number to reconcile by editing either side.
...
revision carried 86 rows and its r19 revision carried **87**, re-derived at that time with the
plan's published `awk` over its mechanism column, whose count was 87 and whose `SKILL.md`-naming
rows were exactly 1, so the split was confirmed against a sibling's body rather than asserted — and
that reading was stamped to the tree the r19 batch shipped, not to `0021c77`.
**That sibling reading is NOT re-taken in this revision and no successor integer is written for
it**: the 5d decisions §D6 and §D7 move the design's matrix and `design-author` had not run when
this revision was written, so a number here would be a reading of bytes that did not exist. The
reading is owed at the commit this batch lands, and it is taken by running that same `awk` against
`docs/02-design/features/doc-block-exec.design.md` — the command is the claim, and the integer is
whatever it returns then. **The reading is RE-TAKEN each round rather than carried**, which is the
...
none of which this task touches. **Expected passing is not 0. Both integers are stated here, because
`h_mad_assemble_tdd.py` REFUSES a 5d dispatch for a new-behaviour task that carries only the rule
that derives them** (halt reason `counts_required`): **`--expect-pass` = 98** and
**`--expect-fail` = 62**.
**`--expect-pass` = 98 is RUN, not carried**: the rule is unchanged — it is the `passed` figure of
Task 3's GREEN summary over this file — and at `861efbe`
`python3.11 -m pytest tests/test_h_mad_doc_block_exec.py -q` reads `98 passed in 37.42s`. The same
figure is reachable without pytest, which is the second derivation that stops a transcription
standing alone: `grep -c '^def test_' h-mad/tests/test_h_mad_doc_block_exec.py` is also **98**, and
the two agree because the file carries **0** `parametrize` markers, so one `def` is one pytest item.
**`--expect-fail` = 62, and THE NAIVE DERIVATION IS WRONG — the failure mode is recorded here rather
than left for the 5d dispatch to rediscover.** `grep -oE '\btest_[a-z0-9_]+'` over this task's AC
checklist returns **66** distinct names, and 66 must not be published: the checklist also NAMES
tests that EARLIER TASKS LANDED, cited for their mechanism or to discriminate a killer, which this
task does not add. **The classification is settled against the SHIPPED FILE and not against prose**,
because two names that look like cross-references are not: exactly **4** of the 66 are already
`def`s in `test_h_mad_doc_block_exec.py` — `test_communicate_oserror_is_launch_failed_collect`
(Task 3), `test_empty_key_is_refused_by_the_api` and `test_substitute_refuses_intersecting_spans`
(Task 2), and `test_invalid_utf8_document_is_unreadable` (Task 1) — while
`test_cli_invalid_utf8_document_is_unreadable` and `test_cli_subst_overlap_detail_lines` appear in
an EARLIER task's checklist as forward references to this one, are absent from the shipped file, and
are therefore additions of this task. Membership of an earlier checklist is not the predicate;
membership of the shipped file is. `66 − 4 = 62`. The whole derivation, re-runnable:

```
sed -n '/^## Task 4: CLI, stream artifacts, and the registry entry$/,/^## Task 5: /p' \
  docs/01-plan/features/doc-block-exec.impl-plan.md \
  | awk '/^\*\*Acceptance Criteria\*\*:/{f=1;next} f&&!/^- \[ \]/{exit} f' \
  | grep -oE '\btest_[a-z0-9_]+' | sort -u > /tmp/t4.txt
wc -l < /tmp/t4.txt                                    # 66  (distinct NAMES, not occurrences)
while read t; do grep -q "^def $t(" h-mad/tests/test_h_mad_doc_block_exec.py \
  && echo "$t"; done < /tmp/t4.txt | wc -l             # 4   (already landed — cited, not added)
# expect: 66 - 4 = 62
```

**The UNIT is TEST FUNCTIONS, and the one condition under which 62 stops being the failing-ITEM
count is stated rather than assumed**: the file's established style writes a multi-case AC as one
function iterating its cases (`test_malformed_invocation_is_a_verdict` runs two invocations in one
body), and the 98 landed tests carry 0 `parametrize` markers, so a function count IS an item count
here. An implementer who writes any of this task's ACs — `test_verdict_table_exit_codes` over its 23
heads, `test_help_anywhere_exits_zero_without_a_verdict` over its five shapes,
`test_cli_nul_composition_is_a_verdict_on_both_paths` over its two — as
`@pytest.mark.parametrize` moves the failing-item count above 62, and the integer must then be
re-derived from the collected node IDs rather than from this list.
**Neither integer carries a test twice**, which is the property both rest on: no AC of this task
adds an assertion to a test an earlier task landed, so nothing is inside `--expect-fail` and
`--expect-pass` at once.
...
`doc_block_exec.json` must report `ALL_CAUGHT` over all **91** rows
before this task is GREEN.
**THE CLOSURE PREDICATE IS NOT SATISFIABLE BY RUNNING THE HARNESS AS THE SPEC STANDS, and a 5e
reader looks here for that** (v1.60): the file holds **90** rows and the set difference against the
design's matrix is exactly `MATRIX-NOT-ON-DISK ['alias-leftover-unreported']` with `NOT-IN-MATRIX []`
— non-vacuous by control, since dropping one on-disk row takes that difference 1 → 2. So the harness
returns `MUTATION: ALL_CAUGHT mutations=90` over a 91-row matrix: **a green token scored against a
population one member short, which is not a failure the token can express.** The missing row's
killer, `test_alias_refusal_unlink_failure_reports_leftover`, is already LANDED and inside this
task's `--expect-fail 62`, so what is owed is the SPEC ROW, not a test. **Adding it is the 5e
implementer's artifact and is deliberately not done by this document**; until it lands, read
`mutations=91` — not `ALL_CAUGHT` alone — as this task's 5e gate.

**RED gate**: `hmad-dispatch run --timeout 600 -- python3.11 -m pytest tests/test_h_mad_doc_block_exec.py -q` before any production code — every Task 4 test fails and Tasks 1–3 stay green — **62 failing, 98 passing**. Judge it on the pytest summary, never on `$?` alone, and keep the recorded output beside the task as the 5d dispatch's `--out` file; `rc=124` is the wrapper's expiry, not a RED result. This is what `h_mad_assemble_tdd.py --phase red` dispatches, with `--test-path` set to the file named above, **`--expect-fail 62 --expect-pass 98`** — the integers the split above derives, carried literally because the assembler HALTS with `counts_required` on a new-behaviour task that states only the rule, while a wiring task omits them by design (Tasks 1 and 5 state their RED in prose, as the assembler allows) — `--out` the recorded report kept beside the task, and `--timeout 600`. **Re-derive both at the dispatch rather than copying them from here**: `--expect-pass` reads `test_h_mad_doc_block_exec.py` as it stands when this RED runs, so any commit adding a test to that file after `861efbe` moves it off 98, and `--expect-fail` moves with any revision that adds an AC to the checklist above.
...
hmad-dispatch run --timeout 600 -- python3.11 scripts/h_mad_mutation_harness.py tests/mutation-specs/doc_block_exec.json        # MUTATION: ALL_CAUGHT mutations=91  (reads 90 until `alias-leftover-unreported` is added to the spec — see Task 4's closure predicate)
...
- v1.56: **Phase 5d decision batch, NOT an audit cycle** — this revision answers `docs/03-analysis/doc-block-exec.5d-decisions.md` §D1, §D2, §D5, §D6 and §D7, settled 2026-09-06 against the Task 1 RED and GREEN dispatches and the Task 3 pre-dispatch review, at the freeze `acb2b56`. **No gating claim, no two-surface clean, no exit gate**: no audit leg ran against this body and none is claimed. **(1) D7 — Task 3's `BadTimeout` upper-bound OPEN-DECISION is SETTLED as (a)**: `_MAX_TIMEOUT_SECONDS = (2**31 - 1) / 1000` (== 2147483.647), written in that derived form and never as the bare literal, and the guard becomes `math.isfinite(t) and 0 < t <= _MAX_TIMEOUT_SECONDS` at all three sites this document states it (Task 3's Description, the `_validate_timeout` docstring in Task 3's Code structure, and Task 4's `main` pipeline). `OverflowError` is NOT mapped downstream, because it fires inside `communicate` — after the spawn and after `mkdtemp` — which is the leak AC-5.6's pre-spawn rule exists to prevent. ONE test is added, `test_unrepresentable_timeout_refuses_before_spawn`, with BOTH arms: refusal at `2147483.648` and a CONTROL arm at `2147483.647` on `echo hi` that RUNS, which is what stops the guard degenerating into "refuse everything large". Neither arm is a fault injection, so the injected populations 13 / 9 / 4 / 6 are unmoved. Mutation row `timeout-upper-bound-removed` added, mutually discriminating with `timeout-validation-removed` in both directions and stated so at the row. D7's residual is carried verbatim: the constant pins CPython's INT_MAX-milliseconds limit as measured on 3.11.8 / darwin, and an `except OverflowError` backstop is deliberately NOT added because an unreachable branch cannot be tested or mutation-verified. **(2) D6 — `preamble-composed-with-unsubstituted-text` is STRUCK from Task 3 and re-lands in Task 4 as `substitution-result-not-passed-to-run_block`.** The premise was re-derived: `run_block` takes ONE already-substituted `Block`, so `block.text` IS `text′` and the mutation as specified is a no-op. **Task 4's AC list held NO test that could kill the moved row** — every `--subst` test there is a refusal rendering, and the mutant moves only the success path — so rather than bind it to a test that cannot see it, this revision ADDS the test the row needs: `test_cli_subst_value_reaches_the_child` (subprocess, success path, asserts the stream artifact carries the substituted VALUE and not the key, with a value carrying no `=` so it cannot stand in for `test_subst_value_may_contain_equals`). `test_preamble_and_substitution_compose` STAYS in AC-3.11 unchanged and Task 3's test count is unmoved by D6. **(3) D2 — the Conventions mutation-spec bullet now says the spec FILE is written at 5e, not created unanchored at 5d.** The old rule was unreachable: `tests/mutation-specs/*.json` is swept by filesystem glob and `h-mad/git-hooks/pre-push` by tracked-file list, and both score an unanchored row `ANCHORS_UNREADABLE` — measured `2 failed, 96 passed` with the RED in the working tree, against a positive control of `11 passed` with both specs moved aside. The parked paths, the load-bearing `.json.pending` suffix, the post-parking `ANCHORS_OK specs=46 mutations=490` sweep and D2's residual (nothing detects a parked spec never moved back) are all carried. Consequently each task's `ALL_CAUGHT` gate is named as a 5e gate. **(4) D5 — a new Conventions bullet states the LIMIT of the new-module RED shape**: a RED whose failure mode is a COLLECTION error certifies nothing about the bodies of the tests it collects, measured at 22 of Task 1's 46 tests surfacing in the first second of the first GREEN run. It is a property of the shape, not of one fixture. **(5) D1 — the `_FenceEvent` neutral-fields rule is stated as the settled contract** in Task 1's Code structure (`marker=None run=0 indent=0 info=''` on every non-`open` event; `heading` additionally carries its own `level` and `text`), with its four reasons, its residual (a `close` event does not export the closer's own run or indentation, and recovering it is a DESIGN change) and the reason no mutation row follows. **COUNTS, EVERY ONE RE-DERIVED OVER THE LIST BESIDE IT AND NOT CARRIED.** Task 3's AC list holds **39** distinct test identifiers, derived over the FULL AC span with `sed -n "<AC-3.1 line>,<AC-5.6 line>p" | grep -oE "\btest_[A-Za-z0-9_]+" | sort -u | wc -l` — 38 before this revision, 39 after, the single addition being `test_unrepresentable_timeout_refuses_before_spawn` (`diff` of the two derivations shows exactly that one line). A per-bullet `awk` over the checklist reads only the FIRST line of each bullet and silently skips AC-5.5's continuation lines, so it cannot produce this figure. Mutation rows: Task 3 stays at **26 by CANCELLATION and not by inaction** (D6 −1, D7 +1, both addends named at the site), Task 4 goes 29 → **30**, and the feature total is 25 + 7 + 26 + 30 = **88 rows**, split **87** of the helper's source and **1** of `h-mad/SKILL.md`. Both per-task figures were re-derived from their own row lists before the edit (Task 3's block yielded 27 kebab names of which `duplicate-heading-takes-first` is a cross-reference to a Task 1 row, giving 26; Task 4's yielded 30 of which `intersect-check-removed` is a cross-reference to a Task 2 row, giving 29) so the shipped figures move a verified base. **THREE PRE-EXISTING DEFECTS FOUND WHILE RE-DERIVING, REPAIRED OR FILED RATHER THAN ABSORBED.** (a) The payload-timing bullet read "**85** of them anchor in `h_mad_doc_block_exec.py` … the eighty-sixth is `registry-row-removed`" against a stated total of 87 and a stated split of 86 + 1 — an arithmetic disagreement between two sentences of the same document; it now reads 87 and eighty-eighth, on the re-derived base. (b) Task 4's discrimination sentence read "which is NOT one of the 28 above" in a paragraph that totals 29; the integer is REPLACED by membership rather than re-typed, because a restated size beside an enumeration is the second-authority class this document already forbids. (c) **FILED, NOT DECIDED**: `timeout-validation-removed` drops the whole predicate from the ONE validator both `run_block` and `main` call, so its mutant reds Task 4's `test_cli_bad_timeout_values` as well as its canonical key — which makes it a member of the "rows whose mutant reds a second named test" enumeration by that enumeration's own scope sentence, and it is not listed. **It was already a member before this revision, on the CLI half alone**, so this is a finding against the enumeration and not a defect D7 introduced; the count is left as the round last derived it and the question is reported rather than resolved on an author's authority. **WHAT THIS REVISION DELIBERATELY DOES NOT WRITE.** The DESIGN's matrix total is not restated at either site that carried it. D6 and D7 move it and `design-author` had not run when this was written, so any integer here would be a reading of bytes that do not exist; both sites now name the plan's published `awk` over the design's mechanism column as the command that takes the reading at the commit this batch lands, and a disagreement with the 88 above is a finding for that round rather than a number to reconcile by editing either side. **No `.py:` line pin is added by this revision and none is retired** — the D2 bullet reaches `test_h_mad_mutation_harness.py` and `git-hooks/pre-push` by `grep` locator precisely so the four `.py:` screens and their 23 folded paragraphs are not moved by it — and no new blank-line-separated paragraph is opened inside a pin-bearing bullet. **OWED ELSEWHERE, recorded in `docs/03-analysis/doc-block-exec.5d-decisions.md` §D6 and §D7 rather than as body prose about a sibling's current bytes**, which is what this document's own Conventions require: `spec-author` owes AC-5.6's wording, and `design-author` owes the guard sentence, the `_MAX_TIMEOUT_SECONDS` constant, the `timeout-upper-bound-removed` row, the rewrite of the row D6 moved (name, mechanism and killer all three), and the matrix total re-derived from its own table. **PRECHECK ON THE SHIPPED BODY, RUN AFTER THE LAST EDIT AND AGAINST A BASELINE IN THE SAME PASS, BECAUSE AN ABSOLUTE ALONE CANNOT SAY WHO MOVED IT**: `PRECHECK: FAIL issues=19` — 11 `PLACEHOLDER`, all of them the FR-4 grammar's unfilled detail-line slots, and 8 `PINDRIFT`. The same command against `git show acb2b56:<this file>` returns **the identical `FAIL issues=19`, 11 + 8**, so **this revision introduces ZERO precheck issues and moves the floor by 0** — the differential is the measurement and the absolute is not. **The floor ROSE from the `issues=11` v1.55 published, and it rose without this document changing**: all 8 `PINDRIFT` are pre-existing `.py:` pins into `h-mad/tests/docsections.py`, `h-mad/tests/test_docsections.py` and `h-mad/tests/test_h_mad_audit_cycle.py`, flagged because Phase 5c/5d COMMITS moved those files under a provenance stamp of `0021c77` that this document has not re-taken. That is the open 5d item (`docs/03-analysis/doc-block-exec.5d-decisions.md` §D3) observed in the field rather than predicted: a document whose pins point into the very files its feature is landing will drift at every GREEN, and re-stamping the provenance is a decision for the round, not an author's edit — so it is reported and deliberately not done here.
- v1.57: **Phase 5e document round, NOT an audit cycle** — this revision answers `docs/03-analysis/doc-block-exec.5d-decisions.md` §D10 and the round call recorded against the collateral-red enumeration, at the freeze `6a1693c`. **No gating claim is made and no audit report is answered.** **(1) §D10 ANSWERED: the per-task spec file is STAGING, not a convention — `doc_block_exec.json` is meant to hold every row of Tasks 1 through 4, and the merge is owed by the IMPLEMENTATION at 5e, not by any document.** The question was which of two readings is true, and it is settled by the file that already exists rather than by either document: `python3 -c "import json;print([m['name'] for m in json.load(open('h-mad/tests/mutation-specs/doc_block_exec.json'))['mutations']])"` prints **32** names that are exactly Task 1's 25 followed by Task 2's 7, and the file's own `_why` reads `Task 1 and Task 2 mutation contracts` — **one file already holds TWO tasks' rows, which refutes a per-task convention outright**, because under such a convention Tasks 1 and 2 would be two files and they are one. Three further readings agree and none disagrees: the two files' row-name sets are DISJOINT (`overlap: []`) and their `command` targets the SAME test file, so this is one spec split in two rather than two specs; `doc_block_exec_task3.json` is the only `*_task[0-9]*.json` among the **42** specs in `h-mad/tests/mutation-specs/` (`ls h-mad/tests/mutation-specs/ | grep -cE '_task[0-9]+\.json$'` prints `1`); and the commit that created it (`ebd8e27`) justifies its TIMING under D2 and offers no per-task rationale at all. **So impl-plan:19's three-spec enumeration is correct as written and NO total moves in this revision.** Classified before deciding, exactly as §D10 required: at the freeze `6a1693c` the body of this document held **18 occurrences of `88` on 16 matching lines** (unit stated because a `grep -n` stream would have counted the line-number prefixes as hits, and did on a first attempt), every one of them the mutation-matrix total, none a line number, and none of them moves. **That is a count over a class this revision itself adds to, so it is re-derived after the write rather than frozen**: the body now holds **19 occurrences on 17 matching lines** — `awk '/^## Version History/{exit}{print}' <this file> | grep -oE '\\b88\\b' | wc -l` and the same pipeline with `grep -cE` — the one addition being the sentence at the settled membership question that says the `88` did NOT move. No pre-existing occurrence was touched. The separate question of whether that total is still **88** after D6/D7/D9 moved Task 3's row count is NOT this revision's and is deliberately untouched. **What this revision DOES owe the document is the merge obligation made checkable**, added to the Conventions bullet beside the parked-spec residual it is the sibling of. **(2) The collateral-red enumeration is resized SIX → SEVEN.** `timeout-validation-removed` is the seventh member and was already one before this round, on the CLI half alone; the previous revision recorded the finding and left the count for a round to decide, and the round decided it. **Its membership is MEASURED, not asserted** (decision Q): the mutant applied to a scratch copy of `h-mad/{scripts,tests}` and the module suite run there reds `test_nonpositive_timeout_refuses_before_spawn` (its canonical key) **and `test_unrepresentable_timeout_refuses_before_spawn`** — `3 failed, 95 passed`, of which one failure is an artifact of the scratch tree, not of the mutant. **The positive control was run before the reading was believed**: the UNMUTATED copy is `1 failed, 97 passed`, failing on `test_block_leaves_the_working_tree_untouched` alone, which shells `git status` and cannot pass outside a repository — so that failure is the harness and the mutant's true collateral is one further test today. **The discriminating control was run too**: `timeout-upper-bound-removed` on the same scratch is `2 failed, 96 passed`, reding only `test_unrepresentable_timeout_refuses_before_spawn` and that same artifact, so it is NOT a member and the document's claim about it stands. Task 4's `test_cli_bad_timeout_values` **does not exist yet** (`grep -n 'def test_cli_bad_timeout_values' h-mad/tests/test_h_mad_doc_block_exec.py`, no hit) and its red is a claim about the finished feature, marked as such at the member rather than published as a measurement. **No scope sentence is added**: `spawn-valueerror-unmapped` is already cross-task at this bullet, and a scope sentence narrowing the enumeration to one task would silently drop its third red. **(3) D3 is untouched and stays red**: the impl-plan's own noise floor is settled at 5g by operator decision, `assert 19 <= 12` at `aa89d62`, and nothing in this revision implies the suite is green.
\n- v1.58: **The cross-document reconciliation of the same 5e round, and it CORRECTS v1.57** — four authors answered §D10 in parallel and the design author's reading was right where mine was wrong, which is recorded here rather than smoothed over. **THE MATRIX TOTAL IS 90, NOT 88, AND v1.57 PUBLISHED IT STALE.** The number's authority is the design's Test Plan table; this document MIRRORS it; the file that realizes it is `doc_block_exec.json`. §D10's answer (the per-task file is staging, the merge is the implementation's) is untouched by this and stands — the two questions are orthogonal, which is why one could be answered correctly while the other stayed wrong. **The correction was settled by four commands, not by majority**: (1) the two rows D9 added, `stderr-not-closed` and `kill-skipped-after-collect-failure`, appear on **0** lines of this document at the freeze `6a1693c` and on **0** before this revision, against **3** matching lines each in the plan and in the design; (2) this document's own accounting chain ENDED AT D7 — the sentence at the arithmetic reads "D7 ADDS `timeout-upper-bound-removed` to Task 3, which is 25 → 26 and takes the total 87 → 88" and had no D9 successor, so **88 is the pre-D9 accounting by its own text**; (3) the set walk closes to zero — this document's Tasks 1-3 were 25 + 7 + 26 = **58**, the landed specs hold 32 + 28 = **60**, and the difference of 2 is exactly those two rows; (4) the split reconciles INDEPENDENTLY, from the specs' `file` field rather than from either document — both D9 rows carry `file: scripts/h_mad_doc_block_exec.py`, so 87 + 2 helper and 1 `h-mad/SKILL.md` is **89 + 1 = 90**, the design author's figure to the digit. **Every task row count in this revision is re-derived from the LANDED specs, not from this document**, because the one premise that broke was mine: v1.57 took Task 3's `26` from this document's own enumeration without checking it against the spec that had already landed. `python3 -c "import json;a=json.load(open('h-mad/tests/mutation-specs/doc_block_exec.json'))['mutations'];b=json.load(open('h-mad/tests/mutation-specs/doc_block_exec_task3.json'))['mutations'];i=[m['name'] for m in a].index('missing-key-silently-skipped');print(i, len(a)-i, len(b))"` prints `25 7 28`. **Task 4's 30 is the one figure that CANNOT be re-derived this way and is marked as such at the arithmetic**: Task 4 has not landed, no spec file holds its rows, and 0 of the 60 landed rows target `h-mad/SKILL.md` — so the `1` in the 89/1 split is entirely inside Task 4's unlanded 30 and is a claim about the design's table, not a measurement. **Classification, run before any value moved**: the body holds **19 occurrences of `88` on 17 matching lines**, and **19 of 19 are the matrix total** — none is a line number, none is an unrelated population. That matters because the plan carries four unrelated `88` populations and a value sweep there would have been wrong; here it would not have been, and the classification is what establishes that rather than assumes it. Sites moved: the Executive Summary, the enumeration derivation rule and its stamp, the payload-fixing bullet, the anchor-axis bullet, the two grammar-axis bullets, three AC-level "matrix total at this batch" statements, the arithmetic and its accounting chain, the design-comparison instruction, Task 4's gate and the 5e command block. **The Executive Summary was also a MIRROR-SITE defect independent of the number** — it phrased a table total as a file row count, which is what let a stale mirror read as a measurement of the file; it now names the design table as the authority and the file as what realizes it. **Task 3's enumeration moves 26 → 28 and names the two D9 rows**, and reading them for collateral rather than assuming they added none PRODUCED A MEMBER: `kill-skipped-after-collect-failure` reds `test_poll_oserror_is_launch_failed_collect` — another row's canonical key — so the collateral-red enumeration moves SEVEN → **EIGHT** in the same revision that moves the total. Its sibling `stderr-not-closed` was read the same way and is NOT a member (`2 failed, 96 passed`, its own key only, against `3 failed, 95 passed` for the member; in both, one failure is the scratch-tree artifact fixed by the unmutated control at `1 failed, 97 passed`), which is the control that stops a pair being admitted as a pair. **This is the second enumeration this round found stale for the same reason — a derivation taken over a population that later grew — and it is why the size now lives in ONE place with every other mention a pointer.** **Post-write screens, body-scoped and re-run rather than inferred by subtracting from the pre-write reading** (a partition's total tells you nothing about its parts): `awk '/^## Version History/{exit}{print}' <this file>` piped to `grep -oE '\b88\b' | wc -l` and to `grep -cE '\b88\b'` reads **9 occurrences on 9 lines**, and the same pair on `\b90\b` reads **26 on 24**. **All 9 surviving `88`s were classified individually and NONE is a live total**: four are waypoints in the accounting chains this revision extended (`87 → 88`, `88 → 90` in two places, `reached 88`), three are stamps on what v1.56's derivation read, and two say in terms that 88 was published stale. The two readings are not comparable by subtraction because this revision WROTE new `88`s as chain waypoints while retiring live ones — which is exactly why both are published with their classification instead of one difference. **D3 is untouched and stays red.**
- v1.59: **Phase 5d contract round for Task 4, NOT an audit cycle**, at the freeze `861efbe`. **No gating claim, no two-surface clean, no exit gate**: no audit leg ran against this body and none is claimed. Three settlements land here together because all three fall inside Task 4 and opening that task twice is what this round exists to avoid. **(1) §D10's MERGE IS EXECUTED AND THE PER-TASK SPEC FILE IS GONE** (operator decision `8ef45fc`, executed at `7afc0ef`, execution record at `8bc2fa7`). v1.58 recorded the merge as a checkable 5e obligation; that obligation is DISCHARGED and this document's own closure predicate now returns 0: `ls h-mad/tests/mutation-specs/ | grep -cE '_task[0-9]+\\.json$'` prints `0` at this freeze, against `1` when v1.57 first ran it, and the directory holds **41** specs where it held 42. The four body sites that named `doc_block_exec_task3.json` are repaired **by what each sentence CLAIMS, not by substituting a filename**: two are stamped historical readings at `6a1693c` and keep their sha (a reading stamped at a blob stays stamped there), one asserted a present-tense property of the tree and now carries both the stamped past reading and the current one, and one was an EXECUTABLE derivation that the deletion BROKE — the two-file one-liner deriving `25 7 28` cannot run at all now, and is replaced by a single-file form re-derived here rather than retyped: `python3 -c "import json;n=[m['name'] for m in json.load(open('h-mad/tests/mutation-specs/doc_block_exec.json'))['mutations']];i=n.index('missing-key-silently-skipped');j=n.index('strict-flags-dropped');print(i, j-i, len(n)-j)"` prints `25 7 28`. The merged file holds **60** rows, name-duplicates **0**, and all 60 carry `file: scripts/h_mad_doc_block_exec.py`. **The matrix total is UNMOVED at 90** — 60 landed plus Task 4's and Task 5's 30 unwritten — and no total moves in this revision. **[Bracket-correction, v1.60: TRUE when written at `861efbe`, and SUPERSEDED — design v1.116 answered §D13(a) by adding `alias-leftover-unreported`, so the total is 91, split 90 helper + 1 `h-mad/SKILL.md`, and the +1 lands in Task 4's addend (30 → 31). The entry is stamped history and is corrected here rather than rewritten.]** **(2) §D11 SETTLED — BROADEN**, and the premise it rests on is WIDER than the finding filed: on the pinned 3.11.8 **five** help-bearing argv shapes short-circuit to argparse's own `SystemExit(0)` with usage on stdout, not the two codex named, and the two no-`--help` controls reach `BadArgs` correctly, which is what makes the five a measurement rather than a probe that never reached `error()`. The OPEN-DECISION line is replaced by contract and Task 4 gains **`test_help_anywhere_exits_zero_without_a_verdict`**, over those five shapes, asserting `SystemExit(0)`, usage on stdout, and — the discriminating clause — **no line starting `DOCBLOCK:`**, since a pre-scan that emitted a verdict AND help would pass a weaker test. **(3) §D12 SETTLED — REQUIRE the read-back, and the finding's PREMISE needed correcting before it could be answered**: it reads as though a shipped branch lacked a read-back, and at this freeze `h-mad/scripts/h_mad_doc_block_exec.py` is **465 lines** carrying exactly one `lexists` — the cwd cleanup at `:457` — with no `O_EXCL`, no reservation, no alias branch and no rollback unlink, so the whole stream-path apparatus is Task 4's own deliverable and the finding is a forward constraint rather than a repair. The alternative answer is REFUTED by this document's own AC-3.10, which injects `PermissionError` into the same `os.unlink` on the same kind of artifact: "it cannot fail silently" is not available when the identical call demonstrably can. Task 4 gains **`test_alias_refusal_unlink_failure_reports_leftover`**, built as AC-3.10's rollback test is, with the alias trigger instead of the second-arm `ENOTDIR`; **without the injection the test is vacuous** and is stated so at the AC. **No mutation row is added for either settlement and the total stays 90** — whether the alias read-back earns its own row is the DESIGN's table to answer, and it is reported rather than decided here. **[Bracket-correction, v1.60: the report was ANSWERED. §D13(a) settled it ADD, design v1.116 shipping `alias-leftover-unreported`, on the ground that `rollback-leftover-unreported`'s killer never enters the alias branch; the total is 91. §D11 still adds no row of its own. Stamped history, corrected not rewritten.]** **(4) TASK 4's RED COUNTS ARE NOW INTEGERS, which is what `h_mad_assemble_tdd.py` refuses a 5d without** (`counts_required`): `--expect-pass` is **98**, RUN at this freeze rather than carried — `python3.11 -m pytest tests/test_h_mad_doc_block_exec.py -q` reads `98 passed in 37.42s` — and `--expect-fail` is **62**. **The naive derivation is WRONG and the failure mode is recorded rather than left to be rediscovered**: `grep -oE '\\btest_[a-z0-9_]+'` over Task 4's AC checklist returns **66** distinct names after this revision (64 before it), but that population includes tests earlier tasks LANDED and this task only cites. The classification is settled against the SHIPPED FILE, not against prose — `grep -c '^def test_'` over `h-mad/tests/test_h_mad_doc_block_exec.py` is 98 defs, and exactly **4** of the 66 are already among them: `test_communicate_oserror_is_launch_failed_collect` (Task 3), `test_empty_key_is_refused_by_the_api` and `test_substitute_refuses_intersecting_spans` (Task 2), `test_invalid_utf8_document_is_unreadable` (Task 1). 66 − 4 = **62**. **Two names that LOOK like cross-references are not**: `test_cli_invalid_utf8_document_is_unreadable` and `test_cli_subst_overlap_detail_lines` appear in an earlier task's checklist as forward references to this one and are absent from the shipped file, so both are Task 4 additions — which is why membership of an earlier CHECKLIST is not the predicate and membership of the shipped FILE is. **The dispatch brief's characterisation of `test_rollback_unlink_failure_reports_leftover` as a Task-3 cross-reference is FALSE against the tree and is not carried**: it is absent from the shipped file, it is an in-process `main` test, and `main` does not exist until this task. **The unit is TEST FUNCTIONS, and the condition under which 62 stops being the item count is stated**: the shipped file carries **0** `parametrize` markers and its 98 defs equal its 98 pytest items, so under this file's established style a function count IS an item count; an implementer who writes any Task 4 AC as `@pytest.mark.parametrize` moves the failing-ITEM count above 62 and the integer must be re-derived. **`--expect-pass` = 98 rests on "none of which this task touches"**, which stays literally true: no Task 4 AC adds an assertion to an inherited test, so no test is carried in BOTH counts. The RED gate paragraph now carries both integers. **D3 is untouched and stays red.**
- v1.60: Phase 5e routing repair for §D13(a) -- NOT an audit cycle, at the freeze `c1d8255` plus the UNCOMMITTED Task 4 GREEN in the working tree (helper, `h-mad/SKILL.md`, mutation spec). **No gating claim, no two-surface clean, no exit gate**: no audit leg ran against this body and none is claimed. **§D13(a) is SETTLED** (`docs/03-analysis/doc-block-exec.5d-decisions.md`): design v1.116 added the mutation row `alias-leftover-unreported`, so **the matrix total is 91, split 90 of the helper source and exactly 1 of `h-mad/SKILL.md`**. The question v1.59 REPORTED as the design table\s to answer is DISCHARGED, and every site that reported it now says so and names what discharged it, rather than having the routing deleted -- a reader who finds a discharge missing cannot tell it from a routing quietly abandoned. **Counts re-derived here at this tree, not carried from the dispatch**: the design\s own awk census over its matrix returns **91**; a second census under a different grammar (python, first backticked cell of every row) returns **91 rows and 91 DISTINCT names**, agreeing; the on-disk spec `h-mad/tests/mutation-specs/doc_block_exec.json` holds **90** rows, `NOT-IN-MATRIX []` and `MATRIX-NOT-ON-DISK [alias-leftover-unreported]`; and the reading is NON-VACUOUS by control -- dropping one on-disk row takes MATRIX-NOT-ON-DISK from 1 to 2. **The +1 lands in TASK 4 addend, 30 -> 31, and the reason is the killer not the AC number**: the design cites AC-3.9, but the row\s killer `test_alias_refusal_unlink_failure_reports_leftover` is an in-process-`main` test and `main` is the new symbol Task 4 introduces, so the killer cannot exist before Task 4; its AC bullet sits in Task 4 Acceptance Criteria list above, and its sibling `rollback-leftover-unreported` (AC-3.10) is already at index 79 of the merged spec, inside Task 4 block of 60..89. So `25 + 7 + 28 + 31 = 91`. **A 5e GAP is stated because nothing else in the tree states it**: the spec is one row short of the matrix, so a harness run over it reports `ALL_CAUGHT` over 90 rows and certifies NOTHING about the alias read-back guard §D12 requires. Adding the row is the 5e implementer artifact and is deliberately not done here. **A published derivation no longer prints its stated output on the working tree and is re-stamped rather than quietly corrected**: the boundary command that `prints 25 7 28` still prints that at `c1d8255`, and prints `25 7 58` on this working tree, because Task 4 30 rows landed uncommitted and the third addend now swallows them. **v1.59 two `unmoved at 90` claims are stamped history and were TRUE when written at `861efbe`**; they are bracket-corrected in place, never rewritten.
- v1.61: **Phase 5g provenance re-stamp (§D3), NOT an audit cycle** — no audit leg ran against this body, no gating claim and no exit gate is made here. The provenance moves `0021c77` to `ed2f1c5` (Task 5 GREEN), which is the decision the round took after Phase 5 closed (Tasks 1-5 shipped, 5e gates closed, 5f `WIREREG: PASS registered=14 verified=14 broken=0`). **The hazard the re-stamp carries was discharged before the stamp was written, not after**: re-stamping asserts every pin is sound at the new sha, so all 20 `PINDRIFT` pins were re-read at BOTH shas in one pass before a byte was edited. **20 of 20 hold at `0021c77`** — that is what makes it the epoch rather than an arbitrary base — and **2 of 20 also hold at `ed2f1c5`** (`test_the_live_phase5_section_extends_past_its_fenced_blocks` in `h-mad/tests/test_docsections.py`, and the `docs/01-plan` glob inside `REAL_AUDIT_REPORTS` in `h-mad/tests/test_h_mad_audit_cycle.py`; neither file carries a source change from this feature). The remaining **18 fail at `ed2f1c5` for reasons this document PLANNED**: Task 1 deleted `_fence_aware_end` and the local `re.search` inside `titled_section` from `h-mad/tests/docsections.py`, and Task 5 hoisted `_gate_block`, `_gate_bash_block` and `_run_recipe` to module level, dropped the one-line docstring and the hand-rolled `re.findall`, and shifted the module head of `h-mad/tests/test_h_mad_collect_report_docs.py` by one line. **NOT ONE of the 18 was renumbered.** Most of their targets no longer exist, so a renumber would be a fabrication; they are declared PRE-implementation readings stamped at `0021c77`, in a new Conventions paragraph that also states the cost — after the re-stamp `PINDRIFT` is blind to this class, so the epoch is carried in prose and made re-derivable in two commands. **COUNTS THIS REVISION COULD MOVE, RE-DERIVED AFTER THE LAST EDIT RATHER THAN ASSUMED.** The four `.py:` screens read **56 / 56 / 0 / 0** at **23** folded pin-bearing paragraphs on the shipped body, identical to the v1.55 reading, because the new paragraph is written with structural locators and bare symbol names and adds no `.py:` line pin at all — deliberate, since a paragraph explaining the pin census must not join it. **PRECHECK ON THE SHIPPED BODY: `PRECHECK: FAIL issues=11`** — the 11 FR-4 grammar detail-line slots alone, `PINDRIFT` **0**, down from `FAIL issues=31`, which clears the D3 noise-floor gate.
- v1.62: **Phase 6a-prime documentation repair, NOT an audit cycle**, at HEAD `b1c8870` (tree clean of tracked changes). No gating claim, no two-surface clean, no exit gate is made here: no audit leg ran against this body. Converts the ONE remaining `OPEN-DECISION (r19, 5d)` at the AC-2.8 boundary (former L2894) to SETTLED, answering 6a-prime's single Important — documentation drift, not a code defect: the shipped tree settled the API/CLI key-validity question at `92ae93a` with one shared predicate `_is_valid_substitution_key` (definition `h-mad/scripts/h_mad_doc_block_exec.py:304`, API call site `:313`, CLI call site `:766`) while the plan still declared it open. The entry records the resolution (shared predicate, caller-specific diagnostics), REFUTES the cross-surface equivalence alternative with the Phase-1 `_dbe.fence_aware_end` precedent rather than merely declining it, names the routing pin `test_empty_substitution_key_validation_routes_api_and_cli_through_one_predicate` and the orchestrator's both-direction mutation verification, warns that an equivalence-shaped test passes on the pre-fix tree, and states the residual (the predicate covers the empty-key rule ONLY; a future rule belongs inside it). **No count, no test name, no mutation row and no task shape moves**: tasks and the 91-row matrix are untouched. Precheck unmoved at `FAIL issues=11`, the same eleven FR-4 grammar specimens.
- v1.63: **Pin re-derivation after an unrelated tooling commit, NOT an audit cycle** — no audit leg ran against this body, no gating claim and no exit gate is made here. The provenance moves `b1c8870` to `0fa825c`, the commit that added Phase 7f and the Agent-tool size tier to `h-mad/scripts/h_mad_assemble_audit.py`. Two of this document's three pins into that file were genuinely stale and are re-derived: `_trim_version_history` moved `:264` -> `:316` (measured `grep -n 'def _trim_version_history'` at `0fa825c`). The third, `_braces_outside_fences:109`, did **not** move and is unchanged — `PINDRIFT` fires on the FILE changing, not on the line moving, so re-deriving all three because three were flagged would have republished a correct pin as if it had been wrong. This is the drift §7's own note anticipates: these pins name a moving value in another file, and the instruction there is to re-derive them rather than trust them.
