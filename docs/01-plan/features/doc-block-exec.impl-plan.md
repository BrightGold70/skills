...
- **No sentence in this document states what a sibling document currently says.** A debt or a
  ...
  size, and the marker is written unbolded — `member (5)` — so that a reference is never counted
  as a member by the marker screen `grep -oE '\*\*\([0-9]\)\*\*'`, which reads **5** over the
  enumeration below on the tree this revision ships.
  ...
  **v1.40 wrote that this document "meets [these conditions] everywhere", and it did not.** Two
  ...
  returns **nothing — 0 occurrences** — against this document at `4e4a00c`, the freeze sha this
  revision is authored against and the commit every figure below was re-derived at, and **0** again
  ...
  **path-qualified line-scoped / path-qualified folded / bare line-scoped / bare folded**.
  ...
  the **paragraph** count rose 19 → 20 because a fenced block this revision inserted split one
...
**Both are INHERITED-UNVERIFIED and they are labelled so here, at the site, rather than only in a
Version History item** (impl-plan audit v41 should-fix). They were measured at `1861157` and have
now gone **three consecutive audit rounds** — v1.41, v1.42/v1.43 and v1.44 — with no surface, on
either leg, re-running them; the same is true of the plan's 263/76/0 and 268/76/0 heading
differentials and their markdown-it-py CommonMark oracles. Not challenged is not the same as
verified, and neither figure is re-derived in this revision: running them costs a full
`--collect-only` pass over the suite, which this authoring pass did not take. They are therefore
transcribed, stamped at the commit they were measured at, and named as owed — not asserted. The
