...
> Source: docs/02-design/features/doc-block-exec.design.md (post-audit, v1.100 — the revision answering design audit v90, its gating round)
> Paired spec: docs/01-plan/features/doc-block-exec.spec.md (v1.60) · paired plan: docs/01-plan/features/doc-block-exec.plan.md (v1.95)
> All three re-derived at `68a70d6` on 2026-09-04 at the moment this revision was written, with `git show 68a70d6:<doc> | grep -oE '^- v1\.[0-9]+' | tail -1` — read out of the **commit**, not the working tree, because the three sibling documents were being revised concurrently by their own authors while this revision was written, and a working-tree read would pin a document no commit contains. The three sibling authors are working **concurrently**, so any of these may be one behind by the time this is read — that is drift, not a finding. v1.35 pinned v1.92/v1.53/v1.85 and all three moved within the same session; v1.36 pinned v1.93/v1.55/v1.86 of which two moved again by `a8e0372`; v1.38's v1.93/v1.56/v1.87 was one behind on the design and two behind on the plan by `335f535`, with only the spec unmoved; v1.40's v1.95/v1.57/v1.90 was one behind on **all three** by `35698f9`; v1.41's v1.96/v1.58/v1.91 was again one behind on **all three** by `6f0ee85`, all three having moved in that commit; v1.42's v1.97/v1.59/v1.92 was one behind on **all three** at `cf3a862` — the **third consecutive** revision behind on every sibling, and `git diff --name-only 6f0ee85 cf3a862 --` names all three sibling files, so all three moved across that span too; and v1.43's v1.98/v1.60/v1.93 was one behind on the **design and the plan** at `4e4a00c` and **correct on the spec** — the first revision in six not behind on all three; and v1.44's v1.99/v1.60/v1.94 is one behind on the **design and the plan** at `68a70d6` and again **correct on the spec**, two consecutive revisions not behind on all three (`git diff --name-only 4e4a00c 68a70d6 -- docs` names the design and the plan and does **not** name the spec). Measured, not inferred: `git diff --name-only cf3a862 4e4a00c --` over the spec is **empty**, and the spec's last version line reads v1.60 at `8909ec4`, `cf3a862`, `7982c18` and `4e4a00c` alike, against v1.59 at `6f0ee85`. That is the measurement behind this sentence, not a supposition.
>
...
- **When each row's payload is fixed — deliberate, not an omission** (impl-plan audit v20, whose
  ...
  one hit, verified at `4e4a00c`: exact `find` anchors are set from the landed source **in the same
...
- **Single source of the fence grammar**: **within the two files this feature touches —
  ...
  **none is a migration target for this feature**:
  - `_titled_section` (`h-mad/tests/test_h_mad_context_budget_docs.py:69`) bounds on
    ...
    — one hit, verified at `4e4a00c`: it anchors on a **substring**, so
...
- **No sentence in this document states what a sibling document currently says.** A debt or a
  ...
  **Residual, demonstrated rather than argued**, on copies under `mktemp -d` with the repository
  ...
  `sed` — `*` is a repetition operator in BRE and `sed` refuses the pattern outright, which is how
  the first attempt at the second mutation silently produced an empty file and was caught by its
  ...
  **v1.40 wrote that this document "meets [these conditions] everywhere", and it did not.** Two
  ...
  `68a70d6`, the freeze sha v1.45 is authored against and the commit every figure below was
  re-derived at, and **0** again
  ...
  The `docs/` figures are the ones that moved and each of those is re-derived at `4e4a00c` in this
  revision. Retained for the earlier chain — `git diff --name-only 8909ec4 cf3a862 -- <this file>` is empty, as is the same diff over
  ...
  **The residual, stated exactly**: the one-hit property is true only at the commit it was
  measured at, so it is re-established every revision or not at all. **Re-swept for v1.44 at
  `4e4a00c`, every one of the 13 re-run in this revision rather than carried from v1.43's sweep or
  from any report: all 13 return exactly one hit** — the fifth consecutive revision at 13/13, and
  ...
  that commit** — `git show 4e4a00c:<sibling> | grep -c -- '<needle>'` — and not against the working
  ...
  the sha v1.42 swept at and this one —
  `git diff --name-only 6f0ee85 cf3a862 -- docs/01-plan/features/doc-block-exec.plan.md
...
`grep -n 'Heading selector differential' docs/01-plan/features/doc-block-exec.plan.md`, one hit, verified at `4e4a00c`).
...
(one hit, verified at `4e4a00c`). The `SUBST_OVERLAP` detail line is `overlap: "<a>" "<b>"` with **both** halves
...
(one hit, verified at `4e4a00c`). Impl-plan v1.24 flagged those two rows as bare and half-quoted and **design
v1.81 answered it** — located with
``grep -n 'both halves of `overlap:`' docs/02-design/features/doc-block-exec.design.md``
(one hit, verified at `4e4a00c`) — so the flags were withdrawn at v1.35. The three `design.md:` line numbers this paragraph carried
...
**re-run at `4e4a00c` in this revision rather than inherited**, because the design moved to a new
revision again (v1.98 → v1.99) and a count over a moved table is not a carried count: over the data rows
...
verified at `4e4a00c`) — count those whose mechanism column names
...
hit, verified at `4e4a00c`), so 80 + 1. The 81 are **81 distinct row names** — the count is over
`sort -u`'d names, not over lines, so a duplicated row could not inflate it — and every one of the
81 occurs in this document, checked at `4e4a00c` by testing each design row name for membership in
this file: **0 missing**.
...
hit, verified at `4e4a00c`). It is `registry-row-removed`'s partner **by
...
measured, not inherited. Two things are **not** verified by that run and are not laundered by it:
the historical values **2748** and **2486** at `b7d0d77`, which would need a checkout of that
commit this authoring pass did not take; and the plan's 263/76/0 and 268/76/0 heading
...
exactly one hit (verified at `4e4a00c`), and that hit is the line to read. A previous
...
paragraph, located with `grep -n 'Bounds: 1200 s' docs/01-plan/features/doc-block-exec.plan.md` (one hit, verified at `4e4a00c`),
