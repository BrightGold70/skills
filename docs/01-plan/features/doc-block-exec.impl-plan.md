...
- **Mutation spec** `h-mad/tests/mutation-specs/doc_block_exec.json`: `root` is `../..`,
  ...
  read each of the 8 `doc_block_exec_wire.json` rows, the 8 `docsections.json` rows and the 88
  `doc_block_exec.json` rows for a per-row clause of the form *goes red with it* / *reds too* /
  *stays a regression test, not this row's key*, and take the members from that reading. **The 88 is
  the v1.56 reading and the wire-side result below is v1.54's, and the two stamps are separated here
  ...
  **A PRE-EXISTING membership question this revision makes visible and deliberately does NOT decide
  on its own authority**: `timeout-validation-removed` drops the WHOLE predicate from the ONE
  validator both `run_block` and `main` call, so its mutant reds Task 4's
  `tests/test_h_mad_doc_block_exec.py::test_cli_bad_timeout_values` as well as its canonical key
  `test_nonpositive_timeout_refuses_before_spawn` — and, once D7's test exists, that one too. By the
  scope sentence below ("the only rows whose mutant reds a second named test") it is a member and it
  is not listed, and it was already a member before this revision, on the CLI half alone. **That is a
  finding against the enumeration, not a defect D7 introduced**, and changing the enumeration's size
  is a decision for the round rather than for this edit; it is reported with this revision and the
  count below is left as the round last derived it,
  ...
  **So through 5d the rows live at
  ...
  spec. The 5e task is the only thing that closes it. **Consequently the `ALL_CAUGHT` gate each task
...
- **When each row's payload is fixed — deliberate, not an omission** (impl-plan audit v20, whose
  ...
  ground; the file held 76 rows at that cycle and holds 88 now, and the ground is unchanged by the
  ...
  the anchor-*text*-rewritten case, not the anchor-*file*-moved one. **`doc_block_exec.json`'s 88
...
**88** at this batch and this axis contributes none of it; a row on THIS axis, if the design wants
...
- the **tab arm is 0, and it is pinned by nothing at all** — no AC in this document prescribed a
  ...
  case. **No mutation row follows from THIS axis**: the matrix total is **88** at this batch and
...
**26 is a RE-DERIVATION over the list above and not a carried figure, and the two moves that
...
25 + 7 + 26 + 30 = **88 rows**, split **87 of the helper's source and 1 of `h-mad/SKILL.md`**.
...
D7 ADDS `timeout-upper-bound-removed` to Task 3, which is 25 → 26 and takes the total 87 → 88. So
Tasks 1 and 2 are unmoved at 25 and 7, Task 3 is unmoved at 26 **by cancellation and not by
inaction**, and Task 4 stands at 30. **The split is re-derived, not carried**: both moved rows
anchor in `h-mad/scripts/h_mad_doc_block_exec.py`, so the `SKILL.md` half stays at exactly 1 —
`registry-row-removed` — and only the helper-source half grew, 86 → 87.
...
`SKILL.md`-naming-row count with the 88 and the 1 above. A disagreement is a finding for that round,
...
`doc_block_exec.json` must report `ALL_CAUGHT` over all 88 rows
...
hmad-dispatch run --timeout 600 -- python3.11 scripts/h_mad_mutation_harness.py tests/mutation-specs/doc_block_exec.json        # MUTATION: ALL_CAUGHT mutations=88
