...
> Source: docs/02-design/features/doc-block-exec.design.md (post-audit, v1.96 — design audit v87, the gating round)
> Paired spec: docs/01-plan/features/doc-block-exec.spec.md (v1.58) · paired plan: docs/01-plan/features/doc-block-exec.plan.md (v1.91)
> All three re-derived at `35698f9` on 2026-09-04 at the moment this revision was written, with `grep -oE '^- v1\.[0-9]+' <doc> | tail -1`. The three sibling authors are working **concurrently**, so any of these may be one behind by the time this is read — that is drift, not a finding. v1.35 pinned v1.92/v1.53/v1.85 and all three moved within the same session; v1.36 pinned v1.93/v1.55/v1.86 of which two moved again by `a8e0372`; v1.38's v1.93/v1.56/v1.87 was one behind on the design and two behind on the plan by `335f535`, with only the spec unmoved; and v1.40's v1.95/v1.57/v1.90 was one behind on **all three** by `35698f9`. That is the measurement behind this sentence, not a supposition.
>
...
- **When each row's payload is fixed — deliberate, not an omission** (impl-plan audit v20, whose
  ...
  one hit, verified at `35698f9`: exact `find` anchors are set from the landed source **in the same
...
- **Single source of the fence grammar**: **within the two files this feature touches —
  ...
  **none is a migration target for this feature**:
  - `_titled_section` (`h-mad/tests/test_h_mad_context_budget_docs.py:69`) bounds on
    ...
    — one hit, verified at `35698f9`: it anchors on a **substring**, so
...
- **No sentence in this document states what a sibling document currently says.** A debt or a
  ...
  **v1.40 wrote that this document "meets [these conditions] everywhere", and it did not.** Two
  ...
  returns **nothing** at `35698f9` after this revision — 3 hits before it. **Residuals on the
  ...
  **quotes the module-level statement verbatim**, which is a stronger anchor than a symbol name:
  ...
  (`awk '/^## Version History/{exit} /^$/{print b;b="";next}{b=b" "$0}END{print b}'` piped to
  `grep -oE '[A-Za-z0-9_./-]*\.py: ?[0-9]+'`) was run beside it at `35698f9` and returns the same
  set. (iii) The **symbol-name** half has no detector at all — the symbol may sit several words
  from the pin — so it is enforced by reading, and its recurrence is what audit v38 and plan audit
  v78 caught.

  **The tree pins this document stamps `335f535` and `74e126f` are unchanged at the freeze sha
  `35698f9`, and that closure is stated once here rather than re-stamped on ~40 pins**:
  `git diff --stat 335f535 74e126f` touches **9** files, all under `docs/`, and
  `git diff --name-only 74e126f 35698f9 -- h-mad handoff` is **empty** — no file under `h-mad/` or
  `handoff/` moved across either step — so every `path:line` derived at `335f535` or `74e126f` is
  byte-identical at `35698f9`, and the older stamps are provenance facts rather than stale pins.
  Sibling `docs/` values are the ones that did move, and each of those is re-derived
  above at `35698f9`. A `docs/` pin has neither property — the
  ...
  **This is NOT a fifth member of the list above, and v1.40 wrote that it was** (impl-plan audit
  v38, which found the document then saying "four" at one site, "the FIFTH" at a second and "a
  fifth has not happened" at a third). The list above is the **prose-agreement** class — a
  ...
  **This preference is stated as a preference, not a hard rule, and the reason is measured**: at
  `35698f9` this document carries **13** distinct `docs/`-sibling locators, and after this
  ...
  **The residual, stated exactly**: the one-hit property is true only at the commit it was
  measured at, so it is re-established every revision or not at all. **Re-swept for v1.41 at
  `35698f9`, every one of the 13 re-run in this revision rather than carried: all 13 return
  exactly one hit.** This was a real re-measurement and not a formality: **all three sibling
  documents were revised** between the sha v1.40 swept at and this one —
  `git diff --name-only 74e126f 35698f9 -- docs/01-plan/features/doc-block-exec.plan.md
  ...
  after ``both halves of `overlap:` `` replaced the one that had returned two.
...
`grep -n 'Heading selector differential' docs/01-plan/features/doc-block-exec.plan.md`, one hit, verified at `35698f9`).
...
at `h-mad/scripts/h_mad_mutation_harness.py:679`, in `run_spec` (`:482`), which asks "what else
noticed" after a row already survived; a
...
(one hit, verified at `35698f9`). The `SUBST_OVERLAP` detail line is `overlap: "<a>" "<b>"` with **both** halves
...
(one hit, verified at `35698f9`). Impl-plan v1.24 flagged those two rows as bare and half-quoted and **design
v1.81 answered it** — located with
``grep -n 'both halves of `overlap:`' docs/02-design/features/doc-block-exec.design.md``
(one hit, verified at `35698f9`) — so the flags were withdrawn at v1.35. The three `design.md:` line numbers this paragraph carried
...
hit, verified at `35698f9`), so 80 + 1.
...
hit, verified at `35698f9`). It is `registry-row-removed`'s partner **by
...
exactly one hit (verified at `35698f9`), and that hit is the line to read. A previous
