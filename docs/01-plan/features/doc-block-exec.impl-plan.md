...
> Paired spec: docs/01-plan/features/doc-block-exec.spec.md (v1.55) · paired plan: docs/01-plan/features/doc-block-exec.plan.md (v1.86)
> All three re-derived on 2026-09-04 at the moment this revision was written, with `grep -oE '^- v1\.[0-9]+' <doc> | tail -1`. The three sibling authors are working **concurrently**, so any of these may be one behind by the time this is read — that is drift, not a finding. v1.35 pinned v1.92/v1.53/v1.85 and all three moved within the same session, which is the measurement behind that sentence, not a supposition.
>
> These three pins go stale the way Task 5's SKILL.md line numbers and AC-6.4's suite floor did, and for the same reason — they name a moving value in another file. **Re-derive them, never trust them**: `grep -oE '^- v1\.[0-9]+' <doc> | tail -1` gives each document's current version, and that is the check to run before acting on anything this header claims. They were last correct at the commit named above; a reviewer finding them behind is looking at expected drift, not a finding.
...
with `archive/` excluded, **25 files**, re-counted here at `1861157` — the old regex and
...
a heading. Over the filesystem glob — **30 files**, the five extra being the gitignored
`.pytest_cache/README.md` artifacts — agreement is **268** with the same `old_only=76` and
`new_only=0`. The differential itself is the plan's measurement, re-derived there at `1861157`
through a throwaway script; this document transcribes it and did not re-run it. What is verified
here is the corpus: `git ls-files -- h-mad handoff | grep '\.md$' | grep -v '/archive/' | wc -l` → 25, the glob → 30, the five extras named in AC-6.1. `titled_section(text: str, heading: str)`
...
(`h_mad_mutation_harness.py:606-607`), so `command` never selects a killer, and three of the eight
killers — `docsections-syspath-setup-removed`'s, `docsections-local-bounder-restored`'s and the
`test_docsections_imports_from_an_unrelated_cwd` binding — live in `test_h_mad_doc_block_exec.py`,
which `command` does not collect. The only consumer of `command` is the survivor-branch diagnostic
...
**The design's verdict table and its exhaustive rule agree, and this document matches both** —
re-derived at `1861157`, not carried. `design.md:1005` renders the head as `BAD_INFO key="<k>"`,
quoted: `key=` is the offending info-string token — document-controlled — and is not among the
seven exempt fields. `design.md:1002` renders the detail line as `overlap: "<a>" "<b>"`, **both**
halves quoted: both elements are caller keys. Impl-plan v1.24 flagged those two rows as bare and
half-quoted; **design v1.81 fixed them** (`design.md:1435`: "`key=` (BAD_INFO) and both halves of
`overlap:` are quoted like every non-exempt field"), eleven design revisions before the v1.92 this
document's header pins, so both flags are **withdrawn** and nothing is owed to the design here
(impl-plan audit v34).
...
(`registry-row-removed`, `design.md:1290`: "one remedy row deleted from the `SKILL.md`
...
emittable line has no registry row (`design.md:1291`). It is `registry-row-removed`'s partner **by
...
**Acceptance Criteria**:
- [ ] AC-6.1 `test_exactly_one_tagged_fence_in_the_tree` (in `test_h_mad_doc_block_exec.py`): opening fences carrying `hmad:exec` equal exactly 1, counted with the module's own `_fence_events` over **`*.md` files only** — a sweep this AC states outright rather than reaching by reference: `Path(REPO_ROOT).glob('*/**/*.md')`, then `rel = p.relative_to(REPO_ROOT)`, keeping `rel.parts[0] in ('h-mad', 'handoff')`, dropping any path with `'archive'` in `rel.parts`, **and dropping any path with a dot-directory component** (`any(part.startswith('.') for part in rel.parts)`). **Two independent clauses, two independent defects, both measured** (impl-plan audit v34 and the design v1.93 back-propagation). *The rebase*: this document's `REPO_ROOT` is **absolute** (`REPO_ROOT = Path(__file__).resolve().parents[2]`, Conventions), while the plan's census walks `Path('.')` from the repository root, so the transcribed `p.parts[0] in ('h-mad', 'handoff')` read `parts[0] == '/'` here and selected **nothing** — the AC would still have predicted 0 correctly at RED and could never have reached 1 at GREEN. `'archive'` is rebased with it, or it would test the components of the absolute prefix. *The dot-directory exclusion*: without it the sweep also walks the five gitignored `.pytest_cache/README.md` artifacts that exist on any tree where pytest has run — build output, not documentation, and not something a cardinality-1 guard should count. **Measured at `1861157`, 2026-09-04, from the repository root, all three keep-counts so the next reader sees why each clause exists**: absolute base **0**; `rel`-rebased **30**; `rel`-rebased **minus dot-directories 25**. The five dropped are exactly `h-mad/.pytest_cache/README.md`, `h-mad/scripts/.pytest_cache/README.md`, `h-mad/tests/.pytest_cache/README.md`, `handoff/.pytest_cache/README.md` and `handoff/tests/.pytest_cache/README.md`. Re-run to check: `python3 -c "from pathlib import Path; R=Path('.').resolve(); print(len([p for p in R.glob('*/**/*.md') if (r:=p.relative_to(R)).parts[0] in ('h-mad','handoff') and 'archive' not in r.parts and not any(x.startswith('.') for x in r.parts)]))"` prints `25` from the repository root; drop the dot clause and it prints `30`; use `p.parts` in place of `r.parts` and it prints `0`. **This defect is LATENT, not live** — worth stating precisely, because a 5d implementer must not go looking for a failing test. No `.pytest_cache/README.md` carries `hmad:exec` (checked over all five at `1861157`: 0 hits), so the cardinality-1 assertion passes today and would have passed at Task 5 GREEN on the 30-file sweep too. What is wrong is that the **set the test walks is machine-state-dependent** — 25 on a clean clone, 30 after anyone has run pytest — so a future tool that drops a generated `.md` under those roots silently enters an assertion that is supposed to be exactly 1. **Which realisation this is, and why it differs from the other**: the design states the exclusion twice, and the two are deliberately not the same command. §Scanning's *measurement* corpus is `git ls-files -- h-mad handoff` filtered to `*.md` with `archive/` excluded — 25 files — because a one-off human measurement has every reason to describe the tracked tree. This AC transcribes the **guard** realisation, the dot-directory exclusion, because a test must still count a tagged fence in an `.md` that has been **written and not yet committed**, which is precisely the accident it exists to refuse and precisely what `git ls-files` would miss. The two agree on this tree and are not the same predicate: measured at `1861157`, the dot-excluded glob set and the `git ls-files` set are both 25 files and **identical** (symmetric difference empty) — an agreement of today's tree, not of the definitions, and it is the definitions the test must implement. **The `*.md` restriction is load-bearing, not tidiness.** (It is no longer stated as "the scope of the census this AC is bound to": reaching a scope by reference imports whatever the referent later becomes, which is how the dot-directory contamination arrived in this AC in the first place, so the sweep is spelled out above instead — the same move spec v1.55 made, for the same reason.) By Task 5, Tasks 1–4 have landed `h-mad/tests/test_h_mad_doc_block_exec.py` under `h-mad/`, and its fixtures carry ` ```bash hmad:exec ` as a column-0 line inside triple-quoted Python strings (AC-1.1's tagged/untagged pair, AC-1.5's section fixtures, AC-1.7's duplicate-heading fixture, AC-3.7's `shell=fish` and `hmad:exec hmad:exec` fixtures). By this feature's own grammar a 0–3-space marker run **is** an opener regardless of the enclosing file's suffix, so an unrestricted sweep counts every one of them and the AC could never pass at GREEN. Worse, the count would not even be the sum of the per-fixture counts: one fixture is a deliberately *unbalanced* four-backtick fence (`test_docsections_unbalanced_four_backtick_fence`), so a whole-file scan of the `.py` carries fence state across fixture boundaries. The scoping rule follows from what `_fence_events` is: a **markdown** scanner whose only inertness rules are markdown ones (a four-backtick fence, a `~~~` fence, a 4-space indent). A Python triple-quoted string is not one of them, so a non-`.md` holder of a fixture is a false positive **by construction**, not an accident of this feature. **Residual, stated exactly**: the sweep does not cover non-`.md` files under the two roots (`.py`, `.sh`, `.json`, `.txt`), `.md` files outside `h-mad/` and `handoff/` (this document and its siblings under `docs/` among them), anything under `archive/`, or anything under a dot-directory — so a tagged fence written into `.pytest_cache/` or any other dot-directory is uncounted, which is the intended trade for not counting build output. The converse is the live edge: **a generated `.md` written under the two roots *outside* a dot-directory IS counted**, correctly — it is then part of the executed documentation surface — but noisily, and if a tool ever starts emitting one this assertion is where it will surface. The guard is "exactly one tagged fence in the executed documentation surface", not "in the repository" — the right scope, because a tagged fence is only ever reachable by a consumer that scans a `.md` doc. **Both sibling clearances this AC carried at v1.35 are superseded, and the debt they denied is taken here rather than flagged** — re-derived at `1861157` against the current siblings, not carried. The design at v1.93 does not merely carry the `*.md` restriction, it goes further and prescribes the dot-directory exclusion for **this sweep specifically**, with the residual; cited by name, not by line, because a line pinned into a document under active revision is the class this document has now hit four times in one day — the Test Plan row for AC-6.1–6.6, located with `grep -n 'The sweep excludes build output' docs/02-design/features/doc-block-exec.design.md` (one hit). Spec v1.55's AC-6.1 states the same sweep in its own words with the same residual. The plan at v1.86 states the same distinction in §"The fence census (68)" ("AC-6.1's tree sweep is deliberately NOT this filter"), so **nothing is owed to the design, the spec or the plan** — but that is now true because this document has adopted their scope, not because 30 matched anything. Under this restriction the RED prediction below is true as stated: measured at `b7d0d77`, `grep -rn 'hmad:exec' h-mad/ handoff/` returns nothing, so before Task 5's SKILL.md edit the `.md` count is zero.
...
AC-6.4 gate command from v1.50 on. It is cited **by name, not by line** — the spec's AC-6.4 entry,
located with `grep -n 'git rev-parse --show-toplevel' docs/01-plan/features/doc-block-exec.spec.md`
— because a line pin into a sibling is exactly the perishable reference the SKILL.md locator rule
forbids, and the `:458` this document carried through v1.34 had already drifted: at `1861157` the
subshell is at `:462` and `:458` is the `full_collected >= 2748 + new_module + len(tuple)`
assertion. The **subshell** agrees across the three documents; its **inline comment** does not.
Re-derived at `1861157`, the comment on `spec.md:462` still reads "from the REPOSITORY ROOT: the
2747 baseline is the root count; from h-mad/ the same command collects 2485" — the pre-v1.53
numbers — while `spec.md:451`, `spec.md:458`, the plan and this document all carry **2748** and
**2486**. **One thing is therefore owed upstream**: that comment needs 2747→2748 and 2485→2486
(flagged for the spec author, not edited here — impl-plan audit v34).
