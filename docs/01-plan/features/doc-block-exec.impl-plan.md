...
- **No sentence in this document states what a sibling document currently says.** A debt or a
  ...
  **The marker screen — its scope, its target and its three readings.** It is published here in
  ...
  On the tree v1.52 ships, re-run **after** v1.52's own edits landed rather than
  ...
  **The site is stamped to the revision that ships it, not to the revision whose repair last
  ...
  rather than avoided it. The four sites that carried the v1.50 stamp into v1.51 are this one, the
  restated-cardinal screen, both `SKILL` screens and the `.py:` series, and all four are re-run and
  re-stamped at v1.52 below. They are unmoved by v1.49's and by v1.50's edits — and re-running
  ...
  **0** at v1.40, v1.41 and v1.42, **1** at v1.43, and **0** at v1.44 and at every revision after
  ...
  narrower one: at `af19d53`, the sha v1.52 is authored against, body **0** / **0** and whole file
  **2** lines / **3** occurrences; on the tree v1.52 ships, re-run after v1.52's own edits landed
  rather than before them (decision K), body **0** / **0** and whole file **2** lines / **3**
  occurrences. What the two readings establish is that v1.52's edits did not reopen the class, not
  ...
  **(iv) The four screens above cover `.py:` pins only, and the `SKILL.md`-pin class this document
  ...
  this document's body, both returning **0** on the tree v1.52 ships and both re-run after
  v1.52's edits landed rather than before them (the stamp is the shipping revision's, re-derived
  and not carried — v1.49 left it at v1.48 and v1.51 left it at v1.50, both while shipping further
  edits; impl-plan audit v45 and v46):
  ...
  **Re-swept for v1.50 at `b3be433`, and the population is now FOURTEEN, not thirteen**
  ...
  own header comment, which `grep -Fn '# h-mad/tests/docsections.py  (delta)'` reaches at exactly
  ...
  **This sweep is itself exposed to the needle-inside-scope rule stated in Conventions, and it is
  ...
  ships**, re-run after this revision's last edit landed rather than before it (decision K), over the
  ...
  **26**, `3f70eb3` **23** / **25**, `af19d53` **23** / **25**. **The rule that closes the class is
  ...
  taken at and does not move when the freeze moves; and **a reading taken over THIS revision's own
  post-edit body is stamped to the tree this revision ships and never to the freeze**, because the
  ...
  four screen sites carrying the other half of the same error — a stamp naming a tree the reading
  was not taken on — are re-run and re-stamped with it, which is the enforcement condition the
  marker screen's rule gained in the same revision.
  ...
  the tree v1.49 ships, **26** on the tree v1.50 ships, **25** on the tree v1.51 ships and **25**
  on the tree v1.52 ships — so the
  ...
  hits on the tree v1.52 ships, re-derived by classifying every one of them rather than carried
  ...
  **NOTHING moved at v1.52, and that is a reading rather than an assumption**: this revision writes
  several new sentences into this bullet and into the Conventions residual above it, so the sweep
  and its split are both figures its own edits could move; re-run after the last edit landed
  (decision K), the total is unmoved at 25 and all three bins are unmoved at 18 / 5 / **2**, with
  the split re-derived by classifying all 25 hits rather than carried from v1.51's. What that
  establishes is that this revision's edits did not reopen the class, not that they closed anything.
