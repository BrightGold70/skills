...
**OPEN-DECISION (r19, 5d):** decide whether `preamble-composed-with-unsubstituted-text` moves — the mutation and its killer together — to the caller that holds both the pre- and post-substitution texts, or is respecified with a different discriminating mechanism, since `run_block` receives the already substituted block and composes from `block.text`, so no distinction is implementable at that seam — source: `docs/01-plan/features/doc-block-exec.impl-plan.audit.v50.codex.md` must 1; premise as filed by codex, re-derive before choosing.

**OPEN-DECISION (r19, 5d):** decide whether `BadTimeout` gains a representable upper bound, coordinated with the spec's AC-5.6 wording, or `OverflowError` is mapped into the launch-failure path, and test the controlled pair either way, since `communicate(timeout=1e300)` raises `OverflowError` on 3.11.8 while `math.isfinite(t) and t > 0` accepts that value and the error mapping excludes that type — source: `docs/02-design/features/doc-block-exec.design.audit.v99.codex.md` must 1; premise as filed by codex, re-derive before choosing.
