...
> Source: docs/02-design/features/doc-block-exec.design.md (post-audit, v1.93 — design audit v83, the gating round)
> Paired spec: docs/01-plan/features/doc-block-exec.spec.md (v1.56) · paired plan: docs/01-plan/features/doc-block-exec.plan.md (v1.87)
> All three re-derived at `a8e0372` on 2026-09-04 at the moment this revision was written, with `grep -oE '^- v1\.[0-9]+' <doc> | tail -1`. The three sibling authors are working **concurrently**, so any of these may be one behind by the time this is read — that is drift, not a finding. v1.35 pinned v1.92/v1.53/v1.85 and all three moved within the same session, and v1.36 pinned v1.93/v1.55/v1.86 of which two moved again by `a8e0372`; that is the measurement behind this sentence, not a supposition.
>
...
- **When each row's payload is fixed — deliberate, not an omission** (impl-plan audit v20, whose
  ...
  kill (`h_mad_mutation_harness.py:609–623`). **The axis is not which spec a row belongs to — it is whether the row's anchor file exists at
  ...
  **current** payloads forward is specifically forbidden: today's two anchors in
  ...
  kill (`h_mad_mutation_harness.py:609–623`), against AC-1's last bullet, which requires
...
- **Single source of the fence grammar**: marker-run **recognition** — the literals ```` ``` ```` and
  ...
  **Residual, and it is deliberate**: the rule is scoped to marker-run recognition, so a
  test-local `##`-slicer that performs none of it is outside the rule and outside
  `test_docsections_has_no_second_bounder`, which is scoped to `docsections.py`. One such slicer
  is live and this document names it rather than leaving it to be re-found: `_titled_section` at
  `h-mad/tests/test_h_mad_context_budget_docs.py:69`, which bounds on `text.index(title)` and
  `text.find("\n## ", start + 1)` with **no fence state at all**, reached from eight call sites
  at `:301`–`:372` over `h-mad/SKILL.md` (re-derived at `a8e0372`: `grep -n '_titled_section'`
  on that file gives one `def` at `:69` and eight call sites). It violates no invariant this
  feature states and it is **not** a migration target — the design's Single-source contract
  already names it and records the measurement that rules it out as a drop-in
  (`grep -n 'anchors on a substring' docs/02-design/features/doc-block-exec.design.md`, one hit:
  `docsections.titled_section` cannot find `Run-context ceiling`, because `_titled_section`
  anchors on a substring rather than on a heading). It is stated here only so a 5d or 5e reader
  who greps for `## `-slicing does not read its survival as an oversight.
...
- **No sentence in this document states what a sibling document currently says.** A debt or a
  ...
  recurred four times and every recurrence cost a full dual-surface audit cycle: v1.24's two
  quoting flags against the design, already answered eleven design revisions earlier (withdrawn
  at v1.35); the AC-6.1 restriction flag, already landed in design v1.92 (withdrawn at v1.35);
  the 5f note's clearance citing a drifted `spec :458` (repaired at v1.35); and the 5f note's
  **debt**, still asserted at v1.36 after spec v1.54 had paid it (withdrawn at v1.37) — the same
  class in the opposite direction, a stale clearance replaced by a stale debt. Two forms are
  ...
  `grep -n 'The sweep excludes build output' docs/02-design/features/doc-block-exec.design.md` —
  ...
  **provenance** citation of the form "(design v1.85)" or "(plan audit v67)", naming the version
  ...
  reads …", "nothing is owed to the plan". The header's three version pins are the one further exception, and only because they
  ...
  **Residual, stated exactly**: only form (a) has a detector, and a weak one — a locator that has
  ...
  assertions about it, so a fifth recurrence is prevented by the sentence not being written, not
  by anything catching it. The one mechanical aid is a pre-dispatch sweep the reviser runs —
...
`grep -n 'Heading selector differential' docs/01-plan/features/doc-block-exec.plan.md`, one hit).
...
`a8e0372`: `git ls-files -- h-mad handoff | grep '\.md$' | grep -v '/archive/' | wc -l` → **30**,
...
read from `h-mad/tests/mutation-specs/docsections.json` at `1861157` — is **left exactly as it is**,
and deliberately (impl-plan audit v34): scoring runs `target_command + [test]`
(`h_mad_mutation_harness.py:606-607`), so `command` never selects a killer, and **two** of the eight
...
eight**, re-derived at `a8e0372`: the four shipped rows in
...
because a 5d implementer who counts finds two and reads the gap as a missing row. The only consumer of `command` is the survivor-branch diagnostic
at `h_mad_mutation_harness.py:679`, which asks "what else noticed" after a row already survived; a
narrower whole-file view there costs nothing a widened `command` would buy.
...
**This document's own constraint, and the design carries the same two rows** — re-derived at
`a8e0372` and cited **by name, never by line**, under the Conventions rule that forbids a sibling
line pin. The `BAD_INFO` head is `BAD_INFO key="<k>"`, **quoted**: `key=` is the offending
...
`grep -n 'unrecognised info-string token' docs/02-design/features/doc-block-exec.design.md`
(one hit, verified). The `SUBST_OVERLAP` detail line is `overlap: "<a>" "<b>"` with **both** halves
quoted, because both elements are caller keys; its row is located with
`grep -n 'one key is a substring of another' docs/02-design/features/doc-block-exec.design.md`
(one hit, verified). Impl-plan v1.24 flagged those two rows as bare and half-quoted and **design
v1.81 answered it** — located with
`grep -n 'both halves of' docs/02-design/features/doc-block-exec.design.md` (one hit, verified) —
so the flags were withdrawn at v1.35. The three `design.md:` line numbers this paragraph carried
through v1.36 had all drifted by `a8e0372`; they are dropped rather than re-pinned, which is the
Conventions rule, not a one-off repair.
...
`grep -n 'guard it removes'` — count those whose mechanism column names
`SKILL.md` as the file the harness edits — **81 data rows, exactly 1 of them**
(`registry-row-removed`, whose design row is located with
``grep -n '^| `registry-row-removed`' docs/02-design/features/doc-block-exec.design.md`` — one
hit, verified at `a8e0372` — and reads "one remedy row deleted from the `SKILL.md`
Helper-scripts entry (the mutation targets `SKILL.md`)"), so 80 + 1.
...
hit, verified). It is `registry-row-removed`'s partner **by
...
**Class and residual, derived at `a8e0372`, not carried.** The class is *a pre-existing `@pytest.mark.parametrize` whose argvalues come from a filesystem glob this feature writes into*; the rule is to enumerate those globs **before** fixing the constant and add one node per (parametrised test × added file). The residual is the enumeration itself: across `h-mad/tests`, `handoff/tests` and `handoff/scripts` — the whole of `testpaths` — exactly **two** module-level glob-fed parametrize sources exist. `_SCANNED` is one, and contributes the `+ 2` above. The other is `REAL_AUDIT_REPORTS` (`h-mad/tests/test_h_mad_audit_cycle.py:15`, consumed at `:1672`), which globs `docs/` and is sliced `[:8]` over 460 candidates today — **saturated, so it adds 0**, and this feature's landed source writes nothing under `docs/` in any case. Every other `.glob`/`.rglob` in those trees is inside a function body and so adds no collection node: `test_h_mad_collect_report.py:287` (globs `h-mad/scripts/*.py` but filters to `writer_modules = ("h_mad_audit_cycle.py", "h_mad_collect_report.py")` at `:285-289`), `test_h_mad_mutation_harness.py:81` and `:1900` (that file contains **no** `parametrize` at all, so `doc_block_exec.json`, `doc_block_exec_wire.json` and the re-pointed `docsections.json` add no node while still being subject to its exact-once anchor sweep), `test_suite_collection.py:81` (rglobs `test_*.py`, which is where the new test module would otherwise have landed a node), `test_h_mad_agy_review_fixes.py:183`, `test_h_mad_skill_reviewer_prompt.py:141`, `test_hmad_dispatch_audit_cycle.py:250` (a `tmp_path` script dir, not the repository's) and `handoff/tests/test_mutation_specs_clean.py:20` (handoff's own `tests/mutation-specs`, which this feature does not write to). **Nothing else in the collected suite globs a directory this feature writes into.**
...
one-line docstring at `test_h_mad_collect_report_docs.py:268` — required, not incidental, because
...
for pytest, not the subshell's `cd`. **The constraint this document states is its own**: the same
root-pinned subshell must appear in the spec's AC-6.4 gate command, and its inline comment must
carry **2748** (the repository-root count) and **2486** (the count from `h-mad/`) — the pair this
document uses everywhere. It is reached **by name, never by line**:
`grep -n 'git rev-parse --show-toplevel' docs/01-plan/features/doc-block-exec.spec.md` returns
exactly one hit (verified at `a8e0372`), and that hit is the line to read. No sentence here says
...
paragraph, located with `grep -n 'Bounds: 1200 s' docs/01-plan/features/doc-block-exec.plan.md` (one hit),
