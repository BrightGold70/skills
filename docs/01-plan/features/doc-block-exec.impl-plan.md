...
(`doc_block_exec.json`, which at completion realizes the design's matrix table one row per row — **90** of them,
89 anchored in the helper's source and 1 in `h-mad/SKILL.md`; `doc_block_exec_wire.json` 8; `docsections.json` 8)
must report `ALL_CAUGHT`. **The 90 is the DESIGN TABLE's count, which that file realizes — it is not a reading
...
- **Mutation spec** `h-mad/tests/mutation-specs/doc_block_exec.json`: `root` is `../..`,
  ...
  **Its sibling `stderr-not-closed` was read the same way and is NOT a member**, which is the control
  ...
  read each of the 8 `doc_block_exec_wire.json` rows, the 8 `docsections.json` rows and the 90
  `doc_block_exec.json` rows for a per-row clause of the form *goes red with it* / *reds too* /
  *stays a regression test, not this row's key*, and take the members from that reading. **The population is 90 and the reading over it is NOT one stamp: 88 of those rows were read at
  ...
  **The middle addend has since MOVED, and the total has not**: after the merge
  ...
  — so the same partition now reads `60 + 30 = 90`, and 90 is the design table's figure this
  document mirrors rather than a count of any file.
  **The 5e obligation was stated so it is checkable rather than assumed, and the checkable half is
  now DISCHARGED**: before the feature is complete all **90** rows live in
  ...
  count**: 60 of the 90 are written, and the remaining 30 are Task 4's and Task 5's, so the
  `ALL_CAUGHT` obligation below is still open on those. **Consequently the `ALL_CAUGHT` gate each task
...
- **When each row's payload is fixed — deliberate, not an omission** (impl-plan audit v20, whose
  ...
  ground; the file held 76 rows at that cycle and its table holds 90 now, and the ground is unchanged by the
...
**90** at this batch and this axis contributes none of it; a row on THIS axis, if the design wants
...
- the **tab arm is 0, and it is pinned by nothing at all** — no AC in this document prescribed a
  ...
  case. **No mutation row follows from THIS axis**: the matrix total is **90** at this batch and
...
25 + 7 + 28 + 30 = **90 rows**, split **89 of the helper's source and 1 of `h-mad/SKILL.md`**.
...
prints `25 7 28`, and all 60 of those rows carry `file: scripts/h_mad_doc_block_exec.py`, 0 of them
`h-mad/SKILL.md`. **The derivation is over ONE file because §D10's merge deleted the other, and the
...
(`0` duplicates). **Task 4's 30 is the one addend that CANNOT be derived that way and is marked as
such rather than presented beside three measurements as if it were one**: Task 4 has not landed, no
spec holds its rows, and the single `h-mad/SKILL.md` row in the 89/1 split lives entirely inside it,
so that `1` is a claim about the design's table and not a reading of any file (v1.58).
**Four moves land here, each one addend, and each is named with the task it lands in.** The move
...
wrong integer. So Tasks 1 and 2 are unmoved at 25 and 7, Task 3 stands at 28 — **unmoved at 26 by
cancellation through D6/D7, then moved to 28 by D9** — and Task 4 stands at 30. **The split is re-derived, not carried**: all four moved rows —
D6/D7's pair and D9's pair — anchor in `h-mad/scripts/h_mad_doc_block_exec.py`, read from the
landed specs' `file` field rather than from this document, so the `SKILL.md` half stays at exactly 1
— `registry-row-removed` — and only the helper-source half grew, 86 → 87 → 89.
**The DESIGN's own matrix total is deliberately NOT restated here**, because D6 and D7 move it and
`design-author` had not run when this revision was written, so any integer written here would be a
reading of bytes that do not yet exist. It is reached by the command instead, exactly as this
document's Conventions require of a sibling claim, and it is re-taken at the commit this batch
lands rather than carried: run the plan's published `awk` over the design's mechanism column against
`docs/02-design/features/doc-block-exec.design.md` and compare its count and its
`SKILL.md`-naming-row count with the 90 and the 1 above. A disagreement is a finding for that round,
...
`doc_block_exec.json` must report `ALL_CAUGHT` over all 90 rows
...
hmad-dispatch run --timeout 600 -- python3.11 scripts/h_mad_mutation_harness.py tests/mutation-specs/doc_block_exec.json        # MUTATION: ALL_CAUGHT mutations=90
