...
- **When each row's payload is fixed — deliberate, not an omission** (impl-plan audit v20, whose
  ...
  one hit, verified in this pass: exact `find` anchors are set from the landed source **in the same
...
- **Single source of the fence grammar**: marker-run **recognition** — the literals ```` ``` ```` and
  ...
  **none is a migration target for this feature**:
  - `_titled_section` (`h-mad/tests/test_h_mad_context_budget_docs.py:69`) bounds on
    ...
    `:301`–`:372` over `h-mad/SKILL.md` (re-derived at `335f535`: `grep -n '_titled_section'` on
    ...
    verified in this pass: it anchors on a **substring**, so
    `docsections.titled_section(SKILL_MD, "Run-context ceiling")` cannot find the real heading,
    which reads `## Run-context ceiling — halt the run at 80%` (`h-mad/SKILL.md:1606` at
    `335f535`).
...
- **No sentence in this document states what a sibling document currently says.** A debt or a
  ...
  **The rule's axis is `docs/` siblings under concurrent authorship, and it is not a ban on line
  ...
  that writes it** rather than carried from a report. A `docs/` pin has neither property — the
...
by **a space or a tab** — *both* `#`-run delimiters in ATX take spaces-or-tabs, the same axis the
opening delimiter's `request-predicate-space-only` closes, and the closing run was the one member
left at space-only; oracle: on markdown-it-py 2.2.0, CommonMark preset, `'## Text\t##\n'` renders
`<h2>Text</h2>`) and trailing whitespace stripped. The comparison is never against the raw source line,
...
- [ ] AC-1.5/1.7 `test_closing_hash_run_does_not_change_heading_identity`: pins the normalization rule from both sides, **on both delimiters**. On a document whose only heading is `## Text ##`, both `find_heading(text, "## Text")` (full form) and `find_heading(text, "Text")` (bare form) find it and return the same `(end, 2)` — the closing run is stripped before the comparison, so the raw line is never what is matched. **The fixture carries the tab-preceded form `"## Text\t##"` beside the space-preceded one**, asserted identically, because the closing `#`-run delimiter is spaces-or-tabs and a space-only strip would leave `## Text\t##` unequal to `Text` and so unfindable in either form. On a document holding both `## Text` and `## Text ##`, the full form raises `AmbiguousHeading` with `n == 2`, because the two lines normalize to the same heading rather than to two distinct ones (design v1.67 §Scanning, design audit v63); the tab leg is asserted the same way, on a document holding `## Text` and `## Text\t##`. **Residual, measured at `335f535`**: `git ls-files -- h-mad handoff | grep '\.md$' | grep -v '/archive/' | xargs grep -cP '^ {0,3}#{1,6}[ \t].*\t#+[ \t]*$'` reports **0** files with a match, so no live document or fixture outside this test depends on the tab arm — shipping it space-only would be a silent divergence from the renderer the scanner grammar was oracled against, not a currently failing document, which is why a fixture rather than a corpus instance is what pins it.
...
**This document's own constraint**, re-derived at `335f535`, with the corresponding design rows
...
hit, verified in this pass), so 80 + 1.
...
exactly one hit (verified in this pass at `335f535`), and that hit is the line to read. A previous
