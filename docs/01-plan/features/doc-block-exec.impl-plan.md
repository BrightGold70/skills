...
> Source: docs/02-design/features/doc-block-exec.design.md (post-audit, v1.95 — design audit v85, the gating round)
> Paired spec: docs/01-plan/features/doc-block-exec.spec.md (v1.57) · paired plan: docs/01-plan/features/doc-block-exec.plan.md (v1.90)
> All three re-derived at `74e126f` on 2026-09-04 at the moment this revision was written, with `grep -oE '^- v1\.[0-9]+' <doc> | tail -1`. The three sibling authors are working **concurrently**, so any of these may be one behind by the time this is read — that is drift, not a finding. v1.35 pinned v1.92/v1.53/v1.85 and all three moved within the same session; v1.36 pinned v1.93/v1.55/v1.86 of which two moved again by `a8e0372`; and v1.38's v1.93/v1.56/v1.87 was one behind on the design and two behind on the plan by `335f535`, with only the spec unmoved. That is the measurement behind this sentence, not a supposition.
>
...
- **When each row's payload is fixed — deliberate, not an omission** (impl-plan audit v20, whose
  ...
  one hit, verified at `74e126f`: exact `find` anchors are set from the landed source **in the same
...
- **Single source of the fence grammar**: **within the two files this feature owns —
  ...
  **none is a migration target for this feature**:
  - `_titled_section` (`h-mad/tests/test_h_mad_context_budget_docs.py:69`) bounds on
    ...
    — one hit, verified at `74e126f`: it anchors on a **substring**, so
    ...
    verified at `74e126f`, reading `## Run-context ceiling — halt the run at 80%`. **The line
  ...
  **Residual (b) — the file axis, the complement of (a), and it is the half v1.39 left unswept.**
  ...
  control. The other **six** are pre-existing, none is `_fence_events`, **three of them are
  production code under `h-mad/scripts/`**, and **no guard in this repository covers any of them** —
  `test_docsections_has_no_second_bounder` reads `docsections.py` and nothing else:
  ...
  **What this sweep still cannot see, measured rather than asserted.** It selects on the *name* of
  ...
  double-quoted shell context, where a literal backtick is command substitution. **Both controls
  were run before this paragraph was written**: the sweep prints `_fence_aware_end` — the positive,
  the one in-scope body — and does not print `_braces_outside_fences`, the negative named above.)
...
- **No sentence in this document states what a sibling document currently says.** A debt or a
  ...
  **The rule's axis is `docs/` siblings under concurrent authorship, and it is not a ban on line
  ...
  into the tree). Pins into `h-mad/` and `handoff/` source — `test_h_mad_portable_timeout.py:151`,
  `test_h_mad_context_budget_docs.py:69`, `test_h_mad_collect_report_docs.py:270` and the rest —
  stay admissible on two conditions this document meets everywhere: each carries the **symbol
  name** beside it (`_TIMEOUT_CMD`, `_SCANNED`, `_ABSENCE_CLAIMS`, `_titled_section`, `_section`,
  `section_text`, `_second_surface`), so a drifted pin self-repairs under one `grep -n` on that
  symbol — `grep -n '_TIMEOUT_CMD' h-mad/tests/test_h_mad_portable_timeout.py` is the shape — and
  the reader is never left guessing what was meant; and each is **re-derived in the revision
  that writes it** rather than carried from a report. **The tree pins this document still stamps
  `335f535` hold unchanged at `74e126f`, and that is measured, not assumed**:
  `git diff --stat 335f535 74e126f` touches **9** files, **all of them under `docs/`** — no file
  under `h-mad/` or `handoff/` moved between the two commits — so every `path:line` derived at
  `335f535` is byte-identical at `74e126f`, and the older stamp is a provenance fact rather than a
  stale pin. Sibling `docs/` values are the ones that did move, and each of those is re-derived
  above at `74e126f`. A `docs/` pin has neither property — the
  ...
  **How a needle is chosen, and the hard condition on it** (added at v1.40; impl-plan audit v37,
  the FIFTH recurrence of this class and the first one *inside* the form (a) prescribes as safe).
  ...
  **This preference is stated as a preference, not a hard rule, and the reason is measured**: at
  `74e126f` this document carries **13** distinct `docs/`-sibling locators, and after this
  ...
  **The residual, stated exactly**: the one-hit property is true only at the commit it was
  measured at, so it is re-established every revision or not at all. **Swept for v1.40 at
  `74e126f`: all 13 locators return exactly one hit** — 12 did before this revision and
  ``both halves of `overlap:` `` replaces the one that returned two.
  ...
  **provenance** citation of the form "(design v1.85)" or "(plan audit v67)", naming the version
  ...
  expires exactly as fast as "the spec carries X" — the fourth recurrence the list above records
  *was* a debt, not a description, so a rule scoped to the present tense would have let its own
...
**Residual, stated exactly**: the **fence opener's** 0–3-space indent is the same axis and is
likewise measured in columns — oracle, same preset: `'\t```bash\n'` renders as indented code while
`'   ```bash\n'` opens a fence — and **no corpus instance exercises either arm**, so both are
pinned by fixtures and by nothing else. That predicate is implemented **once**, in `_fence_events`, which reports such a
...
`grep -n 'Heading selector differential' docs/01-plan/features/doc-block-exec.plan.md`, one hit, verified at `74e126f`).
...
**This document's own constraint**, re-derived at `74e126f`, with the corresponding design rows
...
(one hit, verified at `74e126f`). The `SUBST_OVERLAP` detail line is `overlap: "<a>" "<b>"` with **both** halves
...
(one hit, verified at `74e126f`). Impl-plan v1.24 flagged those two rows as bare and half-quoted and **design
v1.81 answered it** — located with
``grep -n 'both halves of `overlap:`' docs/02-design/features/doc-block-exec.design.md``
(one hit, verified at `74e126f`) — so the flags were withdrawn at v1.35. The three `design.md:` line numbers this paragraph carried
...
hit, verified at `74e126f`), so 80 + 1.
...
hit, verified at `74e126f`). It is `registry-row-removed`'s partner **by
...
exactly one hit (verified at `74e126f`), and that hit is the line to read. A previous
