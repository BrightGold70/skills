...
- **No sentence in this document states what a sibling document currently says.** A debt or a
  ...
  **one** was named by impl-plan audit v44 rather than found by the walk — member (vii);
  ...
  **Residual of the WALK — stated as categories of what a walk of this file cannot reach, not as
  ...
  checkable by reading rather than by a grep. And they are reached regardless, because the walk's
  **second** signature is the bound variable itself, which both screen lines write — so they are
  members of signature two, not a residual of it. What (a) actually rests on is a **naming
  ...
  **This sweep is itself exposed to the needle-inside-scope rule stated in Conventions, and it is
  ...
  literal strings and all four occur in the body it counts. **This reading moves with every
  revision and is therefore stamped**: the sweep returns **23** body hits at `700c599` and **25**
  on the tree v1.49 ships, the two added by v1.48's own edits — so the round-thirteen audit's 23
  was **correct at the sha it read**, and what this document corrects is that audit's
  classification of the 23, not its count. **The composition of the 25 body hits on the tree v1.49
  ships, derived by classifying every one of them rather than by naming the ones a reader
  notices**:
