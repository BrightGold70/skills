...
> Source: docs/02-design/features/doc-block-exec.design.md (post-audit, v1.98 — the revision answering design audit v88, its gating round)
> Paired spec: docs/01-plan/features/doc-block-exec.spec.md (v1.60) · paired plan: docs/01-plan/features/doc-block-exec.plan.md (v1.93)
> All three re-derived at `cf3a862` on 2026-09-04 at the moment this revision was written, with `git show cf3a862:<doc> | grep -oE '^- v1\.[0-9]+' | tail -1` — read out of the **commit**, not the working tree, because the three sibling documents were being revised concurrently by their own authors while this revision was written, and a working-tree read would pin a document no commit contains. The three sibling authors are working **concurrently**, so any of these may be one behind by the time this is read — that is drift, not a finding. v1.35 pinned v1.92/v1.53/v1.85 and all three moved within the same session; v1.36 pinned v1.93/v1.55/v1.86 of which two moved again by `a8e0372`; v1.38's v1.93/v1.56/v1.87 was one behind on the design and two behind on the plan by `335f535`, with only the spec unmoved; v1.40's v1.95/v1.57/v1.90 was one behind on **all three** by `35698f9`; v1.41's v1.96/v1.58/v1.91 was again one behind on **all three** by `6f0ee85`, all three having moved in that commit; and v1.42's v1.97/v1.59/v1.92 was one behind on **all three** at `cf3a862` — the **third consecutive** revision behind on every sibling, and `git diff --name-only 6f0ee85 cf3a862 --` names all three sibling files, so all three moved across that span too. That is the measurement behind this sentence, not a supposition.
>
...
- **When each row's payload is fixed — deliberate, not an omission** (impl-plan audit v20, whose
  ...
  one hit, verified at `cf3a862`: exact `find` anchors are set from the landed source **in the same
...
- **Single source of the fence grammar**: **within the two files this feature touches —
  ...
  **none is a migration target for this feature**:
  - `_titled_section` (`h-mad/tests/test_h_mad_context_budget_docs.py:69`) bounds on
    ...
    — one hit, verified at `cf3a862`: it anchors on a **substring**, so
...
- **No sentence in this document states what a sibling document currently says.** A debt or a
  ...
  cycle. **The count is `len(list)` and this document writes it out in prose nowhere**, because a
  restated integer is a second authority and a second authority drifts — this one did, at v1.40
  (three sites disagreeing, impl-plan audit v38) and again at v1.42, which repaired two of the
  four sites and left two saying "four" after the list had reached five (impl-plan audit v40).
  Every reference below points **into** the enumeration by its marker rather than restating its
  size. The list: **(1)** v1.24's two
  ...
  **v1.40 wrote that this document "meets [these conditions] everywhere", and it did not.** Two
  ...
  returns **nothing — 0 occurrences** — against this document at `cf3a862`, the freeze sha this
  revision is authored against and the commit every figure below was re-derived at, and **0** again
  over the revised working tree. (`cf3a862` is the commit v1.42 was audited **at**; it is the base
  v1.43 is written **against**, and the shipping
  commit of this revision does not exist while it is being written, which is why every figure here
  names `cf3a862` and not a promise. `cf3a862` contains this document byte-identically as `8909ec4`
  does — `git diff --name-only 8909ec4 cf3a862 -- <this file>` is empty, as is the same diff over
  ...
  **quotes the module-level statement verbatim**, which is a stronger anchor than a symbol name:
  ...
  `cf3a862` (which holds v1.42, this revision's input): **49 occurrences across 19 folded
  ...
  is claimed for the after-reading — it is **49 occurrences across 19 folded paragraphs**, unmoved,
  because none of v1.43's edits adds or removes a `.py:` pin. (`| wc -l` on `grep -oE` for
  ...
  **The control is run over a fixture file, not over this document, and the fixture's needle is
  ...
  reproducible verbatim (run under both `bash` and `zsh`, byte-identical output):

  ```bash
  printf '%s\n' 'a residual reaching docsections.pyX' '  270 in _gate_bash_block' \
    | sed 's/pyX/py:/' > /tmp/fold-control.txt
  ```
  ...
  which is checked, not assumed: the four screens over this document return the same 49 / 49 / 0 /
  0 after this revision as before it. The fixture file holds the wrap. Folding it with the same
  ...
  colon and its digits, so all four screens agree at `cf3a862` — but "inert" is a fact about
  ...
  **The tree pins this document stamps `335f535`, `74e126f` and `35698f9` are unchanged at
  `cf3a862`, the freeze sha this revision is authored against, and that closure is stated once here
  rather than re-stamped on ~40 pins**:
  `git diff --stat 335f535 74e126f` touches **9** files, all under `docs/`, and
  `git diff --name-only 74e126f cf3a862 -- h-mad handoff` is **empty** — no file under `h-mad/` or
  `handoff/` moved anywhere across that span, which contains `35698f9`, `6f0ee85` and `8909ec4` — so every `path:line`
  derived at `335f535`, `74e126f` or `35698f9` is
  byte-identical at `cf3a862`, and the older stamps are provenance facts rather than stale pins.
  Re-run in this revision, not carried from v1.42's: the `74e126f cf3a862` form is the one that
  matters now, because it is the only one that reaches the freeze sha, and `git diff --name-only
  cf3a862 -- h-mad handoff` is empty too, so the working tree a 5d implementer reads is that same
  tree.
  Sibling `docs/` values are the ones that did move, and each of those is re-derived
  above at `cf3a862`, the freeze sha this revision is authored against. A `docs/` pin has neither property — the
  ...
  **This is NOT a member of the list above at all, and v1.40 wrote that it was** (impl-plan audit
  ...
  of the list above is written nowhere in this document, here included** — it is `len(list)`, read
  ...
  **The residual, stated exactly**: the one-hit property is true only at the commit it was
  measured at, so it is re-established every revision or not at all. **Re-swept for v1.43 at
  `cf3a862`, every one of the 13 re-run in this revision rather than carried from v1.42's sweep or
  from any report: all 13 return exactly one hit.** Each needle was run against the sibling **as of
  that commit** — `git show cf3a862:<sibling> | grep -c -- '<needle>'` — and not against the working
...
`grep -n 'Heading selector differential' docs/01-plan/features/doc-block-exec.plan.md`, one hit, verified at `cf3a862`).
...
(`h-mad/scripts/h_mad_mutation_harness.py:482`); re-read in this revision at `cf3a862`, and
...
(one hit, verified at `cf3a862`). The `SUBST_OVERLAP` detail line is `overlap: "<a>" "<b>"` with **both** halves
...
(one hit, verified at `cf3a862`). Impl-plan v1.24 flagged those two rows as bare and half-quoted and **design
v1.81 answered it** — located with
``grep -n 'both halves of `overlap:`' docs/02-design/features/doc-block-exec.design.md``
(one hit, verified at `cf3a862`) — so the flags were withdrawn at v1.35. The three `design.md:` line numbers this paragraph carried
...
**re-run at `cf3a862` in this revision rather than inherited**, because the design moved to a new
revision in that commit and a count over a moved table is not a carried count: over the data rows
...
verified at `cf3a862`) — count those whose mechanism column names
...
hit, verified at `cf3a862`), so 80 + 1.
...
hit, verified at `cf3a862`). It is `registry-row-removed`'s partner **by
...
exactly one hit (verified at `cf3a862`), and that hit is the line to read. A previous
...
paragraph, located with `grep -n 'Bounds: 1200 s' docs/01-plan/features/doc-block-exec.plan.md` (one hit, verified at `cf3a862`),
