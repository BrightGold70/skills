...
- **When each row's payload is fixed — deliberate, not an omission** (impl-plan audit v20, whose
  ...
  one hit, verified at `1cbddb7`: exact `find` anchors are set from the landed source **in the same
...
- **Single source of the fence grammar**: **within the two files this feature touches —
  ...
  **none is a migration target for this feature**:
  - `_titled_section` (`h-mad/tests/test_h_mad_context_budget_docs.py:69`) bounds on
    ...
    — one hit, verified at `1cbddb7`: it anchors on a **substring**, so
...
- **No sentence in this document states what a sibling document currently says.** A debt or a
  ...
  *class* can still be matched by prose containing no literal copy of anything — the
  ...
  edit lands.
  **The sweep for this exposure across the whole document, enumerated rather than asserted.** Six
  screens are scoped to *this document* and are therefore exposed at all; every other screen here
  counts `h-mad/`, `handoff/`, a sibling under `docs/` or a `.py` source file, where this
  document's own prose is out of scope by construction. The six, and the disposition of each:
  ...
  **(vi)** the member-(5) survivorship chain's `grep -c` over this file, whose needle is a fragment
  ...
  3 at the sha after it (the two shas are named where the chain is written). Four of
  the six had the rule applied before it was named, one is the regex-class residual, and one is the
  instance that named it — which is the argument for writing it down here rather than rediscovering
  it a third time.
  ...
  **The residual, stated exactly**: the one-hit property is true only at the commit it was
  measured at, so it is re-established every revision or not at all. **Re-swept for v1.47 at
  `1cbddb7`, every one of the 13 re-run in this revision rather than carried from an earlier sweep
  ...
  that commit** — `git show 1cbddb7:<sibling> | grep -c -- '<needle>'` — and not against the working
  ...
  `git diff --name-only 7d8e797 1cbddb7 -- docs/01-plan/features/doc-block-exec.plan.md
...
`grep -n 'Heading selector differential' docs/01-plan/features/doc-block-exec.plan.md`, one hit, verified at `1cbddb7`).
...
(one hit, verified at `1cbddb7`). The `SUBST_OVERLAP` detail line is `overlap: "<a>" "<b>"` with **both** halves
...
(one hit, verified at `1cbddb7`). Impl-plan v1.24 flagged those two rows as bare and half-quoted and **design
v1.81 answered it** — located with
``grep -n 'both halves of `overlap:`' docs/02-design/features/doc-block-exec.design.md``
(one hit, verified at `1cbddb7`) — so the flags were withdrawn at v1.35. The three `design.md:` line numbers this paragraph carried
...
**re-run at `1cbddb7` in this revision rather than inherited**, because the design moved again
...
covers: `git diff --stat 7d8e797 1cbddb7 -- docs/02-design/features/doc-block-exec.design.md` is
**136 insertions / 33 deletions** (the previous span, `4e4a00c`..`7d8e797`, was 303 / 39). **A stamp in this document is DATED, not standing** — it records
...
authority. Its **whole** table for this revision is published in the Version History entry rather
than replaced by a closing sentence or by a chosen subset of the rows. v1.45's closure that "the `docs/`-facing figures were
...
verified at `1cbddb7`) — count those whose mechanism column names
...
hit, verified at `1cbddb7`), so 80 + 1. The 81 are **81 distinct row names** — the count is over
`sort -u`'d names, not over lines, so a duplicated row could not inflate it — and every one of the
81 occurs in this document, checked at `1cbddb7` by testing each design row name for membership in
this file: **0 missing**.
...
hit, verified at `1cbddb7`). It is `registry-row-removed`'s partner **by
...
exactly one hit (verified at `1cbddb7`), and that hit is the line to read. A previous
...
paragraph, located with `grep -n 'Bounds: 1200 s' docs/01-plan/features/doc-block-exec.plan.md` (one hit, verified at `1cbddb7`),
