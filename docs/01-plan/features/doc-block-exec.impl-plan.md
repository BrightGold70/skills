...
**OPEN-DECISION (r19, 5d):** decide whether the CLI and the API share ONE key-validity predicate while each caller keeps its own diagnostic payload, or the cross-surface equivalence test is added instead, since the empty-key predicate is prescribed independently on both surfaces and each spelling carries its own mutation row — source: `docs/02-design/features/doc-block-exec.design.audit.v99.codex.md` must 3; premise as filed by codex, re-derive before choosing.
