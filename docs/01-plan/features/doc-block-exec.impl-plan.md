...
(`doc_block_exec.json` 85 rows, `doc_block_exec_wire.json` 8, `docsections.json` 8) must report `ALL_CAUGHT`.
...
- **When each row's payload is fixed — deliberate, not an omission** (impl-plan audit v20, whose
  ...
  ground; the file held 76 rows at that cycle and holds 85 now, and the ground is unchanged by the
  ...
  the anchor-*text*-rewritten case, not the anchor-*file*-moved one. **`doc_block_exec.json`'s 85
  rows are NOT wholly on the second side, and saying so was an over-statement of this rule against
  its own row list** (impl-plan audit v47 codex should 2): **84** of them anchor in
  `h-mad/scripts/h_mad_doc_block_exec.py`, which does not exist today, and are wholly on the second
  side; the eighty-fifth is `registry-row-removed`, whose anchor file is `h-mad/SKILL.md` — a file
...
- **Single source of the fence grammar**: **within the two files this feature touches —
  ...
  **class** — *hand-rolled `##`-slicers that recognise no marker run* — and it is given as a scope
  ...
  below prints **23** named helpers on the tree v1.53 ships, up from **22** at `335f535` and at
  ...
  **23**, `09e9307` **23**, and **23** on the tree v1.53 ships. The new member is
  `h-mad/scripts/h_mad_assemble_audit.py:247` `_trim_version_history`, verified at `fbc2ea0` by
  `sed -n '247p'` reading `def _trim_version_history(text: str, keep: int | None, *, ref: str) -> str:`
  — the function the round-sixteen freeze commit itself added, which
  `git diff --name-only 3f70eb3 af19d53` names among its files. It is a **non-slicer the over-count
...
- **No sentence in this document states what a sibling document currently says.** A debt or a
  ...
  **The marker screen — its scope, its target and its three readings.** It is published here in
  ...
  On the tree v1.53 ships, re-run **after** v1.53's own edits landed rather than
  before them (decision K): body **5** occurrences, whole file **5** occurrences, and the third
  reading prints **five lines, each with count 1** — unmoved from v1.52's readings, which is a
  result and not an assumption, since v1.53 rewrites prose inside this very bullet. The first two must agree, since a bolded
  ...
  **0** at v1.40, v1.41 and v1.42, **1** at v1.43, and **0** at v1.44 and at every revision after
  ...
  narrower one: at `fbc2ea0`, the sha v1.53 is authored against, body **0** / **0** and whole file
  **2** lines / **3** occurrences; on the tree v1.53 ships, re-run after v1.53's own edits landed
  rather than before them (decision K), body **0** / **0** and whole file **2** lines / **3**
  occurrences. What the two readings establish is that v1.53's edits did not reopen the class, not
  that they closed anything. **Both stamps are the shipping revision's, re-derived rather than
  ...
  **Re-run at v1.53 against base `fbc2ea0`, which holds v1.52** (decision K a sixth time — v1.53
  ...
  `h-mad/scripts/h_mad_assemble_audit.py:247` (the `_trim_version_history` member residual (a)'s
  ...
  the file at `fbc2ea0` rather than transcribed. **The occurrence count rises by SIX for three new
  ...
  (iii) already carries, arriving through a naming rather than a screen. It is left standing rather
  ...
  **(iv) The four screens above cover `.py:` pins only, and the `SKILL.md`-pin class this document
  ...
  this document's body, both returning **0** on the tree v1.53 ships and both re-run after
  v1.53's edits landed rather than before them (the stamp is the shipping revision's, re-derived
  ...
  **7** satisfied the preference; **on the tree v1.53 ships the population is 14 and the split is
  8 + 6**, the eighth being v1.50's own addition and v1.51 through v1.53 having added none.
  **This site is a member of the stamp-carry class the marker screen sweeps, and it carried the
  ...
  re-stamped: all **14** needles were run again at `fbc2ea0` against the three siblings read out of
  that commit, one needle per invocation, and **every one returns exactly one hit in its stated
  target**, with the split still **10 design / 2 plan / 2 spec** and the same **two** second-target
  caveats and no others. The two lists below are the v1.50 ones. The
  **8** satisfying the preference (`` `_titled_section` anchors on a substring ``,
  ...
  that needs `grep -F`) while **6** are bare
  ...
  **This sweep is itself exposed to the needle-inside-scope rule stated in Conventions, and it is
  ...
  per needle rather than asserted as a universal** (delta self-review r15). **On the tree v1.52
  ships**, re-run after v1.53's last edit landed rather than before it (decision K), over the
  body, by line: the debt word **26**, `spec\.md:` **1**, `design\.md:` **1**, `plan\.md:` **0** —
  and 26 + 1 + 1 + 0 is the **28** the whole sweep returns, so no line is reached by two needles.
  ...
  **26**, `3f70eb3` **23** / **25**, `af19d53` **23** / **25**. **The rule that closes the class is
  ...
  on the tree v1.52 ships and **28** on the tree v1.53 ships — so the
  ...
  and five-needle forms both return 28 on this body, because the one prose pin the fifth needle
  was written for was repaired in the same revision that added it. **The composition of the 28 body
  hits on the tree v1.53 ships, re-derived by classifying every one of them rather than carried
  from the previous revision's split**:
  **21** match only through a word that *contains* the debt word — `followed`, `allowed`,
  `narrowed` — so the confound this bullet already named is **wider than the one participle it
  names**, which is the correction; **5** write that word on its own, and **4** of those five are
  this rule's own text (the two sentences quoting the forbidden phrasing, the sweep line above, and
  the confound note itself), leaving **1** genuine use elsewhere in this document; and **2** write a
  sibling-filename-plus-colon needle, both in prose recording that pins were withdrawn. So **6** of
  the 28 are the exposure, **1** is a live use, **21** are the substring confound, and **0** are the
  defect the sweep looks for. **One bin moved at v1.51 and only one, and it moved DOWN**: v1.50's
...
85; a row on this axis, if the design wants one, is the design's to add). What is added is the
...
- the **tab arm is 0, and it is pinned by nothing at all** — no AC in this document prescribed a
  ...
  case. **No mutation row follows**: the matrix total stays **85**, and a row on this axis, if the
...
there is exactly one member. **It is located by a needle here and deliberately not by a line number, even though the 09-03 probe's own output line names one** —
`grep -c '^#$' h-mad/SKILL.md` returns **1** and `grep -n '^#$' h-mad/SKILL.md` reaches it at one
hit, verified at `fbc2ea0` — because a path-qualified `SKILL.md` line pin is the class this
document has reintroduced six times and the standing control
`h-mad/tests/test_h_mad_precheck_doc.py::test_control_todays_impl_plan_dropped_the_six_stale_SKILL_pins`
turns red on one. The member is a **bare `#` on a line of its own**, sitting immediately above the
`## Reading a dispatch verdict` heading; it is CommonMark's empty ATX heading (§4.2 admits a hash
run with no content), which the ATX predicate accepts and the space-required regex rejects.
**It is not NEWLY false, and "false on the live tree" understated it** (round seventeen, the plan author's run; re-derived here rather than taken). The same committed probe, run from a detached worktree at each sha in turn with the probe copied in so the instrument is constant while the corpus varies, reads `new_only=` **0** at `1861157` and **1** at `a8e0372`, `35698f9`, `cf3a862`, `4e4a00c`, `74e126f` and `fbc2ea0` alike — seven readings, one worktree each. The `1861157` run also reproduces the transcribed `files=25 both=263 old_only=76 new_only=0` exactly, which is the first re-derivation of that quadruple this document has made rather than transcribed. So the invariant has been false at **every sha after `1861157`**, through every revision that restated it, and the freeze that surfaced it did not cause it. **Which probe prints WHAT, because the two versions differ and citing the wrong one is how a reader fails to reproduce this**: the **09-03** probe, `docs/03-analysis/probes/doc-block-exec/heading_differential.2026-09-03.cd979362.py`, is glob-only and is the one that prints the `NEW-ONLY` line naming the member; the **09-04** probe prints the `--- TRACKED` / `--- GLOB` pair of quadruples and its `OLD-ONLY` lines and **no** `NEW-ONLY` line at all — verified by running both at `fbc2ea0`.
...
rather than a property of the two grammars. **The line is NOT repaired this round, deliberately**:
editing `h-mad/SKILL.md` moves the freeze and expires the readings three sibling documents stamp
against it, so it is left in place and its status recorded. **Residual, stated as a category**:
...
matrix and adding one would put this document's total at 86 against a matrix of 85. What stands in
...
- **The substring predicate** (unchanged, map-static): if any key is a substring of another, the
  refusal carries `pairs`, the sorted list of unordered `(shorter, longer)` tuples, one
  `overlap: "<a>" "<b>"` detail line each.
- **The span-intersection predicate** (new, text-dependent) runs **beside** it, never instead of
  ...
  refusal carries `intersections`, the sorted list of `(a, b, offset)` triples — one per unordered
  pair of keys, sorted by `(offset, a, b)` — one `intersect: "<a>" "<b>" "<offset>"` detail line
  each. **Two determinism rules fix which triple a pair yields, and they are stated because the
...
gains a second field**: `def __init__(self, pairs, intersections=())`, whose body binds
`self.intersections = list(intersections)` so both fields are lists at every read site. **The signature is this
document's own decision**, on the same ground and by the same mechanism as
`StreamPathUnwritable.__init__` below — the detail lines are fixed by the verdict grammar and a
field on the exception is this document's only way to carry one — and the default keeps every
existing raise site valid. **Ordering against `MissingSubstitution`, stated rather than left
...
**Residual, stated exactly**: a key intersecting **itself** (`aa` in `aaa`) is not an intersection
between two keys and is deliberately outside the predicate. It needs no guard, because
`text.count`'s non-overlapping count equals the regex's non-overlapping match count on the same
key — measured: `aaa` under `{aa→Z}` returns `Za` with `text.count` reading `1` and the callback
fired `1`, so the reported count still equals the replacements performed and AC-2.7's promise
holds. A second uncovered shape would be a key whose matches are made to overlap by the
...
**Acceptance Criteria** (ten tests — the ten enumerated below, one per `test_` name):
...
- [ ] AC-2.7 (substring clause) `test_overlapping_keys_refuse`: keys `a`, `ab`, `abc` → `OverlappingSubstitution` with `pairs == [("a","ab"),("a","abc"),("ab","abc")]`, `intersections == []` (no text is needed to reach this arm and the fixture text holds no `abc`), and three distinct keys implicated. **Both fields are LISTS, empty-list default included**, so an assertion never has to distinguish a tuple from a list; the constructor signature below is written `intersections=()` only because a mutable default is not a legal one, and the body binds `list(intersections)`.
- [ ] AC-2.7 (intersection clause) `test_substitute_refuses_intersecting_spans`: on a block whose text is `abc`, `substitute(block, {"ab": "X", "bc": "Y"})` raises `OverlappingSubstitution` with `pairs == []` (neither key contains the other) and `intersections == [("ab", "bc", 1)]`, two distinct keys implicated, and the block's text unchanged — nothing was replaced. **The emitted detail line is asserted verbatim**, `intersect: "ab" "bc" "1"`, which is what pins all three determinism rules at once: the three-value quoted shape with no connective, `ab` before `bc` because it is lexicographically smaller, and `1` because that is the smallest index the spans `[0, 2)` and `[1, 3)` share — `0` under the superseded wording, so the assertion discriminates the two readings rather than merely accepting either. **The true-negative arm is in the same test and is what keeps the predicate from being a blanket refusal**: on a block whose text is `ab bc ab bc` the same map substitutes to `X Y X Y` with counts `{"ab": 2, "bc": 2}`, because no two spans share an index. Both legs measured on 3.11.8 before this AC was written.
...
**ten** tests this task adds, all with `AttributeError` (`substitute` absent; its three exceptions
already exist from Task 1). **Passing**: every test Task 1 landed in
...
(`references/codex-implementer-prompt.md:62`, read at `fbc2ea0`; the assembler only prints the pair
...
stale. `--expect-fail` is **10**, the count of the AC list above.
...
round resolved it the other way: the matrix moves to **85** and the design owes the row, which is
the correct direction whenever the guard is real and the only obstacle is a count. The tab-arm
...
other composition path — the third row in this document whose mutant reds a second named test, and
the Conventions bullet enumerates it there. **It is discriminated from `launch-oserror-unwrapped`
...
and `__suppress_context__ True` — the cleanup error suppressed rather than selected, which is the
exact defect the two branches exist to prevent; killed by
...
`_field(value)`, and the other 7 are rendered bare by construction — **re-derived on the tree v1.53
ships from the `DETAIL_KEYS` tuple as v1.53 edits it below rather than carried: 12 members
in the tuple, 15 head field names, 15 + 12 = 27 slots; 8 quoted head fields + 12 quoted detail
values = 20 through the renderer; 27 - 20 = the 7 bare** (design v1.75/v1.78/v1.80,
...
25 + 6 + 26 + 28 = **85 rows**, split **84 of the helper's source and 1 of `h-mad/SKILL.md`**.
...
verified at `700c599`) — count those whose mechanism column names
...
this batch, and until then the 85 rests on the decision rather than on a sibling reading.
...
the mode the earlier text got right.
...
`doc_block_exec.json` must report `ALL_CAUGHT` over all 85 rows
...
hmad-dispatch run --timeout 600 -- python3.11 scripts/h_mad_mutation_harness.py tests/mutation-specs/doc_block_exec.json        # MUTATION: ALL_CAUGHT mutations=85
...
and **2809 → 2814** / **2547 → 2552** between `700c599` and `fbc2ea0`, both **+5** — and by this
