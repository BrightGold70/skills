...
- **No sentence in this document states what a sibling document currently says.** A debt or a
  ...
  **The marker screen — its scope, its target and its three readings.** It is published here in
  ...
  On the tree v1.50 ships, re-run **after** v1.50's own edits landed rather than
  ...
  **The site is stamped to the revision that ships it, not to the revision whose repair last
  ...
  three readings belonged to. They are unmoved by v1.49's and by v1.50's edits — and re-running
  ...
  **0** at v1.40, v1.41 and v1.42, **1** at v1.43, and **0** at v1.44 and at every revision after
  ...
  narrower one: at `b3be433`, the sha v1.50 is authored against, body **0** / **0** and whole file
  **2** lines / **3** occurrences; on the tree v1.50 ships, re-run after v1.50's own edits landed
  rather than before them (decision K), body **0** / **0** and whole file **2** lines / **3**
  occurrences. What the two readings establish is that v1.50's edits did not reopen the class, not
  ...
  **(iv) The four screens above cover `.py:` pins only, and the `SKILL.md`-pin class this document
  ...
  this document's body, both returning **0** on the tree v1.50 ships and both re-run after
  v1.50's edits landed rather than before them (the stamp is the shipping revision's, re-derived
  and not carried — v1.49 left it at v1.48 while shipping further edits; impl-plan audit v45):
  ...
  **7** satisfied the preference; **on the tree v1.50 ships the population is 14 and the split is
  8 + 6**, the eighth being v1.50's own addition. The two lists below are the v1.50 ones. The
  ...
  **Re-swept for v1.50 at `b3be433`, and the population is now FOURTEEN, not thirteen**
  ...
  repair of a stale prose line pin in Task 3's delta block replaces it with a locator on the
  ...
  **A sibling location written as a SECTION REFERENCE — `§Name` — is the SAME class**, and unlike
  ...
  re-derivation are in Task 3's delta block). **The rule over the class**: a sibling `§`-reference is admissible
  ...
  **Applied over the population rather than at the instance**: the members are found by reading
  ...
  section's line span in the shipping sibling. Exactly one failed, the one Task 3 carried; the
  ...
  **This sweep is itself exposed to the needle-inside-scope rule stated in Conventions, and it is
  ...
  per needle rather than asserted as a universal** (delta self-review r15). At `dfae038`, over the
  ...
  the tree v1.49 ships, **26** on the tree v1.50 ships and **25** on the tree v1.51 ships — so the
  ...
  and five-needle forms both return 25 on this body, because the one prose pin the fifth needle
  was written for was repaired in the same revision that added it. **The composition of the 25 body
  hits on the tree v1.51 ships, re-derived by classifying every one of them rather than carried
...
- **No body sentence in this document identifies the shipping revision by a RELATIVE phrase.**
  ...
  **The screen, and it is a gate rather than a triage grep, because after the relabel there are no
  ...
  It reads **0** on the body v1.50 ships, re-run after v1.50's last edit landed — the stamp is the
  shipping revision's and was re-derived, not carried (v1.49 left it at v1.48 while shipping
  further body edits, and this site is a fourth member of the class impl-plan audit v45 filed
  against three). **The needles are
