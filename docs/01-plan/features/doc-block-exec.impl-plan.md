...
- **Mutation spec** `h-mad/tests/mutation-specs/doc_block_exec.json`: `root` is `../..`,
  ...
  **That per-task file is STAGING, not a convention, and the merge is owed by the implementation**:
  ...
  files. Three further readings agree: the two files' row-name sets are disjoint and their `command`
  names the SAME test file, so this is one spec split in two; `doc_block_exec_task3.json` is the only
  `*_task[0-9]*.json` among the 42 specs in `h-mad/tests/mutation-specs/`; and `ebd8e27`, the commit
  that created it, justifies its timing under D2 and gives no per-task rationale. **So the three-spec
  ...
  **The 32 above and the end state below are the SAME population read at two times, and saying so is
  the repair**: 32 of the table's 90 rows are in that file today, 28 more are staged in the per-task
  file, and Task 4's 30 have not been written at all — `32 + 28 + 30 = 90`. The 5e obligation, stated
  so it is checkable rather than assumed: before the feature is complete all **90** rows live in
  ...
  '_task[0-9]+\.json$'` must print `0`, and the merge must reconcile the two files' `target_command`
  (the staged file carries the absolute `/opt/anaconda3/bin/python3.11`, which is not portable and
  is not what the merged spec ships). **Consequently the `ALL_CAUGHT` gate each task
...
**OPEN-DECISION (r19, 5d):** decide whether the alias-refusal branch verifies its unlink with a read-back and a discriminating failure test, or records why that unlink cannot fail silently, since the branch deletes a reservation it created while this task states it grows no read-back — source: `docs/01-plan/features/doc-block-exec.impl-plan.audit.v50.codex.md` must 2; premise as filed by codex, re-derive before choosing.

**OPEN-DECISION (r19, 5d):** decide whether the standalone-`--help` exemption is enforced explicitly by pre-scanning argv, with negative tests for the mixed help forms, or the contract is broadened and its boundary tested, since the prescribed parser prints help and exits 0 for `--bogus --help` and for a valid document plus `--help`, bypassing the malformed-invocation verdict — filed by BOTH legs — source: `docs/01-plan/features/doc-block-exec.impl-plan.audit.v50.codex.md` must 3; premise as filed by codex, re-derive before choosing.
...
**Tasks 1, 2 and 3 are re-derived from the LANDED specs, not from this document** — `python3 -c
"import json;a=json.load(open('h-mad/tests/mutation-specs/doc_block_exec.json'))['mutations'];b=json.load(open('h-mad/tests/mutation-specs/doc_block_exec_task3.json'))['mutations'];i=[m['name'] for m in a].index('missing-key-silently-skipped');print(i, len(a)-i, len(b))"`
prints `25 7 28`, and all 60 of those rows carry `file: scripts/h_mad_doc_block_exec.py`, 0 of them
`h-mad/SKILL.md`. **Task 4's 30 is the one addend that CANNOT be derived that way and is marked as
