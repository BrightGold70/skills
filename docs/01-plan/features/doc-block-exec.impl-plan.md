...
- **No sentence in this document states what a sibling document currently says.** A debt or a
  ...
  **(vii)** the pre-dispatch sweep in the sibling-claim bullet below, whose five needles are
  ...
  class v1.50 adds — the first four are every one of them literal and every one of them present
  in the body it counts, and the fifth cannot match itself by construction. **Its disposition is that it
  ...
  *next* such pin being triaged rather than this one being found. **Residual, as a concrete
  category**: a sibling location in a further synonym — *at line N of*, *the Nth line* — matches
  neither the fifth needle nor any other instrument here, so the class is narrowed, not closed.
  **This sweep is itself exposed to the needle-inside-scope rule stated in Conventions, and it is
  ...
  literal strings and all four occur in the body it counts**; the fifth is the character-class
  form above and cannot. **This reading moves with every
  revision and is therefore stamped**: the sweep returns **23** body hits at `700c599`, **25** on
  the tree v1.49 ships, and **26** on the tree v1.50 ships — so the round-thirteen audit's 23
  ...
  and five-needle forms both return 26 on this body, because the one prose pin the fifth needle
  was written for is repaired in the same revision that adds it. **The composition of the 26 body
  hits on the tree v1.50 ships, re-derived by classifying every one of them rather than carried
  from the previous revision's split**:
  **19** match only through a word that *contains* the debt word — `followed`, `allowed`,
  ...
  the 26 are the exposure, **1** is a live use, **19** are the substring confound, and **0** are the
  defect the sweep looks for. **The one hit v1.50 adds is its own residual sentence in the
  paragraph above**, which reaches the sweep through a participle ending in the debt word — a
  substring confound and not a needle, which is why the confound bin moved and the other two did
  not, and why this sentence describes that participle instead of writing it. **It publishes no
  reading**, so a corrupted count is not its failure mode; it is a triage aid read hit by hit, and
  what the exposure costs is 25 lines of triage for one real hit — which is worth stating, because
...
# ^ the design's spelling verbatim (design v1.79 §Scanning) and the same idiom every
#   test under h-mad/tests/ uses for SCRIPT_DIR. The design's location for that spelling is
#   written as a LOCATOR and never as a number: `grep -Fn` on the insert line above, run over
#   docs/02-design/features/doc-block-exec.design.md, returns exactly one hit, re-run at `b3be433`
#   on the design's shipping bytes. The -F is load-bearing and not tidiness: the bracket makes this
#   needle a BRE character class and a plain grep returns zero on text that is present. The plan
#   carries the same line once, so the one-hit property holds under the stated target file — the
...
them), and the feature adds nothing under `references/`, `hooks/` or `scripts/*.sh`. **`_SCANNED`
is GLOB-FED, so this membership statement is a reading of a tree and not a property of a list**
(impl-plan audit v45): three of the eight sources are globs, and a later `references/*.md` or
