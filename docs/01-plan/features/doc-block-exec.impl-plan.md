...
- **No sentence in this document states what a sibling document currently says.** A debt or a
  ...
  **The marker screen — its scope, its target and its three readings.** It is published here in
  ...
  On the tree v1.48 ships, re-run **after** v1.48's own edits landed rather than
  ...
  **ten** instruments scoped to *this document* and therefore exposed at all; every other screen
  ...
  derived by recalling what it had repaired rather than by walking, and it was short by three**
  (impl-plan audit v44) — an enumeration published as complete is a completeness measurement under
  decision G, and a measurement is derived, never recalled. The tenth is the self-reference screen
  ...
  **(vii)** the pre-dispatch sweep in the sibling-claim bullet below, whose four needles — three
  sibling-filename-plus-colon forms and one short word naming a debt — are every one of them
  literal and every one of them present in the body it counts. **Its disposition is that it
  ...
  **character classes** so that the published screen cannot match itself — the only member of the
  ...
  be a hard **0** instead of a triage list. Five of
  the ten had the rule applied before it was named, one is the regex-class residual, one is the
  instance that named it, **two were found by the walk and by nothing else**, and one was written
  under the rule — which is the argument for deriving this enumeration rather than rediscovering it
  a third time.
  **Residual of the WALK — stated as categories of what a walk of this file cannot reach, not as
  residual of the rule.** (a) An instrument whose scope reaches this file through a variable bound
  several lines earlier and never re-named: the two `SKILL` screens are exactly that shape, and the
  walk sees them only because the fence binding the variable is the same fence. A screen written in
  a later paragraph against a variable bound in an earlier one would be invisible to both
  signatures. (b) An instrument scoped to a **directory** that contains this file, so that this
  ...
  **0** at v1.40, v1.41 and v1.42, **1** at v1.43, and **0** at v1.44 and at every revision after
  ...
  narrower one: at `700c599`, the sha v1.48 is authored against, body **0** / **0** and whole file
  **2** lines / **3** occurrences; on the tree v1.48 ships, re-run after v1.48's own edits landed
  rather than before them (decision K), body **0** / **0** and whole file **2** lines / **3**
  occurrences. What the two readings establish is that v1.48's edits did not reopen the class, not
  that they closed anything. Every surviving
  ...
  **(iv) The four screens above cover `.py:` pins only, and the `SKILL.md`-pin class this document
  ...
  this document's body, both returning **0** on the tree v1.48 ships and both re-run after
  v1.48's edits landed rather than before them:
  ...
  **The tree pins this document stamps `335f535`, `74e126f` and `35698f9` are unchanged at
  ...
  that
  matters now, because it is the only one that reaches the sha v1.48 is authored against, and `git diff --name-only 700c599 -- h-mad handoff` is empty too, so the working tree a 5d implementer reads is that same
  tree.
  ...
  **The residual, stated exactly**: the one-hit property is true only at the commit it was
  measured at, so it is re-established every revision or not at all. **Re-swept for v1.48 at
  `700c599`, every one of the 13 re-run at v1.48 rather than carried from an earlier sweep
  ...
  that commit** — `git show 700c599:<sibling> | grep -c -- '<needle>'` — and not against the working
  tree, because the three sibling authors are revising those files concurrently while v1.48
  ...
  `git diff --name-only 1cbddb7 700c599 -- docs/01-plan/features/doc-block-exec.plan.md
  ...
  **This sweep is itself exposed to the needle-inside-scope rule stated in Conventions, and it is
  member (vii) of the enumeration there** (impl-plan audit v44). All four of its needles are
  literal strings and all four occur in the body it counts. **The composition of its 25 body hits,
  derived by classifying every one of them rather than by naming the ones a reader notices**:
  **18** match only through a word that *contains* the debt word — `followed`, `allowed`,
  ...
  the 25 are the exposure, **1** is a live use, **18** are the substring confound, and **0** are the
  defect the sweep looks for. **It publishes no
  reading**, so a corrupted count is not its failure mode; it is a triage aid read hit by hit, and
  what the exposure costs is 24 lines of triage for one real hit — which is worth stating, because
...
- **No body sentence in this document identifies the shipping revision by a RELATIVE phrase.**
  ...
  **The screen, and it is a gate rather than a triage grep, because after the relabel there are no
  ...
  It reads **0** on the body v1.48 ships, re-run after v1.48's last edit landed. **The needles are
...
INHERITED-UNVERIFIED label here as well as at AC-6.4 and in the §Verification (Phase 5f)
paragraph** — this is the site a 5d implementer reads them from, and the register rule says the
label stands at every site that uses a member, not only where the register is written (impl-plan
audit v44). Not challenged is not the same as verified: no round has re-run `263/76/0` or
