...
- **When each row's payload is fixed — deliberate, not an omission** (impl-plan audit v20, whose
  ...
  one hit, verified at `7d8e797`: exact `find` anchors are set from the landed source **in the same
  ...
  the anchor-*text*-rewritten case, not the anchor-*file*-moved one. `doc_block_exec.json`'s 81 rows are wholly on the second side.
...
- **Single source of the fence grammar**: **within the two files this feature touches —
  ...
  **none is a migration target for this feature**:
  - `_titled_section` (`h-mad/tests/test_h_mad_context_budget_docs.py:69`) bounds on
    ...
    — one hit, verified at `7d8e797`: it anchors on a **substring**, so
...
- **No sentence in this document states what a sibling document currently says.** A debt or a
  ...
  **The marker screen — its scope, its target and its three readings.** It is published here in
  ...
  On the tree this revision ships, re-run **after** this revision's own edits landed rather than
  before them (decision K): body **5** occurrences, whole file **5** occurrences, and the third
  reading prints **five lines, each with count 1**. The first two must agree, since a bolded
  marker inside the Version History would be a member standing outside the enumeration. **The
  ...
  **Residual, demonstrated rather than argued**, on copies under `mktemp -d` with the repository
  ...
  `sed` — the mechanism has **two steps and they belong to different programs**, re-run in this
  revision on a `mktemp -d` copy: `sed` does **not** fail silently, it exits **1** and prints
  `sed: 1: "s/**(4)** the 5f/(4) th ...": RE error: repetition-operator operand invalid` on stderr,
  because `*` is a repetition operator in BRE and `sed` refuses the pattern outright; the **empty
  ...
  **The screen for the class, with both of its readings.** The defect is a cardinal standing beside
  ...
  At `4e4a00c`, the freeze sha this revision is authored against and **before** this revision's
  edits: body **1** line / **1** occurrence, whole file **3** lines / **4** occurrences. On the
  revised tree this revision ships, re-run after the edits landed rather than before them
  (decision K): body **0** / **0**, whole file **2** lines / **3** occurrences. Every surviving
  ...
  **v1.40 wrote that this document "meets [these conditions] everywhere", and it did not.** Two
  ...
  `68a70d6`, **0** again at `7d8e797`, the sha v1.46 is authored against and stamps its re-runs at,
  and **0** again
  over the revised working tree. (`4e4a00c` is the commit v1.43 was audited **at**; it is the base
  v1.44 is written **against**, and the shipping
  commit of this revision does not exist while it is being written, which is why every figure here
  names `4e4a00c` and not a promise. `4e4a00c` contains this document byte-identically as `7982c18`
  ...
  (`awk '/^## Version History/{exit}{print}' <this file> | grep -oE 'at .<sha>.' | wc -l`, one
  reading per sha, published in the Version History entry). v1.45 asserted such a closure and it
  ...
  **path-qualified line-scoped / path-qualified folded / bare line-scoped / bare folded**.
  ...
  taken on the tree the revision produces** rather than on a commit that predates it. Base
  `4e4a00c` (which holds v1.43, this revision's input, byte-identically as `7982c18` does —
  `git diff --name-only 7982c18 4e4a00c -- <this file>` is empty): **49 occurrences across 19
  folded
  paragraphs**, re-derived in this revision and not carried. On the revised working tree this
  revision ships — the commit for it does not exist while it is being written, which is why no sha
  is claimed for the after-reading — it is **49 occurrences across 20 folded paragraphs**. **The
  two halves of that pair moved differently and the difference is the point** (decision K — this
  was re-run after the edits landed, and it is the one figure this revision's own edits changed):
  ...
  **49 / 49 / 0 / 0** at **20** folded paragraphs on the base and the same four integers at **20**
  ...
  the 19 → 20 move above stays a v1.44 fact and is not restated as this revision's.
  (`| wc -l` on `grep -oE` for
  ...
  **(iv) The four screens above cover `.py:` pins only, and the `SKILL.md`-pin class this document
  ...
  this document's body, both returning **0** on the tree this revision ships and both re-run after
  the edits landed rather than before them:
  ...
  **The tree pins this document stamps `335f535`, `74e126f` and `35698f9` are unchanged at
  `4e4a00c`, the freeze sha this revision is authored against, and that closure is stated once here
  rather than re-stamped on ~40 pins**:
  `git diff --stat 335f535 74e126f` touches **9** files, all under `docs/`, and
  `git diff --name-only 74e126f 4e4a00c -- h-mad handoff` is **empty** — no file under `h-mad/` or
  `handoff/` moved anywhere across that span, which contains `35698f9`, `6f0ee85`, `8909ec4`, `cf3a862` and `7982c18` — so every `path:line`
  derived at `335f535`, `74e126f` or `35698f9` is
  byte-identical at `4e4a00c`, and the older stamps are provenance facts rather than stale pins.
  Re-run in this revision, not carried from v1.43's: the `74e126f 4e4a00c` form is the one that
  matters now, because it is the only one that reaches the freeze sha, and `git diff --name-only 4e4a00c -- h-mad handoff` is empty too, so the working tree a 5d implementer reads is that same
  tree.
  Sibling `docs/` values are the ones that did move, and each of those is re-derived
  above at `4e4a00c`, the freeze sha this revision is authored against. A `docs/` pin has neither property — the
  ...
  **This preference is stated as a preference, not a hard rule, and the reason is measured**: at
  `cf3a862` this document carries **13** distinct `docs/`-sibling locators (7 + 6, the two lists
  ...
  **The residual, stated exactly**: the one-hit property is true only at the commit it was
  measured at, so it is re-established every revision or not at all. **Re-swept for v1.46 at
  `7d8e797`, every one of the 13 re-run in this revision rather than carried from an earlier sweep
  or from any report: all 13 return exactly one hit**, and
  the split is unchanged at 9 design / 2 plan / 2 spec = 13, with the same single caveat a bare
  13/13 hides (`git rev-parse --show-toplevel`, the spec locator, also returns one hit in the plan,
  so its one-hit property holds only under its stated target file; all 13 were run against all
  three siblings to establish that it is the only one). Each needle was run against the sibling **as of
  that commit** — `git show 7d8e797:<sibling> | grep -c -- '<needle>'` — and not against the working
  ...
  `git diff --name-only 4e4a00c 7d8e797 -- docs/01-plan/features/doc-block-exec.plan.md
  ...
  landed rather than only that it was one: **9** target the design, **2** the plan, **2** the spec,
  and 9 + 2 + 2 = 13. One caveat the split makes visible and a bare 13/13 does not:
  `git rev-parse --show-toplevel` — the spec locator — also returns one hit in the **plan**, an
  unrelated occurrence of the same command. Its one-hit property therefore holds only under its
  stated target file, which is how the rule reads it, and it is recorded here so the next sweep
  does not read that second occurrence as a break.
  The **count is unchanged at 13** and the 7/6 preference split with it: v1.43 added no locator —
...
scan without the fence toggle would have counted.
**That `2` is a composite and it is published split, because a total over a disjunction is not
per-branch evidence** (impl-plan audit v41; decision O). The decline predicate in the block above
...
`grep -n 'Heading selector differential' docs/01-plan/features/doc-block-exec.plan.md`, one hit, verified at `7d8e797`).
...
(`h-mad/scripts/h_mad_mutation_harness.py:482`); re-read in this revision at `4e4a00c`, and
...
(one hit, verified at `7d8e797`). The `SUBST_OVERLAP` detail line is `overlap: "<a>" "<b>"` with **both** halves
...
(one hit, verified at `7d8e797`). Impl-plan v1.24 flagged those two rows as bare and half-quoted and **design
v1.81 answered it** — located with
``grep -n 'both halves of `overlap:`' docs/02-design/features/doc-block-exec.design.md``
(one hit, verified at `7d8e797`) — so the flags were withdrawn at v1.35. The three `design.md:` line numbers this paragraph carried
...
**re-run at `7d8e797` in this revision rather than inherited**, because the design moved again
...
covers: `git diff --stat 4e4a00c 7d8e797 -- docs/02-design/features/doc-block-exec.design.md` is
**303 insertions / 39 deletions**. **A stamp in this document is DATED, not standing** — it records
...
screen is `awk '/^## Version History/{exit}{print}' <this file> | grep -oE 'at .<sha>.' | wc -l`,
one reading per sha, and its readings for this revision are published in the Version History entry
rather than replaced by a closing sentence. v1.45's closure that "the `docs/`-facing figures were
...
verified at `7d8e797`) — count those whose mechanism column names
...
hit, verified at `7d8e797`), so 80 + 1. The 81 are **81 distinct row names** — the count is over
`sort -u`'d names, not over lines, so a duplicated row could not inflate it — and every one of the
81 occurs in this document, checked at `7d8e797` by testing each design row name for membership in
this file: **0 missing**.
...
hit, verified at `7d8e797`). It is `registry-row-removed`'s partner **by
...
count) and **2486** (the count from `h-mad/`) — the pair this document uses everywhere.
...
the predicate behind 2675 is not published here, so the **invariance** is verified and the
absolute is not.)
...
fourth consecutive round with no surface on either leg re-running them). **Executed at `68a70d6`
in this revision**, both commands run rather than transcribed:
`python3.11 -m pytest --collect-only -q -p no:cacheprovider` collects **2809** from the repository
root and **2547** from `h-mad/` — a divergence of exactly **262**, which is the divergence this
paragraph publishes, reproduced on a live tree. The *relation* the floor rests on is therefore
measured, not inherited. Three things are **not** verified by that run and are not laundered by it:
the historical values **2748** and **2486** at `b7d0d77`, which would need a checkout of that
commit this authoring pass did not take; **the `2675` function census**, whose published predicate
...
differentials with their markdown-it-py CommonMark oracles, which are the plan's figures, are only
cited here, and remain inherited-unverified with no round having re-run them. What the live run
does establish about the historical pair is its **staleness**: the root count has moved
**2748 → 2809** since `b7d0d77` and the `h-mad/` count **2486 → 2547**, both **+61** — and by this
...
exactly one hit (verified at `7d8e797`), and that hit is the line to read. A previous
...
paragraph, located with `grep -n 'Bounds: 1200 s' docs/01-plan/features/doc-block-exec.plan.md` (one hit, verified at `7d8e797`),
