...
- The two extractors named in FR-6 are the only in-repo consumers that anchor on a bare
  ...
  **A second, distinct sub-class arrived in v1.61 and v1.62 and is named rather than left implicit**:
  ...
  to agree; re-run here rather than carried — and `104` over the v1.64 draft; the unit is
  *occurrences of the
  token*, not lines
  ...
  floor — are each stated in negatives. The reason is
  ...
  admitting `no`/`none` would put up to `104` candidate occurrences in front of a reader to
