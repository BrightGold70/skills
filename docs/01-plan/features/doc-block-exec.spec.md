...
- **Acceptance Criteria**:
  ...
  - AC-2.7: **Two keys whose replacements are not independent refuse rather than resolve by
    ...
    **The second clause is added beside the first, never in place of it, because the two are
    ...
    first. **`<offset>` is the smallest character index shared by *any* intersecting span pair of
    the two keys**, a 0-based index into `block.text` — *not* the start of the earlier span, which
    is a different number: for `ab`/`bc`
    over `abc` the spans are `[0,2)` and `[1,3)`, they share only index `1`, and the line reads
    `intersect: "ab" "bc" "1"`. **The minimum is taken over span pairs, not over one pair, because
    the scan below enumerates every occurrence of every key**: two keys may intersect at several
    indices, one line is emitted for the unordered pair, and it carries the smallest of them.
    **Residual, stated as a concrete category**: the diagnostic under-reports the *number* of
    intersections a pair has, never their *existence* — the refusal is total and nothing is
    executed under either count, so this is a diagnostic-completeness residual and not a safety
    one, and no caller branches on the missing line.
    Both rules are written down because "the first intersecting
    ...
    **"Every match span" means every occurrence, OVERLAPPING OCCURRENCES OF THE SAME KEY
    INCLUDED**, and that is a requirement on the scan and not an implementation note, because a
    non-overlapping enumeration misses whole intersections and publishes a clean run over them.
    Measured on Python 3.11.8 for the text `aaab` under keys `{aa, ab}`: a non-overlapping
    enumeration yields `aa` at `[0,2)` and `ab` at `[2,4)`, which share **no** index, while
    enumerating every occurrence also yields `aa` at `[1,3)`, and `[1,3)` and `[2,4)` share index
    `2` — so this map must refuse, with the line `intersect: "aa" "ab" "2"`, and the
    non-overlapping scan would have executed the block. **The class this closes is
    "an enumeration that drops occurrences drops intersections", not the `aaab` instance**: any
    key whose occurrences can overlap each other is a member, which is every key that is a
    non-trivial repetition of a shorter string, and the rule is written over the axis rather than
    over the fixture.
    **The class is "two keys' matches are not independent"; containment was one member and span
    intersection is the other. Residual, restated — v1.63's residual was true in its claim and
    wrong in its reason, and the reason was load-bearing**: a key that intersects *itself*
    (`aa` in `aaa`) is still not refused, because both clauses are predicates over two **distinct**
    keys, and `"aaa".count("aa")` is `1` on Python 3.11.8 with `1` non-overlapping match, so
    AC-2.5's count-equals-replacements claim holds over that single-key case. What v1.63 got wrong
    is what followed from it: that reading was used as the reason the scan need not enumerate
    overlapping occurrences, and it is not — the self-overlapping occurrence a non-overlapping
    enumeration drops is exactly the span that intersects a *different* key, which is the `aaab`
    case above. The self-overlap is therefore **enumerated and never refused on its own**.
...
- **Acceptance Criteria**:
  ...
  - AC-3.9: `--stdout` and `--stderr` naming the **same path** refuses with
    ...
    no bytes. **The unlink is read back, exactly as AC-3.10's rollback is** — this branch creates a
    reservation by the same exclusive-create open and removes it by the same `os.unlink`, so it
    fails in the same way and under the same injection, and "it cannot fail silently" is not
    available as an answer when the identical call on the identical artifact demonstrably can. The
    branch re-reads with `os.path.lexists` after its unlink and, on a surviving file, refuses with
    a `leftover: "<path>"` detail line carrying the reservation's name rather than a bare
    `UNREADABLE reason=stream_paths_alias`. Tests: symlink, spelling, and `os.link` alias; plus
    `test_alias_refusal_unlink_failure_reports_leftover`, built as AC-3.10's rollback test is —
    the same `os.unlink` fault injection, an alias trigger in place of the second reservation's
    failure — asserting the `leftover:` line names the created path. **Without the injection that
    test is vacuous**: the unlink succeeds, nothing is left over, and a branch with no read-back at
    all passes it. Both read-backs use the same seam, so AC-5.5's injection list gains no member.
  ...
  - AC-3.11: **Fixture preamble.** `run_block` accepts an optional `preamble` — shell text run in
    ...
    drives both together). **Where that guarantee is enforced is stated exactly**, because the
    clause above reads as a choice `run_block` makes and it is not one: `run_block` takes a
    single `Block` and composes against `block.text`, and by the time it is called the caller
    has already substituted, so at that seam `block.text` **is** `text′` and there is nothing
    else it could prepend to. The ordering — substitute, then run — belongs to the caller, and
    AC-2.1 is what requires the executed block to carry the substituted value end to end.
    Residual, stated as a concrete category: a **library** caller that hands `run_block` a block
    it never substituted executes the unsubstituted text, which is that caller's own defect
    rather than this module's — the module's surface cannot express it, and it is closed by
    AC-2.1 on the CLI path and by nothing at the API boundary.
    A newline boundary is always inserted, so a
  ...
  - AC-3.14: **Cleanup is verified, not assumed.** After every run — normal, timeout, or
    ...
    promises.
    **What the test asserts, and the one assertion it must not make**: on each branch the test
    asserts the *identity* of `__cause__` — `__cause__ is cleanup_error` on a run that succeeded
    and whose cleanup then failed, and `__cause__ is` the pending outcome on a run that had one —
    and it asserts **nothing** about `__suppress_context__`. Measured on Python 3.11.8, raising
    inside an `except` block: `raise ... from X` sets `__suppress_context__` to `True` for
    **every** `X`, the `from cleanup_error` branch included (`cause=Cleanup('boom') suppress=True`,
    beside `cause=Pending('p') suppress=True` on the other branch). An assertion that the flag is
    `False` therefore rejects the two-branch chaining this AC prescribes instead of pinning it, and
    a flag that reads `True` on both branches discriminates nothing between them — `__cause__`
    carries the whole of the contract, which is why the rule is written over it and the test
    follows the rule.
    The fixture is a block that leaves an unreadable subdirectory
...
- **Acceptance Criteria**:
  ...
  - AC-5.6: **The bound is validated before anything is spawned.** `timeout` must be a finite
    number greater than zero **and no greater than `(2**31 - 1) / 1000` seconds** — the upper
    bound is written in that derived form rather than as its literal `2147483.647`, so the reason
    travels with the number: it is CPython's `INT_MAX`-**milliseconds** selector limit, about
    24.855 days, not a chosen round figure. `0`, a negative value, `nan`, `inf`, **a value above
    that bound** and a non-numeric
    ...
    `communicate`, a negative value raises `ValueError` *after* the spawn, `inf` makes the
    promised bound unbounded, and a value above the representable limit raises `OverflowError`
    *inside* `communicate` — an `ArithmeticError`, neither an `OSError` nor a `ValueError`, so
    every error mapping this helper defines excludes it and the traceback escapes.
    **The upper bound is not a new rule**: it joins the same refusal list as `inf`, for the same
    reason and no other — a bound the platform cannot represent is a bound that cannot be
    honoured. **And it stays pre-spawn**: that `OverflowError` fires *after* the spawn and after
    the disposable cwd is created, so mapping it downstream would launch a child and create a
    directory for a bound already known to be unhonourable, which is exactly the leak this AC's
    pre-spawn rule exists to prevent; it would also have to surface as a launch failure, filing a
    caller-input error as a fault of the launch.
    **Measured, not reasoned**, on the pinned interpreter `/opt/anaconda3/bin/python3.11`
    (**3.11.8**, darwin), every arm re-run for this revision rather than carried:
    `math.isfinite(1e300)` and `1e300 > 0` are both `True`, so a predicate carrying only the lower
    clause *accepts* `1e300`, and `communicate(timeout=1e300)` then raises
    `OverflowError: timestamp too large to convert to C _PyTime_t`. The edge is nowhere near
    `1e300`: on `echo hi`, `communicate(timeout=2147483.647)` returns normally while
    `communicate(timeout=2147483.648)` raises `OverflowError: timeout is too large`, and
    `(2**31 - 1) / 1000 == 2147483.647` is `True`. **The control that licenses a fixed constant**:
    the boundary re-measured 3 s apart moves by `-1.91e-05` s — search noise, not wall-clock drift
    — so this is a limit of representation and not an absolute deadline; had it been a deadline a
    fixed constant would have been flaky by construction, and that control, not the pair alone, is
    what makes the constant sound. **Residual, stated exactly**: the constant pins CPython's limit
    *as measured on 3.11.8 / darwin*. On an interpreter or platform whose limit is lower, a value
    between that limit and this constant still reaches `communicate` and raises an unmapped
    `OverflowError`. That residual is deliberately **not** closed by an `except OverflowError`
    backstop — on the pinned interpreter that branch is unreachable, an unreachable branch can be
    neither tested nor mutation-verified, and shipping one would be the appearance of coverage
    rather than coverage. It is closed the way this feature closes its other interpreter-pinned
    reading: by re-running the accepted/refused pair probe on any interpreter or platform this
    feature is later supported on. On the CLI the value is taken as a string and validated by `main`,
    ...
    unknown option or a missing value is a verdict and there is no non-`DOCBLOCK` exit. **The one
    carve-out is argparse's own help action, and its boundary is drawn where argparse draws it,
    not by position in `argv`**: `--help` or `-h` **recognised as an option** — wherever it sits,
    including after an unknown option, after a valid option and its value, before a later unknown
    option, and while a *required* option is absent — short-circuits to `SystemExit(0)` with usage
    on stdout, never reaching the `error()` override and never emitting a `DOCBLOCK:` line. The
    same characters **consumed as data** are not the carve-out and route to the contract like any
    other argument: as an option's value (`--heading --help` → `BAD_ARGS`, `argument --heading:
    expected one argument`) and as a positional after the `--` terminator (`-- --help` →
    `BAD_ARGS`). Measured on Python 3.11.8 against this exact parser, each case its own parser
    instance; the two `BAD_ARGS` rows are the controls that make the help rows a measurement of
    the carve-out rather than a probe that never reached `error()` at all. AC-4.1's
    `test_help_anywhere_exits_zero_without_a_verdict` pins the exit-0 side, and its discriminating
    assertion is that **no stdout line starts `DOCBLOCK:`** — an implementation that pre-scanned
    `argv` and emitted a verdict beside the help text would satisfy the exit code and the usage
    text and fail only that clause. **Residual, stated exactly**: this fixes the two spellings
    argparse's default help action registers (`-h`, `--help`); a revision that declares its own
    help option or changes `add_help` moves this boundary and re-runs the probe.
  - AC-5.5: **The timeout path has no unhandled race.** Two windows, both specified and both
    ...
    the read-back on **both** unlinking branches — AC-3.10's reservation rollback and AC-3.9's
    alias refusal, one seam with two callers, so this count is unmoved —
    **`os.lstat` for AC-3.10's rollback identity guard**,
    the module's
...
- **Acceptance Criteria**:
  ...
  - AC-6.4: The full suite passes, and the count is no lower than the pre-change count plus the
    ...
    **Why that comment names no second number, stated as a rule rather than as this one repair.**
    ...
    superseded, and the pair that superseded it has moved again since: at `fbc2ea0` the two
    collections were **2814** from the repository root and **2552** from `h-mad/`, and at the
    `cac6edc` freeze they are **2836** and **2574**, each measured in its **own** shell invocation
    of `python3.11 -m pytest --collect-only -q -p no:cacheprovider` (a `cd h-mad` chained into the
    same invocation persists and makes both "controls" read the same tree, which is how the pair
    was mis-measured once already). **Both figures are COLLECTED, never passed**: a collection
    count is not a pass count, the `h-mad` suite is *not* green on the committed tree at this
    freeze — `docs/03-analysis/doc-block-exec.gating-decision-sheet.r18.md` records the failing
    test and routes it — and no sentence in this document may turn either number into a claim about
    a green run. The gap on the retired `2748`/`2486` pair is **a function of both endpoints and
    moves whenever either does** — `+66` at `fbc2ea0`, `+88` at `cac6edc` — so it is stated per
    stamp with its arithmetic and never carried forward as a constant of its own. The rule that
    closes the class: **a comment embedded
    ...
    value sweep of this document has already missed twice. **The pair is spelled once, in prose,
    with its command and its sha, and is referred to rather than re-spelled anywhere below**: this
    sentence used to carry a third spelling of it, stated nowhere else in the body and left behind
    when the figures last moved, so the same paragraph has now been missed three times and the fix
    is a reference instead of a fourth site. That pair is a *scale* for the
    root-versus-`h-mad/` gap, not a floor, and the floor remains `2748` until 5c re-measures it.
...
- The two extractors named in FR-6 are the only in-repo consumers that anchor on a bare
  ...
  **Eight** hits at `fbc2ea0`; the unit is matching lines, one match to a line. The last two hits
  ...
  every arrival was a fixture, a comment or a probe and none was an extractor. One further consumer
  reads `SKILL.md`, and it is re-derived here rather than pinned, because it moved:
  `h-mad/tests/docsections.py` does not bound fences itself any more. Both of its exported helpers
  delegate to this feature's own scanner. With `D=h-mad/tests/docsections.py`,
  `grep -c '_dbe\.fence_aware_end' "$D"` → `2` at `eaa10ca`, against
  `grep -c 'startswith("```")' "$D"` → `0` at the same sha, where the **prefix** match this
  document cited through v1.64 used to be
  (`git log --oneline -1 -- h-mad/tests/docsections.py` → `7548faa`, the Task 1 GREEN commit that
  rewired it). **The conclusion is unchanged and its ground is stronger**: the consumer bounds with
  `fence_aware_end`, which implements FR-1's grammar, so an info-string tag is an opener by
  construction rather than by a prefix match that happens to ignore it. **The residual is a
  coupling this document did not carry before**: that bound is now *downstream* of FR-1, so a
  change to the scanner's opener or closer grammar moves it, and what catches that is the
  `_dbe.fence_aware_end` count above — a content predicate over the consumer — rather than a line
  number, which is what went stale here in the first place.
  ...
  **verbatim substring of this document's body**, not a paraphrase of one — a paraphrased control
  ...
  The split of the 12 lines is 10 + 2
  ...
  The split of the 12 lines is 10 + 2                        MATCH
  ...
  **A second, distinct sub-class arrived in v1.61 and v1.62 and is named rather than left implicit**:
  ...
  `fbc2ea0`, `101` at `cac6edc` — the value v1.63's body carried *after its reopen*, its entry
  having published `100` over the pre-reopen draft, so the entry and the body were never restamped
  to agree; re-run here rather than carried — `104` over the v1.64 draft and `106` over the
  v1.65 draft; the unit is
  *occurrences of the
  token*, not lines
  ...
  half), `89` once v1.62's exclusion sub-class and AC-4.2's exhaustiveness clause landed, `101`
  once v1.63's AC-2.7 second clause, AC-4.6 NUL paragraph, AC-1.8
  collection-only residual and the renderer-environment paragraph its reopen rewrote landed, and it
  moves again here because v1.64's three additions — the enumerate-every-occurrence rule under
  AC-2.7, AC-3.14's assertion rule, and the collected-never-passed sentence beside AC-6.4's
  floor — are each stated in negatives, and `106` once v1.65's AC-5.6 upper bound landed, whose
  predicate clause and whose joins-the-same-list clause are each stated in negatives — exactly the
  two occurrences the delta is made of, counted rather than quoted. The reason is
  ...
  admitting `no`/`none` would put up to `106` candidate occurrences in front of a reader to
  ...
  ```
  $ sed '/^## Version History/,$d' "$S" | grep -cE '\.py:[0-9]+'                      # -> 12
  $ sed '/^## Version History/,$d' "$S" | grep -oE '[A-Za-z0-9_./-]*\.py:[0-9]+' | sort -u | wc -l   # -> 10
  ```

  **Three units, three commands, because one `grep` over this class yields three different true
  numbers and v1.59 published one of them bare.** The first command counts **12 body lines
  carrying at least one pin**. The second counts **10 distinct fully-qualified `path.py:N`
  ...
  **2**: it was **6** at `6f0ee85` and **3** at `eaa10ca`, because **v1.60** rewrote §Assumptions'
  broad fence-literal census to print `-n` output instead of a `| wc -l` total, which turned three
  of the six into command output, and **v1.65** replaced the fourth outright — the `startswith`
  fence bound, whose pin had drifted and whose quoted predicate is absent from the tree at
  `eaa10ca` — with a content predicate over the same consumer. **`6` and `three`, not the `7` and `four` v1.60 and v1.61 published**: this
  ...
  figure moving with the two census output lines that revision added and the pin figure not moving
  at all, which is the two units staying apart under a change that touches only one of them; over
  the v1.65 draft they are `2` and `12`, both moving by one, which is the complementary case — a
  change that removes a member of *both* units moves both, and publishing one figure alone could
  not tell the two situations apart. The
  ...
  → `2`. **It discriminates output from prose by the shape of `git grep -n` output itself — the
  ...
  so the old form counts them as prose and returns **`4`** over this same draft. The class is
  *any* discriminator keyed on a corpus's current directory names; the shape of the output it is
  ...
  not a needle*. The split of the 12 lines is 10 + 2 — 10 are output of the two
  `git grep` census commands above and re-quote nothing, and 2 are prose. Both of the prose
  2 are illustrative Phase-6 collectors, each beside a predicate that reproduces its candidate
  set; a drift in either changes no requirement. **The third was the load-bearing one and it is
  gone**: the `startswith` fence bound in §Assumptions carried the literal code token and grepped
  to exactly one hit, which is precisely the self-repair the obligation promises — and the repair
  it forced was replacement, not a re-pin, because the predicate itself is absent from the tree at
  `eaa10ca`. The other load-bearing pins — the two `re.findall` extractor assignments — are now
...
- v1.64: Round eighteen. This document has NO audit report of its own; THREE decisions land in it from the round-eighteen decision sheet (docs/03-analysis/doc-block-exec.gating-decision-sheet.r18.md — FACT 4 (b) and (c), FACT 5's spec bullet, FACT 8's spec column), whose verified findings are the r17 sheet's C33 item 1 and C34 item 2 (docs/03-analysis/doc-block-exec.gating-decision-sheet.r17.md). NO gating claim, NO second surface, NO exit-gate claim: round eighteen is FAIL to revision and the gate after this batch is design c98 / plan c89 / impl-plan c49. Freeze sha cac6edc; HEAD at dispatch was f6849bb and that range is docs-only — git diff --name-only cac6edc -- '*.py' | wc -l returns 0 over the WORKING TREE and git status --porcelain -- '*.py' | wc -l returns 0 — so the two collection readings below, taken over the working tree, are readings of cac6edc's code and are stamped there. (c) AC-3.14 NOW NAMES THE ASSERTION AND FORBIDS THE ONE THAT REJECTS THE PRESCRIBED IMPLEMENTATION. Measured on Python 3.11.8 (/opt/anaconda3/bin/python3.11 -V), raising inside an except block: with a pending outcome, cause=Pending('p') suppress=True; with none, from the cleanup error, cause=Cleanup('boom') suppress=True — __suppress_context__ is True on BOTH branches, so an assertion that it is False rejects the two-branch chaining this AC prescribes. The AC now states that the test asserts the IDENTITY of __cause__ per branch and asserts nothing about __suppress_context__, and says why. Confirmed and reported rather than assumed, and EVERY HALF IS NAMED AT THE SHA IT WAS TAKEN AT, because this revision's own prose moves both counts — a present-tense self-count of a token the revision introduces is the defect this correction closes, filed by the advisory delta review docs/03-analysis/doc-block-exec.spec.delta-review.r18.md and re-derived here rather than taken on its word. __suppress_context__ occurs 0 times in this document at cac6edc and 10 after this revision (whole file, tr '\n' ' ' < $S | grep -oF -- '__suppress_context__' | wc -l), of which 2 are in the body — the same command after awk '/^## Version History/{exit}{print}', and both sit in the AC-3.14 prohibition, which has to name the assertion it forbids or the prohibition is unstatable — and 8 are inside this entry. In the SIBLINGS the count is of the TOKEN and not of an assertion: 4 occurrences in the design and 4 in the impl-plan at cac6edc, of which exactly ONE asserts False — the impl-plan's, restated once in its cleanup-chain mutation row — while the design asserts True in three and spends the fourth inside a Python format string, which is not a sentence at all (git show cac6edc:<doc> | tr '\n' ' ' | grep -oE '__suppress_context__.{0,200}' | grep -c 'False' → design 0, impl-plan 1; a 30-character window returns the same, and the design carries False elsewhere, so the token is common and simply not attached to this one). Counting a string without checking which construct it sits in is the recorded GRAMMAR species, and this entry's first draft committed it. AC-3.14 has 7 sites here at cac6edc — one AC body, six Version History entries — and 13 after this revision — 2 in the body (the AC-3.14 prose itself, and the no-token census sentence that names its assertion rule), 5 in this entry, and the six earlier Version History entries unchanged; no restatement carries the assertion, so the AC body is the whole sweep. (b) AC-2.7's SECOND CLAUSE NOW STATES WHAT THE SCAN ENUMERATES, WHICH IT DID NOT. Measured on 3.11.8 for text aaab under keys {aa, ab}: re.finditer(re.escape(k), text) yields aa [(0,2)] and ab [(2,4)], which share NO index, while re.finditer('(?=' + re.escape(k) + ')', text) with span (m.start(), m.start()+len(k)) yields aa [(0,2),(1,3)] and ab [(2,4)], and (1,3) against (2,4) share index 2 — so the non-overlapping enumeration MISSES a cross-key intersection and publishes a clean run. The AC now requires every occurrence of every key, overlapping occurrences of the same key included, and names the fixture: aaab under {aa, ab} emits intersect: "aa" "ab" "2". THE v1.63 RESIDUAL IS WITHDRAWN AS WRITTEN AND REPLACED, NOT DELETED: its CLAIM (a key intersecting itself is not a refusal) stays true because the predicate is over two DISTINCT keys, but its stated REASON — the non-overlapping count still equals the number replaced — was the reason the scan stayed under-specified, since the self-overlapping occurrence a non-overlapping enumeration drops is exactly the span that intersects a different key. "aaa".count("aa") is 1 on 3.11.8 and AC-2.5 still holds over the single-key case, which is all that reading ever established. NEW RESIDUAL, STATED AS A CONCRETE CATEGORY: over-inclusive enumeration lets ONE unordered key pair intersect at several offsets while the grammar emits ONE line per pair, so <offset> is defined as the smallest index shared by ANY intersecting span pair of those two keys, and a second, later intersection of the same pair is not separately reported; nothing executes under either, so this is a diagnostic-completeness residual and not a safety one. (FACT 1) THE TWO COLLECTION COUNTS ARE RE-STAMPED AND A THIRD SPELLING OF THEM WAS STALE. At cac6edc, python3.11 -m pytest --collect-only -q -p no:cacheprovider collects 2836 from the repository root and 2574 from h-mad/, each measured in its OWN shell invocation (2836 tests collected in 2.07s; 2574 tests collected in 0.31s). COLLECTED, never passed: the h-mad suite is NOT green on the committed tree at the freeze (r18 sheet C2 (i) records 1 failed, 2573 passed), the authors of this batch do not run it, and a collection count is not a pass count. The fbc2ea0 pair 2814/2552 keeps its blob stamp beside the new one (freeze-sha rule clause 2). THE MISS THIS SWEEP FOUND: the paragraph's own closing sentence restated the pair as 2809/2547 — a pair stated NOWHERE else in the body, left behind when the figures last moved — so a value sweep of this document has now missed the same paragraph three times. The fix is a rule and not a fourth site: the sentence refers to the pair above instead of naming it again. The DERIVED figure moved with it — +66 on the retired 2748/2486 pair is a function of both endpoints, not a second statement of either, and is +88 at cac6edc — so it is stated per stamp with its arithmetic. The 2748 floor and its e8eaf6f stamp are untouched. VALUE SWEEP, newline-collapsed and fixed-string over the whole file including fences, tables and comments embedded in commands: 2814 1, 2552 1, 2809 3, 2547 3 before (the surplus of each sits in Version History entries, which are stamped history and are not rewritten), and 2836/2574 0 before. OWED BY OTHER DOCUMENTS, read from their bytes at cac6edc with git show and NOT edited: the DESIGN says "all match spans of all keys are collected" and "one per unordered pair ... sorted by (offset, a, b)" — it owes the overlapping enumeration and the same smallest-index-across-ALL-intersecting-span-pairs wording, and its AC-matrix row still emits the retired connective token ahead of a ZERO offset for the `abc` fixture, where the design's own body and this document both read the smallest index those two spans share, which is 1 (the literal is not re-spelled here: writing it would create a site in THIS document for a value the design is retiring); the IMPL-PLAN says the raised CleanupFailed's __cause__ is the injected cleanup error "and __suppress_context__ is False", which is the assertion this revision's AC-3.14 forbids, and its cleanup-chain mutation row restates it. Both are those authors' rows this round. DISCHARGE NOTE, added at the reopen and measured against the SIBLINGS' STAGED bytes (git show :<doc>) rather than against cac6edc, because an owed-elsewhere claim is a claim about a moving tree: both debts are discharged by THIS SAME BATCH and neither survives to the commit. The impl-plan's body now reads "It asserts nothing about __suppress_context__, and the earlier __suppress_context__ is False clause is WITHDRAWN", so the only False still attached to the token there is that withdrawal quoting the clause it retires; the design's AC-matrix row now emits intersect: "ab" "bc" "1", its two surviving connective-token occurrences being quoted history that explains the correction. ONE EXCEPTION REMAINS OPEN AND IS NOT MINE TO CLOSE: the design's grammar paragraph still defines the offset as the smallest character index the two matched spans share, which is the single-span-pair wording this revision generalised, while its own pseudocode takes a minimum across pairs and agrees with this document. That is prose against prose on a diagnostic three documents must spell identically, routed to the design author. SCREENS RE-RUN AFTER THE LAST EDIT OF THIS REOPEN, which is the mechanism whose absence let the two self-counts ship wrong in the first draft: PRECHECK: PASS issues=0 (h_mad_precheck_doc.py --phase spec --root /Users/kimhawk/orca/skills), advisories all of known-exempt kinds and unchanged in population from the freeze — LINEPIN on FR-6's bare ordinals per residual (a), PATH on the module 5d/5e builds, STALESHA on historic blob stamps, and COUNT lines only inside Version History, which residual (1) exempts; a fixed-string sweep of this file for the current collected count followed by the word passed returns 0, and the literal is deliberately NOT spelled in this entry, since spelling it would create the very site the screen exists to find — the same reason the collection paragraph now refers to its pair instead of re-spelling it; no present-tense pass claim for any suite is made anywhere here; the value sweep above re-run over the shipped bytes; and the body-scoped no-token census re-run to its fixed point.
- v1.65: 5d decision D7 (source: docs/03-analysis/doc-block-exec.5d-decisions.md §D7), ONE change and its sweep; no audit report of this document's own this cycle. AC-5.6 gains a representable UPPER bound in BOTH halves — the predicate now reads `a finite number greater than zero and no greater than (2**31 - 1) / 1000 seconds`, and the refusal list gains `a value above that bound` beside `0`, a negative value, `nan`, `inf` and a non-numeric `--shell-timeout`. The constant is written in the derived form, never as its literal 2147483.647, because the reason is the number: CPython's INT_MAX-MILLISECONDS selector limit, about 24.855 days. MEASURED, NOT CARRIED — every arm re-run in this revision on the pinned interpreter /opt/anaconda3/bin/python3.11 (3.11.8, darwin): math.isfinite(1e300) and 1e300 > 0 are both True, so the pre-D7 predicate ACCEPTED 1e300 and communicate(timeout=1e300) then raised `OverflowError: timestamp too large to convert to C _PyTime_t`; the edge is five orders of magnitude below that filed value — communicate(timeout=2147483.647) on `echo hi` returned b'hi\n' and communicate(timeout=2147483.648) raised `OverflowError: timeout is too large`; and (2**31 - 1) / 1000 == 2147483.647 is True. The exception's TYPE is load-bearing and is stated in the AC: OverflowError is an ArithmeticError, NEITHER an OSError NOR a ValueError, so every error mapping this helper defines excludes it and the traceback escapes. THE CONTROL THAT LICENSES A FIXED CONSTANT is carried into the AC rather than left in the sheet: the boundary re-measured 3 s apart moves by -1.91e-05 s, i.e. search noise and not wall-clock drift, so this is a limit of representation and not an absolute deadline — had it been a deadline a fixed constant would have been flaky by construction. THE REFUSAL STAYS PRE-SPAWN and the AC says why: the OverflowError fires INSIDE communicate, after the spawn and after the disposable cwd is created, so mapping it downstream would launch a child and create a directory for a bound already known to be unhonourable — the exact leak this AC's pre-spawn rule exists to prevent — and would have to surface as a launch failure, filing a caller-input error as a fault of the launch. NOT A NEW RULE: the upper bound joins the same refusal list as `inf` for the same reason and no other, a bound the platform cannot represent being a bound that cannot be honoured. RESIDUAL, STATED EXACTLY AND DELIBERATELY NOT SMOOTHED: the constant pins CPython's limit as measured on 3.11.8 / darwin; on an interpreter or platform whose limit is LOWER, a value between that limit and this constant still reaches communicate and raises an unmapped OverflowError. It is deliberately NOT closed by an `except OverflowError` backstop — on the pinned interpreter that branch is unreachable, an unreachable branch can be neither tested nor mutation-verified, and shipping one would be the appearance of coverage rather than coverage; it is closed by re-running the accepted/refused pair probe on any interpreter or platform this feature is later supported on. VALUE SWEEP, run over the WHOLE file including fenced blocks, table cells and comments inside commands, with its commands: `grep -n 'BAD_TIMEOUT\|BadTimeout\|shell-timeout\|isfinite\|finite number\|_MAX_TIMEOUT' $S` returns the AC-5.6 body plus two token-list sites (FR-4's exit-0 enumeration and AC-4.2's verdict-table partition) and one Version History line — the two token lists name BAD_TIMEOUT as a HEAD and state no predicate, so they are unaffected and were checked rather than assumed; `grep -n -- '--shell-timeout' $S` returns the AC-5.6 body alone, so the CLI half is stated in exactly one place; `grep -n 'OverflowError\|2147483\|2\*\*31' $S` returned NOTHING before this revision, so no prior site states the bound and this is the first. AC-1.9's `--index 0`/`--index -1` refusal is a DIFFERENT predicate on a different flag and is untouched. ONE DOC-SCOPED FIGURE MOVED, and it is the one this revision's own prose moves: the `no`-token scale, `awk '/^## Version History/{exit}{print}' "$S" | grep -oEi '\bno\b' | wc -l`, was 104 over the v1.64 draft and is 106 over this one, because AC-5.6's predicate clause and its joins-the-same-list clause are each stated in negatives — exactly the two occurrences the delta is made of. THE FIGURE DERIVED FROM IT WAS MOVED WITH IT rather than left stale: the `up to N candidate occurrences` bound in the same paragraph is a FUNCTION of the scale, not a second statement of it, and now reads 106. Every other published screen re-run over this draft AFTER the last edit landed and each returned its published value unmoved: anchored awk needle 1; AC-1.2 anchor 1; opener census 21 openers over 11 distinct tokens (this revision adds no fenced command); reproduced-output lines 8; py-pin lines 13 and distinct pins 5, identical at eaa10ca and over this draft, so the edge-case numbers this revision adds create no path:line pin; split_only empty over this draft with its synthetic positive control FIRING first (an unproven probe's zero is indistinguishable from a dead one); and all nine control strings verbatim at 1 with the Version History and the control block cut — the nine EXTRACTED FROM THE SHIPPED HERE-DOC BY THE FILE ITSELF, never retyped, after a first attempt that retyped them scored 0 on three and was the retyping, not the document, that was wrong. Ownership NOT decided here: this document states the CONTRACT and names no source constant, no mutation row and no test name, all of which are the design's and the impl-plan's, both of which were being revised concurrently and neither of which is quoted, counted or version-cited by this entry. [Appended after the entry landed, and every screen re-run after it. A SECOND change was made in this revision and it was NOT part of D7: the shared precheck returned PRECHECK: FAIL issues=1 on this document at eaa10ca, BEFORE this revision touched it — PINDRIFT on the one §Assumptions line pin into `h-mad/tests/docsections.py` (the pin itself is deliberately NOT reproduced here, since quoting it re-arms the same finding — measured: this addendum's first draft did quote it and took the precheck back to FAIL), verified pre-existing by running the same precheck over `git show eaa10ca:<path>`. It was a hard finding, so it is resolved rather than carried, and the finding UNDERSTATED it: the pin had not merely moved, the predicate it quoted is GONE. With `D=h-mad/tests/docsections.py`, `grep -c 'startswith("```")' "$D"` -> 0 at eaa10ca and `grep -c '_dbe\.fence_aware_end' "$D"` -> 2, because `git log --oneline -1 -- "$D"` -> 7548faa, the Task 1 GREEN commit, rewired the consumer onto this feature's own scanner. The document's claim — that an info-string tag does not disturb that bound — SURVIVES and its ground is stronger, since the consumer now bounds with FR-1's grammar rather than with a prefix match that happened to ignore the info string; the residual is stated in §Assumptions as the coupling this creates, the bound being downstream of FR-1 from 7548faa onwards. The pin is replaced by a CONTENT PREDICATE, never a re-pinned line number, which is the rule that would have prevented it. THE CASCADE, and it is the reason this addendum exists rather than a silent second edit: removing one prose pin moves FOUR published figures over a class this document counts, and all four are re-derived with the document's OWN commands over this draft rather than decremented — body lines carrying a pin 13 -> 12, distinct fully-qualified pins 11 -> 10 (both inside the `# ->` comments of a fenced block, which is exactly where a value sweep misses), bare pins standing in prose 3 -> 2, and the retired `^  h-mad/` discriminator's control figure 5 -> 4. The prose split sentence moves with them, 'the 13 lines is 10 + 3' -> 'the 12 lines is 10 + 2', AND SO DOES THE CONTROL STRING THAT QUOTES IT — the sixth positive of the nine-string battery, in both its input and its printed-output half, which loses its committed half in v1.65 for the third time and for exactly the reason this document already names as a class: a control that quotes a figure this document publishes cannot be checked at both halves in the revision that moves the figure. All nine strings re-verified verbatim at 1 with the Version History and the control block cut, six positives MATCH and three true negatives NO MATCH under the pattern extracted from the published block by its own anchored needle. ONE MEASUREMENT ERROR OF MY OWN, caught by re-running rather than by reasoning and recorded because the fix would otherwise look clean: the first draft of the replacement wrapped two lines so they BEGAN with `h-mad/tests/docsections.py`, which took the reproduced-output discriminator from 8 to 10 — a repair introducing a defect in the very figure it was restoring. It was rewrapped behind a `D=` variable and the count is 8 again. Post-addendum re-run of every screen over this draft: no-token scale 106, body lines with a pin 12, distinct pins 10, bare prose pins 2, old-form control 4, reproduced-output lines 8, opener census 21 openers, anchored awk needle 1, quoted `git grep -n` needle 1, AC-1.2 anchor 1, split_only empty with its synthetic positive control FIRING, nine control strings verbatim at 1, PRECHECK: PASS issues=0.]
- v1.66: 5e document round, freeze 6a1693c (HEAD at dispatch; this file is byte-identical to it — `git diff --stat 6a1693c -- $S` is empty). Source `docs/03-analysis/doc-block-exec.5d-decisions.md` §D10, §D7 and §D6. NO audit report of this document's own this cycle, NO gating claim, NO second surface, NO exit-gate claim. **NOTHING HERE CLAIMS ANY SUITE IS GREEN**: D3 is settled (operator, remedy 3) and the impl-plan's own noise floor stays RED through 5e, settled at 5g. **(1) D10 — THE ANSWER FOR THIS DOCUMENT IS ZERO, AND THE ZERO IS A MEASUREMENT, NOT AN OVERSIGHT.** No revision is owed here by the per-task mutation-spec split, because this document publishes no mutation figure of any kind to move. Run at 6a1693c over the WHOLE file, newline-collapsed and fixed-string so fenced blocks, table cells and comments inside commands are all in scope — `tr '\n' ' ' < $S | grep -oF -e 'ALL_CAUGHT' -e 'mutation-specs' -e 'doc_block_exec.json' -e 'doc_block_exec_task3' -e 'docsections.json' -e 'doc_block_exec_wire.json' | wc -l` returns **0**, every needle individually 0. The three raw `\b88\b` hits this document does carry were CLASSIFIED before anything was moved, per §D10's own anti-pattern warning, and not one is a mutation-row count: one is the body's `+88` collection DELTA at cac6edc (a function of the 2836/2574 pair against the retired 2748/2486 floor, stated per stamp with its arithmetic), and two are Version History prose (`13 of 88 test files` in v1.63, the same `+88` restated in v1.64). **WHY THE ZERO IS ZERO, and whether that reason is load-bearing or incidental** — it is LOAD-BEARING and it is a division of labour, not an accident: this document states the CONTRACT, and the mutation population is enumerated only by the design's matrix and the impl-plan's per-task row lists, both of which own the total. The incidental half is named too, because a reason that is only load-bearing today is a defect waiting for the constraint to move: were an AC ever to state a mutation floor the way AC-6.4 states a suite floor, §D10 WOULD move it, and this zero would have to be re-derived rather than carried. The self-limit this document has claimed elsewhere is CORRECTED here rather than restated: v1.65 said this document "names no source constant, no mutation row and no test name", and the middle clause is the true one — `awk '/^## Version History/{exit}{print}' $S | grep -oE '\btest_[A-Za-z0-9_]+' | sort -u | wc -l` returns 14 distinct test identifiers in the body, so the "no test name" half was FALSE when it was written. It is corrected in this entry and NOT rewritten in v1.65, which is stamped history. **(2) #121 / D7 — ALREADY DISCHARGED AT v1.65, VERIFIED RATHER THAN REDONE, and this is reported as a finding about the dispatch and not silently absorbed.** The brief routed AC-5.6's upper bound to this round; v1.65 landed it, and AC-5.6 today carries the derived form `(2**31 - 1) / 1000`, its literal named once as the thing it is deliberately not written as, the upper bound in BOTH halves of the predicate and `a value above that bound` in the refusal list beside `0`, a negative value, `nan`, `inf` and a non-numeric `--shell-timeout`. So this revision writes NO change at AC-5.6. Every arm was nevertheless RE-RUN here rather than carried, because a control's stated properties are claims about code: on `/opt/anaconda3/bin/python3.11`, `communicate(timeout=2147483.647)` on `/bin/sh -c 'echo hi'` returned `b'hi\n'` and `communicate(timeout=2147483.648)` raised `OverflowError: timeout is too large` whose base class prints `ArithmeticError`, and `(2**31 - 1) / 1000 == 2147483.647` is True with `/86400` giving 24.855 days. THE TREE NOW AGREES, which v1.65 could not check because the code did not exist: `h-mad/scripts/h_mad_doc_block_exec.py` defines `_MAX_TIMEOUT_SECONDS = (2**31 - 1) / 1000` in the derived form with the INT_MAX-milliseconds reason as its comment, and `_validate_timeout` reads `if not (math.isfinite(t) and 0 < t <= _MAX_TIMEOUT_SECONDS)` — located by `grep -n '_MAX_TIMEOUT\|isfinite' <path>`, never by line number. **(3) D6 — ONE ADDITION AT AC-3.11, AND IT IS BEYOND THE LITERAL BRIEF, ANNOUNCED WITH ITS REASONING AND ITS REVERT.** §D6 routes to `design-author` and `implplan-author` and routes NOTHING here, and the dispatch named AC-5.6 alone; both are recorded so the deviation is visible. What §D6 established is that the design's row `preamble-composed-with-unsubstituted-text` described a distinction absent at the seam it was assigned to, and the prose ancestor of that row's mechanism cell is this document's AC-3.11 clause "the preamble is prepended to what will actually run, never to the unsubstituted fence body". That clause is TRUE end to end and was NOT weakened; what it did not say is WHERE the guarantee is enforced, and a contract that reads as a choice `run_block` makes will regenerate the same false row the next time a mutation matrix is derived from it. VERIFIED AGAINST THE TREE BEFORE WRITING, not taken from §D6: `grep -n 'def run_block' h-mad/scripts/h_mad_doc_block_exec.py` returns the single-`Block` signature `run_block(block: Block, *, preamble: str | None = None, timeout: float = 30.0)`, the same signature the design and the impl-plan publish, and the body composes `_compose(preamble, block.text)`, so at that seam `block.text` IS `text′`. AC-3.11 now names the enforcement point (the caller substitutes first; AC-2.1 is what requires the executed block to carry the substituted value end to end) and states §D6's residual (1) as a concrete category: a LIBRARY caller that hands `run_block` a block it never substituted executes the unsubstituted text, which is the caller's own defect, outside this module's surface and inexpressible as a mutation of its source. **Offered for revert as a unit** — it is additive, it retires no requirement, and striking it leaves v1.65's AC-3.11 intact. **SCREENS RE-RUN AFTER THE LAST EDIT, over this draft, because a self-counting screen run before the write measures the wrong bytes.** The `no`-token scale is the one figure this revision's own body prose could move and it is the one that was watched: the addition was written to carry no standalone `no`, and `awk '/^## Version History/{exit}{print}' $S | grep -oEi '\bno\b' | wc -l` returns 106 over this draft as over 6a1693c, so the DERIVED `up to 106 candidate occurrences` bound in the same paragraph is unmoved with it rather than left to be discovered stale. This revision adds no fenced block, no `.py:` pin and no bare line pin, so the opener census, the pin counts and the reproduced-output discriminator are all structurally untouched and were re-run to confirm rather than assumed. **OWED BY OTHER DOCUMENTS, reported and NOT edited**: §D6 and §D7 both name `design-author` and `implplan-author` debts, and both READ AS DISCHARGED at 6a1693c from the siblings' own bytes rather than from the sheet — the design carries `substitution-result-not-passed-to-run_block` with `test_cli_subst_value_reaches_the_child` as its killer and a matrix total re-derived 87 -> 88 (v1.112), and the impl-plan carries the moved row, the new test and the settled OPEN-DECISION (v1.56). That leaves §D10's question live for those two documents and for the plan, whose 88 is a MUTATION total and not this document's collection delta; NOTHING in this entry should be read as answering it on their behalf. One item is REPORTED AND NOT TAKEN, unchanged from v1.63 and still unrouted: AC-1.8's claim that `docsections.py` reaches the module "the way every test in `h-mad/tests/` already reaches `h-mad/scripts/`" is an over-broad universal the design's own codex leg filed as false, and it stays spec debt for a round that routes it rather than an unrouted repair here. [Appended after the entry landed, and every screen re-run after it. THIS ENTRY WROTE ITS OWN NEEDLE, which is the class this document already records and is here recurring: the D10 sweep above, run over the WHOLE file, returns 6 over the shipped bytes rather than 0, because this entry SPELLS its six needles in publishing the command. It returned 0 at 6a1693c, and the fix is scope plus a control rather than a re-typed integer. THE SWEEP IS BODY-SCOPED, `awk '/^## Version History/{exit}{print}' "$S"` first, then the same newline-collapsed fixed-string pipe: 0 in total and 0 for each of the six needles taken singly. ITS PIPE WAS PROVED ALIVE BEFORE THE ZERO WAS BELIEVED — a positive control through the identical pipe, the needle `AC-5.6`, returns 5, so this is a measured zero and not a dead one. UNIT, stated because `grep -n` output lines are not occurrences: at 6a1693c the `\b88\b` population is 3 MATCHING LINES and 3 OCCURRENCES — the two units AGREE there, so D10's `spec 3` is right under either reading — and over the shipped bytes it is 4 lines and 8 occurrences, the whole of that delta being this entry. Body-scoped it is 1 line and 1 occurrence, the `+88` collection delta, unmoved. ONE FIGURE OF THIS ENTRY'S OWN IS CORRECTED, caught by re-running rather than by reasoning: the body carries 13 distinct `test_` identifiers, NOT the 14 stated above. The 14 came from `grep -oE 'test_[a-z0-9_]+'` with no leading `\b`, which matched `test_cache` INSIDE `.pytest_cache` — a substring of a path, never an identifier — and `diff` of the two derivations prints exactly that one line. The corrected command is `grep -oE '\btest_[A-Za-z0-9_]+' | sort -u | wc -l` over the body, returning 13; the missing word boundary is the recorded species, and the CLAIM it supports is untouched, since v1.65's "names no test name" is false at 13 exactly as it was at 14. Post-addendum: the `no`-token scale is body-scoped, so nothing written in Version History can move it — re-run, 106, with its derived `up to 106 candidate occurrences` bound equal to it; this addendum adds no fence, no `.py:` pin and no bare line pin; PRECHECK: PASS issues=0.] [Second addendum, and it is the SAME species one level up, caught by re-running and corrected rather than left: the whole-file occurrence figure published two sentences ago was taken BEFORE the first addendum was appended, and that addendum's own prose moved it. Re-derived over the SHIPPED bytes under THREE pipes chosen to fail in OPPOSITE directions and required to agree — newline-collapsed `tr | grep -oE`, per-line `grep -oE`, and a Python `re.findall` — all three return 9, so the published 8 is superseded and the WHOLE-FILE occurrence count is 9 across the same 4 matching lines `grep -c` reports. THE LESSON IS THE REASON THIS IS THE SECOND CORRECTION IN ONE ENTRY, and it is a rule rather than a third integer: any whole-file count of the D10 value written INSIDE this entry is moved by the sentence that writes it, so the whole-file figure is not a stable measurement of this document and must never be the one a later round carries. The stable figure — the one that is load-bearing for D10 and that no Version History prose can move — is the BODY-scoped one: 1 matching line and 1 occurrence, the collection delta at cac6edc, identical at 6a1693c and over this draft. This addendum spells the D10 value nowhere, which is what keeps the 9 above true as shipped. Post-addendum, re-run after this last edit: body-scoped D10 needles 0 with the `AC-5.6` positive control at 5, `no`-token scale 106 with its derived bound equal to it, body test identifiers 13, body-scoped D10 value 1 line and 1 occurrence, PRECHECK: PASS issues=0.]
- v1.67: §D10 EXECUTED IN THE TREE, and the answer for this document is ONE occurrence that is a SEARCH TERM and not a reference, measured two ways and deliberately NOT substituted. Source: the operator's §D10 decision recorded at 8ef45fc, executed at 7afc0ef, execution record with its evidence table at 8bc2fa7 in `docs/03-analysis/doc-block-exec.5d-decisions.md`. Freeze 861efbe. NO audit report of this document's own this cycle, NO gating claim, NO second surface, NO exit-gate claim, and NOTHING HERE CLAIMS ANY SUITE IS GREEN. **WHAT MOVED IN THE TREE, re-derived here rather than carried from the dispatch.** `git show --stat 7afc0ef` prints the per-task staging spec deleted at −243 lines against +228 folded into `h-mad/tests/mutation-specs/doc_block_exec.json`; that surviving file today holds **60** mutation rows, counted by parsing the JSON and taking `len` of the `mutations` list, so the UNIT is list elements and not `grep` lines; `ls h-mad/tests/mutation-specs/ | grep -cE '_task[0-9]+\.json$'` returns **0**, and the retired path is absent from the working tree. The per-task file was STAGING, never a convention, which is why the merge retires a name rather than a requirement. **THE MEASUREMENT, TWO DERIVATIONS THAT MUST AGREE, because a hard wrap has hidden a value from a single-line grep in this feature before.** Per-line `grep -c` over the whole file returns **1 matching line**; newline-collapsed fixed-string `tr '\n' ' ' < $S | grep -oF <retired name> | wc -l` returns **1 occurrence**. They AGREE, so no wrap concealed a second instance, and the two UNITS coincide here only because the single line carries the name once. A THIRD derivation was run as a control against a differently-spelled sibling — the bare substring `_task3` under the same collapsed pipe — and also returns 1, the same instance, so this document carries no second reference under a shorter spelling. **SCOPE IS THE WHOLE FINDING, and it is what makes the repair a non-substitution.** BODY-SCOPED, `awk '/^## Version History/{exit}{print}' $S` first and then the same collapsed pipe, the count is **0** for the retired name, 0 for `mutation-specs` and 0 for the surviving filename: the contract prose names no mutation spec at all. The single whole-file occurrence lives inside v1.66's stamped Version History entry, as the FOURTH of six fixed-string NEEDLES in that entry's published D10 sweep command. It is a search term handed to `grep -oF`, not a path this document depends on, and the sentence containing it claims only that the sweep returned 0 at 6a1693c — a stamped measurement over this document's own bytes that the deletion of a file in the tree cannot falsify. **SO IT IS NOT REWRITTEN TO NAME THE SURVIVING FILE, and the reasoning is stated rather than the edit assumed**: substituting the merged filename into a historical needle list would change what was measured, would make a true stamped sentence false, and would silently alter that entry's "every needle individually 0" arithmetic by duplicating a needle already in the same list. Nor is it deleted, for the same reason. **THE NEEDLE SET IS ALREADY CORRECT FOR THE MERGED TREE**, which is why no forward instrument is owed: the surviving `doc_block_exec.json` is ALREADY the third needle in that same published command, so a later round re-running it verbatim still screens the name that exists. The RESIDUAL, stated as a concrete category rather than left implicit: the retired needle is now INERT rather than stale — it can only ever contribute 0 to that sweep — and a round wanting a forward-only sweep drops it and gets the identical body-scoped 0; the retired name becomes live again only if some future artifact reintroduces a per-task spec file, which §D10 settled against. **NO COUNT THIS DOCUMENT PUBLISHES IS MOVED BY THE MERGE, verified rather than asserted.** The figures in play are 32, 28, 60, 90 and 30; a body-scoped `grep -nE '\b(32|28|60|90|30)\b'` was READ and not merely counted, and every hit is a version-number substring of `v1.60`, `v1.61` or `v1.62`, not a quantity. The word `mutation` occurs 6 times in the body and not one is a row total or a file name: they are AC prose and the `MUTATION: PRECHECK_FAILED` refusal token. **WHY THE ZERO IS ZERO, and whether that reason is load-bearing or incidental** — LOAD-BEARING, and the same division of labour v1.66 recorded: this document states the CONTRACT, and the mutation population is enumerated only by the design's matrix and the impl-plan's per-task row lists, which own every total. The incidental half is named because a reason true only today is a defect waiting for its constraint to move: were an AC ever to state a mutation floor the way AC-6.4 states a suite floor, this zero would have to be re-derived rather than carried. **THIS ENTRY DELIBERATELY SPELLS THE RETIRED NAME NOWHERE**, which is v1.66's own recorded lesson applied instead of restated — an entry that spells its own needle moves the whole-file figure it publishes, and that species has already recurred twice in this file. So the two figures reported above stay true as shipped. This entry DOES spell the surviving filename, which moves the whole-file total of v1.66's D10 sweep; that figure was declared non-carryable by v1.66 itself, the body-scoped 0 is the stable one, and both are re-derived in the post-write addendum below rather than predicted here, because a self-counting screen run before the write measures the wrong bytes. **OWED BY OTHER DOCUMENTS, reported and NOT edited**: 32, 28, 60, 90 and 30 belong to the design's mutation matrix and the impl-plan's per-task row lists — 60 rows on disk against a 90-row design census leaves 30 unwritten for Tasks 4 and 5 — and nothing in this entry answers §D10 on their behalf. Carried forward unchanged and still unrouted: AC-1.8's claim that `docsections.py` reaches the module "the way every test in `h-mad/tests/` already reaches `h-mad/scripts/`" is an over-broad universal the design's own codex leg filed as false, and it stays spec debt for a round that routes it. [Addendum, appended after the entry landed and every screen re-run after it. ONE FIGURE OF THIS ENTRY'S OWN IS CORRECTED, caught by re-running rather than by reasoning, and it is the species this file already records twice now recurring a third time one level down. The THIRD derivation above — the short substring form of the retired name, run as a control against a differently-spelled second reference — returned 1 at the freeze and returns **2** over the SHIPPED bytes, because the sentence that publishes it SPELLS that substring. The full retired name was deliberately kept out of this entry and is therefore unmoved at 1 matching line and 1 occurrence, exactly as reported; the shorter control needle was not, and that asymmetry is the whole of the error. THE CLAIM IT SUPPORTS IS UNTOUCHED: body-scoped, that substring is still 0, so the entire delta is this entry's own prose and no second reference exists in the contract. THE LESSON IS A RULE AND NOT A THIRD INTEGER, because restating the number is what failed twice already: no whole-file count of ANY needle an entry spells is a stable measurement of this document, and a later round must carry only the body-scoped figures. POST-WRITE SCREENS, re-run over this draft after the last edit, body-scoped, `awk '/^## Version History/{exit}{print}' "$S"` first and then the newline-collapsed fixed-string pipe: the retired name 0, all six of v1.66's D10 needles 0 in total and 0 singly, with a POSITIVE CONTROL through the identical pipe — the needle `AC-5.6` returns 5 — so these are measured zeros and not dead ones. The `no`-token scale is 106 with its derived `up to 106 candidate occurrences` bound equal to it; distinct body test identifiers 13; the body is 1482 lines. Every one is structurally unmoved because this revision writes ONLY inside Version History, and each was RUN to confirm rather than assumed. Whole-file, v1.66's six-needle D10 sweep now totals 11 against the 6 stamped there; that figure was declared non-carryable by v1.66 itself and is restated here only to show that it moved. This addendum spells none of the six needles, adds no fenced block, no `.py:` pin and no bare line pin. PRECHECK: PASS issues=0.]
- v1.68: Phase 5d decisions D11 and D12 land in this document, routed as sheet §D13 items (b) and (c) (source: docs/03-analysis/doc-block-exec.5d-decisions.md §D11, §D12, §D13). Freeze 0251b9e; the design-author ran concurrently on the design and nothing else moved. TWO one-site contract repairs, both of them a stated SCOPE that is narrower than the mechanism it describes, which is why they are one entry and not two. (b) AC-5.6 read "(`--help` alone keeps argparse's exit-0 help text)" and the word "alone" was measured FALSE. THE PROBE WAS RE-RUN HERE RATHER THAN CARRIED, on /opt/anaconda3/bin/python3.11 (3.11.8, conda-forge), against the exact parser Task 4 prescribes -- `allow_abbrev=False`, `error()` overridden to raise `BadArgs`, `exit_on_error` left at its default -- with `doc` positional and `--heading` required, each case its own parser instance and its own redirected stdout/stderr. All five §D11 rows reproduce: `--help`; `--bogus --help`; `d.md --heading H --help`; `--help --bogus`; and `d.md --help` with the required `--heading` absent -- every one `SystemExit(0)` with 439 bytes of usage on stdout and nothing on stderr. Both controls reproduce and are what make those five a measurement rather than a probe that never reached the override: `--bogus` alone raises `BadArgs('the following arguments are required: doc, --heading')` and `d.md --heading` with no value raises `BadArgs('argument --heading: expected one argument')`. THE DECISION IS BROADEN, AND "IN ANY ARGV POSITION" IS NOT THE SENTENCE THIS DOCUMENT COULD WRITE, because writing it bare would repeat the defect being repaired one scope wider. §D11 names three shapes in PROSE, as reasons a pre-scan would be new surface, and asserts none of them; all three were EXECUTED here and two of them BOUND the class from the other side. `-h` short-circuits identically (`SystemExit(0)`, the same 439-byte usage), so the class is the option token and not the spelling `--help`. `d.md --heading --help` raises `BadArgs('argument --heading: expected one argument')` -- consumed as `--heading`'s VALUE, it is data and never reaches argparse's help action. `-- --help` raises `BadArgs('the following arguments are required: --heading')` -- after the `--` terminator it is the positional `doc`, again data. So the rule is written over the AXIS argparse itself decides: `--help` or `-h` RECOGNISED AS AN OPTION, wherever it sits, exits 0 with usage and reaches no verdict; the same characters consumed as an option's value or as a positional after `--` are data and route to the `DOCBLOCK:` contract like any other argument. RESIDUAL, a concrete category: this fixes the two spellings argparse's own default help action registers (`-h`, `--help`) and does not fix a spelling a later revision adds by declaring its own help option, nor a change to `add_help`; both are argparse-configuration changes and each would move this boundary. (c) AC-3.9 was the only `stream_paths_alias` site and promised head + exit 2 + block-not-run with no `leftover:` clause, while all seven `leftover` hits in the body belonged to AC-3.10's ROLLBACK arm or to Version History -- a HALF-APPLIED boundary, since two branches create a reservation and delete it and only one was specified to read the deletion back. §D12's decision REQUIRE is applied: the alias branch re-reads with `os.path.lexists` after its unlink and, on a surviving file, refuses with the `leftover: "<path>"` detail line rather than a bare `UNREADABLE reason=stream_paths_alias`, with `test_alias_refusal_unlink_failure_reports_leftover` built exactly as AC-3.10's rollback test is -- the same `os.unlink` injection, an alias trigger in place of the second-arm failure -- and the vacuity condition stated in the AC, because without the injection the unlink succeeds, nothing is left over, and a branch with no read-back at all passes. §D12'S PREMISE CORRECTION IS CARRIED AND NOT DROPPED: this is a FORWARD constraint on what Task 4 writes, never a missing read-back in shipped code -- at `7afc0ef` the helper is 465 lines with exactly one `lexists`, the cwd cleanup at the end of the file, and no `O_EXCL`, no stream-path reservation, no alias detection and no rollback unlink anywhere, so an implementer sent after "the branch that is missing its read-back" would find no branch. A FIGURE THIS REVISION'S OWN FIX MOVES, found by sweeping rather than by the brief, which named neither: AC-5.5's injection list describes the `os.unlink` seam as "for the reservation rollback's read-back", and after (c) that seam has TWO callers. The COUNT is unmoved -- still exactly nine named injections and eight module seams beside the instance-level one, because the alias test reuses a seam already on the list and adds none -- so the number is frozen deliberately and the DESCRIPTION is broadened to name both arms. That is the distinction between a figure a revision cannot affect and a description of a class the revision adds a member to; only the second moved. Nothing else is owed inside this document: `leftover:` is already a registered detail key (FR-4's enumeration and AC-4.5's bidirectional registry walk both carry it, so no key count and no remedy-row count moves), and §D11 states in terms that AC-4.1's broadening adds no mutation row and leaves the matrix total at 90. THE SWEEPS, run over the whole file including fenced blocks, table cells and comments inside commands, and with the body newline-collapsed for the value greps: `grep -n -- '--help'` (one hit before, and after the repair the hits are the AC-5.6 clause alone -- no second site ever stated the narrow scope); `grep -n 'alias'` (three body hits, all AC-3.9, plus two Version History entries); `grep -n 'leftover'` (seven before: AC-3.10 twice, FR-4's key enumeration, one unrelated prose use of the ordinary English word inside FR-6, and three Version History lines); `grep -c 'lexists'` on the body. OWED BY OTHER DOCUMENTS, reported and NOT edited, and this is a claim about a tree that was MOVING while the design-author ran, so it must be re-measured after the batch lands: §D13 item (a) is a DESIGN-author decision and is untouched here -- the design's mutation matrix carries `stream-alias-check-removed` and `rollback-leftover-unreported` and neither pins the ALIAS arm's read-back that §D12 now requires, so either a row is added (moving the matrix total 90 -> 91, with the killer already landed as `test_alias_refusal_unlink_failure_reports_leftover` inside Task 4's `--expect-fail 62`) or the design records why the alias read-back needs no row of its own, which is the answer §D12 already refused for the read-back itself; it blocks 5e, not Task 4's RED. NO gating claim, NO second surface, NO exit-gate claim is made by this entry. [Appended after the entry landed, every sweep re-run over the body AFTER the last edit, the body cut at the Version History heading with awk so no screen counts its own entry. The retired narrow scope is GONE and the zero is a run, not a reasoning: `grep -c 'alone keeps'` on the body -> 0, and the newline-collapsed form `tr '\n' ' ' | grep -o '\-\-help. \{0,3\}alone' | wc -l` -> 0, which is the screen that matters because the old clause was HARD-WRAPPED between "`--help`" and "alone" and every single-line grep for the phrase would have returned 0 while it was still there. `grep -n -- '--help'` on the body returns FOUR matching lines, all four inside the AC-5.6 carve-out paragraph this revision wrote (unit: matching lines, not occurrences). `leftover` moves from seven matching lines to seven-plus-three: AC-3.9 now carries three (the detail line, the test name, the assertion), AC-3.10 keeps its two, FR-4's key enumeration and FR-6's ordinary-English use are untouched, and the Version History lines are outside the cut. `stream_paths_alias` is 2 OCCURRENCES on 2 lines, both AC-3.9 -- its head and the bare form the read-back replaces. `lexists` on the body is 1, the new AC-3.9 read-back, which is the count that was 0 before this revision and is the reason (c) existed. UNMOVED AND RE-DERIVED RATHER THAN ASSUMED, because these are counts over a class this revision touches: `exactly **nine** named fault injections` -> 1 and `eight module seams` -> 2, both unchanged, since the alias read-back reuses `os.unlink` and adds no seam. PRECHECK: PASS issues=0 (h_mad_precheck_doc.py --phase spec --root /Users/kimhawk/orca/skills); advisories read and all kept deliberately, all of them pre-existing -- the STALESHA stamps on 74e126f, a8e0372 and fbc2ea0 are historical readings their own entries explain, the two LINEPIN hits are FR-6's bare ordinals covered by that section's stated residual, and every COUNT hit sits inside a Version History entry, which residual (1) exempts.]
