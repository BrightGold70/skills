...
| J41 | 🟡 | MONITORING | The state schema has no field for the 5c baseline sha, though SKILL.md's 6a-prime (`BASE = 5c sha`) and 5f (`--base <5c sha>`) both take it. `h_mad_state_write.py` correctly REFUSED the invented key rather than letting it reach disk — the guard behaving exactly as designed is the point worth recording. **PREMISE CORRECTED 2026-08-28 — the reason this row gave for NOT adding the field is false.** It claimed the value is derivable because `git merge-base main <branch>` "returned the exact commit". It does not: 5c is defined as `git checkout -b …; commit impl-plan + audit files`, so the 5c sha is the branch's FIRST COMMIT and the merge-base is that commit's PARENT. Measured on the very feature that filed this row: merge-base = `b5c8f41` (the value the dogfood tried to write), actual 5c = `730cc16`, and the gap between them is the 5c commit's own 313 insertions — the impl-plan (249 lines), three audit files and the dogfood doc — which enter the 6a-prime review diff as newly-added content when SKILL.md's contract makes them a separate INPUT to that review. The verification was **circular**: the sha was "confirmed" by re-running the same `merge-base` command that produced it, with no control. Measured correct derivation: `git rev-list --first-parent <branch> --not main | tail -1` → `730cc16`. Store-vs-derive stays genuinely open (a stored sha dies on rebase; a derivation recomputes but assumes the branch's first commit is 5c). Chasing this row's consequence into `--base` surfaced J43, which is the larger finding. |
| J43 | 🔴 | **FIXED** | **The wire registry silently performed the exact removal its own schema forces you to declare.** `register` upserted on `record["id"]` alone and `compare` keyed on the same bare `id`; `id` is the impl-plan task number `"Task N"`, which restarts at 1 for every feature, so any two features that both register a Task N collided by construction — and the successor MASKED the eviction, so `step5f:undeclared_removal` structurally could not fire. The schema meanwhile requires `status`/`removal_provenance`/`removed_by_feature` for a declared removal: the whole apparatus the upsert walked around. Measured over the full history of this repo's `.h-mad/wires.jsonl`: 7 distinct `(owning_feature, id)` pairs ever registered, 6 at HEAD, `audit-cycle-verb :: Task 4` (pin `test_fail_in_either_pass_fails_cycle`) GONE and `masked_by_same_id_at_HEAD=True`. It was also **unrestorable** — probed on a scratch copy, re-registering it evicted the successor, because the registry could hold exactly one `"Task 4"` across all features, ever. **FIX:** identity is now `(owning_feature, id)` via `_record_key()`, used by both `register` and `compare`; halt reasons carry `_record_label()` = `<feature>::<id>`, because a bare id names no single wire once two features have run and two simultaneous removals would otherwise emit the same line twice. `owning_feature` was already a required field, so no data migration was needed. The evicted record was restored **through the fixed write path** (6 → 7 records, both Task 4s coexisting), which is itself the proof: the same call that used to destroy is now correct. SKILL.md's halt-reason contract updated, and `step5f:wire_pin_ambiguous` — emitted but **undocumented**, a gap the fidelity scraper omitted on both sides and so could never see — is now documented and scraped. 7 new tests (RED first), 10 pre-existing format assertions updated to the scoped form, `wire_registry_key.json` mutation spec 5/5 caught, full suite 2197 passed. `challenge`/`partition` were audited this time: their `task['id']` comes from `_parse_tasks(impl_plan)`, where ids are unique within one plan, so they were never exposed. **OPEN, other repo — see J44.** |
| J44 | 🔴 | MONITORING | **HemaSuite's HPW wire registry lost 7 of 19 records to the J43 collision — a 37% loss rate, all masked.** The J43 fix lives in a script shared through the skills symlink, so it stops further losses in every project automatically; the records already gone do NOT self-heal. Measured over the full history of `/Users/kimhawk/orca/HemaSuite/hematology-paper-writer/.h-mad/wires.jsonl` (9 commits, 19 distinct `(owning_feature, id)` pairs ever registered, 12 at HEAD), every one `masked_by_same_id_at_HEAD=True`: `article-path-ledger-wipe::Task 3`, `guideline-seeder-manuscript-path::Task 3`, `knowledge-orchestrator-config-propagation::Task 4`, `::Task 7`, `::Task 11`, `::Task 12`, and `run-report-seam-restoration::Task 3`. Five distinct features. HemaSuite's ROOT `.h-mad/wires.jsonl` is clean (6 commits, 6 ever, 6 at HEAD, 0 lost). Restoration is mechanical now that the key is fixed — every lost record is recoverable verbatim from git history (walk each commit touching the file, collect `(owning_feature, id)` pairs, re-`register()` those absent at HEAD) — but it is a write to another repository and was not performed from here. Re-verify with the same walk after restoring; expect LOST=0. |
