...
- 🟡 **G4** — only `gate-create`/`gate-resolve` existed; a "blocking" gate could be opened but not waited on (`await` waits for `worker_done`, not gates). Added `gate-wait <id> [--timeout][--interval]` polling `orchestration gate-list` for `.resolution`/non-pending `.status`. Live-verified (clean timeout on empty). Caveat: resolved-gate field names (`status`/`resolution`) confirmed against a live *resolved* gate is still outstanding — the empty-list path and timeout are verified.
