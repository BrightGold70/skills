...
| J34 | 🔴 | MONITORING | `h_mad_assemble_tdd.py` interpolates `--module` **verbatim** into `slug = f"{feature}_{module}_{phase}"`, which then composes the default `--prompt`, `--out` and `--log` paths — so a module in a subdirectory yields a path whose parent directory was never created. **SEVERITY CORRECTED 2026-08-28, and this row's own "survivable" claim is false.** It read *"survivable because it prints commands rather than running them"*; that holds for `--out`/`--log`, which are only interpolated into the printed block, but NOT for `--prompt`, which the tool actually WRITES. The run halts before printing anything: `ASSEMBLE-TDD: HALT prompt_unwritable`, exit 2, no prompt on disk. Reproduced with a control — two identical invocations differing only in the module: `--module tools/thing.py` → `HALT prompt_unwritable /tmp/h_mad_probe_tools/thing.py_red.txt: [Errno 2] No such file or directory` (rc=2); `--module thing.py` → `ASSEMBLE-TDD: PASS` (rc=0). **It therefore blocks every 5d/5e staging in HemaSuite, where every module is `tools/…`, and in this repo, where every module is `h-mad/scripts/…`** — i.e. the default invocation is unusable in both projects that run h-mad, and the operator workaround is to pass `--prompt`/`--out`/`--log` explicitly every time. The verdict discipline is correct throughout (a real `HALT` token, exit 2, nothing written), so this fails loudly rather than silently — which is why it reads as an operator error rather than a tool defect. Kept as one row rather than split, because the second finding shares the first's root cause and a duplicate row would fragment it. Fix: slugify the module for filename purposes (`/` → `_`), or key the filenames on the task id, which is already unique per plan. Originally F18 in the anchor-precheck dogfood ledger; the halt half reported by the operator 2026-08-28. |
