...
| J41 | 🟢 | MONITORING | The state schema has no field for the 5c baseline sha, though SKILL.md's 6a-prime and 5f both take it. `h_mad_state_write.py` correctly REFUSED the invented key rather than letting it reach disk — the guard behaving exactly as designed is the point worth recording. Value is derivable (`git merge-base main <branch>`), so the fix is arguably to say so in SKILL.md rather than grow a field (F17) |
| J42 | 🟡 | MONITORING | `h_mad_offcontract_scan.py` — the tool that closed J30 — is absent from SKILL.md's §"Helper scripts" registry, and its verdict token `OFFCONTRACT: NONE|FOUND|UNREADABLE` appears **zero** times in SKILL.md or `references/` (positive control: `h_mad_wire_registry.py` appears twice, so the search is sound; the two `off-contract` hits describe `h_mad_extract_verdict.py` and are unrelated). 182 lines, 10 mutations, tested — and undiscoverable from the docs, so no orchestrator following SKILL.md would ever run it. Pointedly, J30's own finding was artifacts written where nobody can find them. Fix: add the registry entry and a doc test pinning the token, matching the `test_skill_documents_*` pattern. Also: this row exists because J30 sat MONITORING for 5 days after its fix merged — a stale row is a coverage hole in both directions |
