...
| H4 | 🟡 | FIXED | Codex auto-detect decays mid-run (banner scrolls off preview) — added `pin-agents` verb + session pin file (`.h-mad/orca-pins.env`); resolution precedence env pin → pin file → auto-detect |
...
- 🟡 **H4 — Codex identity by preview is not durable.** `env`/`resolve` resolved `codex -> term_41f3e488` at Phase-5 start (preview carried `gpt-5.6-terra`), but mid-run `resolve codex` returned UNRESOLVED: after Codex did work, its preview window showed report/output text and the model-id banner had scrolled off, so H2's `codex|gpt-[0-9]` preview alias matched nothing. H2 remains correct for a *fresh* Codex pane; it cannot survive banner decay. The run pinned `HMAD_ORCA_CODEX_TERMINAL` and proceeded. **FIXED**: added a `pin-agents` verb that resolves codex+agy once and freezes the handles into a session pin file (`${HMAD_ORCA_PIN_FILE:-.h-mad/orca-pins.env}`, already gitignored); `_resolve_target`'s orca branch now reads it with precedence **env pin → pin file → auto-detect**, so a frozen handle survives banner decay while an explicit env pin still overrides. SKILL Phase-5 preflight now calls `pin-agents` after `env`; `agent-substrate.md` documents the precedence + decay. 4 tests; live-verified (pin-agents wrote the file, `resolve` read it with no env pin). Extends F9's "pin the Codex terminal" — the pin is for banner decay too, not just the worktree-title case. [[F9]] [[H2]]
