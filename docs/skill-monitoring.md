...
| J34 | 🔴 | **FIXED** | `h_mad_assemble_tdd.py` interpolated `--module` **verbatim** into `slug = f"{feature}_{module}_{phase}"`, which composes the default `--prompt`, `--out` and `--log` — so a module in a subdirectory produced a path whose parent directory was never created. **This row's own "survivable" claim was false:** it holds for `--out`/`--log`, which are only interpolated into the printed block, but NOT for `--prompt`, which the tool WRITES. It halted — `ASSEMBLE-TDD: HALT prompt_unwritable`, exit 2, nothing on disk. Reproduced with a control, two invocations differing only in the module: `tools/thing.py` → HALT rc=2; `thing.py` → PASS rc=0. Every module in HemaSuite is `tools/…` and every module in this repo is `h-mad/scripts/…`, so the **default invocation was unusable in both projects that run h-mad**, with hand-passed `--prompt`/`--out`/`--log` as the standing workaround. **FIX:** `_filename_slug()` maps anything outside `[A-Za-z0-9._-]` to `_`, per component, **for filenames only**. The module path inside the prompt is untouched, because that is the instruction Codex acts on — sanitising it there would aim the implementer at `tools_thing.py`, a file that does not exist, trading a loud halt for a silent wrong edit, which is strictly worse than the bug. That risk has its own test and its own mutation, anchored at the CALL SITE rather than inside the helper, because the helper is correct and the whole hazard is applying it to the wrong value. Accepted limitation, recorded rather than hashed around: two modules differing only in a separator collapse to one slug, and would also have to share a feature and phase to collide. Verified live on both real shapes (`tools/…` and `h-mad/scripts/…`) with defaults only, and the prompt still names `tools/thing.py` (0 hits for the sanitised form). 7 new tests, `assemble_tdd.json` 23/23 mutants caught, full suite 2235 passed. Originally F18 in the anchor-precheck dogfood ledger; the halt half reported by the operator 2026-08-28. |
| J35 | 🟡 | **CLOSED — premise refuted** | `hmad-dispatch progress` was reported as exiting **1** on a normal LIVE poll, and this row concluded *"the doc's reasoning is the one worth keeping, so the wrapper likely needs the fix"*. **The wrapper needed no fix.** Probed all five observable states — fresh log, dead `--pid`, empty log, MISSING log, stale mtime — and every one returns **0**; `_cmd_progress` ends in an explicit `return 0` with a comment giving the same reasoning as SKILL.md:490, so code and doc already agreed. The non-zero came from the **pipeline the poll was wrapped in**: `progress … | grep -q IDLE` returns grep's verdict, and grep correctly finds no match on a LIVE log — measured, `| grep -q LIVE` → 0 and `| grep -q IDLE` → 1 on the same file. So the exit code was never `progress`'s, and the row blamed the wrong component. **Kept as a doc change, not a no-op close:** the trap is real and the existing warning did not cover it, so SKILL.md now says do not launder that exit code through a pipe, with the measurement. Third row this session whose recorded conclusion was false on re-probing (with J41 and J34). |
| J36 | 🟡 | **CLOSED — doctrine, not code** | An `exec codex` report stated an anchor sweep as `mutations=238` when the tree held **244**, every spec byte-identical to HEAD, inside an otherwise-correct report whose other figures checked out — a fabricated count. There is no code defect here to fix: the harness was right, the tree was right, and the agent's prose was right about its actions. What was missing was a rule, so one was written rather than a guard invented to look like a fix. `invariants.base.md` gains §"Counts a dispatch reports": re-derive every count a dispatched agent reports; never carry it into a document, a commit message or a verdict on the agent's word, because a report that is right about its actions can still be wrong about its numbers and the two are indistinguishable from outside. Deliberately narrower than §"Assumption verification" and aimed at a different moment — there the risk is an assumption written without evidence, here it is evidence that was produced, looks right, and is false. The cheap control existed throughout: `--check-anchors` prints the count in about a second. |
| J37 | 🔴 | **FIXED** | `--check-anchors` and `run_spec` collapsed two distinct cannot-judges into one verdict word: `ANCHORS_DRIFTED` absorbed drifted-and-unreadable (*"An unreadable target is a drift too"*), and `REFUSED` absorbed a drifted anchor and an unreadable TARGET file. The counts were already separate on the summary; the WORD is the part that gets read, and the operator's next action differs — re-anchor a spec versus restore a file that is gone. **FIX, split by what the token is FOR.** `ANCHORS_UNREADABLE` is a new verdict and **outranks** `ANCHORS_DRIFTED`, because a spec cannot be re-anchored into a file that no longer exists — the same precedence that makes `REFUSED` outrank `SURVIVED` and `UNSHAPED` outrank `FAIL`. Both exit 2; both still carry `drifted=`/`unreadable=`, so the word chooses the first action without hiding the other finding. The `MUTATION:` half deliberately keeps ONE `REFUSED` word and gains `unreadable=` instead: it has **five** distinct causes (unreadable target, drifted anchor, named test not green, mutation did not land, run went red without the named test) and a single remedy (`step5e:mutation_unverified`), so five verdicts would be worse than the disease. **The change silently un-guarded the pre-push hook, and that is the finding worth keeping:** the hook matched only `*ANCHORS_DRIFTED*`, so the moment the harness gained the new word a spec with a deleted target fell through to the catch-all and printed *"no ANCHORS_* verdict … Push ALLOWED"* — allowing the push AND misreporting a perfectly good verdict as broken tooling. Reproduced live before fixing the hook, now blocked, and pinned by both a test and a mutation so the coupling cannot break quietly again. One pre-existing test asserted the collapsed verdict — it had pinned the defect as its acceptance criterion — and one mutation anchor drifted onto the rewritten expression; `--check-anchors` refused rather than measuring nothing. 6 new tests, `mutation_harness.json` 36/36 and `prepush_anchor_hook.json` 15/15 caught, full suite 2241 passed. |
| J38 | 🟡 | **FIXED** | `h_mad_ab_dispatch.py --run` rejected any argv token beginning with `-` in the space-separated form SKILL.md documents: `--run --model` failed with `expected one argument` while `--run=--model` worked, because argparse reads a `-`-leading value as an option. Every real dispatch argv starts with flags, so **the documented invocation was broken for essentially every real use** (F6 in the anchor-precheck dogfood ledger). Reproduced live. **FIX:** `_split_run_tokens()` consumes `--run <token>` pairs positionally before argparse sees them; the `=` form is left to argparse so existing callers — who all had to use it — are untouched. Both forms are now tested, and the tests assert **parsing**, never the exit code: `INCONCLUSIVE` legitimately exits 2, so scoring this on `$?` would have passed for a tool that never parsed the argv at all — the same trap this tool exists to enforce against elsewhere, and one my first draft of the test fell into. |
| J39 | 🔴 | **FIXED** | `h_mad_ab_dispatch.py::_observe` took the **FIRST** regex match where every other extractor in this skill takes the **LAST** (`h_mad_extract_verdict.py` uses `matches[-1]` deliberately). An agent's log carries the PROMPT before it carries the answer, so first-match routinely returned the observable's own name quoted back out of the instruction the agent was given. **Reproduced live before the fix:** a log reading `emit a line like RESULT: <n>` / `RESULT: 0` / … / `RESULT: 42` observed **0**. The consequence is specific to the tool's purpose — both arms echo the same prompt, so both observe the same echoed value, so the run reports `SAME` ("the rule is present and not causally effective"), which is a FINDING rather than an error and therefore catchable by nothing downstream: a tool built to establish causality was able to report the prompt back to the operator as its result. **FIX:** `finditer` + `matches[-1]`, pinned against `extract_verdict` itself rather than by restating its rule, so a future change to either is a deliberate decision instead of silent re-divergence. `None` (never `""`) on no match is retained and separately mutation-pinned — two silent arms must not compare equal. The pre-existing no-capture-group guard was probed and already correct, so it is pinned as a regression rather than filed as a fix. 5 new tests, `ab_dispatch.json` 16/16 mutants caught, full suite 2229 passed. |
| J40 | 🟢 | **FIXED** | `h_mad_ab_dispatch.py` substituted only `{prompt}` and `{log}`, so **every other per-arm path in the argv was byte-identical across the two arms** (F5): arm B overwrote arm A's output and the comparison then read B against B — silently, reporting `SAME`, i.e. "the rule is not causally effective", the most believable wrong answer this tool can give and the one its whole verdict set exists to prevent. **FIX:** an `{arm}` placeholder, substituted alongside the other two. A placeholder rather than flag-awareness deliberately — the tool cannot know which token of an arbitrary argv is a path, and a guess would be wrong for the next runner; `{arm}` composes with anything. The second half (F3, no notion of controlling the ENVIRONMENT) is **not codable and is now stated instead**: the tool holds the PROMPT constant apart from the declared variable and knows nothing about a changed file, a model default or an env var, so `UNCONTROLLED` only ever compares the two prompts. An unstated limit reads as a guarantee, so SKILL.md now says so and a test pins that it keeps saying so. `ab_dispatch.json` 17/17 mutants caught, full suite 2244 passed. One test was written, found to call `run_ab` at the wrong layer, and **removed rather than repaired** — the CLI test exercises the fix end to end, and a test that does not test what it claims is worse than no test. |
...
| J45 | 🟡 | **FIXED** | **`--release` had no ownership guard, so `--release` + `--claim` took a LIVE owner's feature with no `--force` anywhere.** `claim()` refuses a live owner and its docstring says `force` is *"the verb for taking a feature from a session that is still RUNNING"*, warning that routing routine cases through it *"teaches an operator to pass it by reflex, which is how the guard stops protecting the case it exists for"*. `release()` — one function below — cleared the owner unconditionally. Measured on a scratch state file with the heartbeat set to NOW: claim refused exit 2, `--release` OK exit 0, claim then OK exit 0, final owner the second session with `phase` still `step5`. **FIX:** `release()` now reads the SAME staleness window (`h_mad_state_ownership`) as `claim()`. Three cases stay free so the guard cannot teach a force reflex — unowned, a STALE owner (ordinary crashed-session cleanup, the case `claim()` explicitly refuses to route through force), and `--session-id` matching the live owner (a session releasing its OWN claim, the routine end of every piece of work). Only a live *foreign* claim needs `--force`. An unidentified caller on a live claim **fails closed**, and the refusal names `--session-id` rather than `--force`, because pointing an operator at force for a case `--session-id` solves is how the reflex gets taught — that message is mutation-pinned as its own guard. Same probe re-run end to end after the fix: all three steps refused, owner unchanged. Both prescribing call sites updated (`handoff/SKILL.md` HANDOVER, which releases its own live claim seconds before handing over, and `h-mad/SKILL.md`). One pre-existing CLI test broke and was updated rather than weakened: it asserted the CLI round-trips, not that release was unguarded. 9 new tests, `release_ownership_guard.json` 6/6 mutants caught — mutated in BOTH directions, because a guard that refuses a routine self-release or a stale cleanup is worse than none. Full suite 2221 passed. |
| J46 | 🟡 | **FIXED** | `hmad-dispatch exec` wrote `--out` non-atomically at **three** sites — one `cp` and two `>` redirects — and a redirect truncates BEFORE it writes, so anything polling for the file could catch it partial or zero-byte. Latent until something polled; the `report-wait --no-done-marker` work made it live, which is why it was fixed FIRST. A truncated verdict reads exactly like a real one — the failure the `.done` marker was invented to prevent, re-entered by another door. **FIX:** one `_write_out_atomic` helper, temp + `mv` in the destination's own directory (`rename(2)` is atomic within a filesystem; same-directory is what keeps it a rename rather than a copy). **The first version shipped a race of its own:** it keyed the temp on `$$`, which is constant across every `--out` a single wrapper invocation writes, and `audit-cycle` writes one per pass — so two writes shared a temp and audit-cycle silently lost its p1 report. Caught by the FULL suite, not by the targeted tests, and not by review. Now `mktemp`, and pinned. Atomicity is asserted **structurally** (the helper must name `mv`, not `cp`) because it has no externally observable trace that `cp` + `rm` does not also produce — mutation testing proved exactly that by leaving every behavioural assertion green. `out_wait_atomicity.json` 5/5 caught, full suite 2256 passed. |
