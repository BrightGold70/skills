...
shared `_agent_pv_re`, which matches prose 36/36 — see AC-1.4). The launch command line is not a
...
- **Description**: A pass between Pass 2 and Pass 3 reads each unresolved candidate's tail
  ...
  INDEPENDENT literals, not a wrapper around `_agent_pv_re` — that helper matches prose 36/36 and
...
- **Acceptance Criteria**:
  - AC-1.1: Given a candidate pool of one pane whose tail matches the agent's `_agent_tail_re`
    ...
    AC-1.4 measures matching ordinary prose 36/36 — and no other pane's, `_orca_find <agent>` prints
