...
(`h-mad/tests/docsections.py`, `h-mad/tests/mutation-specs/docsections.json`,
`h-mad/tests/test_docsections.py` — **rooted, because every command in this document runs from the
repository root and a root-relative path with no root does not resolve there**; §Deliverables
already spells all three this way and the two surfaces now agree). §Deliverables and
...
paths, OS-error text, `leftover:`, `intersect:`) is rendered through one escaper, `_field`, as a double-quoted
...
decorative — but through v1.103 this paragraph named the WRONG ADDRESS for the leak, and the leak
is real.** It said the command "was written first as a bare `sed` range over the whole file" and
returned `1` instead of `0`. **With the ANCHORED address the published command actually uses, that
does not reproduce at any commit in this feature's history.** Run with `/usr/bin/sed`, `/usr/bin/awk`
and `/usr/bin/grep` — the interactive `grep` in this shell is `ugrep`, so the system binaries are
named — at eleven shas (`b3be433`, `00b961f`, `df04e8e`, `dfae038`, `59cc2ad`, `7b182b0`, `3f70eb3`,
`4c1c3a5`, `af19d53`, `09e9307`, `fbc2ea0`), the whole-file and `awk`-scoped readings are **equal at
every one** — `0`/`0` through `dfae038`, `2`/`2` from `59cc2ad` — and
`grep -c '^  - AC-4.2:'` whole-file returns **1** at all eleven, as does `^  - AC-4.3:`. An address
occurring once cannot open a second range.
**The address that DOES leak is the bare token, and it is recovered here rather than the mechanism
being marked unexecuted.** `sed -n '/AC-4\.2/,/AC-4\.3/p' | grep -c 'BAD_ARGS'` — the same range on
the unanchored token — returns **1** whole-file against **0** `awk`-scoped at `b3be433`, `00b961f`
and `dfae038`, and **5** against **2** from `59cc2ad` onward. That `1` is exactly the reading the
old sentence described; the bare token recurs inside the spec's own §Version History (**3**
whole-file against **1** body-scoped at `b3be433`, **7** against **3** at `af19d53` and `fbc2ea0`)
while the anchored address does not. **So two things were conflated and are now separated: the
corpus (`awk`-scoping fixes it) and the address (anchoring fixes it), and only the second one was
ever the mechanism here.**
**The general property is EXECUTED on a fixture rather than reasoned from**, because this document's
own rule is that a property a screen claims is a claim about code and is run:
...
Six lines in, five out: the second range opened at the repeated address, never met a `^B:`
terminator and printed to EOF, dragging the unrelated `BAD_ARGS` line in. **The class: an
unterminated second range prints to EOF, so a range command is written with an address that is
UNIQUE in the corpus it runs over, and the corpus is narrowed to the body before the range is
applied — two independent repairs, both applied here.** Residual: `awk`-scoping fixes the corpus and
not the range, so a body that genuinely repeats the opening address twice would still print two
ranges; that is caught by reading the range's output, which is why it is printed and not only
counted. Second residual, the one this paragraph is an instance of: **nothing detects a prose
description that attributes a real reading to the wrong command**, since both forms are runnable and
only one was run — which is why both forms are now printed with their readings side by side.
...
`sys.path.insert`, in several spellings). **Re-run at the measurement commit `cac6edc`, and the denominator is
the only half that moved**: the same two commands return **13** and **89**, with **48** on the
loose spelling — `b39d9dc` added `h-mad/tests/test_h_mad_agent_definitions.py`, which carries no
`sys.path.insert` at all. The `35698f9` pair is a reading of a committed blob and keeps its own
stamp; the conclusion the pair exists to support is unchanged and is *strengthened* by the move,
since the convention's share of the directory fell while the convention itself did not grow. **The
axis this bullet is on**: the denominator is a directory listing, so it drifts at every commit
adding a test file and must be re-derived at each freeze, while the numerator is a spelling census
that moves only when a test file adopts or drops the exact `SCRIPT_DIR` form — the two are stated
as separate readings for that reason and are never written as a ratio. The instance this feature's own consumer already carries
...
the assertion set; they are the AC-1.8 collect-alone pins Success Criteria names, and **both are
named by node ID here rather than one by name and one by mechanism**:
`test_docsections_imports_when_collected_alone`, which runs
`[sys.executable, "-m", "pytest", "--collect-only", "-q", "-p", "no:cacheprovider", "h-mad/tests/test_docsections.py"]`
as a subprocess from the repository root and requires exit **0** (collected **alone**), and
`test_docsections_imports_from_an_unrelated_cwd`, an isolated `python3 -c "import docsections"`
with the tests directory on `sys.path` and an unrelated cwd.
**THE COLLECT-ALONE PIN IS COLLECTION-ONLY, AND THAT IS A CONTRACT RATHER THAN AN IMPLEMENTATION
DETAIL** — spec v1.63's wording, and this document carries the premise that makes it load-bearing:
§Success Criteria's AC-6.4 adds `test_docsections_delegates_to_the_authoritative_bounder` to
`test_docsections.py` and AC-6.5 requires that node to go RED under
`docsections-delegation-reverted`, so a subprocess that *runs* the file cannot stay green under
the mutant the wire pin exists to fail. A pin asserting exit 0 over a file this feature is adding
a deliberately-failing-under-mutation test to is a contradiction, not a stronger check. **Through
v1.104 this sentence spelled the pin as a plain `pytest` invocation naming that file with `-q` and
no collection flag — a RUN form — so the plan prescribed a pin that is red by construction in
exactly the state the mutation harness creates**; filed by both v88 gating legs and by neither of
them as a style point. **The retired spelling is DESCRIBED rather than quoted**, because a
paragraph reproducing a retired string verbatim puts that string back into every value sweep of
this document and hands the next auditor a hit it must then rule out — the self-quoting hazard the
spec's screen block has already paid for once.
**Residual, stated as a concrete category**: the pre-existing tests in `test_docsections.py` are
no longer run *in isolation* by AC-1.8. They still run — in the full suite under the AC-6.4 floor,
and in the 5e module-scoped run. **The naming half is a defect of its own and is repaired with
it**: the design names `test_docsections_imports_when_collected_alone` (**1** occurrence at
`cac6edc`) and the impl-plan names it (**4**), while this document named **0** at `cac6edc` and gave that pin
only as a shell command, so a 5c author reading this plan alone got one AC-1.8 import pin as a
test name and the other with no node ID to bind — the shape this document's own rule against
unnamed seams exists to prevent. **The existing mutation spec moves with the code:**
...
| The FR-1..FR-5 mutation population, realised on disk in ONE file, `h-mad/tests/mutation-specs/doc_block_exec.json` — **the per-task sibling this row named through v1.108 was STAGING, and it has been folded into that file and DELETED**, the operator's §D10 decision executed at `7afc0ef` and recorded with its evidence table at `8bc2fa7` in `docs/03-analysis/doc-block-exec.5d-decisions.md`. **The population total and its on-disk realisation are separate measurements and the row states both, because the first cannot answer "is it there yet"**: at `861efbe` the file holds **60** rows and **60** distinct names — equal, which is what rules out a name duplicated by the fold, the one failure a row count alone cannot see — and all **60** are matrix members, so the matrix is **60 of 90** realised with Tasks 4 and 5 unwritten. **The `32 + 28` split this cell published through v1.108 is RETIRED rather than re-stamped**: it was a claim about two files and one of them no longer exists. **That realisation figure drifts by construction, is neither a floor nor a target, and is re-derived — never incremented — at every freeze**, by the walk published under §Measurements in the paragraph led by *the mutation population and its ON-DISK REALISATION are two different measurements*, which through v1.108 also derived the two on-disk rows the matrix did not yet carry and routed them to the design — **a routing that same walk now CLOSES rather than carries**, `on-disk-not-in-matrix` returning the empty list at `861efbe` because the design took both rows, which is the `88` → `90` its total moved by. **The set of spec files is read by GLOB and never from a name convention**: the harness sweeps `specs_dir.glob("*.json")`, so a file is covered the moment it lands and stops being swept the moment it goes, and no document has to name it for the gate to reach it — which is why the merge required no gate change and why the closure predicate is a reading of the DIRECTORY rather than a sentence in this row, `ls h-mad/tests/mutation-specs/ | grep -cE '_task[0-9]+\.json$'` → **0** at `861efbe` | mutation spec | FR-1..FR-5 — **90** mutations with a full-node-ID `test` binding each — **89 of the helper's source and 1 of `h-mad/SKILL.md`**. **That total is a COMMITTED-BLOB reading and carries NO batch stamp**: **90** at `861efbe`, where the matrix gained `kill-skipped-after-collect-failure` and `stderr-not-closed` — two arrivals, zero departures, total +2, so pure addition and no rename — and 88 at `da03ca5`, the commit that lands design v1.112 and v1.113, where the matrix gained `timeout-upper-bound-removed` AND renamed `preamble-composed-with-unsubstituted-text` to `substitution-result-not-passed-to-run_block`. **Set-differencing column 1 at `7fc5f94` and `da03ca5` returns TWO arrivals and ONE departure while the total moves by ONE**, because a rename shows as one of each and is invisible to the count — the two figures disagreeing is the evidence a rename happened, and a reader who reconciles this cell against the design BY NAME needs the departure stated or the missing name reads as a dropped row. **The arrivals are named and their mechanisms are NOT restated here**: the design owns the mechanism and the `killed by` binding, a 5c author sizes this spec from the total and the names, and the harness refuses a spec whose rows do not match their anchors. **The ROUND-NINETEEN batch stamp came due when `7fc5f94` landed and is discharged here rather than carried**: `87` was a working-tree reading over that design revision's post-edit body, and re-run as `git show 7fc5f94:` through the same three walks it returns total **87**, distinct names **87**, `SKILL.md`-target **1** — identical to the working-tree triple v1.106 published, so only the FORM was owed and no value moved. At `7fc5f94` the matrix gained `intersect-kind-ignored` — the refusal renderer stops selecting its detail-line prefix from the element's `kind` tag and renders an intersecting pair as an overlap, dropping the offset, killed by `test_cli_subst_overlap_detail_lines` (AC-4.1/4.3 — the subprocess test where the `SUBST_OVERLAP` detail lines are asserted verbatim, which is the task the renderer lands in). **The design's FIRST DRAFT of this row named `test_substitute_refuses_intersecting_spans`, the design SHIPS the row naming `test_cli_subst_overlap_detail_lines`, and the distinction is kept because only one of those has a surface a reader can open**: re-running the matrix walk over the shipped design returns the Task 4 name in the row's `killed by` column, while the wrong name lived in an uncommitted intermediate nobody can check. **The first binding was WRONG rather than merely imprecise**: that test is Task 2's and asserts the exception DATA — `pairs == [("intersect", "ab", "bc", 1)]` and the block text unchanged — so a renderer-only mutant leaves it green and the row would have been scored as caught by a test that cannot see it. **The correction is carried here because a killer that cannot fail is a mutation row with no guard behind it**, which is the one thing this deliverable exists to prevent. At the measurement commit `0021c77` the same walk reads **86**, and that reading is not edited because it is true there — `0021c77` carries design v1.110 and the row arrives with v1.111, at `7fc5f94`. **The PREVIOUS batch stamp expired when its batch landed and is discharged rather than carried**, into the `git show ccd8ebd:` reading of **86** — where the matrix had gained `intersect-scan-non-overlapping`, the span scan's lookahead form degraded to a plain `re.finditer` over the escaped key, killed by `test_substitute_refuses_overlapping_occurrences_of_one_key` (AC-2.7), which the `aaab` fixture under `{aa→X, ab→Y}` reds and the `abc` fixture cannot — and at `cac6edc` the same walk reads **85**. **So this figure has moved at five consecutive design revisions and every reading is published with the stamp that fits it**: all eight at committed blobs and NONE at a batch, because the design revision this total is taken over is committed at `861efbe` before this revision was authored. The obligation, the discharge and the enumeration of the sites that carried the expired stamp live once under §Measurements, in the paragraph closing *the design-matrix derivation*, which records the set going empty. **NEITHER half is carried: the total and the split are re-derived from the same surface, in the same form, at the same sha.** That symmetry is the repair for what this cell shipped through v1.104 — it re-derived the SPLIT from the matrix's mechanism column and *carried* the TOTAL, so the half that moved was the half with no derivation attached to it, and the total sat at `81` while the matrix stood at `85`. **The two commands are published together under §Measurements, in the fenced block introduced as *the design-matrix derivation*, and are pointed at rather than re-typed here** — an `awk` walk over a markdown table cannot be written inside a markdown table cell without escaping the very pipes it matches on, and an unrunnable copy of a command is worse than a pointer to a runnable one. Their readings, each with the sha inside the command and each stating its total and its distinct-name count AS A PAIR rather than quoting one name figure across readings whose totals differ: **81**/**81** at `09e9307`, **85**/**85** at `cb4fe99`, **85**/**85** at the commit v1.105 was measured at `cac6edc`, **86**/**86** at `ccd8ebd`, **86**/**86** at the measurement commit `0021c77`, **87**/**87** at `7fc5f94`, **88**/**88** at `da03ca5` and **90**/**90** at `861efbe`. **Through v1.105 this cell said "85 distinct names at each" across the first three of those, and the walk returns 81 names at `09e9307`** — a uniform figure asserted over a list whose members disagree, which is the class this document already writes a rule against and which the §Measurements walk had already published correctly. The `SKILL.md`-target count is **1** at all eight readings (unit: matching lines, one row per line) — **the split at `861efbe` is 89 + 1, and at `da03ca5` was 87 + 1 rather than 86 + 2, because that count did NOT move when the total did**, which is the check the split is derived from rather than a restatement of it — and that one row is `registry-row-removed`, "one remedy row deleted from the `SKILL.md` Helper-scripts entry (the mutation targets `SKILL.md`)". **The four rows that arrived between `09e9307` and `cb4fe99` are named rather than counted**, because a 5c author sizing this spec from an `81` would have authored four rows fewer than the matrix and the harness refuses a spec whose rows do not match their anchors: `cleanup-chain-selection-flipped`, `intersect-check-removed`, `rollback-identity-check-removed` and `spawn-valueerror-unmapped`. Set-differencing the column-1 names at the two shas shows **four arrivals and zero departures**, so no row was renamed and the delta is entirely new coverage — the round-seventeen design decisions landing. The figure is derived from a **sibling under `docs/`**, so the §Measurements closure does not reach it and it is re-derived at every freeze; through v1.92 it read "Today exactly one does", a count with neither command nor sha. The AC-4.5 pin still has two directions and therefore two rows, but only one of them mutates the registry: the other, `detail-line-undocumented`, mutates the **helper** ("the helper renames one emitted detail line (`missing_key:` → `absent_key:`)"), so its `file` key is the helper's source, not `SKILL.md` — an implementer who writes `"file": "h-mad/SKILL.md"` there gets an anchor that cannot match, which the harness refuses. Each row's `test` binding is enumerated row by row — mutation name, mechanism, `tests/test_h_mad_doc_block_exec.py::<name>` — in the design's §"Test Plan", under the bolded lead-in "Helper mutation spec — `h-mad/tests/mutation-specs/doc_block_exec.json`, entry by entry" (a lead-in paragraph inside the `## Test Plan` heading, not a heading of its own), which is the authoritative matrix this row points at |
...
| `h-mad/tests/mutation-specs/docsections.json` — re-point the two bounder mutations at the authoritative module, convert every row to the named-test form (`target_command` + a full-node-ID `test` key), and add the four connection rows `docsections-delegation-reverted`, `docsections-syspath-setup-removed`, `docsections-heading-lookup-reverted`, `docsections-local-bounder-restored` — **4 + 4 = 8 rows**, written as the arithmetic over the surface it counts (the four the file carries today plus the four connection rows named in this cell) rather than as a free-standing total, so the reader's check is addition on the page; the design's own paragraph derives the same 8 by ordinal enumeration, and the derivation is published under §Measurements beside the mutation-total walk | mutation spec | FR-1 (AC-1.8) |
| `h-mad/tests/test_docsections.py` — gains the delegation spy test `test_docsections_delegates_to_the_authoritative_bounder`, which kills **two** `docsections.json` rows and not one: `docsections-delegation-reverted` and `docsections-heading-lookup-reverted`. **Both are named because this cell is the surface a 5c task split reads first**, and a task provisioning one RED where two are required under-scopes the test — the FR-6 wire-mutation table under §Implementation Strategy is the authoritative binding and states the same pair, so the two surfaces are re-read together rather than one being trusted alone | tests | FR-1 (AC-1.8), AC-6.4 |
...
either changed file is the suite floor in §Success Criteria, and `af19d53` moved it again
...
**A SECOND CLOSURE, OVER A SECOND CORPUS, BECAUSE ONE CLOSURE CANNOT SPEAK FOR TWO.** The interval
above is scoped to `h-mad/` and `handoff/` and says nothing about the repo-wide `.py` censuses this
section also carries. Their own predicate, run per sha:
`git diff --name-only 74e126f <sha> -- '*.py' | wc -l` → **0** at `dfae038`,
**2** at `af19d53`, **2** at `09e9307`, **6** at `fbc2ea0`, and **8** at `cac6edc`, at `bc4688e`
and at the measurement commit `0021c77`. **The series is extended rather than re-stamped, and the
`+2` is attributed by STATUS and not by name-count, because the two are different things**:
`git diff --name-status b39d9dc^ b39d9dc -- '*.py'` shows `b39d9dc` ADDING exactly one file,
`h-mad/tests/test_h_mad_agent_definitions.py`, and MODIFYING three, of which
`h-mad/tests/test_hmad_dispatch_exec.py` already existed at `fbc2ea0` and enters this count only
because `b39d9dc` is its first change since `74e126f`. **A `--name-only` diff counts changed NAMES,
so reading its growth as "two files were added" is wrong about one of the two** — the kind of error
that is invisible until the status column is asked for. **The conclusion the enumeration carries is
re-checked at the new sha rather than inherited**: neither name appears in the bash-fence-bearing
`.py` file list, which reads **5** at `fbc2ea0`, at `cac6edc` and at `0021c77`, and the any-fence
`.py` file LISTS at `fbc2ea0` and `cac6edc` are identical once the rev prefix is stripped —
`diff <(git grep -l '```' fbc2ea0 -- '*.py' | cut -d: -f2-) <(git grep -l '```' cac6edc -- '*.py' | cut -d: -f2-)`
prints nothing — so nothing this commit touched moved a fence census. **The `cut` is load-bearing
and is written into the command rather than left to the reader**: `git grep -l <rev>` prefixes
every path with the rev, so the same `diff` without it reports all 25 lines differing and
contradicts the claim it is quoted to support. **The two readings are read together and the arithmetic
is stated, because the obvious reading of the jump is wrong**: `fbc2ea0` contributes **4** of the
6 — the probe files under `docs/03-analysis/probes/doc-block-exec/` — and the other **2** are
`h_mad_assemble_audit.py` and its test, which `af19d53` landed and which the `h-mad`/`handoff`
closure above already names. So the `.py` interval was **not** empty at `af19d53` either, and no
round noticed because nobody ran this predicate. **THE TWO CLOSURES BROKE AT THE SAME COMMIT, ON
THE SAME TWO PATHS. Through v1.104 this sentence dated the `.py` break one commit earlier than the
other, and the tree refutes it**: `git diff --name-only 74e126f af19d53 -- h-mad handoff` and
`git diff --name-only 74e126f af19d53 -- '*.py'` return the **same** two paths,
`h-mad/scripts/h_mad_assemble_audit.py` and `h-mad/tests/test_h_mad_assemble_audit.py`, and each is
**0** at `3f70eb3`, so neither closure precedes the other — the break is `af19d53` for both.
**The class, because a one-word ordinal is not the interesting part**: a chronology BETWEEN two
closures is a claim about two commit sets and is established by running both predicates at the same
sha series, never by reasoning from which corpus is wider. A `*.py` corpus is a strict superset of
the `h-mad`/`handoff` `.py` corpus, so it *can* break earlier and, at `fbc2ea0`, it did — the four
probe files under `docs/03-analysis/probes/doc-block-exec/` move it while the narrower one stays at
2 — and that is precisely why the general relation cannot be substituted for a measurement at a
particular commit. **What the correction does NOT touch is the reason the two closures are stated
separately at all**: they are different corpora and one cannot speak for the other, which the
`fbc2ea0` divergence demonstrates more sharply than a false chronology ever did. **Residual**: this
comparison is stated at the two shas it was run at, and a third commit touching only one of the two
corpora will separate them again, so the pair is re-run at each freeze rather than carried as a
settled relation, and it has now been re-run twice and moved once. Of the **8** files the reading
at `0021c77` names, only one moves a count here — `grammar_corpus.2026-09-03.cd979362.py`,
which carries markdown fences inside its string literals — and every figure it moves is re-derived
at `fbc2ea0` beside its blob reading under the `.py` census paragraphs below.
...
**A SECOND CLASS, because the sentence above named the wrong section through v1.103 and the
document's own placement rule is what it broke**: it said §Convention Prerequisites, and the suite
floor is in §Success Criteria. The two sections are **adjacent** and both carry a suite-related
bullet — the run-alone rule in §Convention Prerequisites, the `2748` floor in §Success Criteria —
which is why a cross-reference has now landed on the wrong one of the pair twice. **The rule over
that axis: a placement claim names a heading, and the heading is re-derived by walking upward from
the site to the nearest `^## ` line, never by recalling which of two neighbours it was.** Here that
walk is `awk 'NR<=N && /^## /{h=$0} END{print h}'` over the body with `N` the line the site sits on,
and it prints `## Success Criteria`; the companion `grep -n 'test_h_mad_assemble_audit'` returns
this paragraph's own two command lines and the floor bullet, and nothing under §Convention
Prerequisites. **Residual, now realised rather than hypothetical and stated as such**: nothing
detects a *wrong* section name, only a missing one, so the two adjacent sections each carry a
disambiguating clause naming which suite obligation lives there — the cheapest thing that makes a
reader's check local.
...
rather than the phrase beside it. v1.106 is measured at `0021c77`, and at nothing else** — v1.105
was measured at `cac6edc` and v1.104 at `fbc2ea0`, and those two shas survive here only as stamps
on readings those revisions took, never as a second answer to *measured at*.
...
never as a second answer to *measured at*. So every reading v1.106 takes is run at `0021c77` with
the sha inside the command, and every reading it inherits keeps the blob it was stamped at —
`b3be433` for v1.101's, `00b961f` and `dfae038` for v1.102's, `af19d53` for v1.103's, `fbc2ea0` for
v1.104's, `cac6edc` for v1.105's — and does not move. **A third clause the freeze-sha rule needs
and v1.103 did not state:
a reading taken over *this revision's own post-edit body* is stamped to the tree v1.106 ships and
never to `0021c77`, which does not contain these edits** — which is why the two self-counting
passages in this section publish their stamped rows at committed blobs and report the post-edit
re-run as a re-run rather than as a row.
**WHAT "MEASURED AT" DOES AND DOES NOT PROMISE, STATED HERE BECAUSE v1.105's MOVE WAS THE FIRST ONE
THAT EXPIRED `h-mad`-SCOPED READINGS RATHER THAN INHERITING THEM.** The rule says every reading
this revision *takes* is taken at its measurement commit; it does not say every figure in the
document was re-taken this round, and the INHERITED-UNVERIFIED register further down this section
is where the difference is recorded rather than left to a reader's optimism. Between `fbc2ea0` and
`cac6edc` exactly one
commit touches the scoped corpus — `git rev-list --oneline fbc2ea0..cac6edc -- h-mad handoff`
returns `b39d9dc` alone — and it moved four published figures, each re-taken at `cac6edc` at its own
site and named here so the set is enumerable rather than asserted: the two collect counts, the
`h-mad/tests/test_*.py` file count, and the heading-differential `new_only`/`titleless` pair with
the `grep -c '^#$' h-mad/SKILL.md` reading behind it. **`b39d9dc` edits eight `*.md` files under
`h-mad/`, so the standing censuses over that corpus could not be argued clear and were RE-RUN
instead — every one is unchanged at `cac6edc`, and the readings are recorded so "unchanged" is a
measurement rather than a hope**: the bash-fence census `73 across 10 files` with its any-language
control `88`, both from the script published under §Measurements; the tracked `*.md` corpus at
**30** files and its glob at **35**, read out of the 09-04 probe's own output lines; the repo-wide
`git grep -n '```bash' -- '*.py' | wc -l` → **8** and `git grep -l '```' -- '*.py' | wc -l` → **25**;
`ls h-mad/scripts/*.py | wc -l` → **37** with its `git ls-files` control also **37**; and
`grep -l 'sys.path.insert(0, str(SCRIPT_DIR))' h-mad/tests/test_*.py | wc -l` → **13**.
**THREE REPO-WIDE `.py` FIGURES WERE OUTSIDE BOTH THAT MOVED SET AND THAT RE-RUN SET THROUGH
v1.105, WHICH IS THE ARRANGEMENT THIS PARAGRAPH ITSELF CALLS A DEFECT IN THE REGISTER RATHER THAN A
LICENSED GAP.** They are the `.py` scale figure
(`git ls-tree -r --name-only <sha> | grep '\.py$' | grep -vE '^(h-mad|handoff)/' | wc -l`), the
bash-fence-bearing `.py` FILE count (`git grep -l '```bash' <sha> -- '*.py' | wc -l`, distinct from
the `-n` LINE census already listed) and the extractor census. All three are re-run here at
`fbc2ea0`, at `cac6edc` and at `0021c77`, and all three read the same at every one — **415**, **5**
and **2** — so this was an accounting gap and no published value was wrong. **The enumeration is
now FIVE repo-wide `.py` figures and each carries the corpus its command scopes to**, because the
two fence censuses count different units over different populations and reading them as one figure
is how the other three stayed invisible.
**v1.106's OWN INTERVAL IS EMPTY ON BOTH CORPORA AND THAT IS MEASURED, NOT ARGUED FROM THE COMMIT
SUBJECTS**: `git rev-list --oneline cac6edc..0021c77 -- h-mad handoff` returns nothing,
`git diff --name-only cac6edc 0021c77 -- h-mad handoff | wc -l` returns **0**, and
`git diff --name-only cac6edc 0021c77 -- '*.py' | wc -l` returns **0** as well — the second
predicate run because the first cannot speak for it. So every `h-mad`-scoped and every repo-wide
`.py` reading v1.105 took at `cac6edc` is INHERITED at `0021c77` and keeps its `cac6edc` stamp.
**Each was nevertheless RE-RUN at `0021c77` rather than inherited silently, and every one
reproduced**: the bash-fence census `73 across 10 files` with its control `88`; the tracked `*.md`
corpus **30** and the glob **35**; the `.py` fence line census **8**; the any-fence `.py` file
census **25**; the bash-fence-bearing `.py` file count **5**; the `.py` scale figure **415**; the
extractor census **2**; `ls h-mad/scripts/*.py | wc -l` **37** with its `git ls-files` control
**37**; `grep -l 'sys.path.insert(0, str(SCRIPT_DIR))' h-mad/tests/test_*.py | wc -l` **13**
against a denominator of **89**; both collect counts, **2836** from the repository root and
**2574** from `h-mad/`; and the committed 09-04 heading-differential probe on both corpora. **An
empty closure predicate and a re-run that agrees with it are two different pieces of evidence and
this revision has both**, which matters because the closure is a statement about paths and the
census is a statement about contents, and a commit can leave a path list untouched only if it
touches no path — a proposition worth checking rather than assuming when the interval contains ten
commits (`git rev-list cac6edc..0021c77 | wc -l` → **10**). **The one
thing this enumeration cannot do is prove itself complete**, and it is not offered as complete: a
figure outside both the moved set and this re-run set is a defect in the register further down this
section rather than
a licensed gap, which is the rule that register already states about itself. **Residual, unchanged
and now with an instance behind it**: the enumeration of distinctly-scoped corpora is by hand, so a
corpus nobody has named is invisible to the rule — which is exactly how the three `.py` figures
above sat outside it for two revisions, found by an auditor and not by any predicate this document
runs.
...
is one.
**The four documents are byte-identical across `ccd8ebd..0021c77`**
(`git diff --stat ccd8ebd 0021c77 -- <plan> <spec> <impl-plan> <design>` is empty), **and this
document is byte-identical from `ccd8ebd`, its own v1.105 landing commit, through `0021c77`**
(`git diff --stat ccd8ebd <sha> -- <doc>` is empty at `bc4688e`, `093c3ee`, `d27d2ce`, `c7a75eb`
and `0021c77` alike). **The v1.104 reading this replaces is left at its own blobs and not re-run**:
the four documents were byte-identical across `09e9307..fbc2ea0`, and that is a fact about blobs at
commits that have not moved. **A fact about blobs is deliberately not used as a licence to skip a
re-run**: what it licenses is
reading an inherited stamp, and nothing more. **The interval this revision inherits across is the
one that matters and it is checked with the CLOSURE PREDICATES, not with byte-identity alone**:
`git diff --name-only cac6edc 0021c77 -- h-mad handoff` prints nothing and
`git diff --name-only cac6edc 0021c77 -- '*.py'` prints nothing either, so the ten commits in
`cac6edc..0021c77` — the round-eighteen decision-sheet appendices, two `docs/handoffs/` files, the
round-eighteen revision batch `ccd8ebd` and the six round-eighteen gating reports `d27d2ce` landed —
expire no `h-mad`-scoped and no repo-wide `.py` reading, which is the opposite of what `af19d53`
did to v1.103 and is why the
retirement in the closure paragraph above is not repeated here. **The two predicates are both run
because the interval is docs-heavy and a docs-only interval is exactly the shape that has twice
been certified clear on one predicate and moved a census on another** — `af19d53` for the
`h-mad`/`handoff` corpus and `fbc2ea0` for the repo-wide `.py` corpus.
...
**THE VALUE SWEEP THAT DECLARED THAT CORRECTION COMPLETE WAS COMPLETE AT THE BODY IT SWEPT AND NOT
AT THE BODY IT SHIPPED, AND THAT IS THE FINDING RATHER THAN A BLIND NEEDLE.** v1.103 swept the
retired `handoffs/… alone` claim with `tr '\n' ' ' | grep -oE 'handoffs/.{0,2} alone'` and published
`4` — two in the body, two inside a Version History entry — with "all four are corrected or
bracketed". **At `af19d53`, the blob it swept, that verdict is exactly right and the needle is not
blind**: the published needle returns **4** whole-file and **2** body-scoped there, a widened
`grep -oE 'handoffs/.{0,6}alone'` returns the *same* `4`/`2`, and both body hits have the identical
shape `` handoffs/` alone ``. **At `fbc2ea0`, the blob v1.103 shipped, the two readings diverge**:
the published needle gives **5** whole-file / **1** body-scoped and the widened form gives **7** /
**3**, and the two extra body shapes are an emphasised `` handoffs/` *alone `` and a hyphenated
compound `` handoffs/`-alone `` — **both of them written by v1.103's own repair paragraphs**, which
did not exist in the body the sweep ran over. **The class, and it is the general shape behind a
long run of this document's own defects: a value sweep whose completeness verdict is published by
the same revision that edits the swept text is re-run against the POST-EDIT body before the verdict
is written, because the revision's own prose is a new member population the sweep never saw.** This
is a stricter rule than "widen the needle" and it subsumes it: widening finds shapes that already
existed, and only re-running finds the shapes the fix introduced. Residual, stated exactly: a
post-edit re-run is itself a reading of uncommitted text, so it is taken and reported and **not**
published as a stamped figure — the stamped pair above is at two committed blobs, and the shapes it
names are what a later reader greps for.
**v1.104 re-derives no `docs/`-scoped figure that v1.101 stamped at `b3be433`, that v1.102
stamped at `00b961f` or `dfae038`, or that v1.103 stamped at `af19d53`, and does not re-stamp them
either**: those commits are immutable
and those readings are correct there, so a mass re-stamp would replace a set of true sentences with
a set of newly-taken ones and add a defect surface for no argument. **The readings v1.104 *does*
...
heading differential's whole `new_only` accounting, whose `0` was false at every sha it was ever
asserted at; the codex-leg ledger, at `fbc2ea0` and with two rows added to its series; the
`49 across 2 files` cause, deleted rather than re-attributed; and the bare-`sed` range's address —
and readings that are **new**, settling a state no earlier revision measured: the `1861157`
reproduction from a worktree and the `fbc2ea0` reading beside it on both corpora, the enumeration of
the one `new_only` member, the empty-ATX renderer check, the three-working-directory census run,
screen two's sha discrimination, the `af19d53..fbc2ea0` register interval, the `NOT RE-RUN` census
over the four round-fifteen entries, and the two Version History walks that establish
non-execution for the register's two new carve-out members. **v1.103's own list carried a
completeness claim that is corrected here rather than repeated**: it named the `ten of the eleven`
sweep, the `df04e8e`/`dfae038` per-commit path lists and the status at `af19d53` of the two spec
commands, and said each was published at its own site with its own command.
**That was TRUE of two of those three and
FALSE of the first, and the missing site is supplied here rather than the claim being softened.**
Through v1.103 the `ten of the eleven` readings appeared **only** inside a Version History entry,
while this sentence sent a reader to a body site; body-scoped, the phrase occurred exactly **once**
in the whole body — at this list entry, the pointer itself — so the reader arrived at the pointer
and nothing else. **The readings, at three committed blobs, whole-file and body-scoped, each with
the grammar that produced it:**

```
$ for s in af19d53 09e9307 fbc2ea0; do
    git show $s:docs/01-plan/features/doc-block-exec.plan.md \
      | tr '\n' ' ' | grep -oiF 'ten of the eleven' | wc -l                       # whole-file
    git show $s:docs/01-plan/features/doc-block-exec.plan.md \
      | awk '/^## Version History/{exit}{print}' \
      | tr '\n' ' ' | grep -oiF 'ten of the eleven' | wc -l                       # body-scoped
  done
af19d53   whole-file 3   body-scoped 0
09e9307   whole-file 6   body-scoped 1
fbc2ea0   whole-file 6   body-scoped 1
```

**BOTH grammars moved at the landing commit, and v1.103's self-match note covered only one of
them.** That note explained the whole-file reading by observing that the needle appears literally
inside the entry publishing it — true, and it accounts for `3` → `6`. It does not account for
`0` → `1`, which is the **body** reading moving because this list entry is body text and a Version
History line is not. **The class: a self-counting screen publishes a reading per grammar, and the
self-match note is written per grammar too** — one note cannot explain two numbers that move for
two different reasons, and the body reading is the one carrying the substantive claim. Residual,
and it is why the readings above stop at `fbc2ea0`: this revision writes the needle several more
times, in this passage **and** in its own Version History entry, so the reading on the tree v1.104
ships is higher in **both** grammars than `6`/`1` — the two grammars by different amounts, since
only the body additions move the body reading — and it is deliberately **not** published as a fourth
row. A self-count is stamped at a commit that predates the text stating it, or it falsifies itself
in the sentence; the post-edit reading is **taken and reported to the round** rather than written
here, which is the same treatment the `handoffs/`-alone sweep gets two paragraphs above and for the
same reason.
The other two of the three are published at their own body sites with their own commands, as this
sentence claimed for all three. *The freeze* keeps its meaning —
...
**THAT OBLIGATION WAS NOT MET BY v1.102, v1.103 OR v1.104, AND FROM v1.104 THE NON-SATISFACTION IS
RECORDED WHERE THIS DOCUMENT SAYS IT MUST BE** — as a member of the register below, last executed
**v1.101** at `b3be433`. Through v1.103 the walk sat in neither the closure nor the register, which
is precisely the arrangement the sentence at the end of this paragraph calls a defect in the
register rather than a licensed gap, so the document was carrying its own named defect for three
revisions. **The choice between the two admissible resolutions is stated rather than left implicit**:
re-stamping nine sibling-derived values every round is a larger maintained surface than one status
entry, and v1.103's argument against mass re-stamping applies here unchanged, so the walk is entered
in the register and its values stay at `4e4a00c` on the surfaces that own them. **The class: a
standing obligation a revision does not discharge is recorded as an unmet obligation, never left to
the reader to infer from silence** — an obligation with no record of non-satisfaction reads as
satisfied, which is the softer failure the register exists to close. Residual: the register records
*that* the walk was not re-run and cannot record whether any member's value has since moved, so a
reader who needs the current values re-runs the nine commands listed above at the sha they care
about; the entry is a status, never a value.
...
Inherited-unverified at `0021c77`, the commit v1.106 is measured at — re-stamped from `fbc2ea0`
rather than carried, because a register that names a commit older than the revision's own
...
paragraph that defines it**. **v1.105 LEFT THIS STAMP AT `fbc2ea0` WHILE BEING MEASURED AT
`cac6edc`, which is that same detachment for the third time and in the same paragraph**; it is
recorded here rather than silently repaired, because the register's whole subject is the difference
between a figure that was re-run and one nobody looked at, and a stale stamp on the register is
that difference going unrecorded about the register itself. **The `fbc2ea0`→`0021c77` move is
discharged by the interval argument RUN rather than recalled**: `git rev-list fbc2ea0..0021c77`
returns **19** commits, of which exactly **two** touch any of the four feature documents, by
`git diff --name-only <sha>^ <sha> -- <plan> <spec> <impl-plan> <design>` run per commit — `cb4fe99`
and `ccd8ebd`, **4** paths at each, and **0** at each of the other seventeen. `cb4fe99` is the
round-seventeen batch (plan v1.104, design v1.109, impl-plan v1.53, spec v1.63) and `ccd8ebd` the
round-eighteen batch (plan v1.105, design v1.110, impl-plan v1.54, spec v1.64). **Only a revision
of THIS document can execute a member of this register**, by the register's own membership rule, so
the property the stamp needs reduces to what the v1.104 and v1.105 entries record: v1.104's
enumerates, and v1.105's records the PARTIAL run of the `docs/`-scoped walk that the walk's own
bullet below states and no other member. **The predecessor argument is kept as history and not
re-run**: the v1.103→v1.104 move was discharged the same way, `git rev-list --oneline
af19d53..fbc2ea0` returning **four** commits of which exactly **one** — `09e9307`, this document's
own v1.103 landing — touched any of the four documents, printing **3** paths there and **0** at
each of `6fc71ab`, `f91dd2c` and `fbc2ea0`.
**The lead's justification is NARROWED here, because v1.103 asserted a uniform property over four
Version History entries and two of the four falsify it.** v1.103 wrote that *each* of the
round-fifteen revisions' entries enumerates its re-runs and names every member of this register
under `NOT RE-RUN`. Measured at `59cc2ad`, the commit that landed all four:
`git show 59cc2ad:<spec> | awk '/^- v1\.62:/{print}' | grep -ci 'NOT RE-RUN'` → **0**, the same over
the design with `/^- v1\.107:/` → **0**, and neither entry names any member of this register at all
(`grep -oiE '(markdown-it-py|screen-two|Setext|doc-auditor\.md|74e126f self-count|stdlib probe)'`
over each returns nothing). The impl-plan's v1.51 entry **does** carry one such enumeration — the
same grep returns **1** — but every member it names is impl-plan-scoped and **none** is a member of
this register. **So the argument is not that four entries say so; it is that ONE does, and that the
other three could not have re-run a member of this register in any case**: every member is a figure
this document owns and only a revision of this document can execute one, so a sibling revision's
silence carries no weight and is not used as any. The property the stamp actually needs — no member
was re-run inside the interval — is therefore established for the `af19d53..fbc2ea0` move by the
**one** entry in it, v1.103's, which does enumerate. **The class, and it is the rule this document
already states and then broke inside the paragraph stating it**: a lead sentence asserting a uniform
property of a list is checked against **every** member before it is written, and if it fails for
any, the claim is narrowed to the members it holds for and the rest are carried by a different
argument that is stated separately. Residual: the narrowed form rests on the scoping claim that a
plan-scoped figure can only be re-run by a plan revision, which is true by the register's own
membership rule and is not independently checkable by any command.
**Two of this revision's new members are NOT covered even by that one entry, and are carried
differently rather than swept in**: v1.103's entry names the `docs/`-scoped walk but names neither
the `awk` boundary probe nor the fence-body de-indentation case. Their non-execution is derived from
this document's own Version History by the walk the bullets below publish, not from the interval
argument. **This is deliberately not argued from byte-identity, which would be false for `09e9307`.**
Residual, the one `af19d53` made concrete and `fbc2ea0` repeats in the opposite direction: this
argument covers members re-run by a *revision*, and a commit no revision of these documents produced
can still expire a figure — `af19d53` changed `h-mad/` under the register, which is why the
`h-mad/`/`handoff/` closure in §Measurements had to be retired there; `fbc2ea0` changed only
`docs/03-analysis/probes/doc-block-exec/`, which expires nothing and is why the retirement is not
repeated here.
**The v1.102→v1.103 move this one replaces was discharged by the same shape and with the same
over-claim, and the over-claim is corrected rather than left in place**: `git rev-list --oneline
dfae038..af19d53` returns exactly **seven** commits, of which **three** — `59cc2ad`, `7b182b0` and
`3f70eb3` — touch any of the four feature documents; the other four — `4c1c3a5`, `b442a80`,
`7b9d174` and `af19d53` — touch only audit reports under `docs/01-plan/features/` and
`docs/02-design/features/`, and `h-mad/scripts/` and `h-mad/tests/`. Of the three revisions those
landings carry, only this document's own v1.102 enumerates a member of this register, for the
reason and by the measurement given above.
**The v1.101→v1.102 move, two links back in the same chain, was discharged the same way** — the
chain is named by revision number at every link rather than by *this one*, which is the deixis
defect the bullets below already close: `git rev-list --oneline
...
v1.104. **The predicate names one revision — this one — and never a span**: v1.101 executed one
...
the *revision* that ran it, so for that member the register can say the reading is not v1.104's
...
- The markdown-it-py **14-case** grammar corpus, **its `4.2.0` leg only** — stamped
  `markdown-it-py 4.2.0` and by no sha and no revision, being a carve-out probe: it needs a
  throwaway venv installed with `pip install --target`, which no committed sha determines. **The
  split is v1.104's and is stated rather than glossed**: v1.104 re-executed the corpus on the
  interpreter-local `2.2.0` renderer at `fbc2ea0`, 14 of 14 OK, published at the corpus's own site,
  so that leg leaves this register for this revision alone by the same rule the other carve-out
  bullets state; the `4.2.0` leg was **not** re-run and is what this entry covers. A member whose
  two legs have different last-execution stamps is entered by the leg that was not run, never by the
  pair. The fence-body de-indentation probe added in **v1.95** was run on both renderers and is
  *not* part of that fourteen; it has its own entry below.
...
- The **`awk` boundary probe's five legs** — last executed **v1.96**, stamped `awk version
  20200816`, its reading being runtime-determined rather than sha-determined. **This member is
  ENTERED in v1.104 and was wrongly held out through v1.103**, on the words "v1.96 re-ran all five
  of its legs" — which is true and is not an exemption: the register's own rule, stated at the
  `700c599` stdlib bullet and again at the screen-two bullet, is that a probe leaves this register
  **only for the revision that executes it** and re-enters at the next revision that does not. v1.97
  through v1.103 did not. **Non-execution since is derived and not recalled**, by the same walk the
  triage bullet publishes:
  `awk '/^## Version History/{f=1} f && /^- v/{n=$2} f && /awk boundary probe/{print n}'` over this
  document returns, in full and printed rather than described, `v1.94`, `v1.96`, `v1.99` and
  `v1.104` — and **NOT** this revision's own entry, because v1.106 does not name the probe.
  **Through v1.105 this bullet described the output as those three "plus this revision's own
  entry", which is the rule-instead-of-a-list shape v1.104's own entry declared a defect and then
  left standing here**: it was true of v1.104, false of v1.105 and false again of v1.106, since
  whether the current entry appears depends on what that entry happens to say. **The list is
  therefore PRINTED, and it is re-run at every revision rather than extended by assumption.**
  **Each non-v1.96
  hit was READ before being classified rather than assumed structural**: `v1.94` classifies the row
  during the carve-out sweep (`git ls-files awk` → `0`, so genuinely exempt) and `v1.99` argues
  about which rows the stamp-driver's twelve lines resolve to. Neither runs a leg.
- The **fence-body de-indentation case**, the fifteenth of the scanner grammar corpus — last
  executed **v1.95**. **Entered in v1.104 by the same re-entry rule and for the same reason**: it
  was held out on "only the one new case was run", which was v1.95's status and no later revision's.
  The same walk with `/de-indent/` returns, printed in full, `v1.35`, `v1.95`, `v1.96`, `v1.99`,
  `v1.104` and `v1.105`, and NOT this revision's entry — the same repair as the bullet above, and
  the two lists differ at their tails (`v1.105` matches this needle and not the other one), which
  is why each walk is run separately rather than one being inferred from the other. And
  **the three non-v1.95 hits were read, and they are three DIFFERENT things, only one of which is
  even about this probe**: `v1.35` is a homonym — the helper's own body-de-indentation *rule* on
  `extract`, a different subject entirely; `v1.96` records that the round-ten **auditor**
  independently reproduced the oracle on both renderers, which is a second surface's run and not a
  revision of this document executing it, and the register's rule is written over revisions;
  `v1.99` argues about whether the case earns a carve-out row of its own. **The auditor's
  reproduction is named rather than swallowed**, because it is real evidence that the reading held
  at that round even though it does not move the last-execution stamp — and stating it is the
  difference between an un-re-run figure and an unexamined one. It shares a bullet subject with the fourteen above and is stated here rather than folded
  into that bullet, because the two halves carry **different** last-execution stamps and a single
  bullet could only state one of them.
- The **`docs/`-scoped sibling walk** — the nine sibling- and spec-derived figures the closure
  paragraph above lists, all stamped `4e4a00c` and last executed **v1.101**, at `b3be433`.
  **Entered in v1.104 because the paragraph above says in as many words that a figure outside both
  the closure and this register is a defect in the register and not a licensed gap, and through
  v1.103 this walk was exactly that** — v1.103 declined to re-stamp it (correctly, since a mass
  re-stamp is a larger defect surface than a status entry) and then did not enter it either, so it
  sat in the one place the register forbids. Its values are published at their own site under the
  closure paragraph, which owns them; this entry owns their status. **The walk drifts by
  construction** — its corpus is three sibling documents revised in the same rounds this one is — so
  membership here is the default and leaving it is the event that needs a run.
  **v1.105 re-ran PART of this walk and the split is stated rather than rounded either way**, by the
  same rule the `markdown-it-py` bullet states for a member whose two legs carry different stamps:
  the design-derived mutation total and its `SKILL.md` split were re-derived at `09e9307`, `cb4fe99`
  and `cac6edc`, and the `29`-name and `8`-row contract values at `cac6edc`, all published under
  §Measurements with their commands; the remaining sibling- and spec-derived figures of this walk
  were **not** re-run. **A partial run does not remove a member.** The entry stays, its
  last-execution stamp stays **v1.101** for the legs that were not run, and the three that were run
  are named here so a reader can tell which half of the walk is inherited — which is the whole
  distinction between an un-re-run figure and an unexamined one. **Residual, and it is the reason a
  full re-run of this walk is not simply scheduled**: the three siblings are being revised in the
  same batch as this document, so any figure re-derived from them at `cac6edc` is stale the moment
  the batch lands; the orchestrator reconciles cross-document values at collection, and this
  register records the state at the freeze rather than pretending the freeze is the landing.
  **v1.106 re-ran a DIFFERENT part of the same walk and its split is stated the same way**: the
  design-derived mutation total, its distinct-name count and its `SKILL.md` split were re-derived
  at `09e9307`, `cb4fe99`, `cac6edc`, `ccd8ebd` and `0021c77`, all at committed blobs and published
  under §Measurements with their command, and re-derived once more over the design the
  round-nineteen batch ships after that sibling's revision was written; the `29`-name and `8`-row
  contract values and the
  remaining sibling- and spec-derived figures of this walk were **not** re-run. The entry stays and
  its last-execution stamp for the unrun legs stays **v1.101**. **The working-tree reading was
  taken only AFTER the sibling shipped, and the ordering is the point rather than an accident**:
  a reading taken while that author was still writing would have been of a file mid-edit — the
  failure mode of the previous round, where two authors filled one undecided value at once and
  produced two internally consistent documents that disagreed. **A batch reading is admissible
  because it carries the expiry rule with it**; a mid-edit reading is not, because nothing tells
  the reader which edit it caught.

**NO MEMBER LEFT OR ENTERED THIS REGISTER IN v1.106, AND THAT IS STATED RATHER THAN LEFT TO THE
LIST'S SILENCE**: v1.106 re-ran no member in full, so nothing leaves; it added no figure that sits
outside both the closure and this register, so nothing enters; the Setext member that left in
v1.105 STAYS OUT, because v1.106 executed both of its probes again at `0021c77` and the readings
are published at the Setext bullet under §Measurements, which owns them; and the three repo-wide
`.py`
figures the round-nineteen audit found outside the re-run enumeration are NOT new members, because
they were re-run at `fbc2ea0`, `cac6edc` and `0021c77` and are published with those shas at their
own sites — a figure that has been re-measured at the measurement commit is not un-re-run, whatever
enumeration failed to name it. **The membership rule is the test, not the audit finding.**
**ONE MEMBER LEFT THIS REGISTER IN v1.105 AND IT LEFT BY BEING RUN, WHICH IS THE ONLY WAY OUT THIS
REGISTER ADMITS.** The Setext member — carried through v1.104 as "the Setext differential and its
`files=25`/`files=30` printouts, stamped `1861157`" — was in the register on a description that had
itself gone stale twice over: v1.104 re-derived that census at `1861157` from a worktree AND
cross-checked it at `fbc2ea0` against the second committed probe, and it *retired* the
`files=25`/`files=30` pair rather than carrying it, so the register was asserting "not re-run" about
a member the same revision had re-run and about printouts that no longer existed. **Both halves of
that are the register's own failure mode arriving from the other direction**: it exists to stop an
unchallenged figure laundering into a verified one, and here it laundered a verified figure into an
un-re-run one. v1.105 re-ran both probes at `cac6edc` — the readings are published at the Setext
bullet under §Measurements, which owns them — so the member leaves for v1.105 and **re-enters at
the next revision that does not re-run it**, exactly as the `700c599` stdlib and screen-two bullets
state. **The status was reconciled against this document's own §Measurements rather than against
recall**, which is the check that catches this class: a register entry is re-read beside the site
that owns the values every time either moves.

**Residual on the last-execution stamps, stated rather than papered over**: **six** of the **nine**
entries above name an executing *revision* — `v1.99` for the stdlib block, `v1.97` for the
self-counts, `v1.101` for the screen-two legs, `v1.96` for the `awk` boundary probe, `v1.95` for the
de-indentation case and `v1.101` again for the `docs/`-scoped walk, two entries sharing one revision
number; **two** name the **commit** their reading is stamped at — `35698f9` and
`cf3a862`; and **one**, the `markdown-it-py` corpus, names **neither** a revision nor a commit,
carrying only the renderer version of the leg that is actually in — `4.2.0` — which is the third
disjunct the lead states. `6 + 2 + 1 = 9`, and the partition
is stated as a sum because a three-way split that does not add to the cardinal is the
population-short-by-N shape this document writes a rule against. **The cardinal is `nine` because the
list above was walked and its bullets counted for this sentence, by a command and not by hand** —
`awk '/The members:$/{f=1;next} f && /^\*\*ONE MEMBER LEFT THIS REGISTER/{exit} f && /^- /{n++} END{print n}'`
over the body of this document returns **9** on the tree v1.106 ships. **The decrement from
v1.104's `ten` belongs to v1.105 and is named by revision number rather than by a deixis**: v1.105
removed one member and added none, and v1.106 neither removes nor adds, so its **9** is a re-run
that did not move rather than a new arithmetic. **A revision-scoped explanation attached to a
revision-relative phrase expires silently at the next revision** — the same failure the codex-leg
ledger retired this round — which is why the number is re-counted here and the explanation carries
the revision it is about. A cardinal
over a list a revision edits is the exact shape §Measurements writes a rule against, so it is
counted rather than adjusted at every revision, whether or not the list moved. **The walk's stop address moved with the list and is republished with
it**, because v1.105 inserts the departure paragraph above between the last bullet and the residual
that used to follow it — a walk whose stop address is a phrase the same revision rewrites is a walk
that reads the wrong span silently, which is the residual the previous revision already named and
this one had to pay. **Residual on that walk**: its start address is the phrase `The
members:` at end of line and its stop address is the literal
`**ONE MEMBER LEFT THIS REGISTER` — the departure paragraph's bolded opening, which is what the
published `awk` actually tests for and NOT the residual paragraph that follows it, as this sentence
said through v1.105. **A residual that describes a command by a different address than the command
carries is worse than no residual**, because it invites a reader to check the wrong span and find
agreement. So a bullet added *after* the departure paragraph is invisible to this walk and a future
revision that re-words either address breaks it silently — which is why the walk is published with both addresses rather than
described. That is what
...
it says the reading is not v1.104's — named by number; a deixis here would
...
two formerly-exempt rows are now MEMBERS, and the history of their exclusion is kept because it is
the evidence for the re-entry rule and not because either exclusion still stands:**

- The **`awk` boundary probe** row was **out** of this register through v1.103 and is **in** from
  v1.104. Its exclusion has now been wrong twice for two different reasons, which is why the axis
  and not the row is what gets closed. **First**, through v1.95 it was kept out on the strength of
  an `awk --version` stamp and the three per-branch controls, and this section says in as many words
  that those controls are not this probe — the row was out on evidence that did not cover it, which
  v1.96 fixed by executing all five legs, one published command per leg on `awk version 20200816`.
  **Second, and this is the one v1.104 closes**: v1.96's execution was then treated as a standing
  exemption, when the register's stated rule is that execution exempts a probe **for the executing
  revision alone**. Seven revisions passed without a re-run.
- The **scanner grammar corpus** row is **in** this register for **both** halves from v1.104 — the
  fourteen and the fifteenth. Through v1.103 the fifteenth was held out on "only the one new case
  was run", which was v1.95's status and no later revision's, and is the identical shape as the
  `awk` row's second exclusion. Its fourteen are the register entry named above as
  *the markdown-it-py 14-case grammar corpus* and its fifteenth is the entry named
  *the fence-body de-indentation case* — each addressed by name and never by a sentence or
  paragraph ordinal, for the same reason the carve-out table's rows are.
- **The class both exclusions belong to, stated once over the axis**: a status assignment whose
  warrant is "revision N executed it" **decays at revision N+1**, so it is written as a
  last-execution stamp and never as an exemption, and the register's default for a carve-out probe
  is IN. **This is the third and fourth member of that axis** — the `700c599` stdlib probes and the
  six screen-two legs were the first two, each entered a revision late for the same reason.
  **Residual, stated because it is what let both survive**: nothing screens for an exclusion whose
  warrant has decayed; the only detector is walking every non-member of the carve-out table at each
  revision and asking which revision last executed it, which is a human walk and is why the two
  bullets above name the revision rather than the fact of a run.
...
re-derived at `8909ec4` — v1.94 re-ran only their *total*, and **no revision since has re-run
either**. Which revisions say so at their own entries is derivable rather
than recalled: `awk '/^## Version History/{f=1} f && /^- v/{n=$2} f && /triage categor/{print n}'`
over this document returns `v1.94 v1.95 v1.96 v1.99 v1.101 v1.102 v1.103`, so v1.97, v1.98, v1.100
and **v1.104 itself** name it nowhere and their silence is not a claim in either direction.
**This sentence is re-taken on the body that ships it and was NOT carried**: through v1.103 it read
`v1.94 v1.95 v1.96 v1.99` "plus this revision's own entry", a form that was true when v1.96 wrote it
and had been false for three revisions by `fbc2ea0` — the walk had grown three names and the
clause promising a self-match no longer had one. **The class: a derivation whose output is a LIST
of revision numbers grows by construction, so the list is re-printed at each revision and never
described by a rule such as "plus this revision's own"**, which is a prediction about text not yet
written — **which is
...
document returns `v1.94 v1.95 v1.96 v1.97 v1.98 v1.99 v1.101 v1.102 v1.103` and **no `v1.100`** and
**no `v1.104`** — the list is printed in full rather than described as a span with a self-match
clause, because through v1.103 it was described as "every entry from `v1.94` to `v1.99`, plus this
revision's own" and that clause had been false since v1.102: the walk kept growing and the
"this revision's own" member stopped being the last one. The load-bearing reading is the absence of
`v1.100`, and v1.104's own absence is equally a datum — this revision did not re-run the legs
either, which is what the register entry for them records. **Silence is the failure mode the naming
...
**Residual on both screens, stated so the next sweep is checkable rather than trusted, and stated
PER SCREEN because through v1.103 it was stated over the pair and was false of one of them.** What
both share: each is a *shape* filter and never a verdict, so every hit is **read**, not counted.
What is **screen one's alone** is the sha stage — its published program carries `!/[0-9a-f]{7}/`,
which is line-scoped, so a claim whose sha sits in the same sentence wrapped onto the next line
reads as a hit. **Screen two has no sha stage at all**, which this section already says three
paragraphs above and then contradicted here: run verbatim it is an `awk` body-strip piped to a
single `grep -Ei` over counted nouns, and the discrimination is executed rather than asserted —
fed `three files measured` and `three files measured abc1234` through the spec's extracted pattern,
it returns **both**, so a sha on the line changes nothing about whether screen two fires. **The
class: a residual written over a set of screens asserts a uniform property and is checked against
each member before it is written** — the same axis as the register lead's, and the second instance
of it in this document. Residual on the repair: separating the two residuals says which screen has
which stage and does not make screen two provenance-aware; provenance on this document remains
screen one's job and a human's.
...
artifacts that do **not exist yet** (`29` names, the mutation total, `8` rows), which are contract
values this plan must match rather than tree measurements — **the mutation total is named here
rather than quoted, because it has moved at three consecutive design revisions and a bare integer
in a carve-out list is exactly the drift this carve-out was narrowed to stop**; and OS- or interpreter-behaviour probes,
...
**THE SECOND CARVE-OUT IS NARROWED OVER ITS AXIS, BECAUSE IT REMOVED THE ONLY SCREEN THAT COULD
HAVE CAUGHT A DRIFT AND THEN ONE OF ITS THREE MEMBERS DRIFTED.** The mutation total read `81`
correctly at `4e4a00c`, `6f0ee85` and `09e9307` and was **`85`** from `cb4fe99` — *the commit that
landed v1.104* — so the figure was wrong in the same commit that published it, and no provenance
rule reached it because this carve-out exempted it from carrying a sha at all. **It moved AGAIN
one round later, which is the narrowed rule earning itself within one round rather than at
some later freeze**: the design's round-eighteen revision added one matrix row, so the figure this
document states is **85** at `cac6edc` and **86** from `ccd8ebd`, the commit that landed that
revision. **v1.105 could only stamp that second reading to *the batch* because the sibling's edits
were not committed yet; v1.106 discharges it to `ccd8ebd` and the stamp is a sha again**, which is
the third clause of the freeze-sha rule running its full course — a sibling's post-edit body is
stamped to the batch, and the batch stamp expires into a `git show` reading the moment the batch
lands. **It moved a THIRD time in the very next round, which is this rule earning itself twice
running**: the design's round-nineteen revision adds `intersect-kind-ignored`, so the figure is
**86** at `0021c77`, **87** at `7fc5f94` and **88** at `da03ca5`, and it moved a FOURTH time at that
last commit, which lands design v1.112 and v1.113. **It has since moved a FIFTH time, to **90** at
`861efbe`**, when the design took the two 5e rows this document had routed to it as owed — the one
move this document could predict, having named its closing condition, and still a reading rather
than an increment. **A figure that has moved at five consecutive design
revisions is not a candidate for a carve-out under any wording**, which is the narrowed rule's
whole point. Under the old carve-out
that second move would have been invisible for the same reason the first was; under the narrowed
one it is a reading with a command and a sha. **The axis is not
"does the artifact exist yet" but "where does the value come from", and the rule over it: a figure
this plan must MATCH, whose source is a SIBLING DOCUMENT under `docs/`, carries the command that
reads it out of that sibling and the sha it was read at, even though the artifact it counts does
not exist in the tree.** A value read out of another document is a reading of that document, and
the three siblings are revised in the same rounds this one is. **What survives the narrowing is
stated positively so the exemption is not read wider than it is**: these three figures are exempt
from the **tree**-provenance sweep — no `git ls-files`, `git grep` over `*.py` or `pytest`
invocation can produce them — and from nothing else. All three re-derived at `cac6edc` under the
narrowed rule, each command naming the blob it reads:

```
$ IP=docs/01-plan/features/doc-block-exec.impl-plan.md
$ git show "cac6edc:$IP" | tr '\n' ' ' | grep -o '29 names' | wc -l
       2
$ git show "cac6edc:$IP" | tr '\n' ' ' | grep -oE '2[0-9] names' | sort | uniq -c
   1 25 names
   1 28 names
   2 29 names
$ D=docs/02-design/features/doc-block-exec.design.md
$ git show "cac6edc:$D" | grep -oE 'a fifth, `[a-z-]+`|a sixth, `[a-z-]+`|a seventh, `[a-z-]+`|an eighth, `[a-z-]+`'
a fifth, `docsections-delegation-reverted`
a sixth, `docsections-syspath-setup-removed`
a seventh, `docsections-heading-lookup-reverted`
an eighth, `docsections-local-bounder-restored`
```

`29` is the helper's `__all__` when complete, at **2** occurrences in the impl-plan blob, and the
figures that are *not* competitors are named rather than waved past: `25` is Task 1's intermediate
and `28` is Task 3's, both stated by the impl-plan as way-points to the same 29. **The unit is
occurrences and not lines**, which matters because the blob is collapsed to one line first: a
`grep -c` there returns `1` for any nonzero count and would have said the same thing about two
occurrences as about twenty. `8` is `docsections.json`'s row count — the four originals plus the
four ordinals the command above prints from the **design** blob at `cac6edc`, so **4 + 4 = 8**.
Both are unchanged at `cac6edc`; the mutation total is the one member that moved.
**Every command this revision publishes is extracted from the shipped body and executed, fenced or
inline**, and these two are fenced for a reason the class makes concrete: through the first draft
of v1.105 they were *inline*, in double backticks, and the ordinal one carried `\|` between its
alternatives — markdown-motivated escaping that ERE reads as a **literal** pipe, so the command
returned nothing and `rc=1`. It also said "over the same blob", which resolved to the impl-plan
blob quoted immediately before and contains **0** of the four ordinals. **The residual, exact**: an
inline command can acquire escaping that its fenced twin cannot, so the rule is not "publish
commands" but "extract every published command from the shipped bytes and run it", and a fence is
preferred wherever a regex carries an alternation.

**The design-matrix derivation** — published once, here, and pointed at from the §Deliverables row
that states its values, both halves taken at the same sha in the same walk:

```
$ D=docs/02-design/features/doc-block-exec.design.md
$ for s in 09e9307 cb4fe99 cac6edc ccd8ebd 0021c77 7fc5f94 da03ca5 861efbe; do
    printf '%s total=' "$s"
    git show "$s:$D" | awk '/^\| mutation \| guard it removes \(mechanism\) \| killed by/{f=1;next}
                            f&&/^\|---/{next} f&&/^\|/{n++;next} f{exit} END{print n+0}'
  done
09e9307 total=81
cb4fe99 total=85
cac6edc total=85
...
861efbe names=      90
$ for s in 09e9307 cb4fe99 cac6edc ccd8ebd 0021c77 7fc5f94 da03ca5 861efbe; do
    printf '%s skill-md-target=' "$s"
    git show "$s:$D" | awk '/^\| mutation \| guard it removes \(mechanism\) \| killed by/{f=1;next}
                            f&&/^\|---/{next} f&&/^\|/{print;next} f{exit}' \
      | grep -c 'the mutation targets `SKILL.md`'
  done
09e9307 skill-md-target=1
cb4fe99 skill-md-target=1
cac6edc skill-md-target=1
ccd8ebd skill-md-target=1
0021c77 skill-md-target=1
7fc5f94 skill-md-target=1
da03ca5 skill-md-target=1
861efbe skill-md-target=1
```

**THE BATCH STAMP v1.105 CARRIED HAS COME DUE AND IS DISCHARGED HERE, WHICH IS THE ONLY WAY IT WAS
EVER GOING TO CLOSE.** v1.105 published `86` as a WORKING-TREE reading over a sibling's post-edit
body, because the design's round-eighteen revision was uncommitted while v1.105 was written. That
batch landed as `ccd8ebd`. Re-run as the residual required — `git show ccd8ebd:$D` piped through
the same three walks — the readings are total **86**, distinct names **86**, `SKILL.md`-target
**1**, identical to the working-tree pair v1.105 published, so only the FORM was owed and no value
moved. **All eight `git show` readings are of committed blobs and a reader with nothing but a clone can
re-derive every one of them**, which is the property the discharge restores. **The eighth reading,
`861efbe total=90 names=90 skill-md-target=1`, is added rather than substituted for the seventh**:
the matrix moved 88 → 90 when the design took the two rows this document had routed to it as owed
(§Measurements, the paragraph closing *the two realised rows this document routed to the design*),
and a reading that replaced `da03ca5` would erase the evidence of that arrival — the series is what
makes a rename distinguishable from an addition, and it only works if nothing is overwritten. **The
`SKILL.md`-target count is 1 at all eight, so the split at `861efbe` is 89 + 1**, and its staying at
1 across a +2 in the total is the check that the two arrivals target the helper, not the registry.
**THE ROUND-NINETEEN BATCH STAMP HAS COME DUE TOO AND IS DISCHARGED IN THE SAME BLOCK, AND THE
WORKING-TREE INVOCATIONS ARE GONE RATHER THAN REWORDED**: v1.106 published `87` over the design's
round-nineteen post-edit body; that batch landed as `7fc5f94`, and `git show 7fc5f94:$D` through the
same three walks returns total **87**, distinct names **87**, `SKILL.md`-target **1** — again
identical to the working-tree triple, so again only the FORM was owed. **THIS REVISION ARMS NO NEW
STAMP**: the matrix move it publishes is `87` → `88`, and the design revision carrying it is
committed at `da03ca5` BEFORE this revision was authored, so `88` is a blob reading from the moment it is
written and the governed set stays empty. **The row that arrived is NAMED rather than counted**, for
the same reason the four arrivals between `09e9307` and `cb4fe99` are named — a 5c author sizing
the spec from the wrong total authors the wrong number of rows and the harness refuses a spec whose
rows do not match their anchors. It is `intersect-kind-ignored`, killed by
`test_cli_subst_overlap_detail_lines` (AC-4.1/4.3). **The killer is NOT the one the design shipped
with the row, and the correction is a measurement rather than a preference**: the design named
`test_substitute_refuses_intersecting_spans`, which the impl-plan's own acceptance list defines as
asserting the exception DATA and explicitly nothing about an emitted line, so a mutant that changes
only the renderer's prefix selection leaves it green. **A row whose named killer cannot go red under
its own mutant is worse than a row with no killer named**, because the matrix then reports coverage
the suite does not have. **The class over the axis**: a `killed by` binding is a claim that one
named test fails under one named mutation, it is settled by reading what that test asserts, and it
is never inherited from a sibling document on the strength of the row's prose. Residual: this
document cannot execute either test at Phase 5b, so the binding rests on the impl-plan's acceptance
definitions rather than on a run, and it is re-checked when Phase 5c makes both tests real.
**Set-differencing the column-1 names at
`ccd8ebd` and `7fc5f94` shows ONE arrival and ZERO departures**, so no row was renamed there
and that delta is entirely new coverage, and the `SKILL.md`-target count stays **1** across the move,
which is what makes the split at `7fc5f94` **86** helper-source **+ 1** rather than a second registry row.
**The NEXT move is the shape this set-difference exists to distinguish, and it is the first one here
that is not pure addition**: at `7fc5f94` → `da03ca5` the same diff returns TWO arrivals
(`substitution-result-not-passed-to-run_block`, `timeout-upper-bound-removed`) and ONE departure
(`preamble-composed-with-unsubstituted-text`) while the total moves by ONE, which is a rename
composed with an addition. **Arrivals-minus-departures, not arrivals, is what reconciles with the
total** — reading the two arrivals as two new rows over-counts the matrix by one, and reading the
departure as a deleted guard reports coverage loss that did not happen. The `SKILL.md`-target count
is **1** at `da03ca5` too, so the split is **87** helper-source **+ 1**.
**THE MOVE THIS REVISION PUBLISHES IS `88` → `90`, AND IT ARMS NO STAMP EITHER**: the design
revision carrying it is committed at `861efbe`, the freeze this revision is measured at, so `90` is
a blob reading from the moment it is written. **Set-differencing column 1 at `da03ca5` and
`861efbe` returns TWO arrivals and ZERO departures while the total moves by TWO, so this move is
pure addition and no row was renamed**: the arrivals are `kill-skipped-after-collect-failure` and
`stderr-not-closed` — the two names this document had been publishing as realised-but-unmatrixed and
routing to the design as owed, which is what makes this the one matrix move here whose closing
condition was written before it happened. The `SKILL.md`-target count is **1** at `861efbe` too, so
the split is **89** helper-source **+ 1**, and its holding at 1 across a `+2` is the check that both
arrivals target the helper's source rather than the registry.
**The `0021c77` reading of `86` is NOT edited, because it is true there**: `0021c77` carries design
v1.110 and the row arrives with v1.111. **This is the same shape v1.105 shipped one round earlier —
85 at its own measurement commit and 86 over the design its batch shipped — and it discharges the
same way**: the round-nineteen batch landed as `7fc5f94`, the three working-tree invocations were
re-run as `git show 7fc5f94:$D`, and the `87` is a blob reading in the block above. **Residual,
and it is the sharper one, and it is UNDISCHARGED**: this document cannot detect a sibling's matrix
moving — only running the walk can — so the trigger is a sibling revision landing and not a change
this document can observe. **The residual survives the stamp going away**: `88` is stamped to
`da03ca5` and stays true at that blob forever, and it stops describing the CURRENT design the moment
the next design revision touches the matrix, with nothing in this document able to say so. Re-run
the block at every freeze. **THAT RESIDUAL FIRED, WHICH IS WORTH RECORDING BECAUSE IT IS THE ONLY
EVIDENCE THE INSTRUCTION IS LOAD-BEARING**: the next design revision did touch the matrix, `88`
stopped describing the current design, no screen in this document went red, and the eighth reading
`861efbe total=90` exists only because the block was re-run at this freeze as instructed. The
residual is UNCHANGED and undischarged — running the walk is still the only detector — and `90`
inherits it in full.
**THIS PARAGRAPH IS THE SINGLE HOME OF THAT OBLIGATION, AND THE SET IT GOVERNS IS ENUMERABLE
BY COMMAND** — a batch stamp is a reading over a sibling's uncommitted post-edit body, it is
spelled `this batch ships`, and every occurrence of it expires into a
`git show <batch-commit>:` reading when that batch lands. A stamp with no route back to a sha is the
defect; the route is stated here once, and any other site points at this paragraph by its bolded
lead-in rather than repeating it, because one obligation copied to every stamp is one thing to
re-word per stamp when the batch commits. **THE GOVERNED SET WENT EMPTY, WAS RE-ARMED, AND IS EMPTY AGAIN, AND EVERY READING IS
PUBLISHED WITH THE SELF-MATCH AS ARITHMETIC RATHER THAN NETTED OUT**. When the round-eighteen
stamps were discharged into `git show ccd8ebd:` readings the needle returned **1** over the body,
its single occurrence being the quoted spelling in the sentence above, so the governed set was
**1 - 1 = 0**. The design's round-nineteen revision then shipped with a new matrix row, v1.106
took a working-tree reading of it, and the needle returned **7**: the quoted spelling above, plus
**6** live stamps, so the governed set was **7 - 1 = 6** — three in the §Deliverables mutation-spec
cell and three under §Measurements. **All six are discharged in THIS revision and the set is empty
again**, because `7fc5f94` landed the batch they were stamped to and the design revision behind the
new total is committed at `da03ca5` before this revision was authored, so nothing here re-arms it: every
one of the six is now a `git show <sha>:` reading, and the needle returns **1** over this body, its
single occurrence being the quoted spelling in the sentence above, so the governed set is
**1 - 1 = 0**. **The enumeration is by hand and the needle is the check on it**, which is why both
are stated: the count catches a site the enumeration forgets, and the enumeration catches a count
that drifted. **THE DISCHARGE IS THE WHOLE POINT AND IT HAS NOW RUN TWICE**, once at `ccd8ebd` and
once at `7fc5f94`, so the mechanism is measured rather than merely specified. **THIS REVISION ARMS
NOTHING AND THE SET IS STILL EMPTY**: every reading v1.109 adds — the eighth matrix triple and the
realisation walk — is taken at `861efbe`, a COMMITTED blob, so no post-edit body of a sibling is
read anywhere in it; re-running the needle over this body after the last edit returns **1**, its
single occurrence still being the quoted spelling above, so the governed set is **1 - 1 = 0**.
**That reading is taken AFTER the last edit and not after the last edit that seemed relevant**,
because the needle sits inside the corpus it counts. **The self-match is stated because a needle quoted in
the paragraph that defines it is inside the corpus it counts**, and a reader re-running the command
must get the same digit this document does. **Both readings are recorded because the transition is
the evidence**: an empty set and a re-armed one are the same paragraph doing its job at two moments,
and "no site carries this stamp" and "nobody looked" are indistinguishable from a silence. **The
six live stamps expired together at `7fc5f94`, into `git show` readings**, and the needle is re-run
over every revision's own shipped body — including this one, where it returns 1. **The spelling `this revision ships` is a DIFFERENT thing and is
excluded on purpose** — it marks a reading over THIS document's own post-edit body, which is the
freeze-sha rule's third clause and never expires into a `git show` of a sibling; it returns **1**
over this body, and that one occurrence is this sentence's own quotation of the spelling, so the
set of live sibling stamps is **1 - 1 = 0**; conflating the two spellings is how a stamp
count went wrong before. **This reading moved TWICE inside one revision and both moves were the
revision's own edits, which is why it is re-taken after every one of them**: it read `3` while the
§Next Steps pointer residual still carried the spelling, `2` once that residual was reworded, and
`1` once the register cardinal was rewritten to name v1.106 rather than to say *this revision*.
**A needle whose population is this document's own prose is re-run after the LAST edit and never
after the last edit that seemed relevant**, which is the failure mode both of those moves would
otherwise have been. **Both readings
are taken over the shipped body after the last edit and are reported as post-edit re-runs rather
than as rows**, by the same rule that governs every self-count here: the needles sit inside the
corpus they count, and the first draft of this paragraph moved its own figure from six to ten in
the act of stating it. The command is
`awk '/^## Version History/{exit}{print}' <doc> | tr '\n' ' ' | grep -o '<spelling>' | wc -l`,
run once per spelling.

**The walk is anchored on the matrix's header row and on no line number**, because the matrix is
the surface that grows: it opens at that header, skips the separator, counts every following table
row, and stops at the first line that is not one. Piping the same walk's rows through
`sed 's/^| *//; s/ *|.*//' | sort -u | wc -l` is run at every sha the total is run at and its
readings are published in the block above beside them, pair by pair: **81**/**81** at `09e9307`,
**85**/**85** at `cb4fe99`, **85**/**85** at `cac6edc`, **86**/**86** at `ccd8ebd`,
**86**/**86** at `0021c77`, **87**/**87** at `7fc5f94`, **88**/**88** at `da03ca5` and
**90**/**90** at `861efbe`. **The pair is what
carries the claim, so it is printed as a pair at
every reading and never as one figure said to hold "at each"**: totals and names are equal at all
eight, which is what establishes that no row is duplicated, and a single names figure quoted across
readings whose totals differ says the opposite of what it means — through v1.105 this document
quoted **85** names against a total of **81** at `09e9307`, where the walk returns **81** names. **Residual, exact**: the anchor is that header string, so a design revision that
re-words the header breaks this walk *silently* rather than returning a wrong number — which is why
the header is matched in full rather than by a fragment, and why the reading is published beside
its sha rather than carried. A second residual belongs to the **pointer** rather than to the walk:
§Deliverables states these values and this block produces them, so the two surfaces are re-read
together at every freeze, and the pointer names this block by its bolded lead-in rather than by
position.

**The mutation population and its ON-DISK REALISATION are two different measurements, and this
block derives the second one because the first cannot answer "is the deliverable there yet".** The
`90` above is a reading of a **sibling design table**; the realisation is a reading of
`h-mad/tests/mutation-specs/`, and through v1.107 §Deliverables published only the first while
naming one file, so a reader who opened that file found **32** rows against a cell saying **88**
with nothing on any surface able to say whether that is a partial build or a wrong one. **The walk
now reads ONE spec file rather than two**: the operator's §D10 settled the staging split as a
MERGE, executed at `7afc0ef`, folding `doc_block_exec_task3.json` into `doc_block_exec.json` and
deleting it — so the loop that stood here through v1.108 would now read a path that does not exist.
Both halves are derived at `861efbe`, in one walk:

```
$ python3 -c "import json,sys
d = json.load(open(sys.argv[1]))['mutations']
print(len(d), len({m['name'] for m in d}), sys.argv[1])" \
    h-mad/tests/mutation-specs/doc_block_exec.json
60 60 h-mad/tests/mutation-specs/doc_block_exec.json
$ python3 - <<'PY'
import json, subprocess
P = 'h-mad/tests/mutation-specs/'
spec = {m['name'] for m in json.load(open(P + 'doc_block_exec.json'))['mutations']}
body = subprocess.run(['git', 'show', '861efbe:docs/02-design/features/doc-block-exec.design.md'],
                      capture_output=True, text=True).stdout.splitlines()
matrix, seen = set(), False
for L in body:
    if L.startswith('| mutation | guard it removes (mechanism) | killed by'):
        seen = True; continue
    if seen and L.startswith('|---'):
        continue
    if seen and L.startswith('|'):
        matrix.add(L.split('|')[1].strip().strip('`')); continue
    if seen:
        break
print('matrix', len(matrix), 'on-disk', len(spec), 'both', len(matrix & spec))
print('on-disk-not-in-matrix', sorted(spec - matrix))
print('matrix-not-on-disk', len(matrix - spec))
PY
matrix 90 on-disk 60 both 60
on-disk-not-in-matrix []
matrix-not-on-disk 30
```

**A MERGE CAN DUPLICATE A ROW, SO ROWS AND DISTINCT NAMES ARE READ FROM ONE PASS AND PUBLISHED AS A
PAIR**: at `861efbe` `doc_block_exec.json` holds **60** rows and **60** distinct names, and the two
being equal is what rules out a name arriving twice — the one failure the fold could have
introduced and the one a row count alone cannot see. **Unit: distinct mutation NAMES, not rows and
not matching lines.** All **60** are matrix members, so the matrix's realisation is **60 of 90** —
Tasks 1-3 written, Tasks 4 and 5 unwritten. **BOTH TERMS OF THIS FRACTION MOVED, BY DIFFERENT
AMOUNTS AND FOR UNRELATED REASONS, WHICH IS WHY IT IS RE-DERIVED FROM ONE PASS RATHER THAN
SUBSTITUTED TERM BY TERM**: through v1.108 it read **58 of 88** — the numerator rose to 60 because
the §D10 merge left the two 5e rows no longer outside the matrix, the denominator rose to 90 because
the design took those same two rows, and the two moves are only equal in size by coincidence. **A
reader who substituted `88` → `90` and kept the 58 would have shipped a site that looks repaired and
is not.** The rule over the axis: **a partition claim re-derives its numerator, its denominator and
its residual from a SINGLE run, because a total being right proves nothing about its parts** — which
is what the walk above does, printing `matrix`, `on-disk`, `both`, `on-disk-not-in-matrix` and
`matrix-not-on-disk` together rather than bucketing any one of them by subtraction. **The `32 + 28` union that stood here through v1.108 is
RETIRED rather than re-stamped**: it was a claim about TWO files with an intersection of **0**, and
one of those files no longer exists, so re-deriving it at this sha would publish a partition with
no second part. The equal-pair check above is what survives of it — the same question (is any name
realised twice?) asked of the surface that now exists.

**THE WALK'S TWO ZEROES ARE BOTH CONTROLLED, because a zero from a walk that reads nothing looks
exactly like a zero from a walk that reads everything.** Re-running the block above with one
substitution each, at `861efbe`: injecting a name no matrix row carries
(`spec | {'not-a-matrix-row'}`) moves `on-disk-not-in-matrix` from `[]` to
`['not-a-matrix-row']`, so the empty list is a read and not a vacuum; and dropping a real on-disk
name (`spec - {'adjacent-heading-skipped'}`, verified present in the spec) moves `both` from **60**
to **59** and `matrix-not-on-disk` from **30** to **31**, so the other direction is live too. **The
two controls are run SEPARATELY and each with its own substitution**, because one mutation that
moved both figures would leave either of them able to carry the other.

**This figure DRIFTS BY CONSTRUCTION and is neither a floor nor a target**: it rises as Tasks 4 and
5 land, so it is re-derived by the command above at every freeze and **never incremented**, by the
same rule the total obeys. **The SET OF FILES is read by GLOB, never from a name convention**:
`h-mad/tests/test_h_mad_mutation_harness.py` sweeps `specs_dir.glob("*.json")` and its committed
sweep uses `rglob("tests/mutation-specs/*.json")`, both verified present at `861efbe` with
`grep -c 'specs_dir.glob("\*.json")'` → **1** and `grep -c 'rglob("tests/mutation-specs/\*.json")'`
→ **1** (unit: matching lines). **The glob is what made the per-task split harness-legal while it
existed, and it is what makes the merge invisible to the gate**: the sweep reaches whatever files
are there, so neither the split nor its removal needed any document to name a file for the anchors
and the harness to reach the rows. Through v1.108 this document declined to ratify the split as a
convention and equally declined to demand a merge, on the ground that the naming partitioned the
population by no stated rule and that a total was never a claim about a file. **That question is no
longer open and is not re-argued here: it was an OPERATOR decision, recorded as §D10 in
`docs/03-analysis/doc-block-exec.5d-decisions.md` and executed at `7afc0ef` — the per-task file was
STAGING, and it has been folded in and deleted.** The closure predicate the decision names is run
rather than asserted: `ls h-mad/tests/mutation-specs/ | grep -cE '_task[0-9]+\.json$'` → **0** at
`861efbe` (unit: matching filenames), and that zero is *load-bearing rather than incidental* — it
holds because the file was deleted, and any later task staging a new suffixed sibling moves it off
0 by construction, which is exactly what it is there to catch. **The glob remains the contract, and
it is stated as such.**

**A CONTROL THAT WAS DELETED RATHER THAN RE-STAMPED, AND WHY.** Through v1.108 the paragraph above
closed with a control asserting that no surface in any of the four documents named a per-task
convention: `grep -c 'doc_block_exec_task3'` over the four, returning **0 0 0 0** at `6a1693c`.
That reading was true there. Re-running the same command at `861efbe` returns, **per element and at
BOTH SCOPES because the spec is exactly where the two diverge** (spec, plan, design, impl-plan; unit:
matching lines): whole-file **1 5 4 6**, body-scoped — `awk '/^## Version History/{exit}{print}'` —
**0 5 3 4**. The spec's 1/0 is one Version History mention; design 4/3 and impl-plan 6/4 diverge the
same way; **plan 5/5 does NOT diverge, and that agreeing pair is the control on the other three**,
showing the gap is a property of where the needle sits rather than an artifact of the awk. Every one
of those non-zeros is the 5e round's own prose ABOUT the split, including the sentence that carried
the control. **The control counted its own needle, so it could not survive being discussed**, and
re-stamping it to either vector would publish the needle once more in the act of measuring it. It is
removed instead. **v1.109 published this as `0 5 4 6` under a single unit and its first element was
WRONG — not mis-scoped but UNMEASURED**: that sweep read the spec at `docs/00-spec/...`, the spec
lives at `docs/01-plan/features/doc-block-exec.spec.md`, and `git show <sha>:<absent-path>` fails to
stderr while printing nothing to stdout, so `grep -c` scored an empty stream. **The rule over that
axis, which is not about this needle: a `git show <sha>:<path> | grep -c` pipeline FAILS
PERMISSIVELY — a missing path and a true zero are the same number downstream, so the path is
asserted to exist before the count is believed, never inferred from the count being plausible.** **The residual is
exact and is not papered over**: this revision's Version History entry and the merge sentence above
both name the retired path deliberately, because the record of a deletion has to name what was
deleted — so no absence-control over this string can ever be honest in this document again, and the
property it once stood for is now carried by the closure predicate above, which counts FILES ON
DISK rather than mentions in prose.

**THE TWO REALISED ROWS THIS DOCUMENT ROUTED TO THE DESIGN ARE DISCHARGED, AND THE DISCHARGE IS
STATED RATHER THAN SILENTLY DROPPED.** Through v1.108 this block published
`kill-skipped-after-collect-failure` and `stderr-not-closed` — the two rows added at 5e when Task
3's stderr closure and post-collect kill gained discriminating tests — as realised names the matrix
did not carry, declined to bump its own total on their account (the design owns the matrix, and a
`+2` applied here would have made this document the sole attributor of a figure it cannot defend),
and routed them as owed with the closing condition named and its number withheld: *a `not-in-matrix`
list that comes back empty is what closes this routing*. **It comes back empty at `861efbe`**, and
both names are matrix members there, which is precisely the `88` → `90` the design's total moved by.
**The routing is therefore closed by the WALK and not by an edit**, which is the only form that
could close it — the condition was written before the discharge landed and neither figure was
written forward to meet it. **A closed routing is left visible rather than deleted**: a reader who
finds the discharge missing cannot tell it from a routing that was quietly abandoned.

**The difference in the OTHER direction is still MEASURED rather than subtracted, and the reason it
now AGREES with the subtraction is itself the residual**: the matrix carries **30** names no spec
file realises — `len(matrix - spec)` from the same walk, printed above — and `90 - 60` is also 30.
Through v1.108 the two answers *differed*, because the two rows above sat in the 60 and not in the
88, so the naive subtraction under-reported the unrealised set by exactly them. **The sets are now
NESTED — `on-disk-not-in-matrix` is empty — which is the only condition under which the totals may
be subtracted, and it is a fact about this sha rather than a property of the two surfaces.** The
rule is unchanged and the walk still prints both directions: **whenever two sets are not nested the
difference is run and never derived from the totals, and nesting is a reading, not an assumption** —
the moment a 5e round adds a row on disk ahead of the matrix, as it did once already, the agreement
breaks and only the run difference stays right. That **30** is *incidental* rather than
load-bearing — it stands at 30 only because Tasks 4 and 5 have not run, and no rule holds it there.
...
`a8e0372` it is `73`/`88` (above). **The subdirectory claim is EXECUTED rather than reasoned, and it
is the "run from the root" instruction that it supports** — the script's filter tests
`p.parts[0] in ('h-mad','handoff')` against a path relative to the cwd, and from inside either
directory no path can have those `parts[0]`, so a subdirectory run returns **zero** at every commit
rather than a smaller non-zero number. Run at `fbc2ea0`, same script, three working directories:

```
$ ROOT=$(git rev-parse --show-toplevel)
$ for d in "$ROOT" "$ROOT"/h-mad "$ROOT"/handoff; do (cd "$d" && /opt/anaconda3/bin/python3.11 -c "
from pathlib import Path
tot=0; files=0
for p in sorted(Path('.').glob('*/**/*.md')):
    if 'archive' in p.parts or p.parts[0] not in ('h-mad','handoff'): continue
    n=sum(1 for l in p.read_text(encoding='utf-8',errors='replace').split('\n')
          if l.startswith('\`\`\`bash'))
    if n: tot+=n; files+=1
print(f'bash fences: {tot} across {files} files')"); done
bash fences: 73 across 10 files
bash fences: 0 across 0 files
bash fences: 0 across 0 files
```

**A HISTORICAL OBSERVATION WHOSE CAUSE THIS DOCUMENT ASSERTED AND CANNOT SUPPORT.** A plan audit
reported `49 across 2 files` (27 in `h-mad/SKILL.md`, 22 in `handoff/SKILL.md`). Through v1.103 this
paragraph attributed that to a subdirectory run, and the readings above refute it: a subdirectory
run yields `0 / 0`, not `49 / 2`, and it cannot yield `2` in particular because from `h-mad/` the
script cannot see `handoff/SKILL.md` at all. **The observation is preserved and the cause is
deleted: `cause unrecovered — the invocation that produced 49/2 is in no preserved artifact.`**
**The class, which is why this is a deletion and not a re-attribution**: a causal claim about a
measurement is itself a measurement and needs the controlled pair — the invocation that produces
the reported number beside one that does not — and where the original invocation is unrecoverable
the honest record is the observation with its cause marked unrecovered, never the most plausible
mechanism. Residual: `0 / 0` refutes *this* cause and establishes no other, so a later reader who
recovers the invocation should expect it to be neither of the two run above. The script is correct
from the root, which is where its `Path('.')` assumes it runs; a reviewer re-running it must do so
from the root.
...
**Re-run at `fbc2ea0` and still `2` — on the surface that owns the figure and not only in the
neighbouring census paragraph, because a figure re-derived elsewhere is a figure a reader cannot
check where it is stated.** That re-run is not a formality this round: `fbc2ea0` adds four `.py`
files to exactly this corpus and moved every other repo-wide count in this section, and this one
held because the two new fence-carrying lines sit inside a probe's string literals rather than in an
extraction call — the narrow three-branch pattern declines them, which is the pattern discriminating
on what it was written to discriminate on and is worth more than the unchanged digit.
...
`cf3a862`, **411** at `4e4a00c` and **415** at `fbc2ea0` — and it is re-derived on this surface at every
...
**RE-DERIVED AT `fbc2ea0` PER THE SENTENCE ABOVE, AND EVERY FIGURE IN THIS PARAGRAPH MOVED, BECAUSE
`fbc2ea0` ADDS FOUR `.py` FILES TO A REPO-WIDE CORPUS.** This is the case the §Measurements closure
is explicitly said not to reach, arriving: `fbc2ea0` touches neither `h-mad/` nor `handoff/` — so
every figure over the *two roots* is untouched — while adding
`docs/03-analysis/probes/doc-block-exec/*.py`, which this corpus **is** scoped to. At `fbc2ea0`:
the bare-literal count is **8** on all three forms — `grep -rn '```bash' --include='*.py' . | wc -l`
**8**, `git grep -n '```bash' -- '*.py' | wc -l` **8**, and `git grep -n '```bash' fbc2ea0 -- '*.py'
| wc -l` **8** — so the agreement the paragraph rests on **survives**, which is the load-bearing
part; the extractor census is still **2**
(`git grep -nE 'findall.*```bash|split.*```bash|re\.compile.*```bash' fbc2ea0 -- '*.py'`), so the
two new lines are not extractors; `git ls-files '*.py' | grep -vcE '^(h-mad|handoff)/'` is **415**;
`git grep -l '```' fbc2ea0 -- '*.py' | wc -l` is **25** with **6** of them outside the two roots,
and `git grep -l '```bash' fbc2ea0 -- '*.py' | wc -l` is **5**. The whole `6` → `8` delta is one
file, `grammar_corpus.2026-09-03.cd979362.py`, contributing **2** lines to the per-file split below.
**The class, and it is the sharpest instance this document has of it: a freeze checked against a
corpus's closure predicates is NOT checked against every corpus the document carries.** `fbc2ea0`
was certified against `git diff --name-only 09e9307 fbc2ea0 -- h-mad handoff`, which prints nothing
and is a true statement about the two roots — and it moved five figures whose corpus is the whole
repository. The rule over that axis: **a freeze is cleared per CORPUS, not per repository**, by
running each distinctly-scoped census's own command across the interval, and the repo-wide `.py`
censuses are the ones this document has that no `h-mad`/`handoff` predicate can speak for. Residual,
exact: the enumeration of distinctly-scoped corpora in this document is by hand — the two roots,
`docs/01-plan/features/` for the ledger, `.` for the extractor and `.py` censuses — and a corpus
nobody has named is invisible to the rule.
...
the last pair being the two extractors above. **Re-run at `fbc2ea0`, the split gains exactly one
file and no member loses a line**: the same four, plus
`docs/03-analysis/probes/doc-block-exec/grammar_corpus.2026-09-03.cd979362.py` **2** — the whole
`6` → `8` move, and it is a probe's own fenced fixtures rather than anything this feature reads. The four that are not extractors are the one inline
...
and unchanged from `cf3a862`, `6f0ee85`, `35698f9` and `a8e0372` (**23** at `1861157`, and **25** at
`fbc2ea0` by the same command with that sha, the added member being the grammar-corpus probe;
the quantity is deliberately the broad one — `git grep -l '```bash' 4e4a00c -- '*.py' | wc -l`
returns
**4** at `4e4a00c`, `cf3a862`, `6f0ee85`, `35698f9` and `a8e0372`, **3** at `1861157` and **5** at
`fbc2ea0`, the *bash* fence literal, a different and narrower
...
`handoff/`, and at `fbc2ea0` **6** of the 25 do (`git grep -l '```' 4e4a00c -- '*.py' | sed 's/^4e4a00c://' | grep -vcE '^(h-mad|handoff)/'`) — and through v1.94 both figures carried a `6f0ee85` stamp that
...
- **The corpus for every `*.md`-scoped measurement below is the tracked one**, defined as design
  ...
  **What is invariant is the structure, not the pair**: the glob is exactly the tracked set plus
  ...
  named beside them, and they are **not** re-stamped here — a reading of a committed blob stays at
  its blob — but a `fbc2ea0` reading is now published beside each.
  **THE INFERENCE THAT STOOD IN PLACE OF A RE-RUN WAS FALSE AT THE SHA IT WAS ASSERTED AT, AND IS
  RETRACTED RATHER THAN SUPPLEMENTED.** Through v1.103 this residual argued that the five new
  `h-mad/agents/*.md` were *inspected* at `a8e0372` — inspection, not a re-run of the differential —
  and that since none of the five carries any of the three softening shapes, `new_only=0` still
  held. **It did not hold, at `a8e0372` or at any later sha.** The committed 09-04 probe, run in a
  detached worktree per sha, reads `files=30 both=292 old_only=82 new_only=1` with
  `titleless=1` on the TRACKED corpus at **`a8e0372`, `35698f9`, `cf3a862`, `4e4a00c`, `74e126f`
  and `fbc2ea0` alike**, and **`0` from `b39d9dc` onward, including the measurement commit `cac6edc`** — the
  sha list is EXTENDED rather than rewritten, because the specimen existed and was measured at six
  shas and does not have to still exist for the argument that rests on it to hold. The single
  `new_only` member was an **empty ATX heading in
  `h-mad/SKILL.md`** — a bare `#` with no title, which CommonMark accepts as a heading and the old
  space-requiring regex rejects. It entered in `bea1b60`
  (`git log --oneline -S` on that file), which is an ancestor of every one of those shas, so the
  claim was wrong when written and not merely outrun. **The mechanism of the error is the point and
  is what the class closes**: the inference's corpus was *the five files the commit added*, and the
  claim's corpus is *the whole tracked set* — a conclusion about a set was drawn from a run over a
  subset, which is the same population-short-by-N shape this section already writes a rule against,
  here in its most expensive form because the missing member was in the largest file of the corpus
  and had been there all along. **The rule over that axis: an absence claim about a corpus is
  established by running the screen over that corpus, never by inspecting the delta that last
  changed it**, because the member that falsifies it may predate the delta. Residual, exact: this
  correction rests on the committed probe's predicates, so a defect in *those* would move it, which
  is why the probe is in the tree and named by path rather than described.
  **The narrowed guard is RIGHT about that member and the old regex was WRONG about it**, so the
  Guard-narrowing accounting is restated rather than weakened: the invariant is not `new_only = 0`,
  it is **every `new_only` member is enumerated and each is a CommonMark heading**, which the
  enumeration below discharges — with one member at `fbc2ea0` and with **none** at `cac6edc`.
  **v1.104 said the line was deliberately NOT repaired in that round** — touching `h-mad/` moves
  the freeze and expires every `h-mad`-scoped reading in this document at once, which is exactly
  what `af19d53` did to v1.103 — **so it was deferred to the tooling batch after the c88 gate. THAT
  BATCH LANDED AND THE PREDICTION IS DISCHARGED RATHER THAN LEFT IN THE FUTURE TENSE**: `b39d9dc`
  removed the bare `#` and pinned the removal with
  `h-mad/tests/test_h_mad_agent_definitions.py::test_skill_has_no_bare_heading_stub`, and
  `grep -c '^#$' h-mad/SKILL.md` over `git show <sha>:h-mad/SKILL.md` returns **1** at `fbc2ea0` and
  **0** at `b39d9dc` and at `cac6edc`. **It also cost this document exactly what v1.104 predicted it
  would**, which is why the deferral was right and the discharge is recorded rather than celebrated:
  the same commit moved every `h-mad`-scoped collect count and the test-file count, each re-derived
  at `cac6edc` at its own site. **The rule that survives the specimen**: an accounting written as
  "every member is enumerated and each is a heading" holds at any N, including N = 0, whereas one
  written as `new_only = 0` would have been falsified by the specimen and then silently re-satisfied
  by its removal — the same invariant reading TRUE for two opposite reasons is the failure an
  enumeration-plus-predicate form exists to prevent.
  **`both` and `old_only` were predicted to move and DID, so the prediction is replaced by the
  measurement**: `both` 263 → **292** and `old_only` 76 → **82** on the tracked corpus between
  `1861157` and `fbc2ea0`, and `old_only` rising is what the prediction below called the direction
  that *strengthens* the narrowing conclusion. `h-mad/agents/doc-auditor.md` alone carries
...
- **Heading selector differential** — the old `docsections.titled_section` regex
  ...
  on the new side. **THE PROBE IS IN THE TREE AND IS CITED BY COMMITTED PATH, NOT DESCRIBED**:
  `docs/03-analysis/probes/doc-block-exec/heading_differential.2026-09-04.b66afa9c.py`, one
  `re.match` per line per selector, committed in `fbc2ea0` — as are the three others this section
  cites. Through v1.103 all four were called throwaways and named only by a bare filename, so no
  reader could reproduce a single figure below; that is closed, and the naming rule is now that a
  probe is cited by the path it is committed at and by the writing session in its filename.
  **Two versions of this probe exist and they are distinguished by SHAPE, because the figures below
  came from one of them**: the `2026-09-04` version prints the `--- TRACKED` / `--- GLOB` pair
  reproduced here, the `2026-09-03` one (`heading_differential.2026-09-03.cd979362.py`) is
  glob-only. Everything below is the 09-04 version's output.
  **The published `1861157` reading is REPRODUCED rather than asserted, from a worktree, because
  the probe does not exist at that commit and a reader who checks it out finds nothing**:

  ```
  $ ROOT=$(git rev-parse --show-toplevel)
  $ git worktree add /tmp/wt-1861157 1861157
  $ (cd /tmp/wt-1861157 && /opt/anaconda3/bin/python3.11 \
      "$ROOT"/docs/03-analysis/probes/doc-block-exec/heading_differential.2026-09-04.b66afa9c.py)
  ...
  $ git worktree remove /tmp/wt-1861157
  ```

  **The TRACKED half reproduces EXACTLY — `files=25 both=263 old_only=76 new_only=0` — and the GLOB
  half does NOT, and that difference is this bullet's own thesis realised rather than a defect.**
  The GLOB half as published at `1861157` read `files=30 both=268 old_only=76 new_only=0` with
  `closing_hash=5`; from a fresh worktree it reads the tracked figures with `closing_hash=0`,
  because the five-file glob surplus is untracked `.pytest_cache/README.md` build output that
  exists only on a tree where pytest has run — `find . -maxdepth 3 -name '.pytest_cache'` in the
  worktree returns **0**. **So the tracked corpus is reproducible by anyone at any sha and the glob
  corpus is reproducible only on a contaminated tree**, which is the reason §Scanning defines the
  corpus by `git ls-files` and why the glob figures are published as contamination-visible context
  and never as a conclusion. The same applies to the Setext bullet's glob `files=30` below.
  **The `fbc2ea0` reading, taken with the same probe from a clean checkout of this repository**:
  TRACKED `files=30 both=292 old_only=82 new_only=1`, `setext_headings=0`, softening shapes
  `closing_hash=0 tab_form=0 titleless=1`; GLOB `files=35 both=297 old_only=82 new_only=1`,
  `setext_headings=0`, `closing_hash=5 tab_form=0 titleless=1`.
  **THE MEASUREMENT-COMMIT READING, same committed probe, run at `cac6edc`, and ONE FIELD MOVED ON EACH
  CORPUS**: TRACKED `files=30 both=292 old_only=82 new_only=0`, `setext_headings=0`, softening
  shapes `closing_hash=0 tab_form=0 titleless=0`; GLOB `files=35 both=297 old_only=82 new_only=0`,
  `setext_headings=0`, `closing_hash=5 tab_form=0 titleless=0`. `new_only` and `titleless` fall to
  **0** together on both corpora, which is the signature of the single shared member leaving, and
  `files`, `both`, `old_only`, `setext_headings` and `closing_hash` are unchanged on both — so the
  corpus itself did not move, one line in it did. The `fbc2ea0` reading above is a reading of a
  committed blob and keeps its own stamp; this one is the present-tense reading.
  **The one `new_only` member at `fbc2ea0` was ENUMERATED by command, because the committed probe
  does not print one** — it collects at most three `OLD-ONLY` lines and prints no `NEW-ONLY` line at all, so a
  reader handed a `NEW-ONLY …` line has been handed something this probe cannot produce. The
  enumeration runs the probe's own `OLD`, `NEW_LINE` and `fence_events` predicates over its own
  `corpora()`:

  ```
  $ /opt/anaconda3/bin/python3.11 -c "
  import importlib.util as u
  s=u.spec_from_file_location('hd','docs/03-analysis/probes/doc-block-exec/heading_differential.2026-09-04.b66afa9c.py')
  m=u.module_from_spec(s); s.loader.exec_module(m)
  for label,files in m.corpora().items():
      for p in files:
          lines=p.read_text(encoding='utf-8',errors='replace').split('\n')
          st=[f for _,f in m.fence_events(lines)]
          for i,line in enumerate(lines):
              o=bool(m.OLD.match(line)); mm=m.NEW_LINE.match(line)
              if bool(mm) and not st[i] and not o:
                  print('NEW-ONLY', label, p, i+1, repr(line))"
  NEW-ONLY TRACKED (git ls-files) h-mad/SKILL.md 984 '#'
  NEW-ONLY GLOB (filesystem) h-mad/SKILL.md 984 '#'
  ```

  Those two integers are the command's own **output**, reproduced verbatim rather than transcribed,
  which is why they are not pins and are exempt from the shape grep under §Implementation Strategy;
  in prose the member is named structurally, as **the empty ATX heading in `h-mad/SKILL.md`**.
  **That block is a reading of the tree at `fbc2ea0` and is kept as one. RE-RUN VERBATIM AT THE
  MEASUREMENT COMMIT `cac6edc`, THE SAME COMMAND PRINTS NOTHING** — no `NEW-ONLY` line on either corpus,
  because the set it enumerates is empty there. **An empty output is a reading and is published as
  one, not as a silence**: the enumeration did not fail, change corpus or change predicate; it
  returned the empty enumeration, which is the only shape "N = 0" can honestly take for a claim
  whose form is an enumeration. **This is also why the member is named structurally in prose and
  the file offset lives only inside reproduced output**: the offset in that block is now unreachable
  in the tree, and had it been written as a prose pin it would today be a pin into a file that no
  longer has that line, which is the pin-drift failure this document keeps out of its prose by rule.
  `old_only=76` at `1861157` and `82` at `fbc2ea0`: all of them are `#` comment lines
  inside fenced code the old regex read as headings; the migration narrows the guard, and the rise
  is the direction that strengthens that conclusion.

  **The `new_only` accounting did depend on the corpus, and is restated correctly.** The base
  ...
  **At `fbc2ea0` a NEW softened shape appears on both corpora**: `titleless=1` on tracked and on
  glob, with `closing_hash` still `0` tracked / `5` glob and `tab_form=0` on both. Its one member
  is the empty ATX heading enumerated above. **Every statement this bullet makes about the softened
  set names the corpus it is about, because the two corpora have never agreed on it**: the tracked
  set was empty before `bea1b60` and is empty again from `b39d9dc`, and the glob set has carried
  `closing_hash=5` at every reading taken here and has never been empty at all.
  **At `cac6edc`, and again at the measurement commit `0021c77`, the softened set is empty on the
  TRACKED corpus — `titleless=0`, `tab_form=0`, `closing_hash=0` — and is NOT empty on GLOB, where
  `closing_hash` is still `5`.** The conclusion is stated over TRACKED alone because that is the
  corpus this feature reads; the five glob members are the `# pytest cache directory #` lines, one
  per untracked `.pytest_cache/README.md`, so their presence is a fact about build artifacts and is
  no evidence about the feature's corpus in either direction.
  **THREE ZEROES APPEAR IN THIS BULLET AND THEY ARE THREE DIFFERENT KINDS OF ZERO. They are
  separated by name, because a screen reading `0` says nothing about which kind it is.**
  **(1) The `titleless=0` at `1861157` is a TRUE zero over an empty corpus — the specimen did not
  yet exist — and that is MEASURED per commit rather than explained**:

  ```
  for s in a469493 1861157 bea1b60 fbc2ea0 cac6edc 0021c77; do
    printf '%s %s\n' "$s" "$(git show "${s}:h-mad/SKILL.md" | grep -c '^#[[:space:]]*$')"
  done
  a469493 0
  1861157 0
  bea1b60 1
  ...
  `bea1b60` is the commit that writes the specimen and `b39d9dc` the one that removes it, so the
  reading is `0` on both sides of a `1` and the two zeroes bracket the member's whole lifetime.
  **Through v1.105 this bullet said the `1861157` zero meant *the shape had not been looked for
  correctly*, and that was a causal claim with nothing behind it**: the shape was looked for
  correctly and was correctly absent. **(2) The `titleless=0` at `cac6edc` is the opposite kind**:
  the member existed, was enumerated by command above, and left because `b39d9dc` deleted it from
  the corpus as a defect in its own right. **(3) The TRACKED `closing_hash=0` is a third kind
  again** — a true reading of the corpus this feature reads, which through v1.93 was NARRATED as an
  absence in general while the corpus actually measured was the glob, where the same shape reads
  `5`. That is a corpus error rather than a looking error, and it is the zero the mis-corpus story
  above belongs to. **The class: an explanation of a zero is a claim about WHY the set is empty, is
  itself a measurement, and is established by running the corpus predicate at the shas on either
  side of the zero — never by attaching the most available story to the digit.** Residual, exact:
  the per-commit command above settles kinds (1) and (2) and cannot settle (3), because no sha
  distinguishes a mis-stated corpus from a correctly stated one; only re-reading which corpus the
  sentence names does, which is why every sentence in this bullet now carries its corpus word.
  This is also why the bullet publishes the sha series rather than the current value, and why the
  `fbc2ea0` reading is kept beside the `cac6edc` one instead of being overwritten by it. **An empty
  ATX heading is a heading under CommonMark, and that is EXECUTED here rather than asserted from the
  specification**, on the same renderer the grammar corpus below uses:
  ...
  for src in ['#\n','##\n','####### \n','#hashtag\n']:
      print(repr(src),'->',repr(md.render(src)))"
  '#\n' -> '<h1></h1>\n'
  '##\n' -> '<h2></h2>\n'
  '####### \n' -> '<p>#######</p>\n'
  '#hashtag\n' -> '<p>#hashtag</p>\n'
  ...
  The bare `#` renders as an `h1` with an empty title; the two negatives beside it are carried from
  the corpus below so the positive is not a bare assertion of the renderer's agreement. **So this
  member is not a softening at all but a correction: the new
  selector is right about it and the old space-requiring regex was wrong**, which is why the
  Guard-narrowing invariant below is stated as "every `new_only` member is enumerated and each is a
  CommonMark heading" rather than as a zero. **The class this replaces**: an invariant written as a
  count is falsified by its first counterexample and says nothing about whether the counterexample
  is a defect; an invariant written as an enumeration plus a predicate survives the count moving and
  puts the judgement where a reader can check it. Residual, exact: the predicate is "is a CommonMark
  heading", and the corpus below has **no case** for the empty ATX form, so the check is executed in
  this bullet instead and that corpus's `14` does not move; folding the case in there is owed to a
  later revision and is named rather than left implicit.
  **A SECOND RESIDUAL, WRITTEN IN THE WORDS THE ROUND DECIDED SO THAT THE THREE DOCUMENTS WITH A
  SITE STATE IT IDENTICALLY; THE SPEC HAS NO SITE FOR ANY FACT 3 VALUE** (`new_only`, `titleless`
  and `vacuously` are each **0** in it at `cac6edc`), because a vacuous truth read as a verified one is exactly the laundering the
  inherited-unverified register exists to refuse: At `cac6edc` the `new_only` set is empty, so
  "each `new_only` member is a heading under CommonMark" is vacuously true there; it was verified
  ...
  **The empty-ATX case therefore leaves the live corpus and becomes a FIXTURE**, which is the
  hostile-fixture rule applied to its own instance: a corpus specimen is a measurement, not a test,
  and a measurement can be deleted by a commit that never reads the document depending on it —
  which is precisely what happened here. The fixture is a `tmp_path` file with body
  `before\n#\nafter\n` asserted to read `titleless=1 new_only=1` on that file alone; the design
  names it and the impl-plan carries it as a test row, and this plan states only that the case is
  covered by a fixture rather than by pointing at the corpus again.
  ...
  **`both` moved 266 → 268 on the glob (261 → 263 tracked), and nothing is wrong with either.**
  ...
  beside it and the number is not to be carried — **and it drifted again, to `292` tracked / `297`
  glob at `fbc2ea0`, which is the sentence above proving itself rather than a figure to maintain.**
- **Setext census** — the ATX-only assumption measured directly rather than through the selector
  ...
  the same script as the differential above — the committed
  `heading_differential.2026-09-04.b66afa9c.py`, which prints a `setext_headings=` line per corpus —
  and re-derived at `1861157` from a worktree, on both readings of it. **That probe interleaves the
  Setext line with the differential's own `files=`/`both=` line, so the command is FILTERED and the
  output is the filtered output — a `files=` line does not appear here because this command does not
  print one**, and the file counts for this corpus are the ones the differential bullet above
  publishes (`25`/`25` from a clean worktree at `1861157`, `30`/`35` at `fbc2ea0`):
  ...
  **The figure this bullet published through v1.103 was `files=25` / `files=30`, which no committed
  probe prints in that shape** — the 09-04 probe prints its file count on the differential line and
  its Setext count on the next — so the pair is retired here rather than re-stamped: the file counts
  belong to the differential bullet, this bullet owns `setext_headings`, and the glob file count
  from a clean worktree is `25` and not `30` for the contamination reason that bullet states in
  full. The value this census reports — `setext_headings=0` — is unchanged on either reading and on
  either corpus, which is why the conclusion survives the corpus question.
  **At `fbc2ea0`, from a clean checkout of this repository, and cross-checked against the
  second committed probe** `setext_census.2026-09-04.b66afa9c.py`, which counts the same thing and
  labels its two corpora differently:

  ```
  $ /opt/anaconda3/bin/python3.11 docs/03-analysis/probes/doc-block-exec/setext_census.2026-09-04.b66afa9c.py
  [glob] files=35 setext_headings=0
  [tracked] files=30 setext_headings=0
  ```

  **Two independently written probes agreeing on `0` over the same corpus is the point of running
  both**, and the file counts `30`/`35` agree with the tracked/glob pair this section publishes at
  `fbc2ea0`. So no document `docsections` or the helper reads bounds wrongly under the ATX-only
  grammar; a Setext heading that arrives later is still unrecognised silently, which the design
  carries as a limitation rather than a guard.
  **RE-RUN AT THE MEASUREMENT COMMIT `cac6edc`, BOTH PROBES, AND THE READING IS UNCHANGED.** The 09-04
  differential prints `setext_headings=0` on TRACKED and on GLOB, and
  `/opt/anaconda3/bin/python3.11 docs/03-analysis/probes/doc-block-exec/setext_census.2026-09-04.b66afa9c.py`
  prints `[glob] files=35 setext_headings=0` and `[tracked] files=30 setext_headings=0` — the same
  two file counts this section publishes for that sha, so the corpus the census ran over is the
  corpus this section defines and not a different one that happens to answer `0`. **That matters
  more than the value**: a census returning `0` over the wrong corpus and a census returning `0`
  over the right one are indistinguishable from the digit alone, which is why the file counts are
  read out of the same invocation rather than asserted beside it. **This member LEFT
  the inherited-unverified register for v1.105 and STAYS OUT for v1.106, because v1.106 executed
  both probes again**, by the rule that register states about itself — a probe leaves only for the
  revision that executes it, and re-enters at the next revision that does not. **The v1.106 run is
  at `0021c77` and both halves reproduce byte-for-byte**: the committed Setext census prints
  `[glob] files=35 setext_headings=0` and `[tracked] files=30 setext_headings=0`, and the committed
  09-04 differential prints `setext_headings=0` on each of its two corpora with `files=35` and
  `files=30` beside them. **Running the census rather than inheriting it was NOT redundant even
  though the interval is closed**: the closure predicate says no path under the two roots changed,
  and the census says no CONTENT the census reads changed, and the first implies the second only if
  the corpus is exactly those paths — which is true here and is worth one command rather than one
  inference.
- **Scanner grammar corpus** — every fence and ATX rule the scanner implements, rendered through
  ...
  with `pip install --target` for that run), CommonMark preset on both, 14 of 14 agreeing on each.
  **The probe is committed at
  `docs/03-analysis/probes/doc-block-exec/grammar_corpus.2026-09-03.cd979362.py`** — one
  `md.render(src)` per case, a needle asserted on the HTML — and its output is what the design's
  §Scanning cites. **The interpreter is named in the command and is not `python3`**: bare `python3`
  in this environment is 3.14 with no `markdown_it` installed, so the run below would fail on the
  import rather than disagree, which is a failure mode a reader should not have to diagnose.
  **Re-run at `fbc2ea0` on the 2.2.0 leg, 14 of 14 still OK**; the 4.2.0 leg needs the throwaway
  venv and is NOT re-run here, which is why the fourteen sit in the register above:

  ```
  $ /opt/anaconda3/bin/python3.11 -c "import markdown_it; print(markdown_it.__version__)"
  2.2.0
...
- **Fence-body de-indentation, the one case v1.95 ran** — the heading names the executing revision
  by NUMBER, and through v1.104 it read "the one case *this revision* ran", which was v1.95's status
  and no later revision's: by v1.104 the same bullet's closing sentence said in as many words that
  no revision since v1.95 had re-run it, so the heading and its own last paragraph contradicted each
  other on the same screen. **This is the deixis rule the register two sections up states about
  itself, breaking outside the register**, and the fix is the same one: a status assignment whose
  subject is a revision names that revision by number, never by *this* one, because a deixis
  re-points at every revision while the status it labels does not move. **The residual is that the
  rule was written over the register's bullets and the defect landed in a §Measurements heading**,
  so it is restated here over its real axis — every sentence in this document that assigns a
  last-execution status, wherever it sits — and the walk that finds them is the register's member
  list plus the carve-out probe headings, not the register alone. Added because the `extract`
  ...
  $ TGT=$(mktemp -d) && pip install --target "$TGT" 'markdown-it-py==4.2.0' >/dev/null
  $ PYTHONPATH="$TGT" python3.11 - <<'PY'    # byte-identical to the heredoc above
  import markdown_it
  md = markdown_it.MarkdownIt("commonmark")
  src = "   ```bash\n   a\n b\nc\n     d\n   ```\n"     # opener at 3; body at 3, 1, 0, 5
  print("markdown-it-py", markdown_it.__version__, "->", repr(md.render(src)))
  ...
  **The second leg's script is written out rather than referred to as "the same script"**, because a
  reference is not a command and the two legs differ only in `PYTHONPATH`; the `pip install --target`
  line that creates the throwaway target is written out for the same reason, since through v1.103 it
  was named in prose and never as an invocation.
  ...
  false. **This case is NOT one of the fourteen above** — it is a fifteenth, run alone in v1.95, and
  from v1.104 it is a member of the inherited-unverified register in its own right, since no
  revision since v1.95 has re-run it. It is
...
**v1.105 MOVED THIS FIGURE AND PUBLISHED NO READING, which is the obligation in the sentence above
going unpaid by the revision that wrote it**, and v1.106 pays it: re-run over the body v1.106
ships, after the last edit, the unanchored form returns **27**. **It is reported as a post-edit
re-run and not written into the series of stamped rows**, because the tree it was taken on is not
yet a commit — the `8c6539a` and `b3be433` rows are readings of landed blobs and stay exactly as
they are. **The class this closes is not the digit but the SCHEDULE**: a figure whose stated
provenance is "the working tree this revision produces" has to be re-run after the revision's last
edit or it is not that figure at all, so it belongs to the same post-edit pass as every other
self-count here rather than to the pass that repairs prose. Residual: nothing detects a post-edit
re-run that was skipped, since a stale value and a fresh one that happens to agree look identical —
which is why the schedule is stated and the reading is labelled with when it was taken.
...
**Disambiguator, because this section and §Success Criteria are adjacent and both carry a
suite-related bullet**: the **run-alone rule** is here; the **suite floor** and its
`test_h_mad_assemble_audit.py` reading are in §Success Criteria. A cross-reference has landed on the
wrong one of this pair twice, so each section says which of the two it holds.
...
**Disambiguator, the mirror of the one in §Convention Prerequisites**: the **suite floor** and the
one body reading of `h-mad/tests/test_h_mad_assemble_audit.py` are here; the **run-alone rule** is
in §Convention Prerequisites.
...
- The full suite passes at no lower a count than the pre-change baseline plus this feature's tests.
  ...
  **Re-derived at `fbc2ea0` by v1.104, same command, same two working directories: `2814` from the
  repository root and `2552` from `h-mad/`.** The `+5` on **both** is accounted for the same way and
  the accounting is the point, because it lands on a different commit than a reader would guess:
  `git rev-list --oneline 6f0ee85..fbc2ea0 -- h-mad handoff` returns exactly **one** commit,
  `af19d53`, and `git show af19d53 -- 'h-mad/tests/*' | grep -c '^+def test_'` → **5** — the same
  five that moved the suite floor in §Success Criteria. **`fbc2ea0` contributes nothing to either
  figure and that is measured, not assumed**: `git show fbc2ea0 | grep -c '^+def test_'` → **0**,
  and `pytest --collect-only -q -p no:cacheprovider docs/03-analysis/probes` collects **no tests**,
  because the four probe files are not named `test_*.py` and pytest's default discovery never reads
  them. **So the probe commit moved the repo-wide `.py` FENCE and FILE censuses under §Measurements
  and did NOT move these collect counts, and the two must not be attributed together** — the fence
  censuses grep every `*.py` path, the collect counts see only what pytest discovers, and a commit
  can move one corpus without the other. That distinction is the per-corpus rule stated in
  §Measurements applied to its second-nearest neighbour rather than assumed to generalise.
  **RE-RUN AT THE MEASUREMENT COMMIT `cac6edc`, WHERE BOTH FIGURES MOVED AGAIN — and the word is COLLECTED,
  never PASSED.** Same command, same two working directories:
  `python3.11 -m pytest --collect-only -q -p no:cacheprovider | tail -1` returns **2836** from the
  repository root and **2574** from `h-mad/`, each `+22` on the `fbc2ea0` pair, which keeps its own
  stamp because it is a reading of a committed blob. The `+22` lands on ONE commit and is accounted
  for exactly as the `+5` and the `+1` above were:
  `git rev-list --oneline fbc2ea0..cac6edc -- h-mad handoff` returns exactly **one** commit,
  `b39d9dc`, and `git show b39d9dc -- 'h-mad/tests/*' | grep -c '^+def test_'` → **15**. **The gap
  between 15 `def` lines and 22 collected nodes is `@pytest.mark.parametrize`, and it is why these
  are kept as two figures and never written as one** — a `def` census counts source, a collect
  census counts nodes, and a parametrised test is one of the first and several of the second.
  **These two figures are COLLECTION counts and are labelled so deliberately, not as a hedge**: the
  round-eighteen decision sheet records the orchestrator measuring the `h-mad` suite RED on the
  committed tree from `b39d9dc`, for a pin drift that belongs to a sibling document and to no claim
  in this one, so an `N passed` for the current tree would be false and none is written here. The
  suite **floor** in §Success Criteria is a different figure under a different rule — re-measured at
  the 5c branch commit, not here — and the two are deliberately left unreconciled at this site.
  **Residual, and it is structural rather than a caveat**: both collect counts drift at every commit
  adding a test node anywhere in the tree, so each is re-derived at each freeze and neither is ever
  carried across one; the accounting sentence beside them is what makes a drift attributable rather
  than merely visible.
...
rule.** From cycle **73** through cycle **85** every audit of this document ran on the substitute
leg alone. **That span is derived from the file list and never converted from the boundary
predicate beside it**, which is where it went wrong: v1.103 rewrote "the last audit carrying a
codex leg is cycle `72`" as "from cycle `72`", and cycle 72 *did* carry one, so the converted form
was false of the first cycle it named. `git ls-tree -r --name-only af19d53 -- docs/01-plan/features/`
lists `doc-block-exec.plan.audit.v72.codex.md`, which is also why the codex half of the series below
reads `72` at its early shas — **the `72` in that series is a row, an argmax over the file list, not
an endpoint of this span**, and the unit residual numbered below says what that argmax counts.
**The class: a span rewritten from a boundary predicate moves its endpoint by one every time**, so a
span is re-derived from the underlying list — here the `git ls-tree` output this paragraph already
runs — and never obtained by rephrasing the predicate. Residual: nothing detects the conversion; only
re-deriving the span from the list does, which is why the list command sits in the same sentence.
**`4c1c3a5` landed the cycle-86 `codex` report**
...
claimed here — it was not measured**, and the series below reports only the shas listed in it.
**That summary carried a CARDINAL through v1.105 — "the eight shas" — while the series four lines
below it already listed ten, and the series now lists more again**, so the cardinal is deleted
rather than corrected: a free-standing integer summarising a list that the same revision extends
goes wrong at the next extension by construction, and naming the surface costs the reader one
glance at rows that are printed anyway. **The class, over the axis rather than at this cardinal**:
no summary of an enumerable surface in this document is written as an integer unless that integer
is arithmetic over the surface on the same page — the `pgid` census and the §Deliverables total are
both written that way — and where it cannot be, the summary names the surface instead. Residual:
this rule removes the failure and not the temptation; nothing detects a cardinal re-introduced by a
later revision except re-deriving it against the list, which is why the list is printed in full. The rule the debt motivated does not lapse with the debt. This figure's corpus is `docs/01-plan/features/`, which the §Measurements closure
...
and it must be re-measured on every revision, without exception. **v1.105 DID NOT RE-MEASURE IT AND
DID NOT RECORD THE NON-RUN, which is the one thing the rule below forbids**: it shipped the
`fbc2ea0` **87/87** reading unchanged while being measured at `cac6edc`, where the pair already read
**88/88**, and its Version History entry mentions this ledger only inside a value sweep. That is the
fifth consecutive revision outrun by this figure, and the first in which the miss was a
non-run rather than a stale row. **Both halves, re-derived by v1.106 at `0021c77` — v1.106's
one measurement commit, named by revision number and not by a deixis, which is this paragraph's own
deixis rule applied to the sentence that states it — with the
sha inside the command so the reading is reproducible without a working tree:**

```
git ls-tree -r --name-only 0021c77 -- docs/01-plan/features/ \
  | grep -E 'doc-block-exec\.plan\.audit\.v[0-9]+\.codex\.md$' \
  | sed 's/.*audit\.v//;s/\.codex\.md//' | sort -n | tail -1      # -> 89
git ls-tree -r --name-only 0021c77 -- docs/01-plan/features/ \
  | grep -E 'doc-block-exec\.plan\.audit\.v[0-9]+\.teammate\.md$' \
  | sed 's/.*audit\.v//;s/\.teammate\.md//' | sort -n | tail -1    # -> 89
```

**codex `89` against teammate `89` at `0021c77`.** **`fbc2ea0` keeps its own row below and is
v1.104's measurement commit, named by revision number and not by a deixis** — the phrase that stood
here through v1.105 read "this revision's one measurement commit", which was true when v1.104 wrote
it and became a second answer to *measured at* the moment v1.105 shipped without touching it. **A
deixis attached to a sha is a claim that expires silently at the next revision**, which is the same
failure the §Measurements paragraph abolishes by naming one measurement commit per revision, and it
is why every historical stamp at this site is now written with the revision number beside it.
`00b961f` appears below only as the stamp on the
...
`dfae038` **72/85** · `3f70eb3` **72/85** · `af19d53` **86/86** · `09e9307` **86/86** ·
`fbc2ea0` **87/87** · `cac6edc` **88/88** · `ccd8ebd` **88/88** · `bc4688e` **88/88** ·
`093c3ee` **88/88** · `d27d2ce` **89/89** · `c7a75eb` **89/89** · `0021c77` **89/89**.
**The two commits that TOUCHED this corpus inside the round are named and exactly ONE of them
moved the pair, which is the distinction this whole paragraph is about**: `git log --oneline
cac6edc..0021c77 -- docs/01-plan/features/` returns `ccd8ebd` and `d27d2ce` and nothing else;
`ccd8ebd`, the commit landing the round-eighteen revision batch, left the pair at **88/88**, and
`d27d2ce`, the commit landing round eighteen's own six gating reports, took it to **89/89**.
**Touching the corpus and moving the figure are different predicates and the sentence names which
is which**, because a revision batch writes documents into this directory and only a report
landing adds a file the pipelines count. The revision batch is not what moves this figure; the
round's reports are.
...
was not run**, the INHERITED-UNVERIFIED register under §Measurements being the only other place a
non-run is admissible. **That pointer read "the register below" through v1.105 and the register is
ABOVE this paragraph**, under §Measurements, while §Next Steps is the last section before the
Version History. **The class: a relative pointer is a placement claim about two positions in one
file, so this document names the heading and lets the reader's table of contents do the locating**
— the same repair the previous round made to a §Deliverables cell that said "the FR-6 table below"
about a table above it, which is why this is an instance of a class the round had already declared
closed rather than a new defect. **Residual, and it is why the sweep is per-site and cannot be done
by value**: at `ccd8ebd`, the body v1.105 shipped, `grep -c 'register below' <doc>` returned **5**
whole-file and **5** body-scoped, and FOUR of the five were correct — each sitting under
§Measurements ahead of the register's own lead sentence — while the fifth, here in §Next Steps, was
the inverted one. A value-driven replacement would have broken four correct pointers to fix one
wrong one, so each site was checked against its own position relative to that lead sentence, one at
a time. **The reading over the body v1.106 ships is published PER GRAMMAR, PER UNIT, and with its
self-match stated as arithmetic**. Body-scoped the two units agree: `grep -c` returns **5** lines
and the newline-collapsed `grep -o … | wc -l` returns **5** occurrences. Whole-file they do NOT:
**6** lines against **7** occurrences. **The gap is the unit and not the corpus, and it is the
whole reason both are printed** — this document's Version History is ONE LINE PER ENTRY, so an
entry mentioning the phrase twice contributes two occurrences and one line, and a whole-file
`grep -c` silently under-reports by the number of repeats inside any entry. **The rule over that
axis, and it governs every self-count here rather than this one**: a whole-file reading of a
document whose Version History is one line per entry is taken as an OCCURRENCE count, never as a
line count; a body-scoped `grep -c` stays correct only while the body keeps its hard wraps, which
is a property of this body and not a theorem. Of the five body occurrences,
**3** are live pointers — all correct, all preceding the lead sentence — and **2** are the phrase
quoted rather than used, once as the wording this paragraph retired and once inside the command
above, so `3 + 2 = 5`; the remaining **2** whole-file occurrences are both inside this revision's
Version History entry, `5 + 2 = 7`, which is why the whole-file line count reads one lower. **v1.106 also RETIRED
a correct pointer at the re-run enumeration under §Measurements**, replacing the direction with the
heading name, so the live-pointer population fell from four to three rather than holding at four —
a value sweep run against the pre-edit body would have missed that too. Every reading here was
taken after the last edit rather than predicted from it. **Residual,
...
and took the pair to `86`/`86`. **v1.103's reading was outrun the same way, and this revision
watched it happen**: `86`/`86` held at `af19d53` and again at `09e9307`, and `6fc71ab` — the commit
that landed round sixteen's own three gating reports — took the pair to `87`/`87`, which is the
reading v1.104 published at `fbc2ea0`. **v1.104's reading was outrun the same way, and its own
entry predicted it in as many words**: `87`/`87` held at `fbc2ea0` and stopped holding at
`cac6edc`, where `b39d9dc` and the round-eighteen freeze commits had already taken the pair to
`88`/`88`. **v1.105's failure was of a DIFFERENT SHAPE and is recorded as one rather than folded
into the run**: v1.105 was outrun before it began — the pair read `88`/`88` at its own measurement
commit `cac6edc` — and it neither re-ran the pipelines nor recorded the non-run, so what expired
was not a reading but the rule. **That is five consecutive revisions outrun by their own round's
reports, four of them the residual being right and one of them the rule being skipped, and the two
are counted separately because only the second is a defect in this paragraph's own terms.** The
same prediction applies to the reading above and has a named mover this time: `89`/`89` is correct
at `0021c77` and stops being correct the moment round nineteen's own reports are committed, so
v1.107 re-runs the two pipelines at its own measurement commit rather than reading this line, and
if it does not, its Version History entry says so.
...
`00b961f`, **73**/**14** at `af19d53`, **74**/**15** at `fbc2ea0`, **75**/**16** at `cac6edc` and
**76**/**17** at `0021c77` — the codex reports ran
`1..72` contiguously and cycles `86` through `89` are the only ones since, so at `0021c77` a reader
who takes the published `89`/`89` for a file count is off by `89 - 76` on the codex half and
`89 - 17` on the teammate half, the arithmetic
being over the two pairs printed in this sentence rather than a free-standing figure. **This
residual's own series is re-run at each measurement commit for the same reason the headline pair
is**, since both forms read the same directory this round writes into; the gap between the two
forms widens by one on the codex half and by one on the teammate half at every report that lands,
and stating it as subtraction over printed pairs is what keeps it from becoming a carried digit. (2) **The
...
- v1.104: GATING revision, answering the c87 audit that FAILED on BOTH surfaces at freeze 09e9307: the teammate leg must=4 should=4 nit=2 and the codex leg must=2 should=2, two different model families, neither having seen the other. NO GATING CLAIM, NO TWO-SURFACE CLEAN, NO EXIT GATE is claimed by this entry - it answers an audit, it is not one; the gate is c88. ONE MEASUREMENT COMMIT: every committed-blob reading v1.104 takes is run at fbc2ea0 with the sha inside the command, an inherited reading keeps the blob it was stamped at and does not move, and A THIRD CLAUSE THE FREEZE-SHA RULE NEEDED AND v1.103 DID NOT STATE - a reading taken over THIS revision's own post-edit body is stamped to the tree v1.104 ships and never to fbc2ea0, which does not contain these edits, which is why both self-counting passages publish stamped rows at committed blobs and report the post-edit re-run as a re-run. fbc2ea0 adds four probe files under docs/03-analysis/probes/doc-block-exec/ and touches nothing else - git diff --name-only 09e9307 fbc2ea0 -- h-mad handoff prints nothing and git diff --stat 09e9307 fbc2ea0 over the four documents is empty - so every h-mad/handoff figure stamped af19d53 holds at fbc2ea0 and nothing is retired here, which is the opposite of what af19d53 did to v1.103. THE LARGEST MOVE IN THIS REVISION WAS FOUND WHILE MEASURING AND IS IN NO REPORT: new_only=0 STILL HOLDS WAS FALSE AT THE SHA IT WAS ASSERTED AT. v1.94 inferred it from an inspection of the five new h-mad/agents/*.md files at a8e0372; the member that breaks it is in h-mad/SKILL.md, which that inspection never covered. Run in a detached worktree per sha with the committed 09-04 probe, TRACKED corpus: a8e0372, 35698f9, cf3a862, 4e4a00c, 74e126f and fbc2ea0 ALL read files=30 both=292 old_only=82 new_only=1 titleless=1, and the one member is the empty ATX heading in h-mad/SKILL.md, a bare hash with no title, added by bea1b60 which is an ancestor of every one of those shas. So the sentence was wrong when written, not merely stale, and it is CORRECTED rather than supplemented: new_only is 1, its single member is enumerated by command, and that member IS a heading under CommonMark - EXECUTED rather than argued from the specification, md.render on markdown-it-py 2.2.0 turning a bare hash into an empty h1 while seven hashes and a hashtag stay paragraphs - so the narrowed guard is right about it and the old space-requiring regex was wrong. CLASS: an absence claim about a corpus is established by running the screen over that corpus, never by inspecting the delta that last changed it, because the member that falsifies it may predate the delta; the invariant is restated as every new_only member is enumerated and each is a CommonMark heading, which survives the count moving. The line is NOT repaired this round - touching h-mad/ moves the freeze and expires every h-mad-scoped reading here - so it is repaired in the tooling batch after the c88 gate. The 1861157 figures stay at 1861157 per the freeze-sha rule and the fbc2ea0 reading is added beside them; both and old_only, predicted to move, DID - 263 to 292 and 76 to 82 tracked - so the prediction is replaced by the measurement. TWO SHEET CLAIMS MY OWN RUN REFUTED, reported rather than propagated: the r17 decision sheet attributes NEW-ONLY h-mad/SKILL.md 984 hash to the committed 09-04 probe, and that probe CANNOT print it - it collects at most three OLD-ONLY hits and prints no NEW-ONLY line at all, so the enumeration is published here as an importlib one-liner over the probe's own OLD, NEW_LINE and fence_events predicates; and the GLOB half of the 1861157 figures does NOT reproduce from a clean checkout - git worktree add at 1861157 then running the committed 09-04 probe from that root reproduces TRACKED files=25 both=263 old_only=76 new_only=0 EXACTLY, while its GLOB half reads 25/263/76/0 with closing_hash=0 against the published 30/268 with closing_hash=5, because the glob surplus is untracked .pytest_cache/README.md artifacts absent on a fresh worktree (find in that worktree returns 0). That is this document's own thesis realised: the TRACKED half is reproducible by anyone at any sha and the GLOB half only on a contaminated tree, stated beside both the Guard-narrowing and the Setext bullets. CODEX MUST 1, PROBE SOURCES ABSENT FROM THE TREE: closed at fbc2ea0, which commits all four - heading_differential.2026-09-03.cd979362.py, heading_differential.2026-09-04.b66afa9c.py, grammar_corpus.2026-09-03.cd979362.py and setext_census.2026-09-04.b66afa9c.py. Each figure is attributed to the version that produced it BY SHAPE: the 09-04 differential prints the TRACKED/GLOB pair this document publishes, the 09-03 one is glob-only. grammar_corpus re-run at fbc2ea0 on markdown-it-py 2.2.0 under /opt/anaconda3/bin/python3.11, 14 of 14 OK - bare python3 here is 3.14 without markdown_it, which is why the interpreter is in the command; the 4.2.0 leg needs the throwaway venv and was NOT re-run, so the register entry is SPLIT and now covers that leg alone. setext_census at fbc2ea0: tracked 30 / glob 35, 0 Setext on each, agreeing with the differential's own setext line - two independently written probes over one corpus. Also closed here: the fifteenth case's second leg was published as PYTHONPATH=<throwaway target> python3.11 <same script>, a reference rather than a command; both the pip install --target line and the full heredoc are now written out. CODEX MUST 2, THE 49 ACROSS 2 FILES CAUSE: the subdirectory explanation is DELETED and the observation kept. Executed at fbc2ea0 in three working directories: 73 across 10 files from the repository root and 0 across 0 files from h-mad/ and from handoff/ alike, which is structural rather than corpus-dependent - the filter tests p.parts[0] against h-mad and handoff and from inside either directory no path can have those parts[0], so a subdirectory run returns 0 at every commit and can never produce 27 plus 22 across the two top-level SKILL.md files. Marked cause unrecovered. CLASS: a causal claim about a measurement is itself a measurement and needs the controlled pair; where the original invocation is unrecoverable the honest record is the observation with its cause marked unrecovered, never the most plausible mechanism. TEAMMATE MUST 1, A SPAN REWRITTEN FROM A BOUNDARY PREDICATE MOVES ITS ENDPOINT BY ONE: cycle 72 DID carry a codex leg - git ls-tree -r --name-only af19d53 -- docs/01-plan/features/ lists doc-block-exec.plan.audit.v72.codex.md - so the gap is cycles 73 through 85. CLASS: a span derived from a boundary predicate is re-derived from the underlying list and never obtained by rephrasing the predicate; the 72 in the series below is a row, an argmax over the file list, not an endpoint. TEAMMATE MUST 2, A UNIFORM PROPERTY ASSERTED OVER FOUR ENTRIES AND FALSE FOR THREE: the register stamp move rested on each of the four round-fifteen revisions naming every member of this register under NOT RE-RUN. Measured at 59cc2ad: spec v1.62 has 0 occurrences of NOT RE-RUN and 0 of any register-member needle, design v1.107 the same 0/0, and impl-plan v1.51 DOES carry one enumeration (1) but names 0 members of THIS register - every member it names is impl-plan-scoped. Only the plan's own v1.102 entry does what the sentence claimed all four do. NARROWED to that one entry, with the siblings carried by a different and weaker argument stated separately: a plan-scoped figure can only be re-run by a revision of this plan, so a sibling revision's silence carries no weight and is not used as any. CLASS: a lead sentence asserting a uniform property of a list is checked against EVERY member before it is written, and if it fails for any, the claim is narrowed to the members it holds for and the rest are carried by a separate stated argument. TEAMMATE MUST 3, A PLACEMENT CLAIM NAMING THE WRONG HEADING: the suite-floor bullet sits under Success Criteria, not Convention Prerequisites - the nearest heading at or above it is Success Criteria and Convention Prerequisites contains no reading of either file af19d53 touched. CLASS: a placement claim names a heading and the heading is re-derived by walking upward to the nearest ## line, never by recalling which of two neighbours it was; the residual that nothing detects a WRONG section name is realised rather than hypothetical, so both adjacent sections now carry a disambiguating clause naming which suite obligation lives there - which also discharges the audit's second nit. TEAMMATE MUST 4, AND THE MECHANISM IS RECOVERED RATHER THAN MARKED UNEXECUTED: the awk-prefix paragraph said a bare sed range over the whole file returned 1 instead of 0, and with the ANCHORED address that is false at eleven shas from b3be433 to fbc2ea0 - whole-file and body-scoped readings are equal at every one, 0/0 through dfae038 and 2/2 from 59cc2ad, and the anchor occurs exactly ONCE whole-file at all eleven, so sed never opens a second range. The address that DOES leak is the BARE TOKEN: the same range on the unanchored token returns 1 whole-file against 0 body-scoped at b3be433, 00b961f and dfae038 and 5 against 2 from 59cc2ad onward, because the bare token recurs in the spec's Version History while the anchored address does not. So corpus and address were conflated and are now separated, the general property is EXECUTED on a six-line printf fixture that prints five lines through a repeated address, and the residual is stated: nothing detects a prose description that attributes a real reading to the wrong command. FOUND WHILE RE-RUNNING, NOT ROUTED BY ANY LEG: THE CODEX-LEG LEDGER IS OUTRUN FOR THE FOURTH CONSECUTIVE REVISION, exactly as its own residual predicted. Re-run at fbc2ea0 with the sha inside the command, both halves read 87: codex 87 against teammate 87, and 6fc71ab is the commit that landed round sixteen's three gating reports. Series extended, no row re-stamped: af19d53 86/86, 09e9307 86/86, fbc2ea0 87/87. The file-count form moves with it, 73/14 at af19d53 to 74/15 at fbc2ea0, and the unit residual's arithmetic is restated over the fbc2ea0 pair. TWO FURTHER STALE SELF-DESCRIPTIONS FOUND THE SAME WAY, both of the same shape and neither in any report: the screen-two-leg walk was described as returning every entry from v1.94 to v1.99 plus this revision's own, and the triage-category walk as returning v1.94 v1.95 v1.96 v1.99 plus this revision's own. Both clauses had been false for two and three revisions - the walks now return v1.94 through v1.99 plus v1.101 v1.102 v1.103, and v1.94 v1.95 v1.96 v1.99 v1.101 v1.102 v1.103, and neither returns v1.104. CLASS: a derivation whose output is a LIST of revision numbers grows by construction, so the list is PRINTED IN FULL at each revision and never described by a rule such as plus this revision's own, which is a prediction about text not yet written. CODEX SHOULD 1, THE REGISTER'S OWN RE-ENTRY RULE APPLIED TO ITS OWN EXCLUSIONS: the awk boundary probe was out on v1.96 having executed it and the fifteenth renderer case out on v1.95 having executed it, while the register's stated rule is that a probe leaves it only for the revision that executes it. Both re-enter, and TEAMMATE SHOULD 3 adds a third member, the docs/-scoped sibling walk, last executed v1.101 at b3be433, which through v1.103 sat outside both the closure and the register - the one arrangement this document defines as a defect in the register rather than a licensed gap. Non-execution since is derived by the Version-History walk the bullets publish: v1.94 v1.96 v1.99 for the awk probe and v1.35 v1.95 v1.96 v1.99 for the de-indentation case, plus this entry, with the non-v1.96 and non-v1.95 hits read as prose about the rows rather than runs. The member cardinal moves SEVEN to TEN, re-derived by a published awk walk over the bullets rather than incremented, and the residual partition moves with it and is stated as a sum: six name an executing revision, three name the commit their reading is stamped at, one names neither, 6 plus 3 plus 1 equals 10. THE NARROWED MUST-2 LEAD IS RE-CHECKED AGAINST EACH NEW MEMBER rather than assumed to extend: v1.103's entry - the ONE entry inside the af19d53..fbc2ea0 interval this stamp move rests on - names the docs/-scoped walk and does NOT name the awk boundary probe or the fifteenth case, so those two are carried by the Version-History walk and explicitly not by the interval argument. Each non-executing walk hit was READ before being classified: v1.94 and v1.99 for the awk probe are a carve-out-sweep classification and a stamp-driver argument; for the de-indentation case, v1.35 is a HOMONYM (the helper's own body-de-indentation rule on extract, a different subject), v1.96 records the round-ten AUDITOR independently reproducing the oracle on both renderers - a second surface's run, which is named rather than swallowed but does not move a stamp the register writes over revisions - and v1.99 argues about row membership. CLASS: a status assignment whose warrant is revision N executed it DECAYS at revision N+1, so it is written as a last-execution stamp and never as an exemption, and the register's default for a carve-out probe is IN; this is the third and fourth member of that axis after the 700c599 stdlib probes and the six legs of screen two. CODEX SHOULD 2, A RESIDUAL ASSIGNING A STAGE TO A SCREEN THAT HAS NONE: the shared residual said each screen tests for a sha on the same line. Screen two does not - its program is the spec's enumeration run verbatim, an awk body-strip piped to one grep -Ei with no sha stage anywhere, and fed three files measured and three files measured abc1234 it returns BOTH. The sha clause is restricted to screen one, whose program carries the !/[0-9a-f]{7}/ filter. Same axis as MUST 2 and the second instance of it in this document. TEAMMATE SHOULD 1: the ten of the eleven readings were published only inside a Version History entry while the body list called them published at their own site, so body-scoped the phrase occurred exactly once, at the pointer. Both grammars are now published at a body site with their command, at three committed blobs: af19d53 whole-file 3 body-scoped 0, 09e9307 6/1, fbc2ea0 6/1. TEAMMATE SHOULD 4 is the same site: BOTH grammars moved at the landing commit and v1.103's self-match note covered only the whole-file one - the body reading moved 0 to 1 because that list line is body text and a Version History line is not. CLASS: a self-counting screen publishes a reading per grammar and its self-match note is written per grammar too. TEAMMATE SHOULD 2, AND THE FINDING IS SHARPER THAN THE REPORT: the handoffs-alone sweep's completeness verdict was TRUE of the body it swept and FALSE of the body v1.103 shipped. At af19d53 the published needle and a widened form BOTH return 4 whole-file and 2 body-scoped and the two body shapes are identical, so the sweep was complete there; at fbc2ea0 the published needle gives 5/1 and the widened form 7/3, and the two extra body shapes - an emphasised form and a hyphenated compound - were written by v1.103's own repair paragraphs. CLASS, and it is the general shape behind a long run of this document's defects: a value sweep whose completeness verdict is published by the same revision that edits the swept text is re-run against the POST-EDIT body before the verdict is written, because the revision's own prose is a new member population the sweep never saw; this subsumes widening the needle, since widening finds shapes that already existed and only re-running finds the shapes the fix introduced. A THIRD FINDING FROM MY OWN RUN, IN NO REPORT AND CONTRARY TO THE ROUND SHEET: fbc2ea0 IS NOT A NEUTRAL FREEZE FOR THIS DOCUMENT. The sheet certified it with git diff --name-only 09e9307 fbc2ea0 -- h-mad handoff printing nothing, which is a true statement about the two roots and says nothing about the REPO-WIDE .py censuses this document also carries and says in as many words are re-derived at every freeze. All five moved, because the commit adds four .py files to that corpus. Re-derived at fbc2ea0: the bare-literal count is 8 on all three forms - grep -rn with --include, git grep on the working tree, and git grep with the sha - so the AGREEMENT the paragraph rests on survives, which is the load-bearing part; the extractor census is still 2, so neither new line is an extractor; git ls-files '*.py' outside the two roots is 415, up from 411; git grep -l for any fence literal is 25 with 6 outside the two roots, up from 24 with 5; git grep -l for the bash literal is 5, up from 4; and the whole 6 to 8 delta is ONE file, the grammar-corpus probe, contributing 2 lines to the per-file split. Every 4e4a00c-stamped reading with its sha inside the command is untouched and is NOT re-stamped; a fbc2ea0 row is added beside each. CLASS: a freeze is cleared PER CORPUS, not per repository, by running each distinctly-scoped census's own command across the interval - the two roots, docs/01-plan/features/ for the ledger, and . for the extractor and .py censuses - and a closure predicate over one corpus can never speak for another. Residual: that enumeration of corpora is by hand, so a corpus nobody has named is invisible to the rule. A FOURTH FINDING, AND IT PARTLY CORRECTS THE THIRD: the pytest COLLECT counts moved too, but NOT because of the probe commit. Re-derived at fbc2ea0 with the published command in both working directories: 2814 from the repository root and 2552 from h-mad/, up 5 on each from 2809/2547 at 6f0ee85. git rev-list --oneline 6f0ee85..fbc2ea0 -- h-mad handoff returns exactly ONE commit, af19d53, and git show af19d53 -- 'h-mad/tests/*' | grep -c '^+def test_' is 5 - the same five that moved the suite floor. fbc2ea0 contributes NOTHING to either figure and that is measured, not assumed: git show fbc2ea0 | grep -c '^+def test_' is 0, and pytest --collect-only over docs/03-analysis/probes collects no tests, because the four probe files are not named test_*.py and default discovery never reads them. So the probe commit moved the repo-wide .py FENCE and FILE censuses and did NOT move the collect counts, and attributing both to one commit would be wrong in one direction - the fence censuses grep every *.py path while the collect counts see only what pytest discovers. Both figures were already register members as pytest collect counts and stay there; the fbc2ea0 pair is published with its accounting rather than re-stamped over the 6f0ee85 pair. REOPENED AFTER DONE, ANNOUNCED FIRST, ON SHEET CORRECTION C10, WHICH CONVERGES WITH THE THIRD FINDING ABOVE AND ADDS ONE PREDICATE THIS DOCUMENT DID NOT HAVE. A SECOND CLOSURE IS NOW STATED OVER THE .py CORPUS beside the h-mad/handoff one, because one closure cannot speak for two: git diff --name-only 74e126f fbc2ea0 -- '*.py' | wc -l is 6, against 0 at dfae038, 2 at af19d53 and 2 at 09e9307. TWO CORRECTIONS TO C10 AS RELAYED, both re-derived here. (a) C10 gives that interval as 0 at dfae038 AND af19d53; it is 0 at dfae038 and 2 at af19d53, the two files being h_mad_assemble_audit.py and its test. So the .py interval broke one commit EARLIER than the freeze under discussion and no round noticed, because nobody had run this predicate - fbc2ea0 contributes 4 of the 6, not all 6. (b) C10 gives h-mad-scoped collection as 2552 unchanged; it is 2552 now and was 2547 at 6f0ee85, so it moved by 5, and the +5 on BOTH the root and h-mad figures is af19d53's five assembler-audit tests, measured above. Unchanged is right against the probe commit and wrong against the figure this document published. THE SWEEP C10 ASKED FOR WAS RUN OVER THE WHOLE BODY AND IS REPORTED WITH ITS COMMAND rather than asserted complete: a newline-collapsed grep for every body command scoped to '*.py' or to the repository root returns five distinct forms - the git ls-files '*.py' scale figure (three sites, one figure), the two grep -rln 'from docsections import' forms (scoped to the two roots, so the h-mad/handoff closure DOES reach them), and the git grep -- '*.py' census forms - and every repo-wide one now carries a fbc2ea0 row beside its blob reading. The extractor census also gains its fbc2ea0 row ON ITS OWN SURFACE rather than only in the neighbouring paragraph, and it is still 2: the two new fence-carrying lines sit inside a probe's string literals rather than in an extraction call, so the narrow three-branch pattern declines them, which is the pattern discriminating on what it was written for and is worth more than the unchanged digit. THE BYTE-IDENTITY QUALIFIER C10 ASKS FOR WAS SWEPT AND IS NOT OWED: every byte-identical claim in this body is scoped to the four documents, to a program, or to a corpus it names, and none says the tree outside the documents is unchanged - the closure sentence already states its corpus as h-mad/ and handoff/ in as many words. POST-EDIT RE-RUNS OF EVERY SCREEN THIS REVISION'S OWN TEXT CAN MOVE, taken on the tree v1.104 ships and REPORTED rather than published as rows: ten of the eleven whole-file 11 body-scoped 4; handoffs published needle 7 whole-file, widened 12 whole-file and 8 body-scoped; the register bullet walk 10, matching the published cardinal; and the four Version-History walks matching the four lists printed in the body. NIT 1: the boundary sentence now carries the file-list derivation inline, so the reader's local cue for the 72 is in the same sentence rather than sixty lines below. Shared precheck run on this revision: PRECHECK: PASS issues=0. ADVISORIES KEPT DELIBERATELY, all pre-existing kinds: the PATH advisories name files Phase 5c creates; the LINEPIN advisory is this document's own recorded census output, exempt on the shape grep's stated rule; the STALESHA advisories are inherited-blob stamps the freeze-sha rule requires to stay at their blob; the COUNT advisories all fall inside the Version History region, where the heuristic reads one long entry as a list. OWED ELSEWHERE, REPORTED NOT EDITED: to the ORCHESTRATOR, the r17 sheet's NEW-ONLY attribution and its 1861157 GLOB expectation are both refuted above, and its FACT 3f prescription to add the fbc2ea0 reading beside the 1861157 figures was INSUFFICIENT on its own because the new_only=0 claim was false at its own sha and needed correcting rather than supplementing. SIBLING DEBT, MEASURED RATHER THAN ASSERTED: my first draft of this line said nothing was owed, and the grep refutes it. At fbc2ea0, grep -cE 'throwaway|heading_differential\.py|grammar_corpus\.py' returns 6 on the design, 2 on the impl-plan and 1 on the spec, so all three still name the probes as throwaways or by bare filename while the tree now carries them under docs/03-analysis/probes/doc-block-exec/ - the naming repair this revision made here is owed on each of those three surfaces, and FACT 7 of the round sheet says so in general terms without naming the sites. Separately to the IMPL-PLAN AUTHOR, who is implementing the same 3f decision: the sheet's NEW-ONLY h-mad/SKILL.md 984 hash line cannot be produced by the committed probe, so an impl-plan invariant that cites it as probe output would be citing something no command prints; the enumeration command is published in the differential bullet here and is the one to point at.
- v1.105: Round eighteen revisions. Freeze sha `cac6edc`, the commit this revision is measured at; HEAD at dispatch was `f6849bb` and `git diff --name-only cac6edc f6849bb` is docs-only (the r18 decision sheet, two `docs/handoffs/` files, `docs/learnings.md`), so no scoped census moved between them and the freeze field is NOT advanced to HEAD. The decision sheet is `docs/03-analysis/doc-block-exec.gating-decision-sheet.r18.md`; the three reports answered are `docs/01-plan/features/doc-block-exec.plan.audit.v88.{teammate,codex,teammate-b}.md`, whose two musts are the SAME TWO on all three legs (three model runs, one shared must set). NO gating claim and NO exit-gate claim: round seventeen was FAIL and the next gate is plan c89. MUST 1, THE MUTATION TOTAL, 81 -> 85 AT BOTH SITES, AND THE CLASS IS THE MISSING DERIVATION RATHER THAN THE DIGIT. Deliverables published `81 mutations / 80 helper-source + 1 SKILL.md` while the design matrix it names as authoritative carries 85 at `cb4fe99` and at `cac6edc`; the row re-derived the SPLIT from the mechanism column and CARRIED the total, so the half that moved was the half with no command. Re-derived here, structurally anchored on the matrix header row and never on a line number: `git show <sha>:docs/02-design/features/doc-block-exec.design.md | awk "/^\\| mutation \\| guard it removes \\(mechanism\\) \\| killed by/{f=1;next} f&&/^\\|---/{next} f&&/^\\|/{n++;next} f{exit} END{print n+0}"` returns 81 at `09e9307` and 85 at `cb4fe99` and `cac6edc`, with 85 distinct names in column 1, and the SKILL.md-target half `grep -c "the mutation targets \`SKILL.md\`"` over the same rows returns 1 at all three, so the split is 84 + 1. Both plan sites carry the command with the sha inside it, and the Measurements carve-out for design-derived contract values is narrowed over its axis: a figure whose source is a SIBLING UNDER `docs/` carries a command and a sha even though the artifact it counts does not exist yet, because a value this plan must MATCH is a reading of another document and the old carve-out removed the only screen that could catch it. MUST 2, THE AC-1.8 PIN RAN THE FILE IT MAY ONLY COLLECT. Implementation Strategy spelled the collect-alone pin `pytest h-mad/tests/test_docsections.py -q`, which RUNS the file, while spec v1.63 makes it collection-only as a contract; the plan carried the premise that makes the spec right in its own Success Criteria (AC-6.4 adds the delegation spy to that file, AC-6.5 requires that node RED under `docsections-delegation-reverted`), so the pin as written was red by construction in the exact state the harness creates. Replaced with the spec exact invocation `[sys.executable, "-m", "pytest", "--collect-only", "-q", "-p", "no:cacheprovider", "h-mad/tests/test_docsections.py"]` from the repository root requiring exit 0, the pin NAMED `test_docsections_imports_when_collected_alone` beside its twin (design 1 / impl-plan 4 / this plan 0 at `cac6edc`), and the residual stated: the pre-existing tests in that file are no longer run in isolation and are covered by the AC-6.4 floor and the 5e module-scoped run. Value grep `collect-only` over this document: 9 occurrences at `cac6edc`, of which the ONE run-form pin was the defect and the other 8 were read and are correct. SHOULD 1, THE REGISTER CONTRADICTED THIS DOCUMENT OWN MEASUREMENTS. The inherited-unverified register listed the Setext member as not re-run while Measurements records v1.104 re-deriving it at `1861157` from a worktree AND cross-checking it at `fbc2ea0` against the second committed probe, and the `files=25`/`files=30` printouts the bullet names were RETIRED by v1.104 rather than carried. The member is restated by what it actually is (`setext_headings=0` on both corpora), its last execution is v1.104, and the partition arithmetic is RE-DERIVED rather than adjusted. The de-indentation heading said `the one case this revision ran`, a deixis that was v1.95 status and no later revision, contradicting its own closing sentence two paragraphs down; it now names v1.95 by number, which is the rule this register states about itself. SHOULD 2, THE TWO CLOSURES BROKE AT THE SAME COMMIT. The `.py` closure was said to break one commit before the `h-mad`/`handoff` closure. Measured: `git diff --name-only 74e126f af19d53 -- h-mad handoff` and `git diff --name-only 74e126f af19d53 -- "*.py"` return the SAME two paths, `h-mad/scripts/h_mad_assemble_audit.py` and `h-mad/tests/test_h_mad_assemble_audit.py`, and both are 0 at `3f70eb3`, so both closures break at `af19d53` and neither precedes the other. The separate-corpus principle is untouched and is why the two are still stated apart. THE SPECIMEN THIS DOCUMENT MEASURES WAS REMOVED FROM THE TREE BY A TOOLING COMMIT, AND THE ACCOUNTING GOES TO N=0. `b39d9dc` deleted the bare `#` in `h-mad/SKILL.md` on the round-seventeen handoff instruction and pinned the removal with `test_h_mad_agent_definitions.py::test_skill_has_no_bare_heading_stub`; this document treated that line as the single live `new_only` member. The sha list is EXTENDED rather than rewritten: `titleless=1` at `a8e0372`, `35698f9`, `cf3a862`, `4e4a00c`, `74e126f` and `fbc2ea0`, and 0 from `b39d9dc` onward, because the specimen existed and was measured and does not have to still exist for the correction argument to hold. Re-run at `cac6edc` with the committed 09-04 probe: TRACKED `files=30 both=292 old_only=82 new_only=0`, `setext_headings=0`, `closing_hash=0 tab_form=0 titleless=0`; GLOB `files=35 both=297 old_only=82 new_only=0`, `setext_headings=0`, `closing_hash=5 tab_form=0 titleless=0`; the enumeration command that printed the member at `fbc2ea0` prints NOTHING at `cac6edc`, and `grep -c "^#$" h-mad/SKILL.md` is 1 at `fbc2ea0` and 0 at `b39d9dc` and `cac6edc`. The v1.104 prediction that the line would be repaired in the tooling batch after the c88 gate is recorded as DISCHARGED rather than left in the future tense. Residual in the words the round decided, so the four documents state it identically: at `cac6edc` the `new_only` set is empty, so `each new_only member is a heading under CommonMark` is vacuously true there; it was verified non-vacuously at `fbc2ea0` (N=1, the `h-mad/SKILL.md` specimen removed by `b39d9dc`) and is exercised by `test_titleless_heading_is_a_new_only_member`. RE-STAMPED PRESENT-TENSE FIGURES, EVERY ONE RE-RUN AND NONE CARRIED, AND EVERY ONE LABELLED COLLECTED RATHER THAN PASSED. `python3.11 -m pytest --collect-only -q -p no:cacheprovider | tail -1` returns 2836 from the repository root and 2574 from `h-mad/` at `cac6edc`, against 2814/2552 at `fbc2ea0`; the +22 is `b39d9dc` and the historic `fbc2ea0` pair keeps its own stamp. `ls h-mad/tests/test_*.py | wc -l` returns 89 at `cac6edc` against 88 at `35698f9`, where the 13/88 sys.path.insert reading is stamped and stays; the SCRIPT_DIR numerator is 13 at BOTH shas, so the convention-not-property conclusion is unchanged and the denominator is the only half that moved. NOBODY WRITES A PASS COUNT FOR THE CURRENT SUITE HERE: the h-mad suite is RED on the committed tree from `b39d9dc` for an impl-plan pin-drift reason that is not this document defect, so 2836 and 2574 are COLLECTION counts and are labelled as such. CONFORMANCE ROWS VERIFIED AND NOT EDITED, each a measured absence rather than an assumption: this document single `OverlappingSubstitution` mention is a bare name in an exception list and states no representation, so it already conforms to the design single tagged `(kind, a, b, offset|None)` list; `intersections` is 0 here; `__suppress_context__` is 0 here and none of the four body AC-3.14 mentions states a chaining assertion, so all four conform to `__cause__ is cleanup_error`; `wire-unconditional` occurs once, in the FR-6 mechanism table, and the carve-out that must name it is the impl-plan. OWED BY SIBLINGS, REPORTED AND NOT EDITED, measured against their bytes at `cac6edc` and not against their working files: the impl-plan `LaunchFailed.__init__` annotates `err: OSError | subprocess.TimeoutExpired` and must widen to include `ValueError`, and it owns the `intersections` split (9 occurrences), the `^#$` returns 1 re-stamp (3 occurrences) and the assembler pin drift; the design owns its 4 `new_only=1` sites and the one-live-instance sentence; the spec has no site for any FACT 3 value (`new_only`, `titleless` and the bare-hash phrasing are all 0 in it). [Appended before DONE, after the last edit: EVERY SCREEN THIS REVISION'S OWN TEXT CAN MOVE, RE-RUN OVER THE SHIPPED BODY. PRECHECK: PASS issues=0 (h_mad_precheck_doc.py --phase plan --root /Users/kimhawk/orca/skills), advisories read and all kept deliberately: the PATH lines name the module, specs and test files Phases 5c-5e create; the LINEPIN line is the two extractor offsets, residual (a) on the shape greps; the STALESHA lines are the historical stamps the freeze-sha paragraph explains, plus cac6edc itself, which is behind HEAD only because HEAD is the docs-only commit carrying the round's decision sheet -- the paragraph now says so in as many words; the COUNT lines all sit in Version History entries, which the enumeration residual exempts. THE REGISTER CARDINAL MOVED AND WAS COUNTED, NOT ADJUSTED: the published walk returns 9 over this body, and the partition is re-derived as 6 revision-named + 2 commit-named + 1 neither = 9, the Setext member having left by being run. The walk's own stop address moved with the list and is republished with it. BOTH PUBLISHED COMMANDS WERE EXTRACTED FROM THE SHIPPED BODY AND EXECUTED, not retyped: the design-matrix derivation block prints total=81/85/85 and skill-md-target=1/1/1 for 09e9307/cb4fe99/cac6edc, byte-for-byte the output published beside it, and the register walk prints 9. VALUE SWEEPS over the newline-collapsed body, body and Version History counted separately because the entries are historical by construction: '81 mutations' body 0 (VH 2), '80 of the helper' 0/0, '85 mutations' body 1 and the carve-out's '`85` mutations' body 1 -- both plan sites carry it; '2814' body 1 and '2552' body 1, each stamped fbc2ea0, against '2836' body 1 and '2574' body 1 at cac6edc; 'collect-only' body 8, ALL of them --collect-only, the run-form pin gone; 'test_docsections_imports_when_collected_alone' body 2 against 0 before; 'this revision ran' body 0. TWO RETIRED STRINGS WERE DESCRIBED RATHER THAN QUOTED after the first draft quoted both -- the run-form pin and the 'one commit before' chronology -- because reproducing a retired string verbatim puts it back into every value sweep of this document, the self-quoting hazard the spec's screen block already paid for once; that is a defect this revision introduced and removed before DONE, found by its own sweep. THE 'the freeze' VOCABULARY WAS PROTECTED RATHER THAN BROKEN: the first draft wrote 'the freeze `cac6edc`' at six sites, which would have given that phrase a second referent against the paragraph binding it to 4e4a00c throughout; all six now read 'the measurement commit `cac6edc`'. [Corrected in reopen 2: 'the freeze `4e4a00c`' is 16 body-scoped on the shipped body and was 17 at cac6edc, so 'unmoved' was right about the digit and wrong about the word -- the rewritten Deliverables cell dropped the superseded `git grep -c 'the mutation targets SKILL.md' 4e4a00c` reading, which carried one of the 17. Dropping it is correct; describing the set as unmoved was not.] TABLE INTEGRITY: the three edited table rows carry exactly 4 unescaped pipes each, which is why the awk table walk is published in a fenced block under Measurements and pointed at from the Deliverables cell rather than escaped into it. FIGURES RE-RUN AND UNCHANGED AT cac6edc, recorded because b39d9dc edits eight *.md files under h-mad/ and an unchanged census over a touched corpus is a measurement rather than an inference: bash fences 73 across 10 files with the any-language control 88; tracked *.md 30 and glob 35; git grep -n '```bash' -- '*.py' 8 and git grep -l '```' -- '*.py' 25; ls h-mad/scripts/*.py 37 with its git ls-files control 37; the SCRIPT_DIR spelling census 13. ONE DISAGREEMENT WITH A REPORT, RECORDED RATHER THAN SILENTLY FOLLOWED: the teammate leg described the matrix delta as five arrivals against one departure including a rewritten field-escape-removed; set-differencing column 1 at 09e9307 and cac6edc gives FOUR arrivals and ZERO departures, so no row was renamed -- a rename would show as one of each and there is none. The digit both legs prescribed is unaffected.] [REOPEN after the first DONE, announced by the orchestrator before the edit per FACT 6, version held at v1.105 and NOT bumped, and every screen this reopen's own text can move re-run before the second DONE. THE MATRIX MOVED AGAIN INSIDE THIS BATCH, 85 -> 86, AND THE STAMP IS THE BATCH RATHER THAN A SHA. The design's r18 revision (v1.110, working tree, NOT at cac6edc) adds one row, intersect-scan-non-overlapping -- the span scan's lookahead form degraded to a plain re.finditer over the escaped key, killed by test_substitute_refuses_overlapping_occurrences_of_one_key (AC-2.7), which the aaab fixture under {aa->X, ab->Y} reds and the abc fixture cannot. So the matrix is 86 rows = 85 helper-source + 1 h-mad/SKILL.md over the tree this batch ships. FOUR PROSE SITES CHANGED AND THREE COMMITTED-SHA READINGS DELIBERATELY DID NOT: the Deliverables total and split (85/84+1 -> 86/85+1, with the batch stamp and the new row's mechanism and killer named), the Deliverables reading list (86 added as a fourth reading beside 81/85/85, SKILL.md-target 1 at all four), the Measurements carve-out list (`85` mutations -> `86`), and the distinct-names sentence (86 over the working design). The derivation block gains TWO working-tree invocations printing batch total=86 and batch skill-md-target=1, run on $D directly rather than through git show, and the three git show lines 09e9307 total=81 / cb4fe99 total=85 / cac6edc total=85 are UNEDITED because they are true at their blobs. The 'was 85 from cb4fe99' history sentence is kept as history and gains the batch reading beside it, which is the narrowed carve-out earning itself inside one round: under the old carve-out this second move would have been as invisible as the first. BODY-SCOPED \b85\b SWEEP, EVERY HIT READ AND CLASSIFIED rather than replaced by pattern: the matrix-meaning sites are the four changed above plus the two committed readings and the history sentence, all kept; the sites that must NOT move and did not are every 6f0ee85 sha, the probe's own OLD-ONLY output line carrying 85, the 0x85 code point in two fixture probes, 'cycle 85' in the substitute rule, and the whole retired 72/85 and 84/85 codex-leg ledger series -- a blind value substitution would have corrupted all five classes, which is why the sweep is published as a classification and not as a count. POST-REOPEN SCREENS, all re-run after the last edit: PRECHECK PASS issues=0, advisories unchanged in kind; the register walk still returns 9 over the shipped body, unmoved because this reopen touches no register member; both derivation blocks extracted from the shipped body and executed, printing 81/85/85 for the committed shas and 86 for the batch, byte-identical to what is published beside them; body counts '85 mutations' 0, '86 mutations' 1, '84 of the helper' 0, '85 of the helper' 1, '`86` mutations' 1. The old impl-plan spelling test_intersecting_spans_need_an_overlapping_scan is 0 in this document and always was, so the rename costs this document nothing. OWED, unchanged by the reopen except in one figure: the impl-plan's mutation-spec row count moves 85 -> 86 with the design, and its AC-2.7 test row takes the design's name.] [REOPEN 2 after the second DONE, announced by the orchestrator before the edit per FACT 6, version held at v1.105, answering the ADVISORY delta review docs/03-analysis/doc-block-exec.plan.delta-review.r18.md (must=3 should=4 nit=2) on this batch's own diff. Every one of its three musts REPRODUCED against the tree and none was a digit. MUST 1, A PUBLISHED COMMAND THAT COULD NOT RUN, AND THE CLASS BEHIND IT. The 8-row derivation was published INLINE in double backticks with `\|` between its alternatives -- markdown-motivated escaping that ERE reads as a LITERAL pipe -- carried no git show and no file operand, and said 'over the same blob', which resolves to the impl-plan blob quoted immediately before and holds 0 of the four ordinals. Verified: the escaped form over the design blob at cac6edc returns nothing with rc=1; the same regex with bare | prints all four. Both prose derivations (this one and the 29-names one) are now a FENCED block naming its blob, with its output pasted, and both were extracted from the shipped body and executed. THE CLASS, over the axis rather than at the two members: every command a revision publishes is extracted from the shipped bytes and run, fenced or inline -- the first draft executed its two FENCED commands and not its two INLINE ones, and the residual is that an inline command can acquire escaping a fenced one cannot, so a fence is preferred wherever a regex carries an alternation. MUST 2, A DEFINED TERM IS BROKEN BY ITS BARE FORM AND MY SCREEN COULD NOT SEE IT. The body binds 'the freeze' to 4e4a00c throughout; three added sites gave it a cac6edc referent. My reopen-1 screen swept TERM PLUS SHA and returned 0, which is true and irrelevant: a defined term is broken bare. All three reworded (two to 'THE MEASUREMENT-COMMIT READING' and 'the `cac6edc` reading', one to 're-derived at `cac6edc`'). Re-swept with the predicate 'the freeze' NOT followed by a backtick: 29 at cac6edc, 32 on the shipped body, and every one of the 32 read in context is either the generic sense or the 4e4a00c sense -- 0 attach cac6edc. ONE DISAGREEMENT WITH THE REVIEWER, RECORDED RATHER THAN ADOPTED: it reports the bare form as 31 -> 36; my predicate gives 29 -> 32. The DELTA differs too (+5 against +3) because three of its five were the sites this reopen removed. The finding is right and its base is a different predicate; both are published so the next sweep can pick one. THE RULE: a defined-term protection is screened on the bare term with each hit's referent read from context, never on term-plus-value. MUST 3, A CROSS-DOCUMENT CLAIM FALSE FOR THE FOURTH DOCUMENT BY THIS REVISION'S OWN MEASUREMENT. 'ALL FOUR DOCUMENTS STATE IT IDENTICALLY' contradicted this entry, which already said the spec has no FACT 3 site; re-measured, new_only / titleless / vacuously are each 0 in the spec. Reworded to the three documents that carry a site, with the spec's absence stated as the measurement it is. The quoted invariant also differed in QUOTE STYLE -- plan single, design and impl-plan double, confirmed by reading all three -- so the plan's copy now uses double quotes and the three strings are byte-identical on the quoted span. SHOULDS, all four applied: the Deliverables cell pointed at 'the FR-6 table below' when that table is ABOVE it, and now names it by lead-in ('the FR-6 wire-mutation table under Implementation Strategy'), which is the rule the same batch installed for the derivation block; the 29-names clause named one of TWO intermediates and now names both (25 at Task 1, 28 at Task 3, printed by a uniq -c over the blob) and publishes OCCURRENCES via grep -o | wc -l -> 2 rather than the old grep -c, which was a LINE count over a body collapsed to one line and returns 1 for any nonzero count; the batch stamp's expiry now has a single stated home at the derivation block, pointed at from the Deliverables cell. NIT applied: the AC-1.8 naming clause stamps its 0 at cac6edc, since the shipped document names that node 3 times. A DEFECT THIS REOPEN INTRODUCED AND REMOVED BEFORE THE THIRD DONE, and it is the self-count rule this document already carries: the first draft of the batch-stamp paragraph PUBLISHED the site counts (5 and 1) while quoting both census needles in the same paragraph, which moved its own figures to 7 and 3 in the act of stating them. The integers are gone; the paragraph states the rule and the command shape, and the readings are taken over the shipped body after the last edit and reported to the round -- which is exactly what the closure paragraph already prescribes for a figure whose corpus is this document. Post-edit readings, taken after the last edit: the two batch-stamp spellings return 5 and 1, six in all. POST-REOPEN-2 SCREENS: PRECHECK PASS issues=0; register walk 9, unmoved; the design-matrix block extracted and executed, printing 81/85/85 and batch 86 with skill-md-target=1 at all four, byte-identical to the published output; the NEW fenced derivation block extracted and executed, printing 2, the 25/28/29 uniq -c table and the four ordinals, byte-identical; body '85 mutations' 0 and '86 mutations' 1. Siblings owed: unchanged -- impl-plan 85 -> 86 and the design's test name on its AC-2.7 row.]
- v1.106: Round nineteen revisions, answering plan audit v89 - codex leg must 1 / should 3 / nit 1 and teammate leg must 2 / should 4 / nit 1, two model families on one freeze, the must sets DISJOINT and the ledger filed by codex only as a should. NO GATING CLAIM, NO TWO-SURFACE CLEAN, NO EXIT GATE: round eighteen FAILED all three phases and the next gate is plan c90. ONE MEASUREMENT COMMIT: `0021c77`, the commit carrying the r18 decision sheet's C7 shared-facts gate, docs-only over `c7a75eb` which is docs-only over `d27d2ce` which landed the six r18 gating reports. `fbc2ea0` and `cac6edc` survive here only as stamps on readings v1.104 and v1.105 took. THE FREEZE-SHA RULE'S THIRD CLAUSE APPLIES UNCHANGED: a reading over this revision's own post-edit body is stamped to the tree v1.106 ships and never to `0021c77`. THE SCOPED CLOSURE IS CLEAN AND THAT IS MEASURED, NOT ARGUED: `git rev-list --oneline cac6edc..0021c77 -- h-mad handoff` returns NOTHING and `git diff --name-only cac6edc 0021c77 -- h-mad handoff | wc -l` is 0, so every `h-mad`-scoped reading v1.105 took at `cac6edc` is inherited at `0021c77` rather than re-stamped; `git diff --name-only cac6edc 0021c77 -- '*.py' | wc -l` is 0 too, so the SECOND closure over the repo-wide `.py` corpus is clean across the same interval and both closures are checked, not one made to speak for the other. RE-RUN ANYWAY RATHER THAN INHERITED SILENTLY, every one at `0021c77` with its command: the bash-fence census 73 across 10 files with its any-language control 88; the tracked `*.md` corpus 30; the glob 35; `git grep -n '```bash' 0021c77 -- '*.py' | wc -l` 8; `git grep -l '```' 0021c77 -- '*.py' | wc -l` 25; `git grep -l '```bash' 0021c77 -- '*.py' | wc -l` 5; the `.py` scale figure `git ls-tree -r --name-only 0021c77 | grep '\.py$' | grep -vE '^(h-mad|handoff)/' | wc -l` 415; the extractor census 2; `ls h-mad/scripts/*.py | wc -l` 37 with its `git ls-files` control 37; `grep -l 'sys.path.insert(0, str(SCRIPT_DIR))' h-mad/tests/test_*.py | wc -l` 13 against `ls h-mad/tests/test_*.py | wc -l` 89 with 48 on the loose spelling; both collect counts, `python3.11 -m pytest --collect-only -q -p no:cacheprovider | tail -1` returning 2836 from the repository root and 2574 from `h-mad/`; and the committed 09-04 heading-differential probe, TRACKED files=30 both=292 old_only=82 new_only=0 setext_headings=0 closing_hash=0 tab_form=0 titleless=0 and GLOB files=35 both=297 old_only=82 new_only=0 setext_headings=0 closing_hash=5 tab_form=0 titleless=0 - byte-for-byte the `cac6edc` reading. CODEX MUST 1, TWO ZEROES WERE ONE ZERO TOO FEW AND THE HISTORICAL ONE WAS TRUE. The bullet explained the `1861157` titleless zero as "the shape had not been looked for correctly". Measured per commit with `git show "${s}:h-mad/SKILL.md" | grep -c '^#[[:space:]]*$'`: `a469493` 0, `1861157` 0, `bea1b60` 1, `fbc2ea0` 1, `cac6edc` 0 and 0 at every commit through `0021c77`. The specimen DID NOT EXIST at `1861157`, so that zero is a TRUE zero of an empty corpus and the explanation was a causal claim with no evidence behind it. THREE zeroes are now separated by name, each with what makes it that kind: the `1861157` titleless zero (the member had not been written yet - `bea1b60` writes it), the `cac6edc` titleless zero (the member existed and was deleted by `b39d9dc` as a defect in its own right), and the TRACKED `closing_hash` zero (a true reading of the corpus this feature reads, mis-narrated through v1.93 as an absence in general while the glob corpus carried 5). CLASS: an explanation of a zero is a claim about WHY the set is empty and is itself a measurement, established by running the corpus predicate at the shas around it, never by picking the most available story; the three zeroes above differ only in that measurement. CODEX SHOULD 1 IS THE SAME PARAGRAPH AND IS FIXED WITH IT: "the softened set is empty again on both corpora" contradicted the same sentence's own `closing_hash=5` on GLOB, so the conclusion is restricted to TRACKED and the GLOB half is stated as what it is - the five `# pytest cache directory #` lines in untracked `.pytest_cache/README.md` artifacts, a shape that has never been empty on that corpus and is not evidence about the corpus this feature reads. TEAMMATE MUST 1, THE CODEX-LEG LEDGER WAS OUTRUN FOR THE FIFTH CONSECUTIVE REVISION AND v1.105 DID NOT RE-RUN IT - the one thing the paragraph's own rule forbids without an entry recording the non-run, and v1.105's entry records neither a run nor a non-run. Both halves re-derived at `0021c77` with the sha inside the command, one run per sha, no row re-stamped: `1cbddb7` 72/83, `700c599` 72/83, `8c6539a` 72/84, `b3be433` 72/84, `00b961f` 72/85, `dfae038` 72/85, `3f70eb3` 72/85, `af19d53` 86/86, `09e9307` 86/86, `fbc2ea0` 87/87, `cac6edc` 88/88, `ccd8ebd` 88/88, `bc4688e` 88/88, `093c3ee` 88/88, `d27d2ce` 89/89, `c7a75eb` 89/89, `0021c77` 89/89. The deixis "this revision's one measurement commit" beside `fbc2ea0` is rewritten to "v1.104's measurement commit" - it was v1.104's when written and became a second answer to *measured at* the moment v1.105 shipped without touching it. THE RESIDUAL FIRED TWICE MORE AND BOTH ARE NAMED: `ccd8ebd` landing the r18 batch did not move the pair, and `d27d2ce` - the commit landing round eighteen's own six gating reports - took it 88/88 to 89/89, which is the fifth consecutive round in which the round's own reports outran the reading. The unit residual's arithmetic is re-derived over the `0021c77` pair: the `grep -c` form returns 76 codex / 17 teammate there, so a reader taking 89/89 for a file count is off by 89-76 and 89-17. TEAMMATE MUST 2, A CARDINAL OVER A SERIES THAT GROWS BY CONSTRUCTION: "the series below reports only the eight shas it was run at" sat four lines above a series carrying ten, and the series now carries seventeen. The cardinal is DELETED rather than re-derived - a free-standing integer over a list this same revision extends is the defect, and "the shas listed below" is checkable by reading the list, which is what the cardinal was pretending to save the reader. CLASS, stated over the axis: no summary of an enumerable surface in this document is written as an integer unless the integer is arithmetic over that surface on the same page; where it cannot be, the summary names the surface instead. CODEX SHOULD 3, THE VERIFICATION-STATUS LAYER WAS NOT ADVANCED WITH v1.105: the INHERITED-UNVERIFIED register still said "at `fbc2ea0`, the commit v1.104 is measured at" after v1.105 shipped. Re-stamped to `0021c77` and the move discharged by the interval argument RUN rather than recalled: `git rev-list fbc2ea0..0021c77` returns 19 commits, of which exactly TWO touch any of the four feature documents by `git diff --name-only <sha>^ <sha> -- <plan> <spec> <impl-plan> <design>` run per commit - `cb4fe99` (4 paths, the v1.104 batch) and `ccd8ebd` (4 paths, the v1.105 batch) - and 0 at each of the other seventeen. Only this document's own revisions can execute a member, so the property the stamp needs rests on the v1.104 and v1.105 entries, and v1.105's records a PARTIAL run of the `docs/`-scoped walk and no other member, which the register already states. TEAMMATE SHOULD 1, AN INVERTED POINTER IN THE ONE PLACE THE ROUND HAD ALREADY DECLARED CLOSED: the ledger paragraph in Next Steps said "the register below" and the register is under Measurements, ABOVE it. Repaired by naming the heading rather than the direction. RESIDUAL, EXACT AND WHY THE SWEEP CANNOT BE BY VALUE: at the blob v1.105 shipped the phrase occurs FIVE times and FOUR were correct - `git show ccd8ebd:<doc>` returns 5 by `grep -c` and 5 by the newline-collapsed occurrence form alike, the fifth being the inverted one in Next Steps - so each site is checked against its own position; the check is `grep -n 'register below' <doc>` compared against the line the register's own lead sentence sits on. v1.106 repairs the inverted site AND retires one correct pointer at the re-run enumeration in favour of the heading name, so the live-pointer population is THREE over the body this revision ships. Neither 5 nor 3 is four: the cardinal this clause carried at first DONE matched no reading of this document at any commit, and it was wrong because I wrote it from a truncated `head -4400` grep instead of from the whole file. TEAMMATE SHOULD 2, THREE REPO-WIDE `.py` FIGURES SAT OUTSIDE BOTH THE RE-RUN ENUMERATION AND THE REGISTER - the arrangement this document itself calls a defect in the register rather than a licensed gap. The `fbc2ea0`..`cac6edc` enumeration named only the two fence censuses; the `.py` scale figure 415, the bash-fence-bearing `.py` FILE count 5 and the extractor census 2 were not named. All three re-run at `fbc2ea0`, `cac6edc` and `0021c77` and all three are unchanged, so this was an accounting gap and no published value was wrong. The enumeration now names five and the corpus is named beside each. TEAMMATE SHOULD 3, THE SECOND CLOSURE'S SERIES STOPPED AT `fbc2ea0` WHILE ITS OWN RULE SAYS IT IS RE-RUN AT EACH FREEZE: `git diff --name-only 74e126f <sha> -- '*.py' | wc -l` returns 0 at `dfae038`, 2 at `af19d53`, 2 at `09e9307`, 6 at `fbc2ea0` and 8 at `cac6edc`, at `bc4688e` and at `0021c77`. THE REPORT'S OWN MECHANISM IS CORRECTED WHILE ITS FINDING STANDS: `git diff --name-status b39d9dc^ b39d9dc -- '*.py'` shows b39d9dc ADDS ONE file (`h-mad/tests/test_h_mad_agent_definitions.py`, status A) and MODIFIES three, and the +2 is that added file plus `h-mad/tests/test_hmad_dispatch_exec.py`, which already existed at `fbc2ea0` and whose FIRST change since `74e126f` is `b39d9dc` - a name-only diff counts changed names, not added files, and reading it as "adds two files" would be wrong about one of the two. The enumeration is re-worded as a reading at its own sha and the conclusion re-checked rather than carried: neither name enters the bash-fence-bearing `.py` list, which stays 5, and the any-fence `.py` file list is IDENTICAL at `fbc2ea0` and `cac6edc` by `diff` of the two lists. TEAMMATE SHOULD 4, THE BATCH STAMP HAS COME DUE AND IS DISCHARGED: the r18 batch landed as `ccd8ebd`, so the two working-tree matrix invocations are re-run as `git show ccd8ebd:<design>` and give total 86, distinct names 86, `SKILL.md`-target 1 - identical to the published batch reading, so only the form was owed. The committed-blob series is 81/81/1 at `09e9307`, 85/85/1 at `cb4fe99`, 85/85/1 at `cac6edc`, 86/86/1 at `ccd8ebd` and 86/86/1 at `0021c77`. [SUPERSEDED BY REOPEN 1 BELOW, BRACKETED RATHER THAN DELETED SO THE SEQUENCE STAYS READABLE: at first DONE this entry said NO WORKING-TREE READING OF THE DESIGN IS TAKEN THIS ROUND, because a design author was revising that file to v1.111 in parallel and the r19 routing left the kind-selection matrix row undecided, so a reading taken then would have been of a file mid-edit - the r18 collision repeating. That held until the design shipped; reopen 1 then took the reading, and THE SERIES DOES NOT END AT 86: it ends at 87/87/1 over the design this batch ships, which is the total a 5c or 5d implementer sizes `doc_block_exec.json` from. This entry answers the mutation total ONCE, at 87.] The figure is therefore published at `0021c77` and carries an explicit re-derivation obligation at the r19 batch commit. CODEX SHOULD 2, THE DELIVERABLES CENSUS ASSIGNED ONE DISTINCT-NAME COUNT TO THREE DIFFERENT READINGS: it said "85 distinct names in column 1 at each" across readings whose totals are 81, 85 and 85, contradicting the authoritative walk under Measurements which already published 81 at `09e9307`. Corrected to per-reading pairs, total and names given together at every sha, which is what makes "no row is duplicated" checkable at each rather than at one. CODEX NIT, A WALK DESCRIBED BY THE WRONG STOP ADDRESS: the register-walk residual said its stop address is the residual paragraph's opening while the published `awk` stops at the departure paragraph's bolded lead. The description now quotes the address the command actually carries. TEAMMATE NIT, A POST-EDIT RE-RUN THAT v1.105 OWED AND DID NOT TAKE: the unanchored `git ls-files` carve-out reading was published at 24 (`8c6539a`) and 25 (`b3be433`) and the paragraph states that this figure is stamped to the working tree each revision produces; the v1.105 body moved it and no reading was published. Re-run on the tree v1.106 ships and published as a reading, with its two line-anchored siblings re-run beside it. EVERY SELF-COUNTING SCREEN WAS RE-RUN LAST, PER INSTRUMENT, AFTER THE FINAL EDIT, and the readings are reported rather than published where the instrument counts its own text. PRECHECK on this revision: PRECHECK PASS issues=0. ADVISORIES KEPT DELIBERATELY, all pre-existing kinds: PATH advisories name files Phase 5c creates; the LINEPIN advisory is this document's own recorded census output, exempt on the shape grep's stated rule; STALESHA advisories are inherited-blob stamps the freeze-sha rule requires to stay at their blob; COUNT advisories fall inside the Version History region, where the heuristic reads one long entry as a list. OWED ELSEWHERE, REPORTED NOT EDITED [UPDATED AT REOPEN 1, because the condition this clause was written against resolved inside the revision]: to the DESIGN author, the matrix total published here is 87 over the design this batch ships, `git show ccd8ebd:<design>` 86 surviving only as the discharged previous batch reading, and the whole set of walks is re-run as `git show <r19-batch-commit>:<design>` by the first revision written after that batch lands; to the ORCHESTRATOR, the plan ledger figure moved AGAIN by the orchestrator's own commits inside this round (`d27d2ce` took it to 89/89) and will move again the moment the r19 reports land, which is the figure's stated nature and not a defect. TWO FURTHER STALE SELF-DESCRIPTIONS FOUND WHILE RE-RUNNING THE SCREENS, IN NO REPORT AND OF THE SHAPE v1.104's OWN ENTRY DECLARED A DEFECT: the two register bullets whose non-execution is derived by a Version-History walk each described that walk's output as a list of revision numbers "plus this revision's own entry". Whether the current entry appears depends on what that entry happens to say, so the description was true of v1.104, false of v1.105 and false again here - v1.106 names neither needle. Both lists are now PRINTED IN FULL as the walks return them and were re-run after the last edit: the boundary-probe walk returns v1.94, v1.96, v1.99, v1.104; the fifteenth-grammar-case walk returns v1.35, v1.95, v1.96, v1.99, v1.104, v1.105. The two lists differ at their tails, which is why each walk is run rather than one inferred from the other. THE BATCH-STAMP GOVERNED SET IS NOW EMPTY AND IS PUBLISHED AS A READING WITH ITS SELF-MATCH AS ARITHMETIC: the needle returns 1 over the shipped body and its single occurrence is the quoted spelling in the paragraph defining it, so the governed set is 1 - 1 = 0. The obligation is NOT deleted with the set - the r19 batch lands a design revision that may move the matrix, so the spelling re-arms for the next revision that reads a sibling before that batch commits. The sibling spelling that marks a reading over THIS document's own post-edit body is a different thing, returns 1 over the body this revision finally ships (its one occurrence being the paragraph's own quotation of it, so the live set is 0), and is excluded on purpose; conflating the two is how a stamp count went wrong before. POST-EDIT RE-RUNS, TAKEN LAST AND PER INSTRUMENT ON THE TREE v1.106 SHIPS, REPORTED RATHER THAN PUBLISHED WHERE THE INSTRUMENT COUNTS ITS OWN TEXT: the unanchored carve-out reading 27 (published at its site, since that figure's stated provenance IS the working tree each revision produces and v1.105 owed it); its two line-anchored siblings 15 and 13; the register bullet walk 9, matching the published cardinal, which is therefore NOT adjusted; the two batch-stamp spellings 1 and 2 at that moment [SUPERSEDED: 7 and 1 on the body finally shipped - see the reopen-3 readings at the end of this entry]; the inverted-pointer needle read PER UNIT as well as per grammar, because the two disagree whole-file: body-scoped 5 lines and 5 occurrences, whole-file 6 LINES but 7 OCCURRENCES, the gap being that this Version History is one line per entry and this entry carries the phrase twice. The body partition is 3 live pointers (every one correct, every one preceding the register's lead sentence) plus 2 quotations of the phrase in the residual that publishes the reading, and 5 + 2 = 7 whole-file. A whole-file `grep -c` on this document is a LINE count and under-reports by the number of repeats inside any entry, so every whole-file self-count here is taken as an occurrence count; and the closure sha, whose pair is restated PER UNIT at the end of this entry rather than as the bare `70 and 54` first published, since those two were line counts. THE POINTER POPULATION FELL FROM FOUR TO THREE and not from four to four: v1.106 also retired a correct pointer at the re-run enumeration in favour of the heading name, which a value sweep run against the pre-edit body would have missed. THE SETEXT MEMBER STAYS OUT OF THE REGISTER because v1.106 executed BOTH of its probes at 0021c77 rather than inheriting them: the committed census prints [glob] files=35 setext_headings=0 and [tracked] files=30 setext_headings=0, and the committed 09-04 differential prints setext_headings=0 on each corpus with the same two file counts beside them. Running them over a closed interval was not redundant - the closure predicate speaks about paths and the census about contents, and the first implies the second only if the corpus is exactly those paths. ONE CORRECTION TO AN AUDIT REPORT'S MECHANISM, THE FINDING STANDING: the teammate leg attributed the changed-.py count moving 6 to 8 to b39d9dc "adding two .py files to that corpus". git diff --name-status b39d9dc^ b39d9dc -- '*.py' shows ONE addition (h-mad/tests/test_h_mad_agent_definitions.py) and THREE modifications; the second name entering the count is h-mad/tests/test_hmad_dispatch_exec.py, which existed at fbc2ea0 and enters only because b39d9dc is its first change since 74e126f. A --name-only diff counts changed NAMES and reading its growth as additions is wrong about one of the two; the count, the series and the conclusion are unaffected. REOPENED AFTER DONE, ANNOUNCED FIRST, AND WITHOUT A VERSION BUMP because a reopen inside one revision is recorded in that revision's own entry rather than by a new number - the same form v1.104 used when it reopened on a sheet correction. THE TRIGGER: the design shipped v1.111 with the kind-selection rendering row ADDED, so the mutation matrix moved 86 to 87 while this revision was already DONE. The three walks re-run against the WORKING-TREE design (v1.111, the file no longer moving this round) return batch total=87, batch names=87, batch skill-md-target=1; set-differencing the column-1 names at ccd8ebd against the shipped design shows ONE arrival and ZERO departures, the arrival being intersect-kind-ignored, killed by test_cli_subst_overlap_detail_lines (AC-4.1/4.3) - THIS SENTENCE NAMED test_substitute_refuses_intersecting_spans AT REOPEN 1 AND IS CORRECTED HERE RATHER THAN LEFT TO THE REOPEN-2 PARAGRAPH BELOW, because reopen 2's own screen for that name was BODY-SCOPED and this entry is not in the body, so the rule reopen 2 writes was shipped one paragraph above an uncorrected instance of itself; an implementer taking the row's `test` key from this line would have written a node ID that cannot go red under the mutant. The 0021c77 reading of 86 is NOT edited and is not wrong - 0021c77 carries design v1.110 and the row arrives with v1.111 - which is exactly the shape v1.105 shipped one round earlier (85 at its measurement commit, 86 over the design its batch shipped), so the batch-stamp form is re-used rather than invented. Sites moved: the fenced derivation block regains its three working-tree invocations; the paragraph that declined a working-tree reading is replaced by one that takes it and states the ordering reason (the reading was taken AFTER the sibling shipped, and a mid-edit reading stays inadmissible because nothing tells the reader which edit it caught); the Deliverables cell moves to 87 with the split 86 + 1, the SKILL.md-target count holding at 1 across the move being what derives the split rather than restating it; the totals-and-names pair list gains 87/87 and its cardinal moves five to six; the carve-out-narrowing paragraph records the figure's THIRD consecutive move; and the register's sibling-walk bullet records the extra re-derivation. THE CARVE-OUT LIST THAT NAMED THE MUTATION TOTAL AS A BARE INTEGER IS THE ONE SITE THE VALUE SWEEP CAUGHT: a newline-collapsed grep for 86 over the body returned one live miss outside the matrix passages, the contract-value carve-out reading "29 names, 86 mutations, 8 rows"; the integer is replaced by the head noun, because a figure that has moved at three consecutive design revisions cannot sit as a bare digit inside the very carve-out that was narrowed to stop this drift. THE BATCH-STAMP GOVERNED SET WENT EMPTY AND WAS RE-ARMED INSIDE ONE REVISION, and both readings are kept because the transition is the evidence: the needle read 1 with the governed set 1-1=0 after the r18 discharge, and reads 7 with the governed set 7-1=6 now, the six enumerated by surface (three in the Deliverables cell, three under Measurements) so that the count checks the enumeration and the enumeration checks the count. All six expire together at the r19 batch commit into git show <r19-batch-commit>: readings. EVERY SELF-COUNTING SCREEN WAS RE-RUN AGAIN AFTER THE REOPEN EDITS, LAST AND PER INSTRUMENT, and one moved: the batch-stamp spelling 1 to 7. Unmoved and re-verified at that moment: the unanchored carve-out reading 27, its two line-anchored siblings 15 and 13, the register bullet walk 9, and both Version-History list walks. [SUPERSEDED BY REOPEN 3 for the three readings that later moved or were mis-united: the sibling spelling, the inverted-pointer needle and the closure-sha pair - final values at the end of this entry.] Precheck re-run on the reopened body: PRECHECK PASS issues=0. REOPENED A SECOND TIME, ANNOUNCED FIRST, STILL WITHOUT A VERSION BUMP, and the trigger is a killer binding I carried from the design and should have checked before writing it. The design bound intersect-kind-ignored to test_substitute_refuses_intersecting_spans; the impl-plan author caught it at collection and the orchestrator verified it. RE-VERIFIED HERE AGAINST THE TREE RATHER THAN ACCEPTED ON RELAY, because a relayed correction is as much a claim about the tree as the thing it corrects: the impl-plan's acceptance list defines that test as raising OverlappingSubstitution with pairs == [("intersect", "ab", "bc", 1)] and the block text unchanged, and states in as many words that it asserts the exception DATA and nothing about an emitted line, the renderer landing in Task 4; the same document defines test_cli_subst_overlap_detail_lines as the subprocess site where the SUBST_OVERLAP detail lines are asserted VERBATIM, and its own matrix paragraph already records the design's binding as wrong and owed back. So a renderer-only mutant leaves the Task 2 test green and the row would have been scored as caught by a test that cannot see it. Both plan sites naming the killer are rebound to test_cli_subst_overlap_detail_lines (AC-4.1/4.3) and each states the retired name once, as the thing being corrected. THE CLASS, over the axis rather than at this row: a killed by binding is a claim that ONE named test fails under ONE named mutation, it is settled by reading what that test asserts, and it is NEVER inherited from a sibling document on the strength of the row's prose - which is exactly what I did when I copied the design's parenthetical, including its claim that the fixture asserts the exact detail line. A row whose named killer cannot go red under its own mutant is worse than a row with no killer, because the matrix then reports coverage the suite does not have. RESIDUAL, EXACT: this document cannot execute either test at Phase 5b, so the binding rests on the impl-plan's acceptance definitions and not on a run, and it is re-checked when Phase 5c makes both tests real. SCREENS THE REBIND CAN MOVE, RE-RUN LAST: the retired killer name reads 2 body-scoped, both inside the two correction sentences; the new killer name reads 2; the row name reads 3; the sibling row's killer test_substitute_refuses_overlapping_occurrences_of_one_key is untouched at 1; and no published count in this document ranges over test names, so nothing else moves. Every other instrument was re-run and none moved: unanchored carve-out 27, line-anchored siblings 15 and 13, register bullet walk 9, batch spelling 7 with governed set 7-1=6, both Version-History list walks unchanged. [SUPERSEDED BY REOPEN 3: the sibling spelling, the inverted-pointer needle and the closure-sha pair are restated per unit at the end of this entry.] Precheck re-run after the second reopen: PRECHECK PASS issues=0. REOPENED A THIRD TIME, ANNOUNCED FIRST, STILL NO BUMP, and every one of the four musts is in THIS ENTRY rather than in the body - which is the finding, not an accident of where they landed. The advisory delta review of the v1.106 diff reproduced every tree-derived figure this revision adds (the ledger at all seventeen shas, the matrix at all six readings including 87/87/1, the specimen series, the changed-.py series, the three previously-unenumerated repo-wide .py figures, both closure predicates, the committed differential, the carve-out 27 with its siblings 15 and 13, the register walk 9) and found the defects only here. THE ROOT IS ONE THING SAID FOUR WAYS: every screen I re-ran at reopen 1 and reopen 2 was BODY-SCOPED, and this entry is not in the body, so a rule written in the body shipped one paragraph above an uncorrected instance of itself. (1) The pre-reopen sentences declaring that no working-tree design reading was taken and ending the series at 86 are bracketed as superseded and the entry now answers the mutation total ONCE, at 87 - the number a 5c or 5d implementer sizes the spec from; the owed-elsewhere clause that still handed the design author 86 is updated with it. (2) The reopen-1 record that re-published the retired killer binding is corrected in place to the Task 4 CLI test. (3) The inverted-pointer residual's population, which said four with three correct, is corrected to FIVE with FOUR correct at the blob v1.105 shipped and THREE live pointers over the body this revision finally ships; neither 5 nor 3 is four, and the four came from my writing the cardinal off a truncated head -4400 grep instead of the whole file. (4) The whole-file figure for that needle was a LINE count presented as an occurrence count. THE UNIT RULE IS NOW STATED IN THE BODY AND GOVERNS EVERY SELF-COUNT HERE: this document's Version History is one line per entry, so a whole-file grep -c under-reports by the number of repeats inside any entry, and a whole-file reading is taken as an OCCURRENCE count; a body-scoped grep -c stays correct only while the body keeps its hard wraps, which is a property of this body and not a theorem. FINAL PER-UNIT READINGS, TAKEN LAST, WHOLE-FILE AS WELL AS BODY-SCOPED, AND THESE SUPERSEDE EVERY EARLIER READING IN THIS ENTRY: the inverted-pointer needle body 5 lines / 5 occurrences (3 live pointers + 2 quotations) and whole-file 6 lines / 7 occurrences; the batch-stamp spelling body 5 lines / 7 occurrences with the governed set 7-1=6 and whole-file 7 lines / 10 occurrences; the sibling spelling body 1 line / 1 occurrence, that one being the defining paragraph's own quotation so the live set is 0, and whole-file 3 lines / 3 occurrences - it read 3, then 2, then 1 as this revision's own edits moved it, which is why it is re-run after the LAST edit and not after the last edit that seemed relevant; the closure-sha needle body 54 lines / 55 occurrences and whole-file 70 lines / 122 occurrences, the whole-file pair first published as a bare 70 and 54 that were both line counts. Unchanged and re-verified: the unanchored carve-out 27, its two line-anchored siblings 15 and 13, the register bullet walk 9, and both Version-History list walks. SHOULDS AND NITS, BOTH MINE, BOTH CLOSED: the register-cardinal paragraph carried v1.105's deixis into v1.106 and now names v1.105 as the revision the decrement belongs to and states v1.106's 9 as a re-run that did not move; the Deliverables sentence claiming the design SHIPPED the wrong killer is corrected to the design's FIRST DRAFT, since the shipped design names the Task 4 test - verified by re-running the matrix walk over it - while the wrong name lived in an uncommitted intermediate. THE REVIEW ASKED ME TO CITE THE DESIGN'S OWN VERSION HISTORY AS THE SURFACE RECORDING THAT FIRST NAME, AND I DID NOT, BECAUSE THE TREE DOES NOT SUPPORT IT YET: grep -c 'first killer named' over the design returns 0 and that file was still being written while I measured, so the claim rests only on what I could run - the shipped row's killed by column - and the citation is owed back to the design rather than asserted here. Nit 1: the identical-lists claim now carries the cut -d: -f2- form, because git grep -l <rev> prefixes every path with the rev and the diff a reader would reach for reports all 25 lines differing. Nit 2: the ledger sentence conflated touching the corpus with moving the figure in a paragraph whose subject is that distinction; it now names both commits from git log --oneline cac6edc..0021c77 -- docs/01-plan/features/ and says which one moved the pair. Precheck re-run after the third reopen: PRECHECK PASS issues=0.
- v1.107: **The mutation-matrix total and BOTH halves of its split, re-derived rather than incremented, and the round-nineteen batch stamp discharged.** Cross-document sweep found this document one design revision behind: it carried 87 mutations = 86 helper-source + 1 h-mad/SKILL.md while D6 and D7 had moved the design's matrix. Re-derived with the document's own published walk over the design at the current tree, never by adding 1 and never from a sibling's figure: 88 total, 88 distinct names, SKILL.md-target 1, so the split is 87 + 1. Both halves moved and both are edited; a headline-only fix would have left the split not summing to it. TWO NEW COMMITTED-BLOB READINGS, 7fc5f94 (design v1.111) and da03ca5 (design v1.112 + v1.113), added to all three walks in the derivation block, which now reads seven shas. THE ROUND-NINETEEN BATCH STAMP CAME DUE AND IS DISCHARGED: v1.106 published 87 as a working-tree reading over the design's uncommitted post-edit body; that batch landed as 7fc5f94, and git show 7fc5f94: through the same three walks returns 87 / 87 / 1, identical to the working-tree triple, so only the FORM was owed and no value moved. All SIX live batch stamps the previous revision enumerated are discharged, the three subjectless working-tree invocations are DELETED rather than reworded, and the governed set is empty again: the needle 'this batch ships' returns 1 over this body, its own quoted spelling, so 1 - 1 = 0. THIS REVISION ARMS NO NEW STAMP, because da03ca5 was committed before it was authored, so 88 is a blob reading from the moment it is written. THE 87 -> 88 MOVE IS THE FIRST THAT IS NOT PURE ADDITION AND THE SET-DIFFERENCE SAYS SO: column-1 names at 7fc5f94 vs da03ca5 give TWO arrivals (substitution-result-not-passed-to-run_block, timeout-upper-bound-removed) and ONE departure (preamble-composed-with-unsubstituted-text) while the total moves by ONE, because a rename shows as one of each and is invisible to the count. Arrivals-minus-departures is what reconciles with the total; reading two arrivals as two new rows over-counts by one, and reading the departure as a deleted guard reports a coverage loss that did not happen. The arriving names are published and their mechanisms and killers are NOT restated, because the design owns them. POST-EDIT SCREENS, all run over the shipped body after the last edit: the derivation block was EXTRACTED from the shipped bytes and executed, and its 21 output lines are byte-identical to what is published beside it; body-scoped newline-collapsed value sweeps return '87 mutations' 0, '86 of the helper' 0, 'all six readings' 0, '86 + 1 rather than 85 + 2' 0, 'batch total=' 0, 'r19-batch-commit' 0, against '88 mutations' 1, '87 of the helper' 1, 'all seven readings' 1, '87 + 1 rather than 86 + 2' 1. A DEFECT THIS REVISION INTRODUCED AND REMOVED BEFORE DONE, and it is the self-count hazard this document already carries: three of my own added sentences used the spelling 'this revision ships', which is a NEEDLE this document publishes a figure for, moving it from 1 to 4 in the act of writing prose that meant something else entirely; all three now read 'before this revision was authored' and both needles return 1. THE FREEZE MOVED UNDER THIS REVISION AND IT IS RECORDED RATHER THAN PAPERED OVER: the dispatch froze HEAD at 0bfcaa7, where the design matrix walk returns 87 because the design's revision was still uncommitted; da03ca5 landed it mid-revision, and every reading here is stamped at da03ca5 or earlier committed blobs, none at 0bfcaa7.
- v1.108: D10 ANSWERED, AND THE ANSWER IS NEITHER OF ITS TWO OPTIONS - NO `88` MOVED IN THIS DOCUMENT, AND NOTHING WAS INCREMENTED. §D10 asks whether the per-task spec split is the intended shape (its option 1) or whether `doc_block_exec.json` is meant to hold all 88 and a merge is owed (its option 2). Both are refuted by the same command run at `6a1693c`. The settling measurement is a NAME-LEVEL set comparison, not a row count, because a row count cannot tell a staged file from a wrong one: the design-matrix walk this document already publishes returns total 88 / distinct names 88 / `SKILL.md`-target 1 at `da03ca5` AND at `6a1693c`, and `git log --oneline da03ca5..6a1693c -- docs/02-design/features/doc-block-exec.design.md` is EMPTY, so the design has not moved since the blob this cell is stamped at and the `88` in it is correct and unmoved. On disk the two spec files hold 32 and 28 rows, their name sets are DISJOINT (`comm -12` over the sorted name lists -> 0), their union is 60 distinct names, and 58 of the 60 are members of the 88-name matrix. So the two files are disjoint SLICES of one 88-row population that is 58/88 realised because Tasks 4 and 5 have not run - which is neither "the split is the intended convention" nor "a merge is owed". Option 1 is refuted because the naming does not partition by any stated rule: Tasks 1-2 sit in the UNQUALIFIED `doc_block_exec.json` while Task 3 got a suffixed file, so there is no convention, only an asymmetric staging artifact. Option 2 is refuted because the cell`s 88 was never a claim about that file`s current row count - it is a blob reading of a sibling design table, explicitly stamped as one - so no merge follows from it and no `+2` is owed anywhere in this document. THE EXACT TWO ROWS THAT ARE NOT MATRIX MEMBERS ARE NAMED RATHER THAN COUNTED, and they are OWED BY THE DESIGN, not absorbed here: `comm -23` of the spec-name union against the matrix names returns `kill-skipped-after-collect-failure` and `stderr-not-closed`, the two rows D9 added at 5e. The design owns the matrix; a plan that quietly bumped 88 to 90 would publish a total no design surface derives. Recorded as owed elsewhere and NOT edited into this document`s total. THE DELIVERABLE ROW NOW NAMES THE POPULATION AND ITS REALISATION SEPARATELY, which is the actual defect §D10 exposed: through v1.107 the row named ONE file as the realisation of FR-1..FR-5 while two exist, so a reader checking the deliverable against the tree found a 32-row file where the cell says 88 and had no way to tell an incomplete file from a wrong one. The row now names both paths, states the union arithmetic over the files that exist with the command that derives it, and says which task each slice came from. A NUMBER THAT WILL DRIFT BY CONSTRUCTION SAYS SO AND SAYS WHEN: 58/88 is not a target and not a floor - it rises when Tasks 4 and 5 land and is re-derived at every freeze by the published walk, never incremented, for the same reason the matrix total is re-derived rather than carried. BOTH DIFFERENCE DIRECTIONS ARE RUN, NOT SUBTRACTED, AND THE NAIVE SUBTRACTION IS WRONG: the matrix carries 30 names no spec file realises (len(matrix - spec)), not the 88 - 60 = 28 the published totals invite, because the two sets are NOT NESTED - the two non-matrix rows are inside the 60 and outside the 88. I wrote 28 into a first draft of this revision by exactly that subtraction and caught it by running the difference; the rule is now stated on the surface that publishes both figures. 60 and 58 are BOTH published for the same reason: quoting either alone mis-states the other. THE 73 `\b88\b` OCCURRENCES IN THIS DOCUMENT WERE CLASSIFIED BEFORE ANY WERE TOUCHED, per §D10`s anti-pattern warning, and the classification is why nothing moved: they are FOUR unrelated populations, not one. Design-matrix total: 12 occurrences (the Deliverables cell, the derivation block`s two walk outputs, and the batch-stamp paragraph). Fence-census CONTROL `88` (the non-bash fence total beside `73 across 10 files`): 8. Test-file ledger `88/88` (`ls h-mad/tests/test_*.py | wc -l` paired with its glob control): 17. Version LABELS `v1.86`/`v1.88` and Version History prose: 36. 12 + 8 + 17 + 36 = 73, so the four partition the population with no remainder, and a value sweep would have moved 61 figures about three other things. THE SPLIT WAS PUBLISHED WRONG FIRST AND THE CORRECTION IS THE POINT RATHER THAN AN ERRATUM: a first draft of this entry carried 12/8/17/36 as 9/8/13/43, hand-added from a per-line listing, and it summed to 73 only because the last bucket had been treated as the remainder - which is exactly how a wrong partition passes its own check. Re-derived by ASSIGNING EACH LINE TO ONE BUCKET IN CODE and printing the four counts and the total from the same pass, over `git show 6a1693c:` so the population cannot move underneath the reading; the totals agree at 73 by construction and the buckets are now measured rather than apportioned. The general form: a partition is published only when its parts and its total come out of ONE pass, and no bucket is ever computed by subtracting the others. Unit stated: occurrences of `\b88\b`, counted with `grep -o ... | wc -l`; the matching-LINE count over the same body is 35, and publishing that as the occurrence count would understate the population by half. §D10`s own characterisation "most are line numbers" DOES NOT HOLD FOR THIS DOCUMENT and is reported rather than adopted: `grep -c ":88\b"` returns 0 here, because this document writes no line pins at all. THE DISPATCH`S TASK 3 IS ALREADY DISCHARGED IN THE TREE AND IS REPORTED AS STALE RATHER THAN RE-APPLIED - the brief describes the v1.92 body at `6f0ee85`, fourteen revisions back. Verified by EXECUTION, not by reading the Version History: the currently published screen gives all THREE alternatives both-sides POSIX word boundaries and case-folds two of them, and run over this body it returns 143 lines against 134 for the brief`s proposed three-branch form and 49 for the brief`s "published form". The 4 lines the brief`s form reaches that the published one declines are the control fixture`s own true negatives (`in this sessionless mode`, `xthis session glued`), which the leading boundary refuses by design - so the published screen strictly dominates on real members. The brief`s three cited sites do not carry what it says they carry: `measured[,)]` appears nowhere outside three historical descriptions of the superseded form, the 5f wrapper stamp already carries `git ls-files h-mad/bin/hmad-dispatch h-mad/scripts/hmad-dispatch.sh` -> two paths at `6f0ee85` in a six-row carve-out table, and the AC-5.2 probe already runs under `python3.11` with its interpreter line. v1.93 closed the `[Mm]easured`/`[Tt]oday` branches, v1.94 closed `[Tt]his [Ss]ession` (which v1.93 had left unboundaried while asserting in bold that every branch was fixed - the half-fix repeating one revision later), and v1.95 gave each branch its own leading- and trailing-glued fixture. No re-sweep of the carve-out population is owed, because the widened screen is the one already running. PRECHECK on this revision: PRECHECK: PASS issues=0.
- v1.109: Reconciles this document to the operator's D10 merge, executed at `7afc0ef` and recorded at `8bc2fa7`: `h-mad/tests/mutation-specs/doc_block_exec_task3.json` is folded into `doc_block_exec.json` and DELETED, so the five sites naming the per-task file are repaired BY CLAIM rather than by substituting a filename — one states the realisation as a two-file split (the §Deliverables mutation-spec row), three sit inside the §Measurements realisation walk, and the fifth is a `0 0 0 0` control asserting that no document names a per-task convention. THE SITE COUNT IS DERIVED TWICE AND THE TWO AGREE, both at `861efbe`: `grep -c 'doc_block_exec_task3' <doc>` returns **5** matching LINES and `tr '\n' ' ' < <doc> | grep -o 'doc_block_exec_task3' | wc -l` returns **5** OCCURRENCES. The wrap-collapsed form is run beside the line form because a hard wrap hid a retired count from every single-line grep in this feature for 92 cycles, and a disagreement between them would have been the finding rather than a nuisance. EVERY REALISATION FIGURE IS RE-DERIVED AT `861efbe` BY THE WALK THIS DOCUMENT DEFINES AND NONE IS CARRIED THROUGH THE MERGE: the merged spec holds **60** rows and **60** distinct names (unit: distinct mutation NAMES, not rows and not matching lines), the design's matrix at that same sha reads **90** rows and **90** distinct names, `on-disk-not-in-matrix` is now **EMPTY** and `matrix-not-on-disk` is **30**, so the realisation is **60 of 90** with Tasks 4 and 5 unwritten. THE `32`/`28`/`58`/`88` QUADRUPLE IS RETIRED RATHER THAN RE-STAMPED: `32 + 28` and the disjointness that made it a union were claims about TWO FILES, and one of them no longer exists, so re-stamping them at this sha would publish a partition with no second part. THIS DOCUMENT'S OWN ROUTED OBLIGATION IS DISCHARGED AND THE DISCHARGE IS STATED, NOT DROPPED: through v1.108 it published `kill-skipped-after-collect-failure` and `stderr-not-closed` as realised rows the matrix did not carry, declined to bump its own total on their account, and routed them to the design as owed; both are matrix members at `861efbe`, which is precisely the 88 → 90 delta, so the difference this document re-ran at every freeze has closed and the walk now returns the empty list that closes it. THE `0 0 0 0` CONTROL IS REMOVED RATHER THAN RE-STAMPED AND WHY IT FAILED IS RECORDED: it counted its own needle, so `grep -c 'doc_block_exec_task3'` over the four documents returns **0 5 4 6** at `861efbe` [correction, v1.111: the first element is WRONG and was UNMEASURED — this sweep read the spec at a path that does not exist (`docs/00-spec/...`; it is `docs/01-plan/features/doc-block-exec.spec.md`), and `git show <sha>:<absent-path> | grep -c` scores an empty stream as `0`. At `861efbe` the vector is whole-file **1 5 4 6** and body-scoped **0 5 3 4**; the conclusion is unchanged and strengthened] against **0 0 0 0** at `6a1693c`, and all three non-zeros are the 5e round's own prose ABOUT the split — a control whose corpus is the documents that must discuss it cannot survive the discussion, and re-stamping it to `0 5 4 6` would publish the needle a fourth time. THE GLOB CONTRACT THE CONTROL WAS ATTACHED TO SURVIVES THE MERGE AND IS RE-VERIFIED AT `861efbe`: `grep -c 'specs_dir.glob("\*.json")'` and `grep -c 'rglob("tests/mutation-specs/\*.json")'` over `h-mad/tests/test_h_mad_mutation_harness.py` each return **1** matching line. THE VERIFICATION SHAPE IS SETTLED BY THE MERGE AND IS READ FROM THE SOURCE RATHER THAN ASSERTED: the harness declares `spec` with `nargs="+"` but guards a RUN with `if len(args.spec) != 1`, erroring with "a mutation run takes exactly one spec; use --check-anchors to sweep several", so several specs are accepted only under the read-only anchor sweep — the split therefore forced one RUN per file and the merged spec is exactly one. NO GATING CLAIM, NO TWO-SURFACE CLEAN, NO EXIT GATE.
- v1.110: CROSS-DOCUMENT CONVERGENCE ON THE MUTATION TOTAL, RECORDED RATHER THAN RE-FIXED, AND THE SPECIES NAMED. A finding raised by spec-author and relayed mid-round — that this document's mutation-matrix total `88` disagrees with the design's own awk census of **90** at `861efbe` — was ALREADY CLOSED by v1.109, which re-derived the census at that sha and published the eighth reading. The convergence is recorded because two surfaces reaching the same repair independently is evidence, and because the relay carried one thing v1.109 had settled by measurement and the relay could not: **the flagged "58 of 88" shape was REAL**, `git show 861efbe:<doc> | tr '\n' ' ' | grep -oE '58 of 88' | wc -l` → **2** (unit: occurrences, at the §Deliverables mutation-spec row and at the §Measurements realisation paragraph), against **0** in the body v1.109 ships and **3** occurrences of `60 of 90` (the two repaired sites plus the v1.109 entry that records them). **BOTH TERMS OF THAT PARTITION HAD MOVED, BY DIFFERENT AMOUNTS AND FOR UNRELATED REASONS — the numerator 58 → 60 because the §D10 merge folded Task 3's rows into one file and the two 5e rows stopped being outside the matrix, the denominator 88 → 90 because the design took those same two rows — so a bare `88` → `90` substitution would have left the site WRONG while looking repaired.** The rule over the axis, which outlives both numbers: **a partition claim is re-derived NUMERATOR, DENOMINATOR AND RESIDUAL FROM ONE PASS, never by substituting a term; a total being right proves nothing about its parts.** The §Measurements walk already obeys it — one run prints `matrix`, `on-disk`, `both`, `on-disk-not-in-matrix` and `matrix-not-on-disk` together — and the §Deliverables row now derives its `60 of 90` from that run rather than from either figure alone. **THREE POPULATIONS SHARE THE LITERAL `88` IN THIS BODY AND ONLY ONE WAS TOUCHED, which is why this was not swept by value**: (1) the bash-fence census control and `ls h-mad/tests/test_*.py | wc -l` → 88, a different measurement that happens to equal 88 — LEFT UNTOUCHED, and editing it would have been a defect introduced by the sweep; (2) `v1.88` version substrings — not a quantity, left; (3) the mutation total — the only one revised. Classification is by READING each site, not by shape: `grep -nE '\b88\b' <doc>` returns **44** matching lines over the whole file and **31** over the body v1.109 ships (`awk '/^## Version History/{exit}{print}'`-scoped), each inspected. **The `88 - 60 = 28` arithmetic still in this file is the v1.108 Version History entry and is NOT rewritten**: it was true of the tree v1.108 shipped at, the body's live form reads `90 - 60` with the nesting stated, and a Version History entry is bracket-corrected, never rewritten. NO GATING CLAIM, NO EXIT GATE.
- v1.111: CORRECTS A VECTOR v1.109 SHIPPED, AND THE ROOT CAUSE IS NOT THE ONE IT LOOKED LIKE. v1.109 published the re-run of the retired absence-control as **0 5 4 6** (spec, plan, design, impl-plan) under one stated unit. A cross-document reader flagged it as a MIXED-SCOPE tuple — one body-scoped element beside three whole-file ones — and the repair that reading prescribes is right, but the cause is worse and is stated rather than smoothed: **the `0` was not a body-scoped reading at all. It was a NONEXISTENT PATH.** v1.109 swept the spec at `docs/00-spec/features/doc-block-exec.spec.md`; the spec lives at `docs/01-plan/features/doc-block-exec.spec.md`, `git ls-tree -r --name-only 861efbe | grep 'doc-block-exec.spec.md'` returns that one path, and `git show 861efbe:docs/00-spec/...` fails to stderr while printing NOTHING to stdout — so `grep -c` scored an EMPTY STREAM and returned a `0` that measured no document. **A missing path and a true zero are indistinguishable downstream of a pipe, which is the species: `git show <sha>:<path> | grep -c` FAILS PERMISSIVELY, reporting absence when it means the corpus was never opened.** The guard is to assert the path exists before counting it, not to read the number. **BOTH SCOPES ARE NOW PUBLISHED PER ELEMENT rather than one vector under one unit, because the spec is precisely the document where they diverge**, all at the frozen blob `861efbe` (unit: matching lines): whole-file **1 5 4 6**, body-scoped (`awk '/^## Version History/{exit}{print}'`) **0 5 3 4**. Spec 1/0 — its single occurrence is a Version History entry at no line this document pins; design 4/3 and impl-plan 6/4 diverge the same way; **plan 5/5 does not diverge, which is the control on the pair**: a document whose two scopes agree shows the divergence is a property of where the needle sits, not of the awk. **The CONCLUSION is unchanged and strengthened**: the retired control read 0 0 0 0 at `6a1693c` and reads non-zero at `861efbe` on either scope, and whole-file `1 5 4 6` is FURTHER from `0 0 0 0` than the `0 5 4 6` v1.109 published. The v1.109 entry is bracket-corrected in place, never rewritten. NO GATING CLAIM, NO EXIT GATE.
