...
assertion that nothing was left over: **13 + 15 + 8 = 36**. The **13** are the nine sites above —
four of them state their figure on two lines each, which is why the line count exceeds the site
count. The **15** are a fence's or a fixture's own printed output on this document together with
the prose disposing of it. The **8** are zeros making no claim about a corpus at all: **two** are
the `index` contract's ordinal and the sentence denying that `0` is an ordinal, **one** is an AC
row's `--shell-timeout` value, and **five** are rationale quoting its own vocabulary — three in
the census's arm-differential paragraph and two in the seam-ordinal check's. **Zero raised lines sit
outside the three parts**, and the sum is the check — a walk that loses a line shows up as a sum
that misses 36, which recalling a site list never could. **One assignment inside that partition is
arguable and is named rather than hidden**: the census arm-(2) line the needle raises states the
arm's *control* (a fixture turning the shipped zero into one), while the arm's own zero is written
in a shape the needle does not raise; reading it as the site gives 13 + 15 + 8 and reading it as
harness output gives 12 + 16 + 8. The total, and the conclusion, are the same either way. The `$STRIP` bound carried its reason in prose and no
...
5. **Task 5 — the wire.** Tag the Second-surface gate fence in `h-mad/SKILL.md` **and** migrate
   ...
   *lines* plus a number, as an `L`-prefixed number, or as a colon-plus-number not preceded by a
   ...
   again on the working file this revision ships, **after the v1.102 entry**, over the same
   ...
   **A fourth blind form, and why this detector is exempt from the fold §Test Strategy imposes on
   ...
   and readable. It was checked rather than assumed — **on the working file this revision ships, after the v1.102 entry**, and the
...
revision ships, **the second run made after the v1.102 entry**, so the bound is **unexercised here** — *vacuous* in the absence rule's vocabulary,
...
the v1.102 entry below was written**, which is the second half of the stamping rule the
...
— on the working file this revision ships, after the v1.102 entry. **The literal `1` stood in this
...
`tests/test_docsections.py::<name>` in `docsections.json` for the **six** rows killed there — **the four originals** (the adjective "re-anchored" is deliberately dropped here, and the reason is stated as a dated reading of the sibling rather than as a present-tense claim about its bytes, which decision E forbids: **read at impl-plan v1.45, sha `7d8e797`**, the impl-plan used that word on the *anchor-file* axis, where only two of the four move — `fence-tracking-removed` and `section-no-longer-owns-its-subsections` into `h-mad/scripts/h_mad_doc_block_exec.py` — while this sentence is on the *`test`-key* axis, a different partition of the same rows. The tree-derived half is separable and is stamped on the tree, not on the sibling: all four rows in `h-mad/tests/mutation-specs/docsections.json` carry `"file": "tests/docsections.py"` at `7d8e797`. One word on two axes with two cardinalities is a collision; the design gives up the word rather than redefine it, so nothing is owed to the sibling by this repair), `docsections-delegation-reverted` and
