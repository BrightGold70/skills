...
created is already gone or renamed away). **That identity check is a mutation-backed guard like
...
over the head raises **seven**, each identified by section and subject rather than quoted, since
...
unwritable-path row, and the duplicate-substitution-key default on the bad-substitution row. All
seven are constructor keywords, **walked rather than counted** — the count
...
so the seven are the same seven — re-derived after v1.109's own text landed rather than
carried across it. A widening that changes no hit is exactly the case in which carrying the
previous number would be invisible if it were wrong. **The rule over the class: this alternation is a copy of a set another document
...
Ninety rows, ninety mutations — **eighty-nine of the helper's source and exactly one of
`h-mad/SKILL.md`**. This revision added two, `kill-skipped-after-collect-failure` and
`stderr-not-closed`, both of the helper's source and both answering
`docs/03-analysis/doc-block-exec.5d-decisions.md` §D9, where an independent verification pass
...
about this table's coverage and never about the source's. The previous revision added one,
...
the helper's source, so the `SKILL.md` half of the split has not moved in any of the four revisions
...
f&&/^\|/{n++} f&&!/^\|/{f=0} END{print n}'` over the shipped file returns 90 after the v1.114
entry below was written. **The split is derived from the mechanism column above, never carried**: count
