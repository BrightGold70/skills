...
endpoints, so it is taken hop by hop: `700c599`→`8c6539a` prints a single hunk of one changed line,
`8c6539a`→`b3be433` prints nothing, and `b3be433`→working prints a single hunk of one changed line.
So v1.104 reworded one, v1.105 reworded none, and v1.106 reworded one — that one being the §Test
...
hop compares against.** The earlier form of these two sentences reported the last hop as empty and
...
because this alternation lives in prose in this document and not in the code; the spec states the
same residual against the same tuple for the same reason, and the check is a reader comparing the
two lists. **Second residual, as a concrete category**: an *emitted detail line* mis-spelled in the
...
- **Assumption verification** — complies: the plan's `## Measurements` section carries both cited
  ...
  fence-aware after this revision's edits rather than carried — leaving `60` prose lines,
