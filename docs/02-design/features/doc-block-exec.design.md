...
being written. At `cf3a862` it raises **36** candidate lines, of which four sites are absence
claims — the `.md`-under-`docs/` invariant, the two untracked-script measurements
(`grep -c grammar_corpus` and `grep -cE 'heading_differential|grammar_corpus'`), and the
`grep -c parametrize` pair under the floor tuple — and every one of the four is labelled where it
is stated. **Residual, stated as a
...
four-site classification is not, and a fifth absence claim written in a shape matching neither
``[`]0[`]`` nor ``expect[ ]0`` would not be raised by it at all.
...
# BOTH sides are committed blobs, so nothing written into the working file can move this output.
...
raw = subprocess.run(f"git diff {base} {head} -- {doc} | grep '^+' | cut -c2-",
...
`BASE=35698f9`, `HEAD=6f0ee85` — at `cf3a862` it prints
...
**Why this screen may quote its own numbers.** Both inputs are committed blobs — `git diff
35698f9 6f0ee85` reads two trees, and `$R` is a report committed at `cf3a862` — so no sentence
this revision adds to the working file can move the output, and the figures above were re-run
*after* the edit landed. That property is not automatic and is the general rule for anything this
document ships as a control: **measure it over bytes the revision cannot touch — an earlier blob
or a fixture file — or the act of publishing the control changes what it measures.**

**Disposition of the 8 body runs — the partition is computed, the disposition is a reading, and
that is the residual here.** Two are commands, three are fixture descriptions and one fixture's
own printed result, two are phrasings of a rule and of its residual, and **one** is the dottedness
...
word list, and in the safe direction, since the list raises candidates and never hides them.
...
1. **§Version History is exempt for ordinals as well as for line pins.** An ordinal there is a
   ...
   `8` on the working file **after the v1.99 entry**, which records its changes without quoting an
   ...
   History is exempt from. Run over the tail, `grep -cE '\b(index|block|blocks)[ =]+[0-9]+|--index
   [0-9]+'` returns `1` at `cf3a862`, and the one hit is `blocks 7` inside a quoted census
   **output** — a printed count, not an index into a span — so no entry states a span ordinal
   without its base and the rule is satisfied there rather than merely asserted.
...
3. **The screen is proximity-based, so a hit is a candidate to read, not automatically a
   ...
   *under*-matches an ordinal written as a digit-plus-suffix rather than a word, a noun outside
   ...
   file after the v1.99 entry, and it returned `0` under the singular-only `$N` on the same bytes,
...
```bash
comm -23 <(grep -oE 'AC-[0-9]+\.[0-9]+' docs/01-plan/features/doc-block-exec.spec.md | sort -u) \
         <(grep -oE 'AC-[0-9]+\.[0-9]+' docs/02-design/features/doc-block-exec.design.md | sort -u) | wc -l
```

Over the whole document it prints **7** of the spec's 49: seven ACs this design addresses only
inside a range and never as a literal identifier. Narrowed to this table's leading column —
