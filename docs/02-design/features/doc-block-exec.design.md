...
which returns `0` at `335f535`, at `74e126f`, at `35698f9` and at `6f0ee85` alike: every `.md` the unscoped diff
names is under `docs/` — this feature's own spec, plan, design and impl-plan, which lie outside
both roots and enter no corpus measured here. **Do not publish the pair.** An unscoped trip-wire
...
at `335f535` 13 and 11, and at `6f0ee85` 31 and 29 — four shas, four pairs, one invariant.) Re-run **the scoped form**
...
the sha it was run at, and **the reason the zero is zero, marked load-bearing or incidental**. A
zero that is right by accident is a defect that has not fired yet. **Residual, stated as a
...
a real fix models list-item container indentation, which is why arm (2) is still owed.
...
revision ships, so the bound is **unexercised here** — and, unlike `census()`'s arm (2), which is
...
on this file rather than argued. *Positive, for
...
```bash
# $D this document, $R the report the revision answers, $BASE..$HEAD the revision's own diff
python3.11 - "$D" "$R" "$BASE" "$HEAD" <<'PY'
import re, subprocess, sys
doc, rep, base, head = sys.argv[1:5]
tok = lambda s: re.findall(r"[a-z0-9][a-z0-9./_-]*", re.sub(r"[`*_]", "", s.lower()))
add = tok(subprocess.run(f"git diff {base} {head} -- {doc} | grep '^+' | cut -c2-",
...
while i < len(add) - 5:
    if tuple(add[i:i + 6]) in grams:
        ...
        runs.append(' '.join(add[i:j + 5])); i = j + 5
    else:
        i += 1
print(f'{len(runs)} verbatim 6-gram runs carried from {rep.rsplit("/", 1)[-1]}')
for r in runs: print(' *', r)
PY
```

Run for the revision that introduced the defect — `D` this file, `R` the v87 teammate report,
`BASE=35698f9`, `HEAD=6f0ee85` — it prints **23 runs**. Twenty-two are commands, sha pairs,
fixture descriptions and quotations of the finding, all of which a Version History entry is
*supposed* to transcribe verbatim. The twenty-third is the dottedness clause corrected at the head
of this paragraph, and it is deliberately not re-quoted here: quoting a wrong figure to report it
puts the wrong figure back in the body. Four other runs carry a figure, and all four were
re-derived rather than taken on trust and all four hold — `a8e0372..74e126f` names 18 files / 16
`.md`, `a8e0372..335f535` names 13 / 11, and exactly 3 files under the two roots import
`docsections`. **Residual, exactly**: this screen finds carried *text*. A figure retyped in
...
1. **§Version History is exempt for ordinals as well as for line pins.** An ordinal there is a
   ...
   `8` on the working file after the entry **this** revision appends, which records its changes
   without quoting an ordinal address — and the tail command was re-run on the working file
   *after* the bump rather than before it, which is the only order in which that check means
   anything. No version is named in this sentence on purpose: a version-pinned self-claim is
   true when written and stale one revision later, and the command is what carries the claim. It rises by one on the next entry that has to quote
   the ordinal it struck.
...
3. **The screen is proximity-based, so a hit is a candidate to read, not automatically a
   ...
   *under*-matches an ordinal written as a digit-plus-suffix rather than a word, a noun outside
   `{seam, injection, primitive}`, and a gap longer than 60 characters. The head returns `0`
   on the working file this revision ships, so none of the three is exercised on this file — which is the honest statement of what
   the `0` is worth, and it is weaker than "the class is closed".
...
is found by enumerating all three, not by trusting this sentence:** the AC-5.5
