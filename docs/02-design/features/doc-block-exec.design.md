...
| Helper mutation spec(s) | `h-mad/tests/mutation-specs/doc_block_exec.json` **and one `doc_block_exec_task<N>.json` per task written after it** (`doc_block_exec_task3.json` is the first) | new | guards for FR-1..FR-5. **The authority is the mutation matrix under Test Plan, not any one file**, and the count is a property of that table: 90 rows, 89 of the helper's source and 1 of `h-mad/SKILL.md` (`registry-row-removed`, the only row whose mechanism names `SKILL.md` as the file the harness edits) — re-derive both figures from the matrix's own census and mechanism column, never from this cell. **The rule over the files, which is what an audit checks:** the spec files are name-DISJOINT and their union is exactly the set of matrix rows whose task has been written; every row is in at most one file, every landed row is in exactly one, and no file contains a name the matrix does not. Task 5 additionally writes `doc_block_exec_wire.json`, whose eight FR-6 connection rows are its own row set and are **not** members of this matrix (see the Wire mutation spec row below); `docsections.json`'s rows are likewise a separate set. **RESIDUAL, not decided here:** whether the per-task files are merged into a single spec before the feature closes. Nothing in the tree settles it, and the constraint that decides it is the harness's arity — `h_mad_mutation_harness.py` declares `spec` with `nargs="+"` but refuses more than one on a RUN, accepting several only under `--check-anchors` — so as long as the split stands, a full run is one invocation per file and the verification commands below must name each one. If the files are later merged, the union rule above is what the merged file must satisfy; the count does not move, because a merge relocates rows and this table defines them |
...
# one on a RUN (`if len(args.spec) != 1`), taking several only under --check-anchors. So while the
# per-task split stands, every doc_block_exec_task<N>.json gets its own line here, and a task that
# lands a new spec file without adding its line is a file no verification run ever executes.
...
hmad-dispatch run --timeout 600  -- python3.11 h-mad/scripts/h_mad_mutation_harness.py h-mad/tests/mutation-specs/doc_block_exec_task3.json
...
# the anchor pass is the one form that takes them together, and is what checks the union rule holds
hmad-dispatch run --timeout 600  -- python3.11 h-mad/scripts/h_mad_mutation_harness.py --check-anchors h-mad/tests/mutation-specs/doc_block_exec.json h-mad/tests/mutation-specs/doc_block_exec_task3.json
