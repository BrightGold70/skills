...
softened.** Measured at `fbc2ea0`: `h-mad/tests/` holds 88 `test_*.py` modules, of which 13 carry
the exact `sys.path.insert(0, str(SCRIPT_DIR))` spelling, 14 mention `SCRIPT_DIR` at all, and 48
perform some `sys.path.insert`. Re-derive, do not read:

```bash
git ls-files -- h-mad/tests | /usr/bin/grep -cE '(^|/)test_[^/]*\.py$'
...
can be repaired out from under a document by a commit that touches nothing the document guards —
which is precisely what happened here, since `b39d9dc` passed every scoped census predicate this
feature publishes (`h-mad handoff`, `'*.py'`) and still moved a **markdown** line three documents
enumerate by content. The pin is `test_titleless_heading_is_a_new_only_member` in
...
returns `0` at `cf3a862` and **4** at `fbc2ea0`. **The needle carries `setext_census` on purpose**:
...
scratch file is not re-derivable, however correct the figure happens to be. **Both scripts this
document has ever cited are now tracked, and that is what discharges the axis** rather than the
in-document proxies alone; each is cited by its committed path and never by a bare filename, since
a bare filename names no blob a reader can fetch. They are
...
glob-corpus predecessor, and
`docs/03-analysis/probes/doc-block-exec/grammar_corpus.2026-09-03.cd979362.py`;
`git ls-files docs/03-analysis/probes/` lists those three and a Setext census beside them.
**Residual, exactly**: a committed script still has to be run, and its output is not in this file,
so this census and the eleven-shape ATX proxy remain the in-document derivation for both figures;
and any script cited in future must pass that same `git ls-files` check before a figure derived
...
this exception is this one, and every sibling cites it rather than restating it. **Residual, as a
...
So on that corpus the two agree on every real failure and differ only by over-refusal. That is a
finite search, not a proof: the class it cannot reach is a key set over a larger alphabet or longer
keys where the bare form's skipped occurrence is the only witness, and the conservative form is
chosen precisely because that class cannot be ruled out from here.
...
1. **§Version History is exempt for ordinals as well as for line pins.** An ordinal there is a
   ...
   *working file*), and a figure naming **neither** — a number with no corpus behind it — is a
   ...
   Run on the working file this revision ships, **after the v1.110 entry** landed rather than
   ...
   **The head is stamped at `cf3a862`: `$P` raises 14 there and `$W` raises 6; and the working-file
   head is published beside that stamp rather than withheld — `$P` **40** and `$W` **12**, both
   re-derived after the v1.110 entry below was written.** **The figure published here as `29` for
   ...
   and `fbc2ea0`, 30 at `cb4fe99` and `cac6edc`, and 37 on the
   working file; `$W` reads 6 at `cf3a862`, 8 at every sha from `1cbddb7` to `cac6edc`, and 12 on
   the working file. **The `+10` and `+4` this revision adds are prose, not new addresses**, and
   they are what the screen is for: v1.110 writes hop counts, hunk shapes and walk splits, all of
   which put a digit beside a span noun (`positions 3, 4, 37 and 61`, `covering five changed
   lines`, `86 rows`), and every one of them is a **printed count or a differential's own output**, which
...
set out of the spec blob at `cac6edc`, and the two are compared:

```bash
python3.11 - <<'PY'
import re, subprocess
spec = subprocess.run(["git", "show", "cac6edc:docs/01-plan/features/doc-block-exec.spec.md"],
                      capture_output=True, text=True).stdout
acs = set(re.findall(r'^  - (AC-\d+\.\d+):', spec, re.M))
head = open("docs/02-design/features/doc-block-exec.design.md",
...
print("spec", len(acs), "covered", len(covered),
      "uncovered", sorted(acs - covered), "not-in-spec", sorted(covered - acs))
print("zero bare-label occurrences in the head:",
      sorted(a for a in acs if a not in set(re.findall(r'AC-\d+\.\d+', head))))
PY
```

On the file this revision ships it prints two lines:

```
spec 49 covered 49 uncovered [] not-in-spec []
zero bare-label occurrences in the head: []
```

**The second line is empty because this paragraph fills it, and that is stated rather than left to
look like a property of the older text.** At `cb4fe99` the same command's second line named
**seven** labels — **AC-1.2, AC-2.3, AC-3.2, AC-3.4, AC-3.5, AC-4.4 and AC-6.3** — and not the five
a value grep over five guessed labels had reported; a reviser who re-runs it here and reads `[]` is
reading the fix, not the original condition. Each of the seven is covered
...
the labels themselves live here and nowhere else, which is also why the screen's second line prints
empty. AC-1.2 (untagged fences → `NOT_FOUND`) and AC-3.2 (`git status --porcelain`
...
Eighty-six rows, eighty-six mutations — **eighty-five of the helper's source and exactly one of
`h-mad/SKILL.md`**. This revision added one, `intersect-scan-non-overlapping`, of the helper's
source; the previous revision added four (`intersect-check-removed`,
`spawn-valueerror-unmapped`, `rollback-identity-check-removed` and
`cleanup-chain-selection-flipped`), also all of the helper's
source, so the `SKILL.md` half of the split has not moved in either revision and only the first
addend does.
...
f&&/^\|/{n++} f&&!/^\|/{f=0} END{print n}'` over the shipped file returns 86 after the v1.110
...
- **Assumption verification** — complies: the plan's `## Measurements` section carries both cited
  ...
  `132` and `71` on the file this revision ships, **re-run after the v1.110 entry below was
  written** — they read `106` and `62` at `fbc2ea0`, `104` and `60` at `b3be433`, and `126` and
  `70` at `cb4fe99`, and every
  ...
  **v1.110 adds three fences and one marker-word line, and both deltas are derived by differencing
  the walks' own line sets against the `cb4fe99` blob rather than by counting what this revision
  believes it wrote** — `diff` over `HEAD | grep -E ':[[:space:]]*`{3,}'` at the two endpoints
  yields exactly six added lines and no removed ones, which is the `+6`: the second heading
  differential's freeze output block, and the AC-coverage expansion's `bash` fence with its output
  block. The intersecting-span probe's output block was *replaced*, not added, so it contributes
  nothing to the delta — which is precisely the kind of thing a from-memory count gets wrong, and
  the reason the difference is taken mechanically. The one added marker-word line is the sentence
  under §Scanning's softening table recording that the instances column was non-empty at `a8e0372`
  and is zero at the freeze; its command is the differential fence in the same section, so it is
  disposed of below rather than left as an uncited figure. **This clause is deliberately written
  ...
  *second* line to the very walk the clause reports, which is the same self-reference the disposal
  ...
  census, the intersecting-span probe, the NUL-payload probe, the cleanup-chaining probe, the five-kinds
  ...
  `os.setsid()`-escape probes do. **The second walk's `71` splits mechanically before anything is
  ...
  leaving `69` prose lines. **The earlier form of this sentence read `69` and `67`, each one below
  ...
  this bullet forbids — and it is recorded rather than silently corrected. **Eight are disposed of
  by name**, the five v1.106 disposed of, the two
  v1.109 added, and the one v1.110 adds (§Scanning's softening-table instances sentence, whose
  command is the differential fence in the same section): the `umask`/`mkdtemp` mode
