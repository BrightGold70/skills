...
**Every `a8e0372` figure in this document reproduces unchanged at `335f535`**, and that is
checked rather than assumed: `git diff --name-only a8e0372 335f535` names two files, both `.py`
(`h-mad/scripts/h_mad_assemble_audit.py` and its test), so the `*.md` corpus every measurement
here runs over — including `h-mad/SKILL.md`, which the Task 5 census reads — is byte-identical
between the two shas. Re-run that one command before trusting any figure below; if it ever names
an `.md`, every count carrying `a8e0372` must be re-derived rather than re-read.
...
At `335f535` the whole fence prints exactly this — the two `git ls-files`/`find` counts, then
...
a throwaway and is not in the tree (`git ls-files | grep -c grammar_corpus` → `0`), and 4.2.0 is
...
5. **Task 5 — the wire.** Tag the Second-surface gate fence in `h-mad/SKILL.md` **and** migrate
   ...
   **This is a rule over the whole document, not a treatment of this one site, and the class is
   ...
   quotes a line pin that was accurate when that entry was authored, and it stays. Two other
   rules are *not* exempt there, because they govern every reference in this document: the
   seam-naming rule (§Test Strategy), and the ordinal-base rule stated just below. Verify the
   line-pin class rather than trusting this sentence, with the corpus split at the §Version
   History heading:
   ...
   **An ordinal over these blocks is informational, not load-bearing, and it is true**: the
   ...
   indexes — and it must never be given as the thing the code selects on.** Stated in full,
...
is not hypothetical — the ordinal this rule removed named the wrong seam as well as a drifting
one. The rule is stated ahead of both enumerations because it governs every seam mentioned below
*and* every seam mentioned in §Error Handling Strategy above.
...
leaving `subprocess` real; this list is the canonical taxonomy the spec and the impl-plan repeat
verbatim:** the AC-5.5
...
- **Single-source contract** — complies on the invariant's FIRST branch for the rule this feature
  ...
  `_fence_aware_end` and `titled_section`'s local heading regex. **The claim is scoped to that rule,
  and the scope is stated because the tree holds three further hand-rolled `##`-slicers that the
  consumer census behind it — a grep for `from docsections import` — cannot see by construction**:
  `def _titled_section` in `h-mad/tests/test_h_mad_context_budget_docs.py` (eight call sites),
  `def section_text` in `h-mad/tests/test_h_mad_batch_doc_rules.py` and `def _section`
  in `h-mad/tests/test_h_mad_collect_report_docs.py` (reached through `_second_surface()`) —
  each addressed by its own `def`, per the enclosing-symbol rule stated in Task 5, so
  `grep -n 'def _titled_section' …` locates it and a `grep` that returns nothing is the signal
  that it was renamed. None is
  a surface applying *this* rule: each is a test-local assertion helper slicing one document for its
  own pins, all three are fence-blind, and the invariant scopes to "a rule applied by more than one
  surface", not to every function that finds a `##`. They are also not drop-ins — measured at
  `1861157`: `_titled_section` anchors on a substring, so `docsections.titled_section(SKILL_MD,
  "Run-context ceiling")` raises `AssertionError: missing section` where the real heading is
