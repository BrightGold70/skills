...
clean up, so the refusal can neither leak a directory nor need the read-back.
...
Eighty-seven rows, eighty-seven mutations — **eighty-six of the helper's source and exactly one of
`h-mad/SKILL.md`**. This revision added one, `intersect-kind-ignored`, of the helper's source; the
previous revision added one, `intersect-scan-non-overlapping`, also of the helper's source; the one
before that added four (`intersect-check-removed`, `spawn-valueerror-unmapped`,
`rollback-identity-check-removed` and `cleanup-chain-selection-flipped`), again all of the helper's
source, so the `SKILL.md` half of the split has not moved in any of the three revisions and only the
first addend does.
...
f&&/^\|/{n++} f&&!/^\|/{f=0} END{print n}'` over the shipped file returns 87 after the v1.111
