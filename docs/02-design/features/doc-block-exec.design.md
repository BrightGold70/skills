...
rather than a scan that finds nothing. It was `0` at `1861157` too, over the 25 files tracked then.
...
`control arm1 shipped 1 repaired 0` and `control arm2 shipped 1 repaired 0`. That matters because
the tracked corpus contains **0** lines matching `^ {0,3}~{3,}` — `git ls-files -- h-mad handoff |
grep '\.md$' | grep -v '/archive/' | xargs grep -chE '^ {0,3}~{3,}'` sums to `0` over the same 30
files — so without the tilde run the `~{3,}` branch carried by `SHIPPED`, `ANYIND` and `$STRIP`
alike would be a branch that no control and no corpus had ever moved, with the healthy backtick
branch standing in for it (decision O). That corpus `0` is itself an absence claim and is
*vacuous*: the shape does not occur, so nothing was exercised.
...
being written. At `cf3a862` it raises **36** candidate lines, of which **seven** sites are absence
...
the arm-(2) zero of the census two paragraphs above** (*vacuous* and *incidental* respectively);
and the `^ {4,}` fence bound stated at `$STRIP` below (*vacuous*). **v1.99 published this
denominator as four, and the two it dropped are the census arms** — they sit on candidate lines
this sweep itself raises, and the paragraph immediately above them had already labelled them in
the rule's own words, so the miss was *inside the needle* rather than at its margin. The `$STRIP`
bound is the seventh: it carried its reason in prose and no label until this revision. The
`headings shipped 0` column is deliberately **not** an eighth — it is the composite whose two arms
are the two sites just added. (A reader re-running the sweep at a later sha gets a larger number,
**42** at `7982c18`, and the delta is the labels each revision writes into candidate lines; that
is why this figure is stamped at a blob and never restated.) **Residual, stated as a
...
seven-site classification is not, and there are exactly two ways to be missed by it. **Outside the
needle** — an absence claim written in a shape matching neither ``[`]0[`]`` nor ``expect[ ]0`` is
never raised at all. **Inside the needle** — a candidate line whose zero is *about a corpus but
printed by, or read off, a control harness* looks like harness output rather than like prose
making a claim, which is exactly how the two census arms were dropped from the denominator; so a
candidate line standing inside a fence's quoted output, or in the prose disposing of that output,
counts as an absence claim until it is shown not to be one.
...
5. **Task 5 — the wire.** Tag the Second-surface gate fence in `h-mad/SKILL.md` **and** migrate
   ...
   *lines* plus a number, as an `L`-prefixed number, or as a colon-plus-number not preceded by a
   ...
   again on the working file this revision ships, over the same head-of-document corpus, with the
   alternation
   ...
   **A fourth blind form, and why this detector is exempt from the fold §Test Strategy imposes on
   ...
   and readable. It was checked rather than assumed — **on the working file this revision ships**, and the
...
revision ships, so the bound is **unexercised here** — *vacuous* in the absence rule's vocabulary,
and the seventh of the seven sites that rule counts — and, unlike `census()`'s arm (2), which is
...
git diff 35698f9 6f0ee85 -- "$D" | grep '^+' | cut -c2- | grep -v '^+*- v1\.[0-9]' >> "$R"
...
Appending this revision's own added **body** lines to the working `$R` and re-running both forms:
the `open()` form moves from `BODY: 8 runs, 3 of them figure-bearing` /
`VERSION HISTORY: 15 runs, 8 of them figure-bearing` to `BODY: 1 runs, 1 of them figure-bearing` /
`VERSION HISTORY: 49 runs, 17 of them figure-bearing`, while the `git show` form shipped above
prints `8`/`3` and `15`/`8` **unchanged**. (The body figure falls rather than rises because the
appended lines merge the whole added body into one contiguous run — a mutation that moves a count
*downward* is still a mutation, and a screen immune to it prints the same number either way.) On a
clean tree the two forms print the same four numbers, so the repair is output-preserving and the
figures published above are the same figures, not new ones. The restore was verified rather than
assumed: `git diff --quiet -- "$R"` exits `0` afterwards. That immunity is not automatic and is
...
it out of the fence and execute it. Never reason it from what the screen is for.**
...
members named exactly: `fourteen` through `nineteen`, `forty` through `ninety`, `hundred`,
`thousand`, and **every** hyphenated compound. The gap is unexercised on the published input —
`git diff 35698f9 6f0ee85 -- $D | grep '^+' | grep -cEi` over that name set returns `0` — so no
figure above moves; the list is a candidate raiser with a stated hole, not a screen.
...
run pins property (ii): dropping `$STRIP` from the pipeline turns the `0` on this file into `1`,
and that one hit is the alternation assigned to `O` above — the check matching its own source,
not an address. (It was `2` at `35698f9`, when the alternation was written out twice —
...
note; hoisting the alternation into a shell variable is why one copy remains. A reader who runs an unstripped fold
...
1. **§Version History is exempt for ordinals as well as for line pins.** An ordinal there is a
   ...
   `8` on the working file **after the v1.100 entry**, which records its changes without quoting an
   ...
   Run on the working file this revision ships, **after** the v1.100 entry landed rather than
   before it: each of the sixteen noun branches prints `1`, the flag branch prints `1`, and the
   true negative prints `0` — every branch fires, and the screen declines a word-ordinal over a
   set, a cardinality phrasing, a number standing *before* a span noun, and a bare word-ordinal
   count. **The head is stamped at `cf3a862` and raises 14 there**, because this revision writes
   span-ordinal examples into the head as controls and a working-file head value would be a number
   this paragraph moved by being written — the same publication rule the candidate sweep and the
   carry screen state. Two of those 14 are the genuine span ordinals `block is **4**` and
   `block is **2**` in §Test Strategy, each naming its base (`enumerate(b, 1)`, 1-based) and its
   span (the named-anchor block span) in the same sentence, so the positive here is **live text in
   this document**, not only a fixture. **The tail raises 12**, and every one is a printed count
   rather than a position: nine mutation-spec row totals, the two census output fields of the
   named-anchor command, and one mention of a fence indented past a column. So no §Version History
   entry states a span ordinal without its base — and that conclusion now rests on a screen with
   seventeen fired branches and a run true negative, not on a `1` from the only branch that
   worked. **Residual, exactly**: the noun set is closed by enumeration, so a span noun this
   feature adopts later (`field`, `cell`, `record`, `frame`) is invisible until it is added to
   `$S`; and the disposition of the twelve tail hits as counts-rather-than-positions is a
   *reading*, exactly as the disposition of the eight seam-ordinals above it is.
...
3. **The screen is proximity-based, so a hit is a candidate to read, not automatically a
   ...
   *under*-matches an ordinal written as a digit-plus-suffix rather than a word, a noun outside
   ...
   file after the v1.100 entry, and it returned `0` under the singular-only `$N` on the same bytes,
