...
exercised. It is one of the absence claims this revision and v1.100 add on top of the eight the
...
being written. At `cf3a862` it raises **36** candidate lines, of which **eight** sites are absence
...
fence bound stated at `$STRIP` below (*vacuous*); and **the `1861157` restatement in the census's
own paragraph** (*vacuous* and *incidental*, its two arms measured at that sha in this revision).
**v1.99 published this denominator as four and v1.100 as seven, and every site either of them
dropped sits on a candidate line this sweep itself raises** — the miss was *inside the needle*
twice running, never at its margin, which is why that half of the residual below is stated as a
category and not as an example. The `$STRIP` bound carried its reason in prose and no label until
v1.100; the `1861157` restatement carried a command and a sha and no label until this revision.
...
**50** at `06ef40f` and at the freeze sha `68a70d6`, all four re-derived with the bracketed needle
above. The delta is the labels each revision writes into candidate lines, which is why the figure
...
eight-site classification is not, and there are exactly two ways to be missed by it. **Outside the
...
and one of the eight sites that rule counts — and, unlike `census()`'s arm (2), which is
...
`grep -cE '^ {0,3}~{3,}' "$D"` returns **0** on the working file this revision ships, an absence
claim and *vacuous* — so a fixture is the only thing that will ever move this branch, which is
...
tokens `['zero', 'runs']` and scores not-figure-bearing. The gap is unexercised on the published input —
...
1. **§Version History is exempt for ordinals as well as for line pins.** An ordinal there is a
   ...
   `8` on the working file **after the v1.101 entry**, which records its changes without quoting an
   ...
   Run on the working file this revision ships, **after** the entry recording this revision
   landed rather than before it: each of the sixteen noun branches of `$P` prints `1`, its flag
