...
Eighty-eight rows, eighty-eight mutations — **eighty-seven of the helper's source and exactly one of
`h-mad/SKILL.md`**. This revision added one, `timeout-upper-bound-removed`, of the helper's source,
...
that hands `substitute`'s result on; the previous revision added one, `intersect-kind-ignored`, also
of the helper's source; the one before that added one, `intersect-scan-non-overlapping`, again of
the helper's source, so the `SKILL.md` half of the split has not moved in any of the three revisions
...
f&&/^\|/{n++} f&&!/^\|/{f=0} END{print n}'` over the shipped file returns 88 after the v1.112
