...
| Pass-number cross-reference | `h-mad/scripts/hmad-dispatch.sh` | modify | SECOND site — the cross-reference in `_orca_handle_live`'s comment reads, before this feature, "the exact call `_orca_find` Pass 3 already declines to make". A value sweep of the old number finds TWO sites, not one; renumbering only the pass header leaves the file calling two different passes "Pass 3". Impl-plan audit v36 (agy) found this table naming only the first. |
...
| Codex fallback enumeration | `h-mad/scripts/hmad-dispatch.sh` | modify | the Pass-1 enumeration in `_orca_find`'s header comment reads, before this feature, "Codex therefore skips Pass 1 entirely and relies on the preview signature or, properly, on a pin/launch" — exhaustive by construction, and this feature adds a term: the tail signature is what Codex falls back to once the preview decays. Impl-plan audit v39/v40. |
| Banner-decay claim | `h-mad/SKILL.md` | modify | the Codex-identity limit paragraph says, before this feature, that the `gpt-N` banner "scrolls off once it works" — the exact decay this feature resolves through, so leaving it describes Codex as unresolvable in the case the wrapper now resolves. Impl-plan audit v40. |
