...
**Take the first sink that exists — this step must never be a no-op.** A todo-authoring tool is *not* guaranteed: some Claude Code installs ship no `TodoWrite`/`TaskCreate` in the main-loop tool set at all, and no user config can add a built-in (measured 2026-08-20: zero `TodoWrite` tool_use calls across every transcript on the reporting machine, with `ToolSearch` returning no match for `select:TodoWrite,TaskCreate`). An instruction that names one task tool unconditionally therefore silently restores nothing while every other part of READ succeeds — the reconciliation reads correct, the action queue evaporates, and the report says neither. Walk the ladder:

1. **A task/TodoList tool**, if one is in your tool set (`TaskCreate`, `TodoWrite`, or equivalent). Do not go hunting: if it is not already listed, it does not exist — one `ToolSearch` for `select:TodoWrite,TaskCreate` settles it, and a plugin that merely *names* the tool in a hook matcher or an agent catalog is not the tool.
