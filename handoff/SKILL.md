...
**Take the first sink that exists — this step must never be a no-op.** A todo-authoring tool is *not* guaranteed, and — the part that matters — **its presence varies between sessions on the same machine**, so this is a live check every time, never a fact you carry from the last resume. `TaskCreate` was in daily use here through 2026-08-19 (465 calls, most recent 07:11 UTC) and was gone the next morning under Claude Code 2.1.236, with no config change; it may equally be back tomorrow. An instruction that names one task tool unconditionally silently restores nothing while every other part of READ succeeds — the reconciliation reads correct, the action queue evaporates, and the report says neither.

**Probe it, do not infer it.** Your own tool list plus one `ToolSearch` is the only authority. Two ways to get this wrong, both measured 2026-08-20:

- **Absence of calls is not absence of the tool.** Grepping transcripts for `TaskCreate`/`TodoWrite` counts *use*; a session where the tool existed and went unused is byte-identical to one where it never existed. The report that opened this defect concluded "no todo tool on this machine" from zero `TodoWrite` hits — while `TaskCreate`, the name actually in use, had 465. The conclusion happened to be right for that session and the method could not have shown it.
...
No user config can add a built-in tool, so when the probe comes back empty, walk the ladder rather than trying to re-enable anything:
