...
- **Orca worktree reconcile** (Orca only) — if `hmad-dispatch env` reports `substrate: orca`, reconcile against Orca's worktree model, which persists across sessions and the mobile app where git+PID state does not:
  - `hmad-dispatch worktree-current` → compare the returned `.branch`/`.path` to the doc's Branch/worktree; a mismatch is a divergence line (you may be resuming in the wrong worktree). Its `.comment` is the last checkpoint the writing session left (see WRITE stamp) — quote it if present.
  - `hmad-dispatch worktree-ps` → list every worktree as `branch · comment` so in-flight siblings (parallel agents, queued fanout modules) are visible before you act.
