...
| 5d | No `STATUS:` line extractable from the codex scrape | `step5d:no_verdict:<module>` | "The agent dispatched and produced no verdict. Re-read with a larger `--lines`; if still absent, `hmad-dispatch clear codex` and re-dispatch. Never proceed on silence — an absent verdict is not a pass." |
...
| 6a-prime | No `ASSESSMENT:` line extractable | `step6a-prime:no_verdict` | "An empty architectural review is not `READY_TO_MERGE`. Re-read with a larger `--lines`; if still absent, `hmad-dispatch clear agy` and re-dispatch." |
