...
| `hmad-dispatch launch <codex\|agy> [--worktree <sel>] [--focus]` | Spawn a FRESH agent terminal via `orca terminal create --command …`, then **resolve its live handle by joining the create response's `paneKey` against `terminal list`** and pin that. The zero-manual durable identity path (H5) — no title/preview dependence. **Not blindly** `.result.terminal.handle`: through 2026-08-02 that field was observed 3× to be a pre-adoption placeholder the pane never adopted, and pinning it made every later dispatch vanish (J1). It is **intermittent, not invariant** — on Orca 1.4.192 `paneKey` was command-discriminated (`sleep 300` 16/16 present, agy 3/3 present, one additional id-selector control present, codex 11/11 absent; every response `surface: visible`), while every paneKey-less codex response handle was genuine. Therefore the paneKey join remains primary; when the key is absent, launch pins the response handle only after that exact handle independently appears in `terminal list`. A missing/unreadable listing or a response handle that never appears still fails loud within `HMAD_LAUNCH_RESOLVE_TIMEOUT` (default 20s). Launch command overridable via `HMAD_ORCA_CODEX_LAUNCH_CMD` / `HMAD_ORCA_AGY_LAUNCH_CMD` |
...
| `hmad-dispatch exec <codex\|agy> <promptfile> [--cd <dir>] [--model <m>] [--effort <e>] [--out <file>] [--timeout <s>]` <br>codex: `[--sandbox <mode>]`, `--effort` → `-c model_reasoning_effort=<e>` · agy: `[--sandbox]`, `--effort` native. **`--model`/`--effort` are overrides, not defaults** — unset, each CLI resolves its own configured setting (codex: `$CODEX_HOME/config.toml`, which under Orca is a per-account home, NOT `~/.codex`; agy: `model` in `~/.gemini/antigravity-cli/settings.json`, whose label carries the effort, e.g. `Gemini 3.1 Pro (High)`) | **Exit-code dispatch** (alternative to the pane REPL): runs the agent HEADLESS as a real subprocess, so it returns the agent's own exit code with no idle poll, and — pane-independent — sidesteps identity resolution (orca#9870) entirely. The final response → STDOUT (the STATUS:/VERDICT: carrier, like `ask`); run chatter → stderr. `$?` answers "did the CLI run"; still pipe STDOUT into `h_mad_extract_verdict.py` for "did the WORK pass" (exit 0 ≠ task passed). **`codex`** — `codex exec`, prompt via stdin (no keystroke cap), final message via `--output-last-message`; the 5d/5e IMPLEMENTER path (default `--sandbox workspace-write`). **`agy`** — `agy --print`, response straight to stdout; the AUDIT/REVIEW path (Phases 3/4/5b + 5e-review), a headless replacement for the agy `ask` scrape, run with `--dangerously-skip-permissions` so a tool request doesn't block. Tradeoff: no cross-dispatch session context, no human-visible pane — use where the task is self-contained and the exit code is the point. `--timeout` is a portable watchdog (returns 124; agy also gets `--print-timeout`), since `timeout` is not a macOS system component. **The prompt file is passed through verbatim — `exec` injects nothing.** h-mad's ASSEMBLED prompts already carry the portable-time-bound rule (an audit prompt splices `invariants.base.md` §"Portable time bounds"; the 5d/5e implementer carries it in `codex-implementer-prompt.md`), so a HAND-WRITTEN prompt is the one dispatch path with no rule in it. When you dispatch ad-hoc work that may need to time-bound a command, paste that rule in. Measured 2026-08-25 on one task needing a bound, dispatched twice: the agent given the rule ran `run --timeout <s> -- <cmd...>`; the agent without it probed `which timeout`, found a `brew install coreutils` copy, and improvised a bare `timeout <n>` form. **Both exited 124** — the exit code cannot tell them apart, only the command form in the log can |
| `hmad-dispatch run --timeout <s> -- <cmd...>` | **Portable time-bounded command runner** — the same `_exec_run` watchdog the row above uses, exposed for any command, not just an agent dispatch. Exits **124** at the deadline (GNU `timeout` convention, so a caller already branching on 124 needs no change) and the child's own code otherwise; stdin/stdout/stderr are inherited, so redirections on the call site reach the child. TERM → 2s grace → KILL, signalled to the whole **process group** (`set -m`; macOS ships no `setsid`), so forked grandchildren die with it rather than orphaning past the 124 — verified live. Exists because **neither `timeout` nor `gtimeout` is a macOS system component**: on a stock box reaching for one returns 127, and the measured reflex is to re-run the command *unbounded*, which converts a deadline into a hang; on a box carrying `brew install coreutils` it works instead, hiding the same unportable form behind a separately-installed CLI and removing the 127 that would have exposed it. A missing time-bounder is a halt, never an unbounded retry — and a local `command -v timeout` that succeeds is not licence to use it |
