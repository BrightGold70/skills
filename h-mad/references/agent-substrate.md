...
| `hmad-dispatch exec <promptfile> [--cd <dir>] [--model <m>] [--sandbox <mode>] [--out <file>] [--timeout <s>]` | **Exit-code dispatch** (Phase 5d/5e alternative to the pane REPL): runs `codex exec` HEADLESS as a real subprocess, so it returns codex's own exit code with no idle poll. Prompt via stdin (no keystroke cap). The agent's final message (`--output-last-message`) → STDOUT (the STATUS:/VERDICT: carrier, like `ask`); the run transcript → stderr. `$?` answers "did the CLI run"; still pipe STDOUT into `h_mad_extract_verdict.py` for "did the WORK pass" (exit 0 ≠ TDD passed). Sandbox default `workspace-write`. Tradeoff: no cross-dispatch session context, no human-visible Orca pane — use where the task is self-contained and the exit code is the point. Timeout is a portable watchdog (returns 124), since macOS has no `timeout` |
