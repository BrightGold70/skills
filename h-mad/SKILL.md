---
name: h-mad
description: "Orchestrate the seven-phase H-MAD development workflow end to end with project invariants, independent audits, test-first implementation, verification, and closure. Use for /h-mad feature, /h-mad do feature, /h-mad status, or /h-mad reset feature."
---
...
# Keep the claim LIVE while you work (#126): pass --session-id on every phase write
# (the owner's --set refreshes the heartbeat), or beat explicitly between long stretches:
...
**A claim that is never beaten looks abandoned two hours after Phase 0 (#126).** `--claim` was the only heartbeat writer, so a claim beat once and then aged toward the staleness window while its owner was busiest — measured live: one beat, then nothing for hours — and a stale-looking live claim is takeable by any other session with a plain `--claim`, which is the collision the window exists to prevent. Two writers now beat it: **pass `--session-id "<session-id>"` on every `--set`** (a phase write by the owner is a sign of life and refreshes the heartbeat in the same write; a bystander's `--set` still lands but refreshes nothing), and `--beat --session-id "<session-id>"` between long dispatch stretches. A beat never claims: it refuses an unowned feature (that is `--claim`'s job) and a claim held by another session, so it cannot keep a dead claim alive. Residual: the beat still depends on the orchestrator writing state; wiring it into `hmad-dispatch exec`'s own watchdog heartbeat is the follow-up.
...
7. **Closure (autonomous)** — **run the precondition gate first; it is what makes the 6-before-7 ordering real rather than documented:**
   ...
   python3 ~/.claude/skills/h-mad/scripts/h_mad_phase7_preconditions.py \
     docs/.bkit-memory.json --feature <feature>
   ```
   Parse the **token**, not `$?` (exit 0 on any verdict, 2 on operational error). `PHASE7: BLOCKED` → halt `step7:verification_not_run` and address each blocker. It refuses to close a feature that never ran Phase 6, whose analysis is missing or states no match rate, whose rate is below threshold, whose 6a-prime returned `WITH_FIXES`/`NO`, that carries an open halt, or that parked a mutation spec (`<name>.json.pending`) and never moved it back (#142). A `SKIPPED_NO_PANE` archreview **blocks** the gate — a headless `exec agy` review satisfies the gate. Then `h_mad_telemetry.py record`, inline report + archive (`references/inline-protocols.md §Phase 7`), then `git add -A -- ':/' ':(top,exclude)*.done' && git add -u -- ':/' && git commit`, then **7f — integrate** (below), then `git push origin <base>`.

   **7f — integrate the branch, which nothing used to do. It runs AFTER 7d's commit and before 7e's push**: 7c's archive `mv`s tracked documents, so a 7f placed before the commit blocks on its own `dirty_tree` gate on every real run. 5c creates `feature/NNN-<slug>` and this phase's exit criterion is "Push to origin/main succeeds", but no step between them merges: the stated commands cannot reach the stated exit, and the merge was performed by hand every time, carried between sessions as a handoff Next Step. Run `python3 ~/.claude/skills/h-mad/scripts/h_mad_phase7_integrate.py --repo-root <repo> --feature <feature> [--base <b>] [--route merge|pr|keep] [--apply]` and read the `INTEGRATE:` token, **never `$?`** — every verdict exits 0 and only `UNREADABLE` (git did not answer) is non-zero. Without `--apply` it PLANS: it evaluates every gate, prints the exact commands, and touches nothing. `BLOCKED` names its reasons and each is a fact about the world, not an error. The full set, which `test_h_mad_phase7_integrate_blockers_are_documented` holds to the script: `detached_head`, `dirty_tree:N_tracked_changes`, `no_default_base`, `unknown_base:<b>`, `base_not_a_local_branch:<b>`, `unknown_branch:<b>`, `base_is_the_feature_branch:<b>`, `base_checked_out_elsewhere:<worktree>`, `untracked_collision:<n>:<path>`, `nothing_to_integrate:<branch>_is_not_ahead_of_<base>`. **`--base` must name a LOCAL branch.** A tag, a raw sha or a remote-tracking ref would make `git checkout` DETACH — the merge would land on no branch, the base would never move, and the verdict would say MERGED at exit 0 — so those are refused as `base_not_a_local_branch:<b>` rather than followed. On a checkout whose trunk exists only as `origin/main` the remedy is to create it (`git branch main origin/main`); 7f will not do that for you, because which commit the local trunk should start at is not 7f's judgement to make. `no_default_base` is the same situation reached without `--base`: no `origin/HEAD` is set and neither `main` nor `master` exists locally. **`identity=y` is the verification half**: when the base is strictly behind, a `--no-ff` merge writes a tree byte-identical to the feature tip, so the green suite run you already have covers the merged result by proof rather than by a second eight-minute measurement (measured 2026-09-07 on `doc-block-exec`: both trees `381aaba2`). `identity=n` means the base moved and the merged tree is NEW — re-run the suite before 7e. It never pushes, never deletes a branch, and never removes a worktree: sibling worktrees are REPORTED with their removal command, because that lane may have a live owner and #111 sat across five handoffs precisely because nothing in the protocol named it. **The route is recorded, never asked** — Phase 7 is autonomous, so a blocking three-option menu would break its contract; `keep` and `pr` integrate nothing and are not failures.
...
- **5g** — `git add -A -- ':/' ':(top,exclude)*.done' && git add -u -- ':/' && git commit -m "feat(<feature>): implement <module>"` per module. **The pathspec is load-bearing, and both halves are.** The audit `.done` markers are untracked and NOT gitignored, so a bare `git add -A` commits every one of them — 88 in this repository at the time of writing, and "do not commit the markers" has ridden every handoff as a manual warning instead. Gitignoring them is the wrong fix in the other direction: `hmad-dispatch` COUNTS them (`^?? .*\.done$`) to exclude them from its `tree delta`, so an ignore would blind a measurement that currently works. `':/'` anchors the sweep at the repository root and `':(top,exclude)*.done'` anchors the exclusion there too — a CWD-relative pair silently stages only the subtree you happen to stand in, and excludes only that subtree's markers (measured: from a subdirectory, 1 of 3 changed files staged and a root marker committed). The second `git add -u` then stages any TRACKED `.done` file's real modification, which the exclusion alone would suppress forever. Write `phase = null` (disarms TDD gate hook). Emit `[H-MAD] <feature> phase5 complete`.
...
### Codex authors Phase 5 — enforced, not just instructed
  ...
  **Parking a mutation spec (D2), and the detector for one never moved back (#142).** Mutation rows are written at GREEN against source that exists, never at RED; a spec drafted before its source lands is *unanchored*, and an unanchored spec under `tests/mutation-specs/` reds the committed-spec sweep and blocks the pre-push hook. Park it instead as `docs/03-analysis/<feature>.pending-mutation-specs/<name>.json.pending` and restore it to `tests/mutation-specs/<name>.json` at that task's GREEN. **The parked copy stays afterwards as provenance — do not delete it.** The name match is the whole convention, so name the parked file exactly as the live one will be named. What nothing used to detect is the other case: `h_mad_phase7_preconditions.py` now blocks with `pending_mutation_spec_not_restored` when a `<name>.json.pending` has no live `<name>.json` in any `tests/mutation-specs/` directory up to two levels below the repo root (`--mutation-specs-dir` names one explicitly), and warns `pending_specs_unverified` when it finds parked specs but no such directory at all — "I could not check" is not "never moved back". A parked spec's guards were never measured, and the sibling sweep cannot see it precisely because parking moved it out of the swept directory.
...
On a revision cycle, before the audit is re-dispatched:

```bash
python3 ~/.claude/skills/h-mad/scripts/h_mad_delta_review.py \
  --repo <repo> --rev <the revision commit> [--path <doc> …]
```

Read the `DELTA:` token, never `$?` — `CLAIMS` is an answer and exits 0; only `UNREADABLE` (git
could not answer for that revision) exits 2. **`unverified` is a cannot-judge and appears in the
verdict line on purpose**: those spans hold shell metacharacters or a verb outside the read-only
allowlist, so they were not run, and a count of what could not be checked that is missing from the
token reads as a pass.

**Why a script and not the habit.** The hand-run version found fix-introduced defects in **3 of 3**
passes at roughly a quarter of a gating round's cost — and failed twice in one sitting, once to a
`</dev/null` on a piped helper and once to zsh applying the `:h` modifier inside `$h:hematology`. A
review whose own instrument fails silently is worse than none, because it reports clean.

**It never executes arbitrary text from a diff.** Prose backticks hold illustrative commands and
destructive examples; `h_mad_doc_block_exec.py` may execute document blocks only because they are
explicitly TAGGED, and nothing in a diff hunk is. A span is run only when it is a bare read-only verb
with no shell metacharacters — matched on the VERB, since `rm -rf / # grep` contains `grep`.

Only spans that could be commands at all are reported: a verb plus an argument. Measured on the real
commit `8af16d9`, the first version treated every backticked span as a claim and returned
`claims=15 executed=0 unverified=15` — every one an identifier (`doc-auditor`, `DIVERGED`,
`[H-MAD]`). An alert that is always wrong is worse than no alert. On `a90c365` the filtered form
returns `claims=2`, both genuine published commands.

This is ADVISORY and does not replace the second surface — see §"Delta self-review" for the round
protocol it mechanises.

### Tag every must-fix with its ORIGIN, at the moment it is verified (#11/H7)

The audit loop's largest class is defects that an earlier cycle's *fix* created, and until this
shipped nobody could say how large. Reconstructed from memory at handoff, `#18`'s ledger could only
report "at least **17 of 98** cycles carry a must-fix that an earlier cycle's fix created (seeded
from recorded evidence — a lower bound; a blank cell means *unclassified*, not *new*)". Tagged as
they were verified, the same loop's cycles 98–103 answer exactly: **51 of 152 must-fixes, 33.6%**,
the largest single class, plus 27 more (`author-self-caught`) that the revising author's own delta
review caught one step earlier. Neither number is recoverable after the fact.

After each cycle's reports are persisted, append one record per must-fix — **at the moment the
must-fix is verified, not at handoff**, which is the whole point:

```bash
python3 ~/.claude/skills/h-mad/scripts/h_mad_audit_origins.py \
  --feature <feature> append --phase design --cycle 99 --leg codex \
  --item "<the finding, 80 chars>" --origin fix-introduced --by-cycle 98 \
  --verified-by-execution
python3 ~/.claude/skills/h-mad/scripts/h_mad_audit_origins.py \
  --feature <feature> summary --per-cycle
```

The eight origins are the ledger's and are not re-derived: `new-mechanism` · `new-consistency` ·
`fix-introduced` (with `--by-cycle`) · `propagation-gap` (a fix reached one document and stopped;
also `--by-cycle`) · `instrument` (the finding is about a command or count the document publishes) ·
`record-stale` (a Version-History record wrong at its own commit) · `author-self-caught` (optional
`--caught-by self|advisor|orchestrator`) · `rejected` (a false finding).

**Read the `ORIGINS:` token, never `$?`.** `OK` and `INVALID` are both answers; only `UNREADABLE`
(no sidecar, or an unreadable one) is a cannot-judge and exits 2.

**An absent sidecar is not a rate of zero**, and the tool says so rather than printing one: an
untagged feature and a feature with no fix-introduced defects are opposite findings that a bare `0`
spells identically. Same reason an unparsable line is named with its line number instead of skipped
— a dropped record makes every count smaller, and smaller reads as progress.

**The fix-introduced denominator excludes `rejected`**, and the output line states that it does.
Counting false findings would mean the cycles that filed the most noise scored best.

**This script never decides an origin.** Classifying a must-fix is a judgement about why a defect
exists, so `append` records one the orchestrator has already made. That is also why nothing hooks
it into `h_mad_audit_cycle.py`: the driver does not know the moment a finding is verified.

**Calibrated before it was wired**, against the 152 records that already existed: every hard rule
fires **zero** times on them. That bar is not ceremony — every `h_mad_precheck_doc.py` detector
first written as hard fired 104 / 49 / 48 times on documents that had just passed 83 and 74 audit
cycles, and all of those hits were correct usage.

### Shared sentences are single-sourced, and adoption is checked BEFORE the commit (#11/H5)

When two or more documents must state the same fact, the design owns the sentence and the siblings
quote it **verbatim**. Before committing a cycle that touched a shared fact, run:

```bash
python3 ~/.claude/skills/h-mad/scripts/h_mad_adoption_check.py \
  --sentence-file <the design's canonical line> \
  --doc <sibling1> --doc <sibling2> [--doc …]
```

Read the `ADOPTION:` token, never `$?` — `DIVERGED` is a verdict and exits 0; only a cannot-judge
(`UNREADABLE`: an unreadable document, an empty sentence, fewer than two documents) exits 2.

**Why this cannot be left to a reviewer.** Measured on `#18 gateway-consolidation`: two authors
independently corrected one shared fact in the same cycle, one to **18** and one to **20**. Each
document was internally consistent, so a reviewer reading any single document found nothing and a
reviewer reading both had no reason to diff one sentence out of thousands. Three of the last five
cycles' must-fixes there were propagation gaps of that shape. `doc-auditor` rule 7 already carries a
cross-document reading, but it runs in the NEXT cycle, after the divergence is committed — this
moves the same question before the commit and answers it mechanically.

Whitespace is normalised on both sides, because documents hard-wrap at different columns and a byte
comparison would report `DIVERGED` on siblings that adopted the sentence perfectly. What is **not**
normalised is the wording: this is an adoption check, not a similarity score, and h-mad has already
refused fuzzy matching once by measurement (§"Why the ack match is not fuzzy").
...
(`docs/03-analysis/probes/<feature>/<name>.<date>.<session>.py`), plus **one** reading stamped at one
sha. It does not publish sha-series of its own readings, ledgers of its own audit reports, trip-wires
over `h-mad/`, counts of its own sections, or any figure that a commit *of this feature's own
artifacts* moves. Measured on `doc-block-exec`: the design at `--vh-tail 1` assembled to 980,076 of
the 1,048,576-character `exec` ceiling, the plan carried a ten-sha ledger that the gating reports
landing advanced by one every round, and 9 of the 15 r18 musts were in that layer. A number that
lives in a probe is re-derived by running it; a number that lives in the document is re-stamped by
an author, and the re-stamp is the next round's must.

Two consequences. **Do not restructure a document mid-flight** — extracting the layer is itself a
revision batch that spawns two more rounds; apply the rule to the next feature, and to a document
whose audit prompt no longer fits (`ASSEMBLE: HALT <phase>:oversize` at `--vh-tail 1` is the trigger
that makes it mandatory, because the audit itself has become impossible). **A measurement-class
finding on an existing document is acknowledged with its re-run command**, never re-stamped by hand
...
Every Must-fix and Should-fix bullet a reviewer files carries a **class**, stated on a continuation
line under the bullet — the same shape as `quote:`, and like `quote:` never a `- ` bullet of its own:
...
The operational test is one question, worded identically here, in `audit-prompt.template.md` and in
`agents/doc-auditor.md`: **would the code or tests a 5d/5e implementer writes differ if this finding were fixed?** Yes → `build` (an interface, a contract, a predicate, a
fixture, a test's assertion or its RED/GREEN count, a false premise about the platform an implementer
would code against). No → `measurement` (a published number, stamp, ledger row, sha series, pointer or
self-count about the tree that is stale or mis-derived — the tree is unchanged by fixing it).
...
put 12 of 12 fix-introduced musts in the same layer. A document that publishes numbers about a tree it
moves is stale by construction, so a gate that scores that layer like a false timeout semantics cannot
converge — it measures itself. The build class is where the two model families kept finding DISJOINT
sets (r16, r17, r18), which is the class the loop exists for.

**Exit rule, both audit loops (Phase 3 auto-cycle and the 5b bullet):** build-class must = 0 on BOTH
surfaces at one commit; every remaining finding fixed or acknowledged. Measurement-class findings never
hold a phase past the round in which build-class reached 0 — they go to `## Acknowledged-not-fixed`
with their **re-run command** as the ack text, so the next reader can re-derive the figure instead of
trusting it. **The decision is made blind:** the orchestrator writes the classification rule into the
round's decision sheet BEFORE reading the round's verdicts, and may re-classify a reviewer's
...
- **Run the project suite with the gate in Phases 3, 4 and 5b, and read `SUITE:` as its own line (#91).** `python3 ~/.claude/skills/h-mad/scripts/h_mad_audit_gate.py <report> --gated <doc> --project-tests <project-test-root> [--suite-cmd "<cmd>"]`. Measured on HemaSuite `#18 gateway-consolidation`: a dual-surface design audit ran **95 cycles** without meeting its exit gate while a repo test enforcing the **audit loop's own invariant** stayed red for the entire life of the feature — `tests/test_audit_phase_inline_summary_sync.py::test_real_features_synced`, whose guard is that every earlier cycle recording a FAIL must be cited in the Version History. It went green for the first time after 91+ cycles. The gate could not have caught it: before this it held **zero** occurrences of `subprocess|pytest|check_call|os.system`, and `pytest` appeared in this file only at 5e and 5f. **The root must be SCOPED** — an unscoped pytest from a repository root collects sibling-project import mismatches (step 5f says so, and HemaSuite carries two suites under a monorepo root). The verdict is scored on pytest's own **summary line, never on `$?`**: a skipped selection and a killed run both exit 0, and `SUITE: UNREADABLE reason=no_summary` is a cannot-judge that is neither a pass nor a failure.
- **A red suite blocks the EXIT, not the cycle.** The per-cycle `GATE:` verdict is untouched — blocking every cycle would make an audit hostage to an unrelated flaky test, and `docs/skill-candidates.md` §`concurrent-suite-runs-manufacture-phantom-failures` records two pytest runs over one working tree producing 6 and 3 failures in *different* sets, and 0 when the file ran alone. The refusal lives at the streak instead: the stamp records `"suite"`, and `--exit-check <stamp>…` prints `EXIT: READY|BLOCKED reason=<r>`, where a cycle whose suite was red **or unmeasured** cannot contribute. `suite: null` is what a cycle run without `--project-tests` records, and it is refused for the same reason silence is never a pass. This is enforced in the gate rather than written here as a step because **nothing counts the streak** — `grep -in 'streak|consecutive'` over `scripts/` returns nothing, so the two-consecutive-both-clean rule existed only as prose, and an instruction an orchestrator can skip is exactly what 91 cycles demonstrated.
- **Name the reviewer legs with `--legs`, and read `GATE-LEGS:` as its own line (#11/H3).** `… --gated <doc> --legs codex --legs agy`. A clean streak counts only over an **unchanged leg set**: `--exit-check` blocks with `reason=legs_changed:<a>|<b>` when the two cycles ran different legs, because a streak over two different leg sets measures two different questions. Measured on HemaSuite `#18 gateway-consolidation` (`gateway-consolidation.audit-ledger.md`): Σmust tracked the **leg count**, not document quality — the 13 design cycles ending at **c99** ran `3 3 7 5 3 5 4 3 8 4 7 12 6` and every rise coincided with a leg added or returning (teammate c87, doc-auditor+crossdoc c92, codex c97). Re-derive the series from the ledger rather than reading that window as current; it has grown since. The exit streak was reset by new legs **four times** and never once approached by the documents — zero at c99 when this was measured, and still zero at c104. Changing the set is legitimate — it just starts a **new baseline**, which falls out with no extra flag, since two fresh cycles under the new set agree with each other.
- **Drive the gate through `h_mad_audit_cycle.py` and pass the flags THERE — the driver feeds every leg.** `--project-tests <root> [--suite-cmd "<cmd>"] --gated <doc> [--legs <name> …]` on the *cycle driver*, not on each gate call. The suite runs **once per cycle** and its verdict is forwarded to every per-leg gate invocation as `--suite-result`; running it inside the gate would be minutes per leg and is the design `audit_suite_gate.json` rejects outright. The leg set is forwarded identically to every leg, so the legs of one cycle cannot disagree about what that cycle ran. Before this, `gate()` built its command with the collected report and `--ack-file` and nothing else, so **every cycle driven through the script recorded `suite: null` with no leg set** and its exit gate was refused `suite_unmeasured` — both gates shipped, correct, and unreachable from the path that actually runs. `--suite-result` and `--project-tests` are mutually exclusive on the gate: one field, one source. **`GATE_RE` now tolerates trailing fields** on line 1, because `--gated` appends ` gated=N` there; the gate's own comment had recorded that collision as latent "because the cycle driver never passes `--gated`", and wiring this made it live — the first gated run died with `ERROR: missing GATE token` against a gate that had answered `GATE: PASS must=0 should=0 gated=1`.
- **A streak is two CYCLES, not two stamps — `--exit-check` groups by cycle.** A cycle emits one stamp **per leg**, so counting stamps counts legs. Measured 2026-09-07 over the real archive: of 17 cycles carrying stamps, **8 have two**, and same-cycle legs sort *adjacently*, so a glob hands the gate two legs of one cycle. Executed against the then-shipped gate, the real pair `doc-block-exec.design.audit.v20.codex` + `…v20.p1` returned `EXIT: READY` — a two-consecutive-clean-cycle streak certified from **one cycle**. H3 made it read worse rather than catching it: both legs carry the same declared leg set, so the leg check passed and lent false confidence. The cycle is derived from the stamp's name (`<feature>.<phase>.audit.v<N>.<leg>.md.gated.json`), ordered **numerically** (`v10` sorts before `v9` as text), **every** leg of a counted cycle must be clean with a green suite, and the legs of one cycle must agree about what that cycle ran (`legs_disagree:<phase>.v<N>`). A stamp whose name is outside that grammar is `EXIT: UNREADABLE reason=stamp_name:<name>` — a cannot-judge, never a pass, because an unplaceable stamp may be a second leg of a cycle already counted. Neither the offline suite nor the field tracer could see this: both used fixtures that were distinct cycles *by construction* (`v1.gated.json`/`v2.gated.json`, and v43/v44). The fixtures now carry the real grammar.
- **An unrecorded leg set does NOT block — it is reported.** `legs: null` is what a cycle stamped without `--legs` records, and it reads as a cannot-judge, not a mismatch: every stamp written before the field existed has none and was not thereby wrong. This is deliberately **unlike** `suite`, which the gate measures itself when asked; a hard fail here would fire on cycles that legitimately passed, which is the calibration error `docs/learnings.md` already records (detectors written as hard fired 104/49/48 times on documents that had passed 74–83 cycles). It is still never silent: `EXIT: READY … legs=unrecorded`, and `GATE-LEGS: unrecorded` on **every** stamped PASS, so the nudge lands each cycle at the point of use rather than once at the exit.
...
- `h_mad_done_gate.py` — author DONE-line gate (§"Teammate authors" rule 6): `find_done_line()` / `parse_fields()` / `digest()` / `run()` + CLI printing `DONEGATE: PASS path=<p> sha256=<hex> lines_check=ok|disagree|absent`, `DONEGATE: FAIL reason=<r>` (exit 1) or `DONEGATE: UNREADABLE reason=<r>` (exit 2). Re-derives the artifact's sha256 and refuses a mismatch, because a subagent's "stopped writing" is an **instant, not a state** — a post-DONE dispatch can make the author resume and write again, so the notification proves nothing about the bytes on disk. `--expect-path` is the half that catches the likelier mistake: an author that hashes the WRONG file reports a digest that verifies perfectly against the file it named. A missing artifact is `FAIL reason=artifact_absent`, not `UNREADABLE` — the author asserted the file exists, so absence refutes the claim rather than obstructing the gate. `lines=` never gates (`wc -l` counts newlines, so no trailing newline reads one short) and is reported as `lines_check=`. The protocol ran on five live author dispatches and matched five for five before anything required it. **A run holds a TREE LOCK for its duration** (`<git toplevel>/.h-mad/mutation.lock`), because a run rewrites files in place and a second run — or a plain `pytest` in another pane, or a sibling session committing into the same clone — measures a TORN tree. The dangerous part is that the resulting verdict is PLAUSIBLE: `REFUSED` for an anchor that is fine is exactly what real drift produces, and nothing at the token separates them (measured: a spec launched while its own anchor lines were still being edited). A second run gets `MUTATION: BUSY holder=<pid>` and exit 2 — a wait instead of a wrong measurement. A lock whose holder is proven DEAD is taken, so a crash cannot wedge the repo; an UNPARSEABLE lock is never taken, because "I could not read it" is not "nobody holds it", and the message names the file to delete so the stuck case is one `rm`. `os.kill(pid, 0)` raising **EPERM means ALIVE** — folding that in with ESRCH steals a live holder's lock, the same defect recorded for `is_pid_alive` (#40). **`--check-anchors` takes no lock**: it only reads, so the cheap diagnostic and the pre-push hook keep working while a run is in flight. The lock cannot stop a SIBLING SESSION committing into the same clone, so the run also reads `HEAD` on both sides of itself and returns `MUTATION: TREE_MOVED before=<sha> after=<sha> inner=<verdict>` (exit 2) when it moved: the mutations were applied to one tree and scored against another, and every number in the inner verdict still looks plausible, which is why it needs a verdict rather than a note. Measured 2026-09-14 — `4915206` landed inside a full-suite run here. The inner verdict is KEPT and printed, since it is still the most informative thing anyone has about those mutations. An unavailable `HEAD` read is NOT a change: both reads must be present AND differ, or every non-repo spec would refuse. Stdlib-only.
...
- `h_mad_check_memory_index.py` — pre-check a memory index against the caps Claude Code enforces at LOAD time: CLI printing `OK|WARN|OVER|UNREADABLE <path> bytes=N/25000 lines=N/200 worst=<dim> frac=N% dropped=N target=N`, exit 0 clean / 1 WARN-or-OVER / 2 unreadable. **It exists because the built-in warning is one write too late and the loss is silent**: past the cap the write SUCCEEDS and, in the binary's own words, *"everything past the limit is silently dropped each time the index is loaded — entries at the end are already invisible to readers"*. Nothing in the tree looks wrong and `wc -c` cannot see it. Caps are decoded from the binary (25000 bytes / 200 lines, warn at 0.8, compact to 0.7 = 17500/140) and **re-grepped by the test on every run**, which SKIPS rather than passes when the binary is unreadable. **Both dimensions are scored and the WORSE one binds** — an index at 40% of its byte cap can still be over on lines, so a byte-only check reports healthy on a broken index; that is the row its mutation spec calls load-bearing. `--show-dropped` prints the invisible text rather than a count, because `dropped=254` is a number and the two entries it is deleting are the finding. Default target is every `~/.claude/projects/*/memory/MEMORY.md` de-duplicated **by inode** — those directories are routinely symlinks onto ONE store (measured: 40+ paths, one inode), and without the de-dup a single problem prints dozens of times. Wired as a pre-check in `handoff/references/auto-memories.md`. Stdlib-only.
- `h_mad_precheck_doc.py` — phase-document pre-dispatch precheck (§"Precheck before you dispatch"): `scan()` + CLI printing `PRECHECK: PASS|FAIL issues=N|UNREADABLE`, exit 0 on a verdict / 2 on operational error. Refutes before the prompt is assembled what a cycle would otherwise spend two dispatches discovering. **Hard** findings are only the provably-wrong ones — `PLACEHOLDER` (an unfilled `key=…` or bare `<slot>`, impl-plan only, plus `TBD`/`TODO`/`FIXME` anywhere), `LINEPIN` past end-of-file (and, in `design`/`plan`/`spec`, ANY line pin — those author contracts forbid one, `NO_LINE_PINS` now enforces it, and `docs/archive/**` is exempt because an archived document cannot be repaired), `PINDRIFT` (a pin into a file that changed since the document's own newest provenance commit — the c33 defect exactly, six `SKILL.md` pins stale by 93 lines), and `UNKNOWNSHA` (a provenance sha naming no commit here). **Advisory, verdict-neutral**: `PATH` and `SYMBOL` (a planning document names files and symbols the feature will CREATE), `STALESHA` (behind-HEAD is the normal condition of every written measurement), ordinary `LINEPIN` **in `impl-plan` and in archived documents only**, and `COUNT`. Every one of those started as a hard finding and was demoted by measurement, not by taste: they fired 104, 49 and 48 times on the design and plan that had just passed 83 and 74 audit cycles, and the hits were correct usage. `--allow` is an input, never inferred — but **the `## Version History` demotion IS inferred, from the document's own content**, and it is the one exception: every hard finding inside that section is demoted to `allowed` (still printed, never dropped), because the section holds prose ABOUT changes and the detectors read it as the changes themselves. Measured: one impl-plan failed entirely on its own changelog saying "**TBD placeholders removed.**". The section is bounded by `h_mad_version_history`'s own notion — sole full-line `## Version History` heading, ending at the next header or `---`, fence-aware — and **two headings demote nothing**, because first-match on a document carrying a template history above its live one would silence the body between them. So a `PASS` may be a demoted `FAIL`; read the `ALLOWED:` lines. The residual it cannot close: a document that NARRATES a stale pin quotes the stale number, and no detector distinguishes that from the defect. Stdlib-only.
- `h_mad_delta_review.py` — delta self-review over a revision's ADDED lines (#11/H4): `is_command_shaped()` / `is_executable()` / `added_lines()` / `claims()` + CLI printing one `DELTA: CLEAN|CLAIMS|UNREADABLE claims=N executed=E unverified=U` line, a `executed:`/`unverified:` line per claim with the source line, then the `[H-MAD]` marker. Exit 0 on a verdict, 2 on a cannot-judge. Executes only a bare read-only verb with no shell metacharacters, matched on the VERB and never as a substring; everything else is `unverified` and counted. Stdlib-only.
- `h_mad_audit_origins.py` — per-must origin tagging (#11/H7): `validate()` / `read_sidecar()` / `aggregate()` / `share()` + two subcommands. `append` validates one must-fix record and appends it to `<docs-root>/03-analysis/<feature>.audit-origins.jsonl` (or `--sidecar`), printing `ORIGINS: APPENDED cycle=N origin=X leg=L`; an invalid record is REFUSED and nothing is written, since a malformed record on disk is counted by every later summary. `summary` prints one `ORIGINS: OK|INVALID|UNREADABLE n=N accepted=A rejected=R fix-introduced=C/A (P%) self-caught=C/A (P%) cycles=lo-hi` line, an origins breakdown, then the `[H-MAD]` marker; `--per-cycle` gives the trend the hypothesis is about and `--cell` prints ledger table cells for the owning lane to paste. Exit 0 on a verdict, 1 on `INVALID`, 2 on a cannot-judge. **The eight origins are the ledger's** and an unknown one is the single hard error over content — a typo that becomes its own bucket lowers `fix-introduced` and reads as the loop improving. `by_cycle` is required for `fix-introduced`/`propagation-gap` only; other origins may carry it (33 of the prototype's 152 do) and unknown keys pass through, because `severity`/`note` are in the real corpus and were never in the schema block. An **absent sidecar is `UNREADABLE`, never a rate of zero**, and an unparsable line is named with its line number rather than skipped — both are the same failure family, an absence that spells like a good result. The fix-introduced denominator excludes `rejected` and the output says so. Calibrated against the 152-record `gateway-consolidation` prototype (cycles 98–103) copied to `tests/fixtures/`: every hard rule fires zero times on it. Stdlib-only.
- `h_mad_adoption_check.py` — shared-sentence adoption gate (#11/H5): `check(sentence, docs)` + CLI printing one `ADOPTION: ADOPTED|DIVERGED|UNREADABLE docs=N adopted=N missing=N` line, then a `missing:` line per sibling that did not quote it, then the `[H-MAD]` marker. Exit 0 on a verdict (`DIVERGED` is an answer), 2 on a cannot-judge — an unreadable document, an empty sentence, or fewer than two documents, since adoption is a relation between siblings. Whitespace is collapsed on both sides so a hard wrap or a bullet is still adoption; the wording is not normalised at all. `--sentence-file` is preferred over `--sentence`: the design owns the line and retyping it is how it drifts. Stdlib-only.
- `h_mad_audit_gate.py` — audit-gate verdict unit (single source of truth): `classify()` (three keys, every caller) / `classify_detail()` (plus the per-class breakdown) + CLI printing `GATE: PASS|FAIL must=N should=N`, then `GATE-CLASS: build=N measurement=M untagged=K ack_refused=R` (§"Finding class — build vs measurement": untagged and unknown count as build; a `class: build` bullet is refused by the sidecar), then, on a stamped PASS, `GATE-LEGS: <a>+<b>|unrecorded` (§"Name the reviewer legs"; `--legs NAME` repeatable, normalised sorted/deduped into the stamp as `legs`, `null` when unnamed — never `[]`, which would claim zero legs ran), then the `[H-MAD]` marker. **Full emission order is four lines, and `SUITE:` comes FIRST** — `SUITE:` → `GATE:` → `GATE-CLASS:` → `GATE-LEGS:` — measured by executing the CLI on 2026-09-07 (`docs/03-analysis/hmad-gate-field-verification.md`); an earlier revision of this entry described three lines starting at `GATE:`. Nothing anchors on the order (`GATE_RE` matches by line), but a documented sequence that omits a line is a claim that does not survive execution. `SUITE:` is absent without `--project-tests` and `GATE-LEGS:` without `--gated`, exit 0 on verdict / 2 on operational error; `--must-only` for the `/h-mad do` precondition. `--exit-check <stamp>…` prints `EXIT: READY|BLOCKED|UNREADABLE`, refusing a streak whose suite was red or unmeasured (#91) or whose **leg set changed between the two cycles** (#11/H3, `reason=legs_changed:<a>|<b>`); an unrecorded leg set is reported as `legs=unrecorded`, never blocked. Imported by `h_mad_do_preconditions.py` Since #15 an `## Acknowledged-not-fixed` bullet matches a finding through canonicalisation (emphasis, backticks, wrapping, case, trailing punctuation) or an explicit leading `[key]`; **never through text similarity** — see §"Why the ack match is not fuzzy" for the measurement that refused it.
...
- `h_mad_assemble_tdd.py` — Phase-5d/5e Codex dispatch assembler: `assemble()` / `task_body()` + CLI printing `ASSEMBLE-TDD: PASS <promptfile> <bytes>B phase=<red|green> task=<id> shape=<s>` followed by the verbatim command block (backgrounded `exec codex` with `--model`/`--effort`/`--out`/`--log`/`--timeout` → `progress` → `h_mad_extract_verdict.py --key STATUS` → an INDEPENDENT scoped pytest), exit 0 on PASS / 2 on `ASSEMBLE-TDD: HALT <reason>`. It stages the prompt and prints the commands; **it does not dispatch** — the dispatch/poll/wait loop is §"Exit-code dispatch for 5d/5e", and a driver that dispatches either blocks blind for the timeout or re-implements `progress`. Cuts §"Task N" out of the impl-plan using the wire-pin gate's own `_TASK_RE`/`_parse_tasks` rather than a second parser (#39), and stamps the phase into the task section because the template carries BOTH the RED and the GREEN instructions and nothing in it says which applies. Halt reasons: `task_not_found`, `impl_plan_unreadable`, `template_unreadable`, `residual_slots`, `no_wire_pin`, `counts_required`, `interpreter_has_no_pytest`, `test_path_misrooted`, `sandbox_read_only`, `prompt_unwritable`. The defaults exist because hand-assembly produced five distinct mistakes, each an invocation default rather than a judgement: a model pinned in the assembler at all — it outranks the codex CLI's own `$CODEX_HOME/config.toml`, so the model the TUI shows and the model 5d/5e runs drift apart; `--model`/`--effort` are OVERRIDES, emitted only when given (effort reaches codex as `-c model_reasoning_effort=<e>`), and the escape hatch they provide is real because a configured `gpt-5.6-luna` cannot execute tools and fails as a well-formed `STATUS: BLOCKED`; a bare `python3` (3.14 here, no pytest) — so the chosen interpreter is PROBED for pytest and a failure names a working one; the prompt passed inline when `exec` takes a FILE PATH; `--sandbox read-only` on a run that executes pytest, which kills pytest's tempdir so the pass measures nothing — refused; and an unscoped pytest that collects the sibling project, so `--test-path` is required and the prompt tells the agent not to widen it. `--test-path` is also **rooted**, which the interpreter always was and the path was not: it is interpolated verbatim into the prompt (run under `--cd <project-root>`) and into the operator's re-run line, so one string had to be correct relative to two roots. Given subproject-relative, pytest exits 4 on the missing file — or **5** when a same-named directory exists at the root, which runs, collects nothing and reports no error. Absence alone is NOT refused, because a 5d RED names the file the agent is about to write; a path that exists SOMEWHERE ELSE under the root is `test_path_misrooted`, and the refusal names the path that would have worked. The operator's re-run line now runs in a subshell `cd`'d to the project root. **What it refuses to guess is the judgement:** a 5d without `--expect-fail`/`--expect-pass` is `counts_required`, never defaulted, because "guard changed" and "test weakened" are otherwise indistinguishable. Supplying HALF a split — one of the pair without the other — is also `counts_required`, for **every shape and every phase**, including the two carve-outs that allow neither count: absent-BOTH is an exemption, absent-ONE is an incoherent invocation whose given half would reach no part of the prompt. A `wiring` task needs no counts — the RED split is identical either way — but its `WIRE`/`WIRE-PIN` are carried automatically and a missing pin is `no_wire_pin`. **Needing no counts is not the same as discarding the ones you gave.** Counts supplied for a `wiring` task are stated with an informational rider (the WIRE-PIN, not the split, is what gates it), and the regression-guard line is emitted for EVERY shape — guards are orthogonal to shape, and the label is what makes an immediate pass legitimate rather than something to manufacture away. Until 2026-09-21 the WIRE lines REPLACED the counts-and-guards block instead of preceding it, so `--expect-fail`/`--expect-pass`/`--guard` were parsed, validated and silently dropped on every `wiring` task while the run still ended `ASSEMBLE-TDD: PASS`. Filed from HemaSuite after four of one feature's wiring dispatches shipped with no stated split at all — rescued only by the orchestrator's habit of hand-writing a preamble, which was not correcting the spliced values but was their sole source. That habit is no longer load-bearing. Stdlib-only.
...
- `h_mad_version_history.py` — the phase-doc `## Version History` bump: `bump()` / `plan_insertion()` + CLI printing `VERSION-HISTORY: OK|DRY-RUN path=<p> version=<v> line=<n> placement=<append|prepend>`, exit 0 on a completed write / 2 on refusal. Refusals print `VERSION-HISTORY: REFUSED path=<p> reason=<r>` carrying **no `line=`**, and an unreadable path prints a bare `VERSION-HISTORY: UNREADABLE` — a cannot-write must never read as a write that landed. Reasons: `anchor_missing`, `anchor_ambiguous`, `table_shape`, `unknown_shape`, `mixed_order`, `duplicate_version`, `bad_version`, `empty_text`, `multiline_text`, `splice_not_additive`, `unreadable`, `fence_grammar_unavailable`. The section boundary is resolved over `h_mad_doc_block_exec._fence_events`, the repo's one home for fence and ATX grammar, which is why the last reason exists: if that grammar cannot be imported the bump REFUSES rather than falling back to the `startswith("```")` counter it replaced, because a wrong boundary splices an entry into a code block or past a real heading and a silent downgrade is worse than no write. That port also fixed the anchor: a fenced `## Version History` is no longer an anchor (this repo quotes its own templates, and `references/inline-protocols.md` carries seven such headings and no live section), and `##Version History` no longer starts a section that the same spelling could not end. **Use this instead of a hand-rolled anchored substitution** — the value is the assert, not the append: a `.replace()` whose anchor has drifted writes nothing and reports success, so a skipped bump and a completed bump are indistinguishable from the caller's side. Placement is derived from the section, never assumed: measured over 713 real sections, 191 of the 246 multi-entry ones are ascending, 29 descending and 26 unsorted, so a blind append-at-end is wrong for 22% of them and silently so. Tables (140 of 713) are refused rather than reformatted, the unsorted case is refused rather than guessed, and every write self-checks that its own splice was insertion-only. Stdlib-only.
...
- `h_mad_doc_block_exec.py` — execute one explicitly tagged `bash hmad:exec` block from one UTF-8 document and heading. CLI: `doc --heading <heading> [--index <one-based index>] [--subst K=V ...] [--preamble-file <path>] [--shell-timeout <seconds>] [--stdout <path>] [--stderr <path>]`. Selection and substitution refusals precede stream reservation. The default shell is strict; `shell=plain` opts out. The finite positive shell bound defaults to 30 seconds and cannot exceed `(2**31 - 1) / 1000`. Stream paths must be distinct regular files; existing bytes survive refusals before final writes. Completed output replaces each selected artifact and is verified immediately. A child failure is still `RAN`; inspect `rc`. Argument errors yield `BAD_ARGS`; argparse help exits zero without a verdict. Every verdict exits zero except environmental failures, which exit two. Dynamic values are double-quoted JSON strings with control and line separators escaped; only `rc`, `blocks`, `count`, `keys`, `shell`, `stage`, and `reason` are bare helper-produced integers or enums.

| Line head / detail key | Value form | Remedy |
|---|---|---|
| `RAN` | `rc=3 blocks=1 shell=plain` | Inspect the child rc and stream artifacts. |
| `NOT_FOUND` | `heading="title"` | Choose a heading containing a tagged block or an in-range index. |
| `AMBIGUOUS` | `blocks=2 heading="title"` | Select one block with --index. |
| `AMBIGUOUS_HEADING` | `count=2 heading="title"` | Make the heading unique. |
| `BAD_INDEX` | `index="raw"` | Use a positive one-based integer. |
| `BAD_TIMEOUT` | `value="raw"` | Use --shell-timeout with a finite positive value within the supported bound. |
| `BAD_ARGS` | `message="parser diagnostic"` | Correct the arguments; consult --help. |
| `BAD_SUBST` | `arg="raw"` | Supply a nonempty, unique key followed by = and its value. |
| `SUBST_MISSING` | `keys=1` | Correct the missing literal keys. |
| `SUBST_OVERLAP` | `keys=2` | Choose keys with neither containment nor intersecting source spans. |
| `BAD_INFO` | `key="token"` | Remove the unknown or repeated info token. |
| `TIMEOUT` | `seconds="1.0"` | Inspect the block and adjust --shell-timeout if appropriate. |
| `CLEANUP_FAILED` | `path="directory"` | Inspect and remove the retained temporary directory. |
| `LAUNCH_FAILED stage=mkdtemp` | `os_error detail follows` | Check temporary-directory access. |
| `LAUNCH_FAILED stage=spawn` | `os_error detail follows` | Check bash availability and remove embedded NULs. |
| `LAUNCH_FAILED stage=reap` | `os_error detail follows` | Inspect the reported process group and terminate it if needed. |
| `LAUNCH_FAILED stage=collect` | `os_error detail follows` | Inspect the process group and pipe failure. |
| `UNREADABLE reason=doc_unreadable` | `reason is the fixed enum in the head` | Check document access and strict UTF-8 encoding. |
| `UNREADABLE reason=preamble_unreadable` | `reason is the fixed enum in the head` | Check preamble access and strict UTF-8 encoding. |
| `UNREADABLE reason=stream_paths_alias` | `reason is the fixed enum in the head` | Choose paths referring to different inodes; inspect any leftover. |
| `UNREADABLE reason=stream_path_unwritable` | `reason is the fixed enum in the head` | Choose writable regular-file paths; inspect any leftover. |
| `UNREADABLE reason=stream_write_failed` | `reason is the fixed enum in the head` | Inspect written, failed, skipped and verify details before retrying. |
| `UNREADABLE reason=stream_close_failed` | `reason is the fixed enum in the head` | Inspect the named stream and OS error before retrying. |
| `missing_key:` | `"key"` | Correct this absent literal key. |
| `overlap:` | `"shorter" "longer"` | Remove one of the containing keys. |
| `intersect:` | `"a" "b" "offset"` | Remove the conflicting source-span substitution. |
| `duplicate_key:` | `"key"` | Supply this key only once. |
| `os_error:` | `"diagnostic"` | Resolve the reported operating-system failure. |
| `pgid:` | `"4242"` | Inspect this process group after a reap or collection failure. |
| `written:` | `"stdout stderr"` | These artifacts completed verification. |
| `failed:` | `"stdout"` | Inspect this artifact before retrying. |
| `skipped:` | `"stderr"` | This artifact was not rewritten; inspect the earlier failure. |
| `verify:` | `"stdout"` | Read-back failed; inspect the file and storage. |
| `stream:` | `"stdout"` | Inspect this stream for a close failure. |
| `leftover:` | `"path"` | Inspect the retained reservation; remove only a file you own. |

- `h_mad_mutation_harness.py` — Phase-5e mutation harness: `run_spec()` + CLI printing `MUTATION: ALL_CAUGHT|SURVIVED|REFUSED|BASELINE_NOT_GREEN|RESTORE_FAILED|UNREADABLE mutations=N caught=K survived=J refused=R unreadable=U crash_kills=C or `MUTATION: BUSY holder=<pid> age=<s> spec=<path>` or `MUTATION: PRECHECK_FAILED specs=N drifted=K unreadable=J` + `[H-MAD]` marker, exit 0 on a verdict (`ALL_CAUGHT`/`SURVIVED`) / 2 on anything that measured nothing, including `MUTATION: PRECHECK_FAILED`. With `--check-anchors`, prints `ANCHORS: <verdict> specs=N mutations=M ok=K drifted=J unreadable=U skipped=S unclassifiable=C` for `ANCHORS_OK`/`ANCHORS_DRIFTED`/`ANCHORS_UNREADABLE`, or `ANCHORS: ANCHORS_NOTHING_SWEPT specs=0 skipped=K unclassifiable=J`; `ANCHORS_OK` exits 0, while `ANCHORS_DRIFTED`, `ANCHORS_UNREADABLE` and `ANCHORS_NOTHING_SWEPT` exit 2 because no usable mutation verdict was measured. **`skipped=` and `unclassifiable=` ride EVERY summary line, not only `ANCHORS_NOTHING_SWEPT`**: both were always counted, and a consumer that renders only the summary could not otherwise tell a sweep that set files aside from one that did not. **A file that is not valid JSON at all (`unclassifiable`) is a finding, not a skip** — it takes the `ANCHORS_UNREADABLE` verdict and exits 2. It rides that existing word rather than a new one because the consuming pre-push hook scores verdicts with an ordered substring `case` whose default arm ALLOWS the push, so a new verdict word would be silently non-blocking there until every consumer was updated in lockstep. `skipped` (`not-a-spec`: valid JSON declaring no `mutations`) stays non-fatal and is the category that earns “not every file beside a spec is a spec”; that rationale never covered a file that will not parse. Takes a JSON spec naming the suite command and each mutation as an exact `find`/`replace`. Refuses any anchor not matching exactly once — the "`.replace()` that matches nothing reports the guard as enforced" failure in `invariants.base.md` §"Mutation verification" — restores the tree on every path including SIGINT/SIGTERM, verifies the restore by re-reading, and re-runs the suite afterwards to prove it. Before the baseline command, the mutation run sweeps sibling specs in the same directory and refuses on sibling drift with `MUTATION: PRECHECK_FAILED`; the precheck summary carries `specs=`/`drifted=`/`unreadable=` counts and exits 2 because no mutation was measured. A mutation may add `"test": "<nodeid>"` (with a spec-level `"target_command"` argv prefix), which changes the question from **did the suite go red** to **did THAT test bite** — the discrimination `ALL_CAUGHT` structurally cannot express, since a mutant that dies on a crash, a timeout or an unrelated assertion is otherwise byte-identical to the guard biting. A named test that PASSES while the suite goes red is a **survivor**, and the detail line names what actually bit; the named test is also required green before the mutation is applied, because a kill credited against an already-failing pin measures nothing and the whole-suite baseline cannot see one red pin. Caught and survived mutations both print a `mechanism:` detail line naming the killer, and a refused anchor prints `hint:` lines — near misses with line numbers when it matched 0 times, the match locations when it matched more than once — because the REFUSED verdict was always right and the recovery was a manual re-grep. A mutation whose `find` occurs verbatim inside its own `replace` is **diagnosed, never refused**: every substring of the original survives such a mutation, so an assertion shaped `assert "<text>" in …` cannot see it, and that is reported as a `self-matching:` detail line under `--check-anchors` and appended to the `mechanism:` line of any survivor — including the untargeted branch, which previously printed a bare name and nothing else. A backlog row asked for a refusal here; measured against the corpus on 2026-09-14 it would have refused **21 of 874** committed mutations, every one a legitimate insertion that is caught (`order = order + order` appended to a sort, `default=False,` added to an argparse line, a frontmatter `name:` renamed), so the diagnosis is kept and the refusal is refuted. It rides detail lines rather than the summary for the same reason `unclassifiable=` had to ride an existing verdict word: the `ANCHORS:` summary is asserted by exact string in the suite and scored by an ordered substring `case` in the pre-push hook whose default arm ALLOWS the push. Relative spec `root` values resolve against the spec file's directory, not the caller's cwd, so committed specs remain portable across checkouts and correct from git worktrees. A named-test kill is then classified further, because "that test failed" still covers two events wearing one `1 failed`: the guard's assertion bit, or the mutant CRASHED before the property was reached (the two measured cases are an unbound-variable crash tripping `assert r.returncode == 0`, and a 60-second `TimeoutExpired`). The scoring run's output is scanned STRUCTURALLY — `File "…"` frames attributed by basename to the mutated file, then a terminal exception line — never by a bare `Traceback` search, which fires on the echoed source of any test whose subject IS a crash. **Tier 1 (`SyntaxError`/`IndentationError`/`TabError` in the mutated file) is REFUSED**, on the same reasoning as the collection break: a file that did not parse cannot have exercised a guard, so it measured NOTHING — and unlike a collection break it is otherwise invisible, because a script consumed by `subprocess.run` can be syntax-broken with pytest's collection fully intact. **Tier 2 (every other exception) is ANNOTATED and COUNTED but never refused**: the `mechanism:` line carries `(crash: NameError in <file>)` and the summary carries `crash_kills=N`. Tier 2 is deliberately not a refusal because a mutation that strips a None-check makes the code raise `AttributeError` and the test asserting the graceful message then fails — there the crash IS the property violation, and the harness cannot tell that from a pre-property crash, so it prints the judgement rather than making it. `crash_kills=0` means "none found", not "none there": a test written `assert r.returncode == 0` with no message discards the child's stderr, and `TimeoutExpired` (no `Error`/`Exception` in its name, attributed to the test file) and the untargeted no-`test` branch are not classified at all. **`crash_kills=` is APPENDED to the end of the summary line** — a subset of `caught`, not a new outcome — so every consumer that parses that line by substring from the left is unaffected; the rest of what is new on a measured verdict is still an additional detail line. It also purges cached bytecode around every run: CPython invalidates a `.pyc` on (source mtime, size), a mutation is frequently byte-size-IDENTICAL, and it lands inside the same filesystem-mtime second as the previous run — so both invalidation inputs match, the stale bytecode is reused, the mutant never executes and the run reports `survived` while the file on disk is genuinely mutated (measured 4 false survivors in 6 trials before the fix). Reporting only: whether the mechanism that fired is the mechanism the spec claims stays with the author. Stdlib-only.
- `h_mad_archreview_cycle.py` — ONE Phase-6a-prime cycle, staged and scored: `stage` substitutes the reviewer template and prints the exact `exec agy` command, `score` runs the gates in the order that is the contract — **evidence FIRST, then verdict, then record, then READ BACK**. Prints `ARCHREVIEW: STAGED prompt=… base=… head=… bytes=N` or `ARCHREVIEW: READY_TO_MERGE|WITH_FIXES|NO tools=N recorded=yes` (exit 0), and `OVERSIZE`/`NO_EVIDENCE`/`LOW_EVIDENCE_CLEAN`/`NO_VERDICT`/`NOT_RECORDED`/`UNSUBSTITUTED`/`DEGENERATE_RANGE`/`UNREADABLE` (exit 2). **`OVERSIZE` refuses a prompt no delivery surface accepts and writes NO file** — `stage()` measured nothing while `h_mad_assemble_audit.py` held the measured ceiling one file over, so a ~690 KB prompt was staged, reported `STAGED`, and was then refused by the surface, which reads as "6a-prime failed" rather than "the prompt was never delivered"; the gate runs AFTER the output-contract prepend because that is the body a surface receives, and `--vh-tail N` is the remedy it names. **`LOW_EVIDENCE_CLEAN` is `NO_EVIDENCE` at the floor this channel's contract sets rather than at zero**: the reviewer template says "you must also WRITE your report file with `run_command`. That is the only write you may make" and asks for no `.done` marker, so `DELIVERY_FLOOR = 1` and a reviewer that read nothing but obeyed the delivery instruction used to score `tools=1` and pass. Derived from the CONTRACT, never from tool names — `h_mad_review_evidence.scan` knows none on purpose, and classifying calls as reads-vs-writes there would re-create the false zero that hardcoding `view_file|grep_search` produced. Asymmetric on purpose: only READY_TO_MERGE is held to the floor, because findings are evidence of reading and a clean is not, so a WITH_FIXES or NO at the same count is still recorded. `h_mad_audit_cycle.py` has carried `DELIVERY_FLOOR = 2` against the same J49 failure; this closes the asymmetry between the two channels. The seven mechanical steps of 6a-prime were hand-assembled **7 times in one session**, and the two with no other home are the two that get skipped: the BASE/HEAD stamp (a stale sha silently reviews the PREVIOUS commit — J41 one level up, which is why `DEGENERATE_RANGE` refuses `BASE == HEAD`) and the evidence gate (a dispatch whose single `view_file` errored returned `READY_TO_MERGE` in 1510 confident bytes, and rc, the extractor and the Phase-7 gate all took it). **`NO_EVIDENCE` and `NO_VERDICT` record NOTHING and carry no verdict word** — a review that read nothing has no verdict whatever its last line says, and recording it is what made that defect survivable. `NOT_RECORDED` exists because `archreview` is not in the schema's `required` array, so strict validation passes over a dropped write and the read-back is the only check. **One cycle per invocation: it does not decide whether to run another**, because that run went to seven when cycle 3 came back clean and cycle 4 then found a Critical vacuous pass. Deliberately NOT a variant of `audit-cycle`, which is a multi-pass verdict COMBINER over already-finished runs and rejects any phase outside plan/design/impl-plan — same word, different machine. Stdlib-only.
...
- `h_mad_review_evidence.py` — 6a-prime evidence gate: `scan()` + CLI printing `EVIDENCE: PASS|NONE tools=N ok=K failed=J thinking=T [status=…]`, exit 0 on a verdict / 2 on `EVIDENCE: UNREADABLE reason=no_log|empty_log|unsupported_format`, which carries **no counts** so a cannot-judge cannot read as a zero. Answers only "did the review read anything" — never whether its findings are right. Counts any tool reaching `DONE`, knows no tool names, and reports `result.status` without gating on it. `thinking=` sums `usage.thinking_tokens` over completed `agent_response` steps and is reported for the same reason `status` is — triage, never a verdict (J49). Stdlib-only.
