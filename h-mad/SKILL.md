...
- `h_mad_wire_registry.py` — Phase-5 wire registry: records passing `wiring` pins and re-verifies them at 5f, emitting the documented `[H-MAD]` halt reasons; its challenge command is warning-only and verdict-neutral. `pin` (and `successor_pin`) accept a string or a list of distinct non-empty strings, so one record can cover a task's every call site; all pins must resolve and pass for the record to verify, and halt reasons gain `#<pin>` only on multi-pin records. Stdlib-only.
